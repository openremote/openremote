...
public interface ProtocolAssetService {
  ...
  /**
   * Protocols may store assets in the context or update existing assets. A unique identifier must
   * be set by the protocol implementor, and the parent must be the {@link Agent} associated with
   * the requesting protocol instance. This operation stores transient or detached state and returns
   * the current state. It will override any existing stored asset data, ignoring versions.
   */
  ...
  /**
   * Get assets by an {@link org.openremote.model.query.AssetQuery}; can only access {@link Asset}s
   * that are within the same realm as the {@link Agent}.
   */
   ...
   *
   * <p>When an agent is unlinked from a protocol then all subscriptions will be automatically
   * removed also; it is safe to call this method multiple times for the same agentId and
   * assetChangeConsumer and only a single subscription would actually be created.
   */
