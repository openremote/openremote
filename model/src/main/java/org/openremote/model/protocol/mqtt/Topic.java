...
public class Topic {
  ...
  public static Topic parse(String topic) throws IllegalArgumentException {
    ...
    if (multiLevelPos >= 0 && multiLevelPos < topic.length() - 1) {
      throw new IllegalArgumentException(
          "Multilevel wildcard token must be at the end of the topic");
    }
  ...
  /**
   * Determines if the specified topic matches this topic; can be a simple equality match or if this
   * topic contains wildcards then determines if the specified topic is covered by the wildcards
   */
