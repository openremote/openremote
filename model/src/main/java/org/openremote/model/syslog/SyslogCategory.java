...
public enum SyslogCategory {
  ...
  protected String getSubCategory(String loggerName) {
    if (!includeSubCategory) return null;
  ...
  public static SyslogEvent mapSyslogEvent(LogRecord record) {
    ...
    if (!record.getLoggerName().startsWith("org.openremote")) return null;
    ...
    for (SyslogCategory category : values()) {
      ...
      if (loggerName != null) {
        ...
        String message =
            level.ordinal() >= SyslogLevel.ERROR.ordinal() && record.getThrown() != null
