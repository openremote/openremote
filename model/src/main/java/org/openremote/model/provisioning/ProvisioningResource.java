...
@Tag(
    name = "Provisioning",
    description = "Super-administrator operations for device provisioning configurations")
@Path("provisioning")
public interface ProvisioningResource {
  ...
  long createProvisioningConfig(
      @RequestBody(
              required = true,
              description = "Complete typed provisioning configuration to create.")
  ...
  void updateProvisioningConfig(
      ...
      @Parameter(description = "Numeric provisioning configuration identifier.", example = "12")
          @PathParam("id")
          Long id,
      @Valid @RequestBody(
              required = true,
              description =
                  "Complete typed provisioning configuration to replace the existing configuration.")
          ProvisioningConfig<?, ?> provisioningConfig);
