...
public abstract class ProvisioningConfig<T, U extends ProvisioningConfig<T, U>> {
  ...
  @Id
  @Column(name = "ID")
  @Min(1) @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = PERSISTENCE_SEQUENCE_ID_GENERATOR)
  @SequenceGenerator(
      name = PERSISTENCE_SEQUENCE_ID_GENERATOR,
      initialValue = 1000,
      allocationSize = 1)
  @Schema(
      description = "Server-assigned numeric configuration identifier.",
  ...
  @Size(min = 1, max = 255, message = "{ProvisioningConfig.name.Size}") @Schema(description = "Human-readable configuration name.", example = "Factory sensors")
  ...
  @Size(min = 3, max = 100, message = "{ProvisioningConfig.type.Size}") @JsonDeserialize
  ...
  @Schema(
      description = "Create the provisioned user with restricted-user access.",
      example = "true")
  ...
  @Schema(
      description = "Prevent new provisioning while retaining the configuration.",
      example = "false")
  ...
  /**
   * Implementors must annotate the data field with @Column as JPA (Hibernate) doesn't work with
   * generic fields
   */
  @JsonProperty
