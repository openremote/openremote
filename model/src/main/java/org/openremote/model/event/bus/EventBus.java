 ...
 *
 * <p>Listeners must match the event type (as in {@link Event#getEventType}) exactly, a listener for
 * an event supertype will not receive events of a subtype.
 *
 * <p>Listeners can be registered to execute in the <em>prepare</em> phase, and cancel dispatch by
...
public class EventBus {
  ...
  public <E extends Event> EventRegistration<E> register(
      Class<E> eventClass, EventListener<E> listener) {
    return register(false, eventClass, listener);
  }

  public <E extends Event> EventRegistration<E> register(
      boolean prepare, Class<E> eventClass, EventListener<E> listener) {
  ...
  public <T extends Event> void dispatch(T event) {
    synchronized (this.registrations) {
      ...
      for (EventRegistration<?> registration : registrations) {
        if (!registration.isMatching(event) || !registration.isPrepare()) continue;
        activePrepareRegistrations.add(registration);
      }
      // Only notify listeners after iterating registrations, some listeners might modify
      // registrations!
