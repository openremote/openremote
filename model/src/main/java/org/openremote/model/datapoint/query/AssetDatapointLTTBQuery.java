...
public final class AssetDatapointLTTBQuery extends AssetDatapointQuery {
    ...
    public String getSQLQuery(String tableName, Class<?> attributeType) throws IllegalStateException {
        ...
        if (isNumber) {
            return "select time::timestamp, value from public.unnest((select public.lttb(cast(timestamp as timestamptz), cast(value as double precision), ?) from " + tableName + " where ENTITY_ID = ? and ATTRIBUTE_NAME = ? and TIMESTAMP >= ? and TIMESTAMP <= ?))";
        } else if (isBoolean) {
            return "select time::timestamp, value from public.unnest((select public.lttb(cast(timestamp as timestamptz), (case when cast(cast(value as text) as boolean) is true then 1 else 0 end), ?) from " + tableName + " where ENTITY_ID = ? and ATTRIBUTE_NAME = ? and TIMESTAMP >= ? and TIMESTAMP <= ?))";
        } else {
