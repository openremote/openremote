...
public final class AssetDatapointLTTBQuery extends AssetDatapointQuery {
    ...
    public String getSQLQuery(String tableName, Class<?> attributeType) throws IllegalStateException {
        ...
        if (isNumber) {
            return "select * from public.unnest((select public.lttb(timestamp::timestamptz, value::double precision, ?) from "
                    + tableName + " where ENTITY_ID = ? and ATTRIBUTE_NAME = ? and TIMESTAMP >= ? and TIMESTAMP <= ?))";
        } else if (isBoolean) {
            return "select * from public.unnest((select public.lttb(timestamp::timestamptz, (case when VALUE::text::boolean is true then 1 else 0 end), ?) from "
                    + tableName + " where ENTITY_ID = ? and ATTRIBUTE_NAME = ? and TIMESTAMP >= ? and TIMESTAMP <= ?))";
        } else {
