...
@Tag(
    ...
    description = "Query and export historical values stored for asset attributes")
@Path("asset/datapoint")
public interface AssetDatapointResource {

  /**
   * Retrieve the historical datapoints of an asset attribute. Regular users can only access assets
  ...
  ValueDatapoint<?>[] getDatapoints(
      ...
      @RequestBody(
              description =
                  "Optional polymorphic datapoint query. Set type to all, interval, lttb, or nearest. Omit the body to return the available stored series.",
              content =
                  @Content(
                      ...
                      examples = {
                        @ExampleObject(
                            name = "All values",
                            summary = "Return every datapoint in a one-day inclusive range",
                            value = DATAPOINT_ALL_QUERY),
                        @ExampleObject(
                            name = "Hourly average",
                            summary = "Average one day of values into hourly buckets and fill gaps",
                            value = DATAPOINT_INTERVAL_QUERY),
                        @ExampleObject(
                            name = "LTTB downsampling",
                            summary = "Downsample one day to at most 500 representative points",
                            value = DATAPOINT_LTTB_QUERY),
                        @ExampleObject(
                            name = "Nearest value",
                            summary =
                                "Return the closest datapoint at or before a Unix-seconds timestamp",
                            value = DATAPOINT_NEAREST_QUERY)
                      }))
  ...
  @OpenApiResponses.BadRequest
  @OpenApiResponses.NotFound
  DatapointPeriod getDatapointPeriod(
      @BeanParam RequestParams requestParams,
      @Parameter(description = ASSET_ID, example = EXAMPLE_ASSET_ID, required = true)
          @QueryParam("assetId")
          String assetId,
      @Parameter(description = ATTRIBUTE_NAME, example = EXAMPLE_ATTRIBUTE_NAME, required = true)
  ...
  @Operation(
      ...
      summary = "Export historical datapoints",
      description =
  ...
  @ApiResponse(
      ...
      headers =
          @Header(
              ...
              schema =
                  @Schema(type = "string", example = "attachment; filename=\"dataexport.zip\"")),
      content =
          @Content(
  ...
  void getDatapointExport(
      @Suspended AsyncResponse asyncResponse,
      @Parameter(
              description = "JSON-encoded array of objects with id and name fields.",
              example = "[{\"id\":\"7A6p4AnLTkKxJUCQAAABAA\",\"name\":\"temperature\"}]",
              required = true)
          @QueryParam("attributeRefs")
          String attributeRefsString,
      @Parameter(
              description = "Start of the export range as Unix time in milliseconds.",
              example = EXAMPLE_TIMESTAMP,
              required = true)
          @QueryParam("fromTimestamp")
          long fromTimestamp,
      @Parameter(
              description = "End of the export range as Unix time in milliseconds.",
              example = "1767312000000",
              required = true)
          @QueryParam("toTimestamp")
          long toTimestamp,
      @Parameter(description = "Export file format.", example = "CSV")
          @QueryParam("format")
          @DefaultValue("CSV")
