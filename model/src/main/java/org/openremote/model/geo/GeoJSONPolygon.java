 ...
 *
 * <p>A Polygon is defined by one or more linear rings. The first ring is the exterior ring (outer
 * boundary), and any subsequent rings are interior rings (holes).
 *
 * <p>Each linear ring is a closed coordinate array where the first and last coordinates must be
 * identical.
 */
@JsonTypeName(TYPE)
public class GeoJSONPolygon extends GeoJSONGeometry {

  public static class CoordinateArrayListConverter
      extends StdConverter<List<Coordinate[]>, double[][][]> {
  ...
  public static class CoordinateArrayListConverterJackson2
      extends com.fasterxml.jackson.databind.util.StdConverter<List<Coordinate[]>, double[][][]> {
  ...
  @com.fasterxml.jackson.databind.annotation.JsonSerialize(
      converter = CoordinateArrayListConverterJackson2.class)
    ...
    for (double[][] ring : coordinates) {
      if (ring.length < 4) {
        throw new IllegalArgumentException(
            "Each ring must have at least 4 coordinates (forming a closed ring)");
      }
      ...
      for (int i = 0; i < ring.length; i++) {
        ...
        if (coord.length > 3) {
          throw new IllegalArgumentException(
              "Each coordinate can have at most 3 values (longitude, latitude, altitude)");
        }
  ...
  private static Coordinate[] clampCoordinates(Coordinate[] coords) {
    ...
    for (int i = 0; i < coords.length; i++) {
      ...
      result[i] =
          Double.isNaN(coord.getZ()) ? new Coordinate(x, y) : new Coordinate(x, y, coord.getZ());
    }
    return result;
  }
  ...
  public String toString() {
    return "GeoJSONPolygon{"
        ...
        + coordinates.size()
        + ", exteriorRingPoints="
        + (coordinates.isEmpty() ? 0 : coordinates.getFirst().length)
        + ", holes="
        + (coordinates.size() - 1)
        + '}';
  }
