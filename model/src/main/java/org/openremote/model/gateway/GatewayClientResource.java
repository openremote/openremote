...
@Tag(
    name = "Gateway",
    description = "Configure this Manager's connections to central OpenRemote gateway services")
@Path("gateway")
...
public interface GatewayClientResource {
  ...
  @ApiResponse(
      responseCode = "204",
      description = "No gateway connection is configured for the realm")
  GatewayConnection getConnection(
  ...
  void setConnection(
      ...
      @RequestBody(
              required = true,
              description =
                  "Central-manager connection settings. localRealm and realms in attribute filters are forced to match the path realm.")
          @Valid GatewayConnection connection);
  ...
  void deleteConnections(
      ...
      @Parameter(
              description =
                  "Realm names whose gateway connections should be removed. Repeat the query parameter for multiple realms.",
