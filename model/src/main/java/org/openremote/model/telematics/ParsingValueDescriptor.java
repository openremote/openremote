...
public class ParsingValueDescriptor<T> extends ValueDescriptor<T> {
    ...
    public ParsingValueDescriptor(String name, Class<T> type, ValueConstraint... constraints) {
        this(name, type, -1, type::cast, constraints);
    }
    ...
    public ParsingValueDescriptor(String name, Class<T> type, int length, Function<ByteBuf, T> parser, ValueConstraint... constraints) {
        super("teltonika_"+name, type, constraints);
        this.parser = parser;
        this.length = length;
    }
     ...
     * </pre>
     *
     * @param name The name for the teltonika attribute (will be prefixed with "teltonika_")
     * @param valueDescriptor The ValueDescriptor to extend (e.g., ValueType.TEXT, ValueType.NUMBER)
     * @param length The byte length from AVL specification (-1 for variable length)
     ...
     */
    public static <T> ParsingValueDescriptor<T> fromValueDescriptor(String name, ValueDescriptor<T> valueDescriptor, int length, Function<ByteBuf, T> parser) {
        return new ParsingValueDescriptor<>(name, valueDescriptor.getType(), length, parser, valueDescriptor.getConstraints());
    }
     ...
     * Example usage:
     * <pre>
     * // Extend ValueType.TEXT with variable-length binary parsing
     * ParsingValueDescriptor&lt;String&gt; varString = ParsingValueDescriptor.fromValueDescriptor(
     *     "name", ValueType.TEXT,
     *     buf -> { ... } // custom parsing logic
     * );
     * </pre>
     *
     * @param name The name for the teltonika attribute (will be prefixed with "teltonika_")
     ...
     * @return A new ParsingValueDescriptor with the specified parser
     */
    public static <T> ParsingValueDescriptor<T> fromValueDescriptor(String name, ValueDescriptor<T> valueDescriptor, Function<ByteBuf, T> parser) {
        return fromValueDescriptor(name, valueDescriptor, -1, parser);
    }
}
