...
public class RequestParams {
  ...
  public UriBuilder getExternalBaseUriBuilder() {
    ...
    String scheme =
        TextUtil.isNullOrEmpty(this.forwardedProtoHeader)
            ? uri.getScheme()
            : this.forwardedProtoHeader;
  ...
  public static String getBearerAuth(@NotNull String authorizationHeader) {
    return authorizationHeader.isEmpty() || authorizationHeader.charAt(0) != 'B'
        ? authorizationHeader
        : authorizationHeader.substring(BEARER_AUTH_PREFIX.length()).trim();
  }
}
