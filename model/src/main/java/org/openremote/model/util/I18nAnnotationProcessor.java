...
public class I18nAnnotationProcessor extends AbstractProcessor {
  ...
  public Set<String> getSupportedAnnotationTypes() {
    return Set.of(
        JsonSchemaTitle.class.getCanonicalName(), JsonSchemaDescription.class.getCanonicalName());
  }
  ...
  private <A extends Annotation> void collectI18nKeys(
      Class<A> annotationClass, RoundEnvironment roundEnv) {
    for (Element element : roundEnv.getElementsAnnotatedWith(annotationClass)) {
      try {
        ...
        Optional<String> i18nSuffix =
            Arrays.stream(annotationClass.getMethods())
        ...
        throw new RuntimeException(
            "Failed to process i18n annotation " + annotationClass.getSimpleName(), e);
      }
    }
  }
    ...
    if (conflicts > 0) {
      processingEnv
          ...
          .printMessage(
              ERROR,
              conflicts
                  + " translation key conflict(s) detected. Please remove the above keys or the conflicting annotations.");
  ...
  // Hacky solution to get around weird Jackson formatting which adds spaces between field names and
  // colons
