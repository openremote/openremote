...
public final class NumberComparator implements Comparator<Number> {
  ...
  public int compare(Number x, Number y) {
    if (isSpecial(x) || isSpecial(y)) return Double.compare(x.doubleValue(), y.doubleValue());
    else return toBigDecimal(x).compareTo(toBigDecimal(y));
  }
  ...
  private static BigDecimal toBigDecimal(Number number) {
    ...
    if (number instanceof Byte
        ...
        || number instanceof Long) return new BigDecimal(number.longValue());
