...
public class AttributeInfoValidator implements HibernateConstraintValidator<AttributeInfoValid, AttributeInfo> {
    ...
    public static final System.Logger LOG = System
            .getLogger(AttributeInfoValidator.class.getName() + "." + SyslogCategory.MODEL_AND_VALUES.name());
    protected ClockProvider clockProvider;

    @Override
    public void initialize(ConstraintDescriptor<AttributeInfoValid> constraintDescriptor,
    ...
    public boolean isValid(AttributeInfo attributeInfo, ConstraintValidatorContext context) {
        ...
        return AssetValidator.validateNameValueMetaHolder(assetModelInfo, attributeInfo, context,
                clockProvider.getClock().instant());
    }
}
