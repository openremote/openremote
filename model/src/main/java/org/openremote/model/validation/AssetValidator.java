...
public class AssetValidator implements HibernateConstraintValidator<AssetValid, Asset<?>> {
    ...
    public static final System.Logger LOG = System
            .getLogger(AssetValidator.class.getName() + "." + SyslogCategory.MODEL_AND_VALUES.name());
    protected ClockProvider clockProvider;

    @Override
    public void initialize(ConstraintDescriptor<AssetValid> constraintDescriptor,
    ...
    public boolean isValid(Asset<?> asset, ConstraintValidatorContext context) {
        ...
        if (assetModelInfo != null && !assetModelInfo.getAssetDescriptor().isDynamic()
            ...
            assetModelInfo.getAttributeDescriptors().values().stream()
                    ...
                    .forEach(requiredAttributeDescriptor -> {
                        ...
                        if (foundAttribute == null) {
                            context.buildConstraintViolationWithTemplate(ASSET_ATTRIBUTE_MISSING)
                                    .addPropertyNode("attributes")
                                    .addPropertyNode(requiredAttributeDescriptor.getName()).addConstraintViolation();
                            valid.set(false);
                        }
                    });
        }
    ...
    public static <U, T extends NameValueHolder<U> & MetaHolder> boolean validateNameValueMetaHolder(
        ...
        boolean valid = true;
        AttributeDescriptor<?> descriptor = assetModelInfo == null ? null
        ...
        if (descriptor != null) {
            if (!Objects.equals(attribute.getType(), descriptor.getType())) {
                context.buildConstraintViolationWithTemplate(ASSET_ATTRIBUTE_TYPE_MISMATCH)
                        .addPropertyNode("attributes").addPropertyNode(attribute.getName()).addConstraintViolation();
                valid = false;
            }
            ...
            if (attribute.getValue().isPresent()) {
                if (!descriptor.getType().getType()
                    ...
                    context.buildConstraintViolationWithTemplate(ASSET_ATTRIBUTE_TYPE_MISMATCH)
                            .addPropertyNode("attributes").addPropertyNode(attribute.getName())
        ...
        ValueUtil.ConstraintViolationPathProvider pathProvider = (
                constraintViolationBuilder) -> constraintViolationBuilder.addPropertyNode("attributes")
                        .addPropertyNode("value").inContainer(Map.class, 1).inIterable().atKey(attribute.getName());

        if (!ValueUtil.validateValue(descriptor, attribute.getType(), attribute, now, context, pathProvider,
