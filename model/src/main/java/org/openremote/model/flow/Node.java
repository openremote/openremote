...
public class Node extends FlowObject {
  ...
  @Transient public Slot[] slots = new Slot[0];

  @ManyToOne(fetch = FetchType.LAZY)
  @JoinColumn(
      name = "FLOW_ID",
      nullable = false,
      foreignKey = @ForeignKey(name = "FK_NODE_FLOW_ID"))
  ...
  @Embedded public EditorSettings editorSettings = new EditorSettings();
  ...
  public Slot findSlot(String slotId) {
    for (Slot slot : getSlots()) {
      if (slot.getId().equals(slotId)) return slot;
    }
    return null;
  }
  ...
  public Slot[] findSlots(String type) {
    List<Slot> list = new ArrayList<>();
    for (Slot slot : getSlots()) {
      if (slot.isOfType(type)) list.add(slot);
    }
  ...
  public Slot findSlotWithPeer(String peerId) {
    for (Slot slot : getSlots()) {
      if (slot.getPeerId() != null && slot.getPeerId().equals(peerId)) return slot;
    }
    return null;
  }
  ...
  public Slot[] findConnectableSlots(String type) {
    List<Slot> list = new ArrayList<>();
    for (Slot slot : getSlots()) {
      if ((type == null || slot.isOfType(type)) && slot.isConnectable()) list.add(slot);
    }
    return list.toArray(new Slot[list.size()]);
  }
  ...
  public Slot[] findNonPropertySlots(String type) {
    List<Slot> list = new ArrayList<>();
    for (Slot slot : getSlots()) {
      if (slot.isOfType(type) && slot.getPropertyPath() == null) list.add(slot);
    }
    return list.toArray(new Slot[list.size()]);
  }
  ...
  public Slot[] findPropertySlots() {
    List<Slot> list = new ArrayList<>();
    for (Slot slot : getSlots()) {
      if (slot.getPropertyPath() != null) list.add(slot);
    }
    return list.toArray(new Slot[list.size()]);
  }

  public Slot findSlotByPosition(int position, String type) {
    if (position > getSlots().length - 1) return null;
    if (getSlots()[position].isOfType(type)) return getSlots()[position];
    return null;
  }
}
