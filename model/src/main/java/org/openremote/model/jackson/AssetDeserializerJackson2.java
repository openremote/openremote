...
/**
 * Jackson 2 wrapper deserializer for {@link Asset} subclasses. Sets the asset type info in the
 * deserialization context before delegating to the standard POJO deserializer, so that {@link
 * AttributeDeserializerJackson2} can resolve attribute types from the asset model.
 */
public class AssetDeserializerJackson2 extends StdDeserializer<Asset<?>>
    implements ContextualDeserializer, ResolvableDeserializer {

  // Must match Asset.AssetDeserializer.ASSET_TYPE_INFO_ATTRIBUTE (avoid loading Jackson3 class
  // here)
  ...
  public Asset<?> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
    ctxt.setAttribute(
        ASSET_TYPE_INFO_ATTRIBUTE, ValueUtil.getAssetInfo(clazz.getSimpleName()).orElse(null));
