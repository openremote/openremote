...
public class AttributeDeserializerJackson2 extends StdDeserializer<Attribute<?>> {
  ...
  @Override
  public Attribute<?> deserialize(JsonParser parser, DeserializationContext context)
    ...
    // Primary: resolve type from asset type info context set by AssetDeserializerJackson2 (mirrors
    // Jackson3 path)
    AssetTypeInfo assetTypeInfo =
        (AssetTypeInfo) context.getAttribute(AssetDeserializerJackson2.ASSET_TYPE_INFO_ATTRIBUTE);
    ...
    Attribute attribute =
        valueDescriptor != null
            ? new Attribute(attributeName, valueDescriptor)
  ...
  protected Object convertValue(JsonParser parser, JsonNode valueNode, Class<?> valueType)
      throws JsonMappingException {
    try {
      return ValueUtil.JSON.readValue(
          valueNode.toString(), ValueUtil.JSON.constructType(valueType));
    } catch (tools.jackson.core.JacksonException e) {
      throw JsonMappingException.from(
          parser, "Failed to deserialize attribute value as " + valueType.getName(), e);
    }
  }
}
