 ...
 *
 * <p>This resource provides endpoints for service discovery, registration, and management of
 * external services through the manager.
 *
 * <p>Registered services are made available via the OpenRemote manager's Web UI and API, enabling
 * centralized service management and monitoring.
 */
@Tag(
    name = "Services",
    description = "Register, discover, renew, and deregister leased external-service instances")
@Path("service")
...
public interface ExternalServiceResource {
   ...
   *
   * <p>Creates a new registration entry and returns the registered external service with its
   * generated instanceId and initial status.
   *
   * <p>This service will be made available only to the realm it is registered for.
   *
  ...
  @Operation(
      ...
      summary = "Register an external service with the OpenRemote Manager",
      description =
        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    ...
                    schema = @Schema(implementation = ExternalService.class))),
        @ApiResponse(responseCode = "400", description = "Invalid external service object"),
        @ApiResponse(
            responseCode = "409",
            description = "ExternalService instance already registered"),
      })
  ExternalService registerService(
      @BeanParam RequestParams requestParams,
      @NotNull @Valid @RequestBody(
              required = true,
              description =
                  "Realm-scoped service metadata and lease settings; instanceId is assigned by the Manager.")
          ExternalService service);

  /**
   * Register a new global external service with the OpenRemote manager. This service will be made
   * available to all realms.
   *
   * <p>Creates a new registration entry and returns the registered external service with its
   * generated instanceId and initial status.
   *
  ...
  @RolesAllowed({Constants.WRITE_SERVICES_ROLE})
  @Operation(
      operationId = "registerGlobalService",
      summary = "Register a global external service with the OpenRemote Manager",
      description =
  ...
  ExternalService registerGlobalService(
      @BeanParam RequestParams requestParams,
      @NotNull @Valid @RequestBody(
              required = true,
              description =
                  "Globally visible service metadata and lease settings; instanceId is assigned by the Manager.")
          ExternalService service);

  /**
   * Retrieve all registered external services for a specific realm.
   *
  ...
  @Operation(
      ...
      summary = "List registered external services for a realm",
      description =
          "Returns all active realm-scoped service registrations for an active realm accessible to the caller.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of registered external services",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ExternalService[].class))),
  ...
  ExternalService[] getServices(
      @BeanParam RequestParams requestParams,
      @Parameter(description = REALM, example = EXAMPLE_REALM, required = true)
          @QueryParam("realm")
  ...
  @Operation(
      ...
      summary = "Retrieve an external service instance",
      description =
      ...
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "ExternalService retrieved successfully",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ExternalService.class))),
  ...
  ExternalService getService(
      @BeanParam RequestParams requestParams,
      @Parameter(
              description = "Stable service type identifier chosen by the service.",
              example = "weather")
          @PathParam("serviceId")
          @NotNull @Size(min = 1) String serviceId,
      @Parameter(
              description = "Numeric instance identifier assigned during registration.",
              example = "1")
  ...
  @Operation(
      ...
      summary = "List globally available external services",
   ...
   *
   * <p>This endpoint is used by external services to indicate they are still running and available.
   * It extends the service's lease duration and maintains its active status in the registry.
   *
  ...
  @Operation(
      ...
      summary = "Renew an external service registration lease",
      description =
   ...
   *
   * <p>Removes the active registration for the specified service, causing it to no longer be
   * available through the external service registry. This is typically called when a service shuts
   * down or needs to be removed from the system.
   *
  ...
  @Operation(
      ...
      summary = "Deregister an external service",
      description =
