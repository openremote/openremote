...
@Schema(
    description =
        "Executable ruleset. Its type discriminator selects global, realm, or asset scope.")
public abstract class Ruleset {
  ...
  @SequenceGenerator(
      ...
      allocationSize = 1)
  @Schema(
      description = "Server-assigned numeric ruleset identifier.",
  ...
  @Schema(
      description = "Server-assigned last modification time.",
      accessMode = Schema.AccessMode.READ_ONLY)
  protected Instant lastModified;

  @NotNull(message = "{Ruleset.name.NotNull}") @Column(name = "NAME", nullable = false)
  @Size(min = 3, max = 255, message = "{Ruleset.name.Size}") @Schema(description = "Human-readable ruleset name.", example = "High temperature alert")
  ...
  @Transient
  @Schema(
      description = "Latest compilation or execution error, when present.",
      accessMode = Schema.AccessMode.READ_ONLY)
  ...
  public boolean isContinueOnError() {
    return Optional.ofNullable(getMeta().get(CONTINUE_ON_ERROR))
        .flatMap(ValueUtil::getBoolean)
        .orElse(true);
  }
  ...
  public CalendarEvent getValidity() {
    return Optional.ofNullable(getMeta().get(VALIDITY))
        .map(calEvent -> ValueUtil.convert(calEvent, CalendarEvent.class))
        .orElse(null);
  }
  ...
  public boolean isTriggerOnPredictedData() {
    return Optional.ofNullable(getMeta().get(TRIGGER_ON_PREDICTED_DATA))
        .flatMap(ValueUtil::getBoolean)
        .orElse(false);
  }
  ...
  public boolean isShowOnList() {
    return Optional.ofNullable(getMeta().get(SHOW_ON_LIST))
        .flatMap(ValueUtil::getBoolean)
        .orElse(false);
  }
