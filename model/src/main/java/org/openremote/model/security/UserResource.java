...
public interface UserResource {
  ...
  Role[] getClientRoles(
      ...
      @Parameter(description = CLIENT_ID, example = EXAMPLE_CLIENT_ID) @PathParam("clientId")
          String clientId);
  ...
  void updateRoles(
      ...
      @RequestBody(
              required = true,
              description =
                  "Complete desired set of OpenRemote client role definitions; omitted existing roles are removed.")
          Role[] roles);
  ...
  void updateClientRoles(
      ...
      @RequestBody(
              required = true,
              description =
                  "Complete desired set of client role definitions; omitted existing roles are removed.")
          Role[] roles,
      @Parameter(description = CLIENT_ID, example = EXAMPLE_CLIENT_ID) @PathParam("clientId")
          String clientId);
  ...
  User[] query(
      ...
      @RequestBody(
              required = true,
              description =
                  "Realm, identity, service-account, asset-link, and field-selection criteria.",
              content =
                  @Content(
                      ...
                      examples =
                          @ExampleObject(
                              name = "Find human users",
                              summary = "Find human users by username in one realm",
                              value = USER_QUERY)))
          UserQuery query);
  ...
  User update(
      ...
      @Valid @RequestBody(
              required = true,
              description =
                  "User profile, enabled state, attributes, and identity-provider fields to update.")
          User user);
  ...
  User updateCurrent(
      ...
      @Valid @RequestBody(
              required = true,
              description = "Editable profile fields for the currently authenticated user.")
          User user);
  ...
  User create(
      ...
      @Valid @RequestBody(
              required = true,
              description =
                  "New user profile; omit the id because it is assigned by the identity provider.",
              content =
                  @Content(
                      mediaType = APPLICATION_JSON,
                      ...
                      examples =
                          @ExampleObject(
                              name = "Create a human user",
                              summary = "Create an enabled user with an English locale",
                              value = USER_CREATE)))
          User user);
  ...
  @ApiResponse(
      responseCode = "405",
      description = "The master-realm administrator cannot be deleted")
  void delete(
      @BeanParam RequestParams requestParams,
      @Parameter(description = REALM, example = EXAMPLE_REALM) @PathParam("realm") String realm,
      @Parameter(description = USER_ID, example = EXAMPLE_USER_ID) @PathParam("userId")
  ...
  @ApiResponse(
      responseCode = "405",
      description = "The caller cannot mutate a user in the requested realm")
  ...
  void updatePassword(
      ...
      @RequestBody(
              required = true,
              description = "New credential value and whether it is temporary.")
  ...
  String resetSecret(
      ...
      @Parameter(description = REALM, example = EXAMPLE_REALM) @PathParam("realm") String realm,
      @Parameter(
              description = "Service-user identifier whose client secret is regenerated.",
  ...
  void updateUserClientRoles(
      ...
      @RequestBody(
              required = true,
              description = "Complete desired set of client-role names for this user.")
          String[] roles,
      @Parameter(description = CLIENT_ID, example = EXAMPLE_CLIENT_ID) @PathParam("clientId")
          String clientId);
  ...
  void updateUserRealmRoles(
      ...
      @RequestBody(
              required = true,
              description = "Complete desired set of realm-role names for this user.")
          String[] roles);
  ...
  void updateCurrentUserLocale(
      ...
      @RequestBody(
              required = true,
              description =
                  "BCP 47 locale identifier for the current user, encoded as a JSON string.")
          String locale);
