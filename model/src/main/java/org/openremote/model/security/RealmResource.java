...
@Tag(
    name = "Realm",
    description = "Discover accessible realms and administer identity-provider realms")
@Path("realm")
public interface RealmResource {
  ...
  @Operation(
      ...
      summary = "Retrieve realms accessible to the caller",
      description =
  ...
  @Operation(
      ...
      summary = "Retrieve an accessible realm",
      description =
  ...
  @ApiResponse(
      responseCode = "405",
      description = "The requested mutation is not allowed for the master realm")
  void update(
      @BeanParam RequestParams requestParams,
      ...
      @Valid @RequestBody(
              required = true,
              description =
                  "Complete realm settings to apply; the path name identifies the existing realm.")
          Realm realm);
  ...
  void create(
      ...
      @Valid @RequestBody(
              required = true,
              description =
                  "Realm name, display name, activation state, and optional identity-provider settings.")
          Realm realm);
