...
public class Realm {
  ...
  public static final List<RealmRole> DEFAULT_REALM_ROLES =
      List.of(new RealmRole(null, RESTRICTED_USER_REALM_ROLE, "Restricted access to assets"));
  ...
  /**
   * List of password policies, using the "policy(value)" format. Example: "specialChars(1)". Stored
   * in column as "policy(value) and policy(value) and policy(value)", and converted using {@link
   * PasswordPolicyConverter}
   */
  ...
  @NotNull(message = "{Realm.realm.NotNull}") @Size(min = 3, max = 255, message = "{Realm.realm.Size}") @Pattern(regexp = "[a-zA-Z0-9\\-_]+", message = "{Realm.realm.Pattern}") public String getName() {
  ...
  @NotNull(message = "{Realm.displayName.NotNull}") @Size(min = 3, max = 255, message = "{Realm.displayName.Size}") public String getDisplayName() {
    return displayName;
  }
