...
public abstract class ValueFilter implements Serializable {
  ...
  public static class ValueFilterSerializer extends StdSerializer<ValueFilter> {
    ...
    @Override
    public void serialize(ValueFilter value, JsonGenerator gen, SerializationContext context)
    ...
    public void serializeWithType(
        ValueFilter value, JsonGenerator gen, SerializationContext context, TypeSerializer typeSer)
        throws JacksonException {
  ...
  public static class ValueFilterDeserializer extends StdDeserializer<ValueFilter> {
    ...
    @Override
    public ValueFilter deserialize(JsonParser jp, DeserializationContext ctxt)
        throws JacksonException {
      ...
      return switch (type) {
        case RegexValueFilter.NAME ->
            new RegexValueFilter(Pattern.compile(text(node, "pattern")))
                .setMatchGroup(integer(node, "matchGroup"))
                .setMatchIndex(integer(node, "matchIndex"));
        case SubStringValueFilter.NAME ->
            new SubStringValueFilter(integer(node, "beginIndex", 0), integer(node, "endIndex"));
        case JsonPathFilter.NAME ->
            new JsonPathFilter(
                text(node, "path"), bool(node, "returnFirst"), bool(node, "returnLast"));
        case MathExpressionValueFilter.NAME ->
            new MathExpressionValueFilter(text(node, "expression"));
        default -> throw DatabindException.from(jp, "Unsupported value filter type: " + type);
      };
    }

    @Override
    public ValueFilter deserializeWithType(
