...
public final class MetaItemType {
     ...
     */
    public static final MetaItemDescriptor<AgentLink> AGENT_LINK = new MetaItemDescriptor<>("agentLink",
            ValueType.VALUE_AGENT_LINK);
     ...
     */
    public static final MetaItemDescriptor<AttributeLink[]> ATTRIBUTE_LINKS = new MetaItemDescriptor<>("attributeLinks",
            ValueType.ATTRIBUTE_LINK.asArray());
     ...
     */
    public static final MetaItemDescriptor<Boolean> ACCESS_PUBLIC_READ = new MetaItemDescriptor<>("accessPublicRead",
            ValueType.BOOLEAN);

    /**
     * Marks the attribute as writable by unauthenticated public clients.
     */
    public static final MetaItemDescriptor<Boolean> ACCESS_PUBLIC_WRITE = new MetaItemDescriptor<>("accessPublicWrite",
            ValueType.BOOLEAN);
     ...
     * UserAssetLink}.
     */
    public static final MetaItemDescriptor<Boolean> ACCESS_RESTRICTED_READ = new MetaItemDescriptor<>(
            "accessRestrictedRead", ValueType.BOOLEAN);

    /**
     * Marks the attribute as writable by restricted clients and therefore users who are linked to the asset, see {@link
     * UserAssetLink}.
     */
    public static final MetaItemDescriptor<Boolean> ACCESS_RESTRICTED_WRITE = new MetaItemDescriptor<>(
            "accessRestrictedWrite", ValueType.BOOLEAN);
     ...
     */
    public static final MetaItemDescriptor<Boolean> STORE_DATA_POINTS = new MetaItemDescriptor<>("storeDataPoints",
            ValueType.BOOLEAN);

    /**
     * How long to store attribute values in time series database (data older than this will be automatically purged)
     */
    public static final MetaItemDescriptor<Integer> DATA_POINTS_MAX_AGE_DAYS = new MetaItemDescriptor<>(
            "dataPointsMaxAgeDays", ValueType.POSITIVE_INTEGER);
    ...
    public static final MetaItemDescriptor<Boolean> HAS_PREDICTED_DATA_POINTS = new MetaItemDescriptor<>(
            "hasPredictedDataPoints", ValueType.BOOLEAN);

    /**
     * The forecast service calculates predicted data points based on the forecast configuration.
     */
    public static final MetaItemDescriptor<ForecastConfiguration> FORECAST = new MetaItemDescriptor<>("forecast",
            ValueType.FORECAST_CONFIGURATION);

    /* RULE META */

    /**
     * Can be set to false to exclude an attribute update from being processed by the rules engines as {@link
     * org.openremote.model.attribute.AttributeInfo} facts, otherwise any attribute that also has an {@link #AGENT_LINK}
     * meta item or {@link #RULE_STATE}
     * is true, will be processed with a lifecycle that reflects the state of the asset attribute. Each attribute will
     * have one
     ...
     */
    public static final MetaItemDescriptor<Boolean> RULE_STATE = new MetaItemDescriptor<>("ruleState",
            ValueType.BOOLEAN);

    /**
     * Used by when-then rules to indicate that the rule should be allowed to re-trigger immediately; this can be useful
     * for attributes
     * that contain event based data rather than state data.
     */
    public static final MetaItemDescriptor<Boolean> RULE_RESET_IMMEDIATE = new MetaItemDescriptor<>(
            "ruleResetImmediate", ValueType.BOOLEAN);
     ...
     */
    public static final MetaItemDescriptor<ValueFormat> FORMAT = new MetaItemDescriptor<>("format",
            ValueType.VALUE_FORMAT);
     ...
     */
    public static final MetaItemDescriptor<String[]> UNITS = new MetaItemDescriptor<>("units",
     ...
     */
    public static final MetaItemDescriptor<ValueConstraint[]> CONSTRAINTS = new MetaItemDescriptor<>("constraints",
            ValueType.VALUE_CONSTRAINT.asArray());
     ...
     */
    public static final MetaItemDescriptor<Boolean> MULTILINE = new MetaItemDescriptor<>("multiline",
            ValueType.BOOLEAN);

    /**
     * If there is a dashboard, some kind of attribute overview, should this attribute be shown.
     */
    public static final MetaItemDescriptor<Boolean> SHOW_ON_DASHBOARD = new MetaItemDescriptor<>("showOnDashboard",
            ValueType.BOOLEAN);
     ...
     */
    public static final MetaItemDescriptor<Boolean> MOMENTARY = new MetaItemDescriptor<>("momentary",
            ValueType.BOOLEAN);
     ...
     */
    public static final MetaItemDescriptor<String> USER_CONNECTED = new MetaItemDescriptor<>("userConnected",
            ValueType.TEXT);
