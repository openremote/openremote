...
/**
 * Built in model provider that scans the model classes for asset classes and also includes {@link
 * MetaItemType} and {@link ValueType} classes.
 */
@TsIgnore
...
public class StandardModelProvider implements AssetModelProvider {
  ...
  protected static Logger LOG =
      SyslogCategory.getLogger(MODEL_AND_VALUES, StandardModelProvider.class);
  ...
  public void onAssetModelFinished() {
    ...
    List<ValueConstraint> constraints =
        ValueType.ASSET_TYPE.getConstraints() != null
            ? new ArrayList<>(Arrays.asList(ValueType.ASSET_TYPE.getConstraints()))
            : new ArrayList<>();
    constraints.removeIf(vc -> vc instanceof ValueConstraint.AllowedValues);
    constraints.add(
        new ValueConstraint.AllowedValues(
            Arrays.stream(ValueUtil.getAssetInfos(null))
                .map(ati -> ati.getAssetDescriptor().getName())
                .toArray()));
