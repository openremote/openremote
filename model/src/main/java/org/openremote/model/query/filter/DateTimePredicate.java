 ...
 * yyyy-MM-dd'T'HH:mm:ssZ or yyyy-MM-dd'T'HH:mm:ss\u00b1HH:mm), offset and time are optional, if no
 * offset information is supplied then UTC is assumed.
 */
...
public class DateTimePredicate extends ValuePredicate {
  ...
  public String
      value; // Sliding window value as ISO8601 duration e.g. PT1H or fixed date time in ISO 8601
  public String
      rangeValue; // Sliding window value as ISO8601 duration e.g. PT1H or fixed date time (used as
  // upper bound when Operator.BETWEEN)
  ...
  public Predicate<Object> asPredicate(Supplier<Long> currentMillisSupplier) {
    return obj ->
        ValueUtil.getValueCoerced(obj, Date.class)
            .map(
                date -> {
