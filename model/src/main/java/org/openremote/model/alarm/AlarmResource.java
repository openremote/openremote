...
public interface AlarmResource {
  ...
  SentAlarm[] getAlarms(
      ...
      @Parameter(
          ...
          @QueryParam("realm")
          String realm,
      @Parameter(description = "Alarm lifecycle status to match.", example = "OPEN")
          @QueryParam("status")
          Alarm.Status status,
      @Parameter(description = "Return alarms linked to this asset.", example = EXAMPLE_ASSET_ID)
          @QueryParam("assetId")
          String assetId,
      @Parameter(
              description = "Return alarms assigned to this identity-provider user.",
  ...
  SentAlarm createAlarm(
      ...
      @RequestBody(
              required = true,
              description =
                  "Alarm to create. The source and sourceId are assigned from the authenticated caller.",
              content =
                  @Content(
                      ...
                      examples =
                          @ExampleObject(
                              ...
                              summary = "Create an open high-severity alarm",
                              value = ALARM_CREATE)))
          Alarm alarm,
      @Parameter(
              description =
                  "Asset IDs to link to the new alarm. Repeat the query parameter for multiple assets.",
  ...
  void removeAlarms(
      @BeanParam RequestParams requestParams,
      @RequestBody(required = true, description = "Numeric IDs of the alarms to remove.")
          List<Long> ids);
  ...
  SentAlarm getAlarm(
      @BeanParam RequestParams requestParams,
      @Parameter(description = "Numeric alarm identifier.", example = "1234") @PathParam("alarmId")
          Long alarmId);
  ...
  List<AlarmAssetLink> getAssetLinks(
      ...
      @Parameter(description = "Numeric alarm identifier.", example = "1234") @PathParam("alarmId")
          Long alarmId,
      @Parameter(description = REALM, example = EXAMPLE_REALM, required = true) @QueryParam("realm")
          String realm);
  ...
  void setAssetLinks(
      ...
      @RequestBody(
              required = true,
              description =
                  "Alarm asset links to add. All items must use the same `id.realm` and `id.alarmId`.",
              content =
                  @Content(
                      mediaType = APPLICATION_JSON,
