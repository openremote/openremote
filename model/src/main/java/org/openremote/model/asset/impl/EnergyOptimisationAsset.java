...
public class EnergyOptimisationAsset extends Asset<EnergyOptimisationAsset> {
    ...
    public static final AttributeDescriptor<Integer> FINANCIAL_WEIGHTING = new AttributeDescriptor<>(
            "financialWeighting", ValueType.POSITIVE_INTEGER).withUnits(UNITS_PERCENTAGE)
            .withConstraints(new ValueConstraint.Min(0), new ValueConstraint.Max(100));
    public static final AttributeDescriptor<Double> INTERVAL_SIZE = new AttributeDescriptor<>("intervalSize",
            POSITIVE_NUMBER);
    public static final AttributeDescriptor<Boolean> OPTIMISATION_DISABLED = new AttributeDescriptor<>(
            "optimisationDisabled", BOOLEAN, new MetaItem<>(READ_ONLY));
    public static final AttributeDescriptor<Double> FINANCIAL_SAVING = new AttributeDescriptor<>("financialSaving",
            NUMBER, new MetaItem<>(READ_ONLY)).withUnits("EUR");
    public static final AttributeDescriptor<Double> CARBON_SAVING = new AttributeDescriptor<>("carbonSaving", NUMBER,
            new MetaItem<>(READ_ONLY)).withUnits(UNITS_KILO, UNITS_GRAM);

    public static final AssetDescriptor<EnergyOptimisationAsset> DESCRIPTOR = new AssetDescriptor<>("flash", "C4DB0D",
            EnergyOptimisationAsset.class);
