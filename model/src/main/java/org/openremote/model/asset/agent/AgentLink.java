...
/**
 * Represents the configuration of an {@link Attribute} linked to an {@link Agent}; each {@link
 * Agent} should have its' own concrete implementation of this class or use {@link DefaultAgentLink}
 * with fields describing each configuration item and standard JSR-380 annotations should be used to
...
@JsonTypeInfo(
    ...
    defaultImpl = DefaultAgentLink.class)
...
public abstract class AgentLink<T extends AgentLink<?>> implements Serializable {
  ...
  public static class AgentLinkSerializer extends StdSerializer<AgentLink<?>> {
    ...
    @Override
    public void serialize(AgentLink<?> value, JsonGenerator gen, SerializationContext context)
      ...
      for (Class<?> type = value.getClass();
          type != null && type != AgentLink.class;
    ...
    public void serializeWithType(
        AgentLink<?> value, JsonGenerator gen, SerializationContext context, TypeSerializer typeSer)
        throws JacksonException {
      serialize(value, gen, context);
    }

    protected void writeFields(
        AgentLink<?> value, Class<?> type, JsonGenerator gen, SerializationContext context)
        throws JacksonException {
      for (Field field : type.getDeclaredFields()) {
        ...
        try {
          ...
          throw DatabindException.from(
              gen, "Failed to serialize agent link field: " + field.getName());
        }
      }
    }
  }
  ...
  public static class AgentLinkDeserializer extends StdDeserializer<AgentLink<?>> {
    ...
    @Override
    public AgentLink<?> deserialize(JsonParser jp, DeserializationContext ctxt)
        throws JacksonException {
      JsonNode node = ctxt.readTree(jp);
      String type =
          node.has("type") && !node.get("type").isNull()
              ? node.get("type").asText()
              : DefaultAgentLink.class.getSimpleName();
      ...
      try {
        ...
        for (Class<?> fieldType = agentLinkClass;
            fieldType != null && fieldType != AgentLink.class;
        ...
        throw DatabindException.from(
            jp, "Failed to deserialize agent link type '" + type + "': " + e.getMessage());
      }
    }
    ...
    protected AgentLink<?> newAgentLink(Class<? extends AgentLink<?>> agentLinkClass, JsonNode node)
    ...
    protected void readFields(AgentLink<?> agentLink, Class<?> type, JsonNode node)
  ...
  @JsonSchemaDescription(
      "Defines ValueFilters to apply to an incoming value before it is written to a"
  ...
  @JsonSchemaDescription(
      "Defines a value converter map to allow for basic value type conversion; the incoming value"
  ...
  @JsonSchemaDescription(
      "Similar to valueFilter but will be applied to outgoing values allowing for inverse filtering.")
  @JsonProperty
  ...
  @JsonSchemaDescription(
      "String to be used for attribute writes and can contain dynamic placeholders to allow dynamic"
  ...
  @JsonSchemaDescription(
      "The predicate to apply to incoming messages to determine if the message is intended for the"
