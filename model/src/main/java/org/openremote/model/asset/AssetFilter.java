...
public class AssetFilter<T extends SharedEvent & AssetInfo> implements EventFilter<T> {
  ...
  public T apply(T event) {
    ...
    if (valueChanged
        && event instanceof AttributeEvent attributeEvent
    ...
    if (restrictedEvents) {
      if (event instanceof AttributeEvent attributeEvent) {
        ...
        attributeFilter =
            (attr) -> attr.getMetaValue(MetaItemType.ACCESS_RESTRICTED_READ).orElse(false);
      }
    }
    ...
    if (publicEvents) {
      if (event instanceof AttributeEvent attributeEvent) {
        ...
        if (!assetEvent.isAccessPublicRead()) {
          ...
          attributeFilter =
              (attr) -> attr.getMetaValue(MetaItemType.ACCESS_PUBLIC_READ).orElse(false);
        }
      }
    }
  ...
  public String toString() {
    return getClass().getSimpleName()
        + "{"
        + "assetIds='"
        + (assetIds != null ? String.join(",", assetIds) : "")
        + '\''
        + "assetTypes='"
        + (assetTypes != null ? String.join(",", assetTypes) : "")
        + '\''
        + ", parentIds='"
        + (parentIds != null ? String.join(",", parentIds) : "")
        + '\''
        + ", realm='"
        + realm
        + '\''
        + ", attributeNames='"
        + (attributeNames != null ? String.join(",", attributeNames) : "")
        + '\''
        + '}';
  }
}
