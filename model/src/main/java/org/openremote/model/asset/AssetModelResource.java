...
public interface AssetModelResource {
  ...
  AssetTypeInfo[] getAssetInfos(
      ...
      @Parameter(
              description = "Optional parent asset used to select the local or gateway model",
              example = EXAMPLE_ASSET_ID)
          @QueryParam("parentId")
          String parentId,
      @Parameter(
              description =
                  "Optional parent asset type, primarily for assets that have not been persisted yet",
              example = "GroupAsset")
          @QueryParam("parentType")
          String parentType);
  ...
  @ApiResponse(
      responseCode = "204",
      description = "No model information is available for the requested asset type")
  AssetTypeInfo getAssetInfo(
      ...
      @Parameter(
          ...
          @QueryParam("parentId")
          String parentId,
      @Parameter(description = "Asset type name", example = "ThingAsset", required = true)
          @PathParam("assetType")
          String assetType);
   ...
   * descriptor schema is only meant to be retrieved once per client. Either when a new {@code name}
   * is requested or the "If-None-Match" header does not match the current ETag. The HTTP client
   * should use the provided ETag to cache the response.
   */
  @GET
  ...
  @Operation(
      ...
      summary = "Retrieve the JSON Schema of the specified value descriptor",
      description =
  ...
  @ApiResponse(
      responseCode = "200",
      description = "The current JSON Schema",
      headers =
          @Header(
              name = "ETag",
              description = "Weak entity tag identifying this schema version",
              schema = @Schema(type = "string", example = "W/\"17af8b28\"")),
      content = @Content(mediaType = APPLICATION_JSON, schema = @Schema(type = "object")))
  @ApiResponse(
      responseCode = "304",
      description = "The supplied ETag still identifies the current schema",
      headers =
          @Header(
              name = "ETag",
              description = "Entity tag supplied by the client",
              schema = @Schema(type = "string")))
  @OpenApiResponses.NotFound
  Response getValueDescriptorSchema(
      @Parameter(description = "Value descriptor name", example = "number", required = true)
          @QueryParam("name")
          String name,
