...
/**
 * Represents an event that occurs at a point in time with a {@link #start}, {#link #end} and
 * optional {@link #recurrence} and loosely follows the following standard for JSEvent. If {@link
 * #end} is not specified then it is assumed the event never ends and so only the {@link #start} is
 * important.
 *
 * <p>CRON expressions were ruled out because intervals are not possible when using days of the week
 * (e.g. every other Thursday).
 *
 * <p>Note that unix timestamp milliseconds is used everywhere as time zone information can be
 * obtained from the associated {@link Asset}; {@link #start} and {@link #end} are required but
 * {@link #recurrence} is optional:
 *
 * <ul>
 *   <li><a href="https://datatracker.ietf.org/doc/draft-ietf-calext-jscalendarbis/">JSCalendar</a>
 * </ul>
 *
 * <p>Example 1 (One day, one time only):
 *
 * <blockquote>
 *
 * <pre>{@code
...
public class CalendarEvent implements Serializable {
  ...
  @com.fasterxml.jackson.databind.annotation.JsonSerialize(
      converter = RecurStringConverterJackson2.class)
  ...
  public static class RecurStringConverterJackson2
      extends com.fasterxml.jackson.databind.util.StdConverter<Recur<?>, String> {
  ...
  public Pair<Long, Long> getNextOrActiveFromTo(Date when) {
    ...
    if (getStart().before(when)
        && getEnd().after(when)
        && (getEnd().getTime() - when.getTime() > 1000)) {
    ...
    Instant whenMillis =
        when.toInstant().minus(getEnd().getTime() - getStart().getTime(), ChronoUnit.MILLIS);
    List<LocalDateTime> matches =
        recurrence.getDates(
    ...
    if (endTime <= when.getTime()) {
      if (instants.size() == 2) {
        return new Pair<>(
            instants.get(1).toEpochMilli(),
            instants.get(1).toEpochMilli() + (getEnd().getTime() - getStart().getTime()));
      }
      return null;
    }
