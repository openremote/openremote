...
public class MapSourceConfig {
  ...
  /**
   * An array containing the longitude and latitude of the southwest and northeast corners of the
   * source's bounding box in the following order: <code>[sw.lng, sw.lat, ne.lng, ne.lat]</code>.
   * When this property is included in a source, no tiles outside of the given bounds are requested
   * by MapLibre.
   *
   * @implSpec Optional array. Possible values: vector.
   */
   ...
   * Mercator) profile is assumed.
   *
   * <p>
   *
   * <ul>
   *   <li><code>xyz</code>: Slippy map tilenames scheme.
   *   <li><code>tms</code>: OSGeo spec scheme.
   * </ul>
   *
   * @implSpec Optional enum. Possible values: <code>xyz</code>, <code>tms</code>. Defaults to
   *     <code>xyz</code>.
   */
  ...
  /**
   * Maximum zoom level for which tiles are available, as in the TileJSON spec. Data from tiles at
   * the maxzoom are used when displaying the map at higher zoom levels.
   *
   ...
   *
   * <p>The Maplibre sources follow the TileJSON specification. The TileJSON specification allows
   * additional properties not in the spec. Those properties MUST be ignored by MapLibre, but can be
   * used in OpenRemote.
   *
