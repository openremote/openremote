...
public class CalendarEventTest {
    ...
    public class TryAdvanceActive {
        ...
        public void shouldReturnSomeForever() {
            ...
            CalendarEvent event = new CalendarEvent(Date.from(start), null, (Recur<LocalDateTime>) null);
        ...
        public void shouldReturnEmptyAt() {
            ...
            CalendarEvent event = new CalendarEvent(Date.from(start), Date.from(end), (Recur<LocalDateTime>) null);
        ...
        public void shouldReturnSomeUntil() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent event = new CalendarEvent(Date.from(start), null, "FREQ=DAILY;UNTIL=20000105T000000");
        ...
        public void shouldReturnNoneAfter() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent event = new CalendarEvent(Date.from(start), null, "FREQ=DAILY;COUNT=4");
        ...
        public void shouldStartAt1730() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent event = new CalendarEvent(Date.from(start), null, "FREQ=DAILY;BYHOUR=17;BYMINUTE=30");
        ...
        public void shouldMinutelyReturnSomeUntil() {
            ...
            CalendarEvent event = new CalendarEvent(Date.from(start), null, "FREQ=MINUTELY;UNTIL=20000101T000500");
        ...
        public void shouldCatchUpWithCurrentTime() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent event = new CalendarEvent(Date.from(start), null, "FREQ=MINUTELY");
    ...
    public class GetDelay {
        ...
        public void getDelayForHourlyRecurrence() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent schedule = new CalendarEvent(Date.from(start), null, "FREQ=HOURLY");

            long now = start.getEpochSecond();
            long timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(100, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());
            ...
            assertEquals(1, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());

            now = Instant.parse("2000-01-01T00:01:40.000Z").getEpochSecond();
            timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(3600, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());
        }
        ...
        public void getDelayForHourlyRecurrenceWithStartDate() {
            ...
            long timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(DAY + 100, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());

            now = Instant.parse("2000-01-01T00:01:00.000Z").getEpochSecond();
            timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(DAY + 40, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());

            now = Instant.parse("2000-01-01T00:02:00.000Z").getEpochSecond();
            timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(DAY - 20, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());
        }

        @Test
        public void getDelayCustomRecurringWithUntil() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent schedule = new CalendarEvent(Date.from(start), null, "FREQ=HOURLY;UNTIL=20000101T020000Z");
            ...
            now = Instant.parse("2000-01-01T00:01:00.000Z").getEpochSecond();
            timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(40, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());

            now = Instant.parse("2000-01-01T00:02:00.000Z").getEpochSecond();
            timeSinceOccurrenceStarted = now - schedule.tryAdvanceActive(now).getAsLong();
            assertEquals(3580, CalendarEvent.getDelay(100, timeSinceOccurrenceStarted, schedule).getAsLong());
        ...
        public void getDelayCustomRecurringWithCount() {
            Instant start = Instant.parse("2000-01-01T00:00:00.000Z");
            CalendarEvent schedule = new CalendarEvent(Date.from(start), null, "FREQ=HOURLY;COUNT=3");
