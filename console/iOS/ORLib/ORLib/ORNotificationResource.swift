...
public class ORNotificationResource: NSObject, URLSessionDelegate {
    ...
    public func notificationDelivered(notificationId : Int64, targetId : String) {
        guard let urlRequest = URL(string: "\(ORServer.scheme)://\(ORServer.hostURL)/\(ORServer.realm)/notification/\(notificationId)/delivered?targetId=\(targetId)") else { return }
    ...
    public func notificationAcknowledged(notificationId : Int64, targetId : String, acknowledgement: String) {
        guard let urlRequest = URL(string: "\(ORServer.scheme)://\(ORServer.hostURL)/\(ORServer.realm)/notification/\(notificationId)/acknowledged?targetId=\(targetId)") else { return }
        let request = NSMutableURLRequest(url: urlRequest)
        request.httpMethod = "PUT"
