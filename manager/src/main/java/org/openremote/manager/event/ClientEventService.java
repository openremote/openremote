...
public class ClientEventService extends RouteBuilder implements ContainerService {
  ...
  protected final Map<String, Map<String, Consumer<? extends Event>>>
      websocketSessionSubscriptionConsumers = new HashMap<>();
  ...
  public void configure() throws Exception {
    ...
    from(WEBSOCKET_URI)
        ...
        .threads()
        .executorService(
            executorService) // Ensure processing is not done on the WebService I/O threads
        .choice()
              ...
              switch (eventType) {
                case ONOPEN -> {
                  WebSocketHttpExchange httpExchange =
                      exchange
                          .getIn()
                          .getHeader(UndertowConstants.EXCHANGE, WebSocketHttpExchange.class);
                  ...
                  // Set an idle timeout value for service account connections; browsers don't
                  // reliably support
                  // ping/pong frames so we can't be too aggressive with those connections but other
                  // clients
                  ...
                  LOG.log(
                      DEBUG, "Client connection created: " + webSocketChannel.getSourceAddress());
                }
                case ONCLOSE -> {
                  AuthContext authContext =
                      (AuthContext) webSocketChannel.getAttribute(Constants.AUTH_CONTEXT);
                  ...
                  LOG.log(
                      DEBUG, "Client connection closed: " + webSocketChannel.getSourceAddress());
        ...
        .process(
            exchange -> {
              AuthContext authContext =
                  exchange.getIn().getHeader(Constants.AUTH_CONTEXT, AuthContext.class);
              ...
              if (exchange.getIn().getBody() instanceof EventSubscription<?> subscription) {
                ...
                LOG.log(
                    TRACE,
                    () -> "Adding subscription for session '" + sessionKey + "': " + subscription);

                if (!authorizeEventSubscription(realm, authContext, subscription)) {
                  sendToWebsocketSession(
                      sessionKey, new UnauthorizedEventSubscription<>(subscription));
                ...
                synchronized (websocketSessionSubscriptionConsumers) {
                  ...
                  Map<String, Consumer<? extends Event>> subscriptionConsumers =
                      websocketSessionSubscriptionConsumers.computeIfAbsent(
                          sessionKey, (s) -> new HashMap<>());
                  String subscriptionKey =
                      subscription.getEventType() + subscription.getSubscriptionId();
