...
public class AssetPredictedDatapointService
    ...
    clientEventService.addSubscriptionAuthorizer(
        (requestedRealm, auth, subscription) -> {
          ...
          if (identityService.getIdentityProvider().isRestrictedUser(auth)) {
            EventFilter<AssetPredictedDatapointEvent> existingFilter =
                predictedDatapointSubscription.getFilter();
            predictedDatapointSubscription.setFilter(
                event -> {
                  AssetPredictedDatapointEvent filteredEvent =
                      existingFilter == null ? event : existingFilter.apply(event);
                  ...
                  Attribute<?> attribute =
                      asset != null ? asset.getAttribute(attributeName).orElse(null) : null;
                  if (attribute == null
                      || !attribute
                          .getMeta()
          ...
          return (auth.isSuperUser()
              ...
              && identityService
                  .getIdentityProvider()
                  .isRealmActiveAndAccessible(auth, requestedRealm);
        });
  ...
  protected void publishPredictedDatapointsEvent(String assetId, String attributeName) {
    ...
    clientEventService.publishEvent(
        new AssetPredictedDatapointEvent(
            new AttributeRef(assetId, attributeName), timerService.getNow()));
  }
