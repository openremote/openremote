...
public class AssetDatapointService extends AbstractDatapointService<AssetDatapoint>
                    ...
                    em.unwrap(Session.class)
                        .doWork(
                            connection -> {
                              ...
                              // Create temp table and insert attribute refs safely via
                              // PreparedStatement
                              ...
                              try (Statement statement = connection.createStatement()) {
                                statement.execute(
                                    "create temp table "
                                        + tempTableName
                                        + " (entity_id text, attribute_name text) on commit drop");
                              }
                              ...
                              try (PreparedStatement insertStatement =
                                  connection.prepareStatement(
                                      "insert into "
                                          + tempTableName
                                          + " (entity_id, attribute_name) values (?, ?)")) {
                                for (AttributeRef attributeRef : attributeRefs) {
                                  insertStatement.setString(
                                      1, validateAssetId(attributeRef.getId()));
  ...
  private String buildCopyToStdoutQuery(
    ...
    final String TO_STDOUT_WITH_CSV_FORMAT =
  ...
  /**
   * Function for building CSV attribute headers when format will make a separate column per
   * attribute. Uses TreeSet for alphabetical ordering required by crosstab queries.
   */
  ...
  protected ExportQuery getSelectExportQuery(
    ...
    StringBuilder sb =
        new StringBuilder(
            "select ad.timestamp, a.name, ad.attribute_name, value from asset_datapoint ad, asset a where ad.entity_id = a.id and ad.timestamp >= to_timestamp(?) and ad.timestamp <= to_timestamp(?) and ");
