...
public class AssetPredictedDatapointResourceImpl extends ManagerWebResource
    implements AssetPredictedDatapointResource {

  private static final Logger LOG =
      Logger.getLogger(AssetPredictedDatapointResourceImpl.class.getName());

  protected final AssetStorageService assetStorageService;
  protected final AssetPredictedDatapointService assetPredictedDatapointService;

  public AssetPredictedDatapointResourceImpl(
  ...
  @Override
  public ValueDatapoint<?>[] getPredictedDatapoints(
  ...
  @Override
  public void writePredictedDatapoints(
    ...
    try {
      ...
      if (isRestrictedUser()) {
        attribute
            .getMeta()
            .getValue(MetaItemType.ACCESS_RESTRICTED_WRITE)
            .ifPresentOrElse(
                (v) -> {
                  if (!v) {
                    throw new WebApplicationException(Response.Status.FORBIDDEN);
      ...
      if (!isAuthenticated()) {
        attribute
            .getMeta()
            .getValue(MetaItemType.ACCESS_PUBLIC_WRITE)
            .ifPresentOrElse(
                (v) -> {
                  if (!v) {
                    throw new WebApplicationException(Response.Status.FORBIDDEN);
      ...
      assetPredictedDatapointService.updateValues(
          assetId, attributeName, Arrays.asList(predictedDatapoints));
