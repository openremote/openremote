...
public class DashboardResourceImpl extends ManagerWebResource implements DashboardResource {
  ...
  public Dashboard[] query(RequestParams requestParams, DashboardQuery query) {
    ...
    if (isAuthenticated()
        && !isSuperUser()
        && !(getAuthenticatedRealmName().equals(query.realm.name))) {
      throw new WebApplicationException(FORBIDDEN);
    }
  ...
  protected DashboardQuery createDashboardQuery(String realm) {
    ...
    Set<DashboardAccess> userViewAccess =
        new HashSet<>(Set.of(query.conditions.getDashboard().getViewAccess()));
    Set<DashboardAccess> userEditAccess =
        new HashSet<>(Set.of(query.conditions.getDashboard().getEditAccess()));
    Set<DashboardQuery.AssetAccess> assetAccess =
        new HashSet<>(Set.of(query.conditions.getAsset().getAccess()));
