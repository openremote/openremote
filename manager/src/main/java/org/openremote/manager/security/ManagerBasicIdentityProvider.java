...
public class ManagerBasicIdentityProvider extends BasicIdentityProvider
          ...
          em ->
              em.unwrap(Session.class)
                  .doWork(
                      connection -> {
                        String sql =
                            "insert into PUBLIC.REALM(ID, NAME, ENABLED) values ('master', 'master', true)";
                        PreparedStatement st = connection.prepareStatement(sql);
                        st.executeUpdate();
                        st.close();

                        sql =
                            "insert into PUBLIC.REALM_ATTRIBUTE(REALM_ID, NAME, VALUE) values ('master', 'displayName', 'Master')";
  ...
  public User createUpdateUser(String realm, User user, String password, boolean allowUpdate) {
    if (!realm.equals(MASTER_REALM)) {
      throw new UnsupportedOperationException(
          "This provider does not support realms other than master");
    }
        ...
        em ->
            em.unwrap(Session.class)
                .doWork(
                    connection -> {
                      String sql =
                          "insert into PUBLIC.USER_ENTITY(ID, REALM_ID, USERNAME, PASSWORD, FIRST_NAME, LAST_NAME, EMAIL, ENABLED) values (?, ?, ?, ?, ?, ?, ?, ?)";
                      if (allowUpdate) {
  ...
  public void updateUserRealmRoles(String realm, String userId, String... roles) {
    throw new UnsupportedOperationException(
        "This provider does not support updating user realm roles");
  }
