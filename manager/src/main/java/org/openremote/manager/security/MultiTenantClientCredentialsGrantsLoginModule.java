...
/**
 * A version of the Keycloak {@link org.keycloak.adapters.jaas.AbstractKeycloakLoginModule} that
 * supports client credentials grant and lookup of {@link org.keycloak.adapters.KeycloakDeployment}
 * by using the {@link KeycloakDeploymentCallback}. Copied from {@link
 * org.keycloak.adapters.jaas.DirectAccessGrantsLoginModule} which has private members
 * un-fortunately.
 *
 * <p>Also supports including the realm roles (as well as resource roles) by setting the {@link
...
public class MultiTenantClientCredentialsGrantsLoginModule extends AbstractKeycloakLoginModule {
  ...
  private static final Logger log =
      Logger.getLogger(MultiTenantClientCredentialsGrantsLoginModule.class);
  ...
  public void initialize(
    ...
    Iterator<MultiTenantClientCredentialsGrantsLoginModule.RefreshTokenHolder> iterator =
        subject
            .getPrivateCredentials(
                MultiTenantClientCredentialsGrantsLoginModule.RefreshTokenHolder.class)
            .iterator();
    if (iterator.hasNext()) {
  ...
  @Override
  protected Auth doAuth(String username, String password)
  ...
  public boolean commit() throws LoginException {
    ...
    if (refreshToken != null) {
      MultiTenantClientCredentialsGrantsLoginModule.RefreshTokenHolder refreshTokenHolder =
          new MultiTenantClientCredentialsGrantsLoginModule.RefreshTokenHolder();
  ...
  protected Auth clientCredentialsAuth(String username, String password)
      throws IOException, VerificationException {
    String authServerBaseUrl = deployment.getAuthServerBaseUrl();
    HttpPost post = new HttpPost(deployment.getTokenUrl());
    List<NameValuePair> formparams = new ArrayList<NameValuePair>();
    formparams.add(
        new BasicNameValuePair(OAuth2Constants.GRANT_TYPE, OAuth2Constants.CLIENT_CREDENTIALS));
    ...
    if (status != 200) {
      ...
      if (entity != null) {
        ...
        OAuth2ErrorRepresentation errorRep =
            JsonSerialization.readValue(is, OAuth2ErrorRepresentation.class);
        errorBuilder
            .append(", OAuth2 error. Error: " + errorRep.getError())
    ...
    AdapterTokenVerifier.VerifiedTokens tokens =
        AdapterTokenVerifier.verifyTokens(
            tokenResponse.getToken(), tokenResponse.getIdToken(), deployment);
  ...
  protected Auth postTokenVerification(String tokenString, AccessToken token) {
    ...
    RefreshableKeycloakSecurityContext skSession =
        new RefreshableKeycloakSecurityContext(
            deployment, null, tokenString, token, null, null, null);
    ...
    final KeycloakPrincipal<RefreshableKeycloakSecurityContext> principal =
        new KeycloakPrincipal<RefreshableKeycloakSecurityContext>(principalName, skSession);
    Set<String> roles;
      ...
      try {
        ...
        if (status != 204) {
          StringBuilder errorBuilder =
              new StringBuilder("Logout of refreshToken failed. Invalid status: " + status);
          if (entity != null) {
            InputStream is = entity.getContent();
            if (status == 400) {
              OAuth2ErrorRepresentation errorRep =
          ...
          // Should do something better than warn if logout failed? Perhaps update of refresh tokens
          // on existing subject might be supported too...
