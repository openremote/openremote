...
public class UserResourceImpl extends ManagerWebResource implements UserResource {
    ...
    public User[] query(RequestParams requestParams, UserQuery query) {
        ...
        boolean isRestricted = !isAdmin
                && authContext.hasResourceRole(ClientRole.READ_USERS.getValue(), Constants.KEYCLOAK_CLIENT_ID);
        ...
        if (!authContext.isSuperUser()) {
            ...
            if (query.attributes == null) {
                query.attributes(new UserQuery.AttributeValuePredicate(true,
                        new StringPredicate(User.SYSTEM_ACCOUNT_ATTRIBUTE), null));
            } else {
                List<UserQuery.AttributeValuePredicate> attributeValuePredicates = new ArrayList<>(
                        Arrays.asList(query.attributes));
                attributeValuePredicates.add(new UserQuery.AttributeValuePredicate(true,
                        new StringPredicate(User.SYSTEM_ACCOUNT_ATTRIBUTE), null));
    ...
    @Override
    public void resetPassword(@BeanParam RequestParams requestParams, String realm, String userId,
    ...
    @Override
    public Role[] getUserClientRoles(@BeanParam RequestParams requestParams, String realm, String userId,
            String clientId) {
        boolean hasAdminReadRole = hasResourceRole(ClientRole.READ_ADMIN.getValue(), Constants.KEYCLOAK_CLIENT_ID);
    ...
    @Override
    public void updateUserClientRoles(@BeanParam RequestParams requestParams, String realm, String userId, Role[] roles,
            String clientId) {
        try {
            identityService.getIdentityProvider().updateUserRoles(realm, userId, clientId,
                    Arrays.stream(roles).filter(Role::isAssigned).map(Role::getName).toArray(String[]::new));
        } catch (ClientErrorException ex) {
    ...
    public void updateUserRealmRoles(RequestParams requestParams, String realm, String userId, Role[] roles) {
        try {
            identityService.getIdentityProvider().updateUserRealmRoles(realm, userId,
                    Arrays.stream(roles).filter(Role::isAssigned).map(Role::getName).toArray(String[]::new));
        } catch (ClientErrorException ex) {
            ex.printStackTrace(System.out);
            throw new WebApplicationException(ex.getCause(), ex.getResponse().getStatus());
    ...
    public UserSession[] getUserSessions(RequestParams requestParams, String realm, String userId) {
        ...
        if (!hasAdminReadRole && !Objects.equals(getUserId(), userId)) {
            throw new ForbiddenException(
                    "Can only retrieve own user sessions unless you have role '" + ClientRole.READ_ADMIN + "'");
        }
        ...
        return mqttBrokerService.getUserConnections(userId).stream().map(connection -> new UserSession(
                ...
                connection.getSubject() != null ? KeycloakIdentityProvider.getSubjectName(connection.getSubject())
                        : userId,
                connection.getCreationTime(), connection.getRemoteAddress())).toArray(UserSession[]::new);
    }
    ...
    protected void throwIfIllegalMasterAdminUserDeletion(RequestParams requestParams, String realm, String userId)
    ...
    protected void throwIfIllegalMasterAdminUserMutation(RequestParams requestParams, String realm, User user)
            throws WebApplicationException {
        if (!realm.equals(MASTER_REALM)) {
            return;
        }
