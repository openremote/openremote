...
public class UserResourceImpl extends ManagerWebResource implements UserResource {
  ...
  protected static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.API, UserResourceImpl.class.getName());
  ...
  public User[] query(RequestParams requestParams, UserQuery query) {
    ...
    boolean isAdmin =
        authContext.hasResourceRole(ClientRole.READ_ADMIN.getValue(), Constants.KEYCLOAK_CLIENT_ID);
    boolean isRestricted =
        !isAdmin
            && authContext.hasResourceRole(
                ClientRole.READ_USERS.getValue(), Constants.KEYCLOAK_CLIENT_ID);
    ...
    if (!authContext.isSuperUser()) {
      throwIfNotRealmActiveAndAccessible(authContext.getAuthenticatedRealmName());
      ...
      if (query.attributes == null) {
        query.attributes(
            new UserQuery.AttributeValuePredicate(
                true, new StringPredicate(User.SYSTEM_ACCOUNT_ATTRIBUTE), null));
      } else {
        List<UserQuery.AttributeValuePredicate> attributeValuePredicates =
            new ArrayList<>(Arrays.asList(query.attributes));
        attributeValuePredicates.add(
            new UserQuery.AttributeValuePredicate(
                true, new StringPredicate(User.SYSTEM_ACCOUNT_ATTRIBUTE), null));
        query.attributes(
            attributeValuePredicates.toArray(UserQuery.AttributeValuePredicate[]::new));
      }
    }
  ...
  public User get(RequestParams requestParams, String realm, String userId) {
    ...
    if (!hasAdminReadRole && !Objects.equals(getUserId(), userId)) {
      throw new ForbiddenException(
          "Can only retrieve own user info unless you have role '" + ClientRole.READ_ADMIN + "'");
    }
  ...
  public void updatePassword(
      @BeanParam RequestParams requestParams, String realm, String userId, Credential credential) {
    throwIfNotSameRealm(realm, userId);
  ...
  @Override
  public String[] getUserClientRoles(
      @BeanParam RequestParams requestParams, String realm, String userId, String clientId) {
    throwIfCannotReadUserInRealm(realm, userId);
    ...
    if (!hasAdminReadRole && !Objects.equals(getUserId(), userId)) {
      throw new ForbiddenException(
          "Can only retrieve own user roles unless you have role '" + ClientRole.READ_ADMIN + "'");
    }
  ...
  public void updateUserClientRoles(
    ...
    try {
      identityService.getIdentityProvider().updateUserClientRoles(realm, userId, clientId, roles);
    } catch (ClientErrorException ex) {
  ...
  @Override
  public void updateUserRealmRoles(
      RequestParams requestParams, String realm, String userId, String[] roles) {
    throwIfCannotAdminRealm(realm);

    try {
      identityService.getIdentityProvider().updateUserRealmRoles(realm, userId, roles);
    } catch (ClientErrorException ex) {
  ...
  public Role[] getClientRoles(RequestParams requestParams, String realm, String clientId) {
    ...
    try {
      return identityService.getIdentityProvider().getClientRoles(realm, clientId);
    } catch (ClientErrorException ex) {
      throw new WebApplicationException(ex.getCause(), ex.getResponse().getStatus());
    } catch (Exception ex) {
      throw new WebApplicationException(ex);
  ...
  @Override
  public void updateClientRoles(
      RequestParams requestParams, String realm, Role[] roles, String clientId) {
    throwIfCannotAdminRealm(realm);

    try {
      identityService.getIdentityProvider().updateClientRoles(realm, clientId, roles);
    } catch (ClientErrorException ex) {
  ...
  public UserSession[] getUserSessions(RequestParams requestParams, String realm, String userId) {
    ...
    if (!hasAdminReadRole && !Objects.equals(getUserId(), userId)) {
      throw new ForbiddenException(
          "Can only retrieve own user sessions unless you have role '"
              + ClientRole.READ_ADMIN
              + "'");
    }

    return mqttBrokerService.getUserConnections(userId).stream()
        .map(
            connection ->
                new UserSession(
                    MQTTBrokerService.getConnectionIDString(connection),
                    connection.getSubject() != null
                        ? KeycloakIdentityProvider.getSubjectName(connection.getSubject())
                        : userId,
  ...
  public void disconnectUserSession(RequestParams requestParams, String realm, String sessionID) {
    String connectionUserId = mqttBrokerService.getConnectionUserId(sessionID);
    if (connectionUserId == null) {
      throw new NotFoundException("User session not found");
    }

    User connectionUser = identityService.getIdentityProvider().getUser(connectionUserId);
    if (connectionUser == null) {
      throw new NotFoundException("User not found");
    }

    String connectionRealm = connectionUser.getRealm();

    if (!connectionRealm.equals(realm)) {
      throw new NotFoundException("User session not found in realm");
    }

    throwIfCannotReadUserInRealm(connectionRealm, connectionUserId);

    boolean hasAdminWriteRole =
        hasResourceRoleOrIsSuperUser(
            ClientRole.WRITE_ADMIN.getValue(), Constants.KEYCLOAK_CLIENT_ID);

    if (!hasAdminWriteRole && !Objects.equals(getUserId(), connectionUserId)) {
      throw new ForbiddenException(
          "Can only disconnect own user sessions unless you have role '"
              + ClientRole.WRITE_ADMIN
              + "'");
    }
  ...
  protected void throwIfIllegalMasterAdminUserDeletion(
      RequestParams requestParams, String realm, String userId) throws WebApplicationException {
  ...
  protected void throwIfIllegalMasterAdminUserMutation(
      RequestParams requestParams, String realm, User user) throws WebApplicationException {
    if (!realm.equals(MASTER_REALM)) {
      return;
    }
  ...
  protected void throwIfNotSameRealm(String realm, String userId) throws WebApplicationException {
    ...
    if (!currentUserId.equals(userId)
        && !hasResourceRole(Constants.WRITE_ADMIN_ROLE, Constants.KEYCLOAK_CLIENT_ID)) {
  ...
  protected void throwIfCannotReadUserInRealm(String realm, String userId)
