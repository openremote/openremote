...
public class UserAssetProvisioningMQTTHandler extends MQTTHandler {
  ...
  public void onConnectionLost(RemotingConnection connection) {
    provisioningConfigAuthenticatedConnectionMap
        .values()
        .forEach(connections -> connections.remove(connection));
  }
  ...
  protected void forceClientDisconnects(long provisioningConfigId) {
    provisioningConfigAuthenticatedConnectionMap.computeIfPresent(
        provisioningConfigId,
        (id, connections) -> {
          // Force disconnect of each connection and the disconnect handler will remove the
          // connection from the map
          connections.forEach(
              connection -> {
                try {
                  LOG.fine(
                      "Force disconnecting client that is using provisioning config ID '"
                          + provisioningConfigId
                          + "': "
                          + MQTTBrokerService.connectionToString(connection));
