...
public class MINAGatewayTunnelFactory implements GatewayTunnelFactory {
  ...
  private static final Logger LOG =
      SyslogCategory.getLogger(GATEWAY, MINAGatewayTunnelFactory.class.getName());
  ...
  private class TunnelSessionManager {
      ...
      if (isRunning.compareAndSet(true, false)) {
        ...
        if (session != null) {
          try {
            ...
            LOG.warning(
                "Error closing session during tunnel stop msg=" + e.getMessage() + ": " + this);
          }
        }
      }
    }
    ...
    /**
     * This method blocks waiting for the server to confirm the port forwarding. It MUST NOT be
     * called from an I/O thread.
     */
    private void setupForwarding(ClientSession session) {
      ...
      try {
        String bindAddress =
            tunnelInfo.getType() == GatewayTunnelInfo.Type.TCP ? "" : tunnelInfo.getId();
        int rPort =
            tunnelInfo.getType() == GatewayTunnelInfo.Type.HTTPS
                ? 443
                : tunnelInfo.getType() == GatewayTunnelInfo.Type.HTTP
                    ? 80
                    : tunnelInfo.getAssignedPort();
        String target =
            localhostRewrite != null && "localhost".equals(tunnelInfo.getTarget())
                ? localhostRewrite
    ...
    private void handleFailure(Throwable t) {
      boolean wasInitialFailure =
          initialFuture.completeExceptionally(
    ...
    private void scheduleReconnect() {
      ...
      scheduledExecutor.schedule(
          this::attemptConnection, RECONNECT_DELAY.toMillis(), TimeUnit.MILLISECONDS);
    }
  ...
  public MINAGatewayTunnelFactory(
    ...
    //       client.setGlobalRequestHandlers(Collections.singletonList((connectionService, request,
    // wantReply, buffer) -> {
  ...
  @Override
  public GatewayTunnelSession createSession(
      String hostname, int port, GatewayTunnelInfo tunnelInfo, Consumer<Throwable> closedCallback) {
    LOG.fine(
        "Creating session: hostname=" + hostname + ", port=" + port + ", tunnelInfo=" + tunnelInfo);
    CompletableFuture<Void> connectFuture = new CompletableFuture<>();
    TunnelSessionManager manager =
        new TunnelSessionManager(hostname, port, tunnelInfo, connectFuture);
