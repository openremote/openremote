...
/**
 * Tracks a gateway tunnel session; the {@link #getConnectFuture} should return a future that tracks
 * the initial connection of the tunnel; the {@link GatewayTunnelFactory} is responsible for then
 * reconnecting the tunnel session on failure. If the initial connection fails then the {@link
 * GatewayTunnelFactory} should do nothing other than report the failure via the {@link
 * #getConnectFuture()} exception.
 *
 * <p>The {@link #disconnect} should disconnect the tunnel session and/or cancel any retry logic; if
 * the initial connection failed then this method should do nothing.
 */
