...
public class GatewayClientService extends RouteBuilder implements ContainerService {
  ...
  private static final Logger LOG =
      SyslogCategory.getLogger(GATEWAY, GatewayClientService.class.getName());
  public static final String CLIENT_EVENT_SESSION_PREFIX =
      GatewayClientService.class.getSimpleName() + ":";
  ...
  protected Map<String, Map<AttributeRef, Long>> clientAttributeTimestamps =
  ...
  public void init(Container container) throws Exception {
    ...
    String tunnelKeyFile =
        getString(container.getConfig(), GatewayService.OR_GATEWAY_TUNNEL_SSH_KEY_FILE, null);
    String localhostRewrite =
        getString(container.getConfig(), OR_GATEWAY_TUNNEL_LOCALHOST_REWRITE, null);
    ...
    if (!TextUtil.isNullOrEmpty(tunnelKeyFile)) {
      ...
      if (f.exists()) {
        ...
        LOG.warning(
            "Gateway tunnelling SSH key file does not exist, tunnelling support disabled: "
                + f.getAbsolutePath());
      }
    }
    ...
    container
        ...
        .addApiSingleton(new GatewayClientResourceImpl(timerService, identityService, this));
    ...
    clientEventService.addSubscriptionAuthorizer(
        (realm, authContext, eventSubscription) -> {
          ...
          if (!authContext.isSuperUser()) {
            ...
            EventSubscription<GatewayConnectionStatusEvent> subscription =
                (EventSubscription<GatewayConnectionStatusEvent>) eventSubscription;
            ...
            .doReturningTransaction(
                entityManager ->
                    entityManager
                        ...
                        .getResultList())
            .stream()
            .collect(Collectors.toMap(GatewayConnection::getLocalRealm, gc -> gc)));

    // Create clients for enabled connections
    connectionRealmMap.forEach(
        (realm, connection) -> {
  ...
  public void configure() throws Exception {
    ...
    from(PERSISTENCE_TOPIC)
        ...
        .process(
            exchange -> {
              @SuppressWarnings("unchecked")
              PersistenceEvent<GatewayConnection> persistenceEvent =
  ...
  protected synchronized void processConnectionChange(
      GatewayConnection connection, PersistenceEvent.Cause cause) {
  ...
  protected GatewayIOClient createGatewayClient(GatewayConnection connection) {
    ...
    try {
      ...
      realmAttributeEventConsumer =
          attributeEvent -> sendAttributeEvent(connection, attributeEvent);
  ...
  protected void sendAssetEvent(GatewayConnection connection, AssetEvent event) {
    ...
    sendCentralManagerMessage(
        connection.getLocalRealm(), messageToString(SharedEvent.MESSAGE_PREFIX, event));
  }
  ...
  protected EventFilter<AttributeEvent> getOutboundAttributeEventFilter(
    ...
    if (gatewayConnection.getAttributeFilters() != null
        && !gatewayConnection.getAttributeFilters().isEmpty()) {
      predicatesWithFilters =
          gatewayConnection.getAttributeFilters().stream()
              .map(
                  filter -> {
    ...
    return ev -> {
                    ...
                    if (filter.getDurationParsed().isPresent()) {
                      boolean allow =
                          filter
                              ...
                              .map(
                                  durationMillis -> {
                                    Map<AttributeRef, Long> attributeTimestamps =
                                        clientAttributeTimestamps.get(
                                            gatewayConnection.getLocalRealm());
      ...
      if (allowEvent && gatewayConnection.getAssetSyncRules() != null) {
        GatewayAssetSyncRule syncRule =
            gatewayConnection
                .getAssetSyncRules()
                .getOrDefault(ev.getAssetType(), gatewayConnection.getAssetSyncRules().get("*"));
        if (syncRule != null
  ...
  protected void onGatewayClientConnectionStatusChanged(
      GatewayConnection connection, ConnectionStatus connectionStatus) {
    LOG.info(
        "Connection status change for gateway IO client '" + connectionStatus + "': " + connection);
    clientEventService.publishEvent(
        new GatewayConnectionStatusEvent(
            timerService.getCurrentTimeMillis(), connection.getLocalRealm(), connectionStatus));
  ...
  protected void onCentralManagerMessage(GatewayConnection connection, String message) {
    ...
    if (event != null) {
      if (event instanceof GatewayDisconnectEvent) {
        if (((GatewayDisconnectEvent) event).getReason()
            == GatewayDisconnectEvent.Reason.PERMANENT_ERROR) {
          LOG.info(
              "Central manager requested disconnect due to permanent error (likely this version of the edge gateway software is not compatible with that manager version)");
        ...
        sendCentralManagerMessage(
            connection.getLocalRealm(), messageToString(SharedEvent.MESSAGE_PREFIX, responseEvent));
        ...
        assetProcessingService.sendAttributeEvent(
            (AttributeEvent) event, getClass().getSimpleName());
  ...
  public boolean deleteConnections(List<String> realms) {
    LOG.info(
        "Deleting gateway connections for the following realm(s): "
            + Arrays.toString(realms.toArray()));
      ...
      persistenceService.doTransaction(
          em -> {
            List<GatewayConnection> connections =
                em.createQuery(
                        "select gc from GatewayConnection gc where gc.localRealm in :realms",
            ...
            if (connections.size() != realms.size()) {
              throw new IllegalArgumentException(
                  "Cannot delete one or more requested gateway connections as they don't exist");
            }
