 ...
 */
public class GatewayService extends RouteBuilder implements ContainerService, AttributeEventInterceptor {
    ...
    @Override
    public boolean intercept(EntityManager em, Asset<?> asset, Attribute<?> attribute, boolean outdated, AttributeEvent.Source source) throws AssetProcessingException {
        ...
        if (connector != null) {
            ...
            if (GatewayAsset.CLIENT_SECRET.getName().equals(attribute.getName())) {
                ...
                if (!TextUtil.isNullOrEmpty(newSecret)) {
                    ...
                    assetProcessingService.sendAttributeEvent(new AttributeEvent(asset.getId(), GatewayAsset.CLIENT_SECRET, gatewayAsset.getClientSecret().orElseThrow(() -> new IllegalStateException("Gateway client secret is null which was not expected"))), getClass().getSimpleName());
                }
            }
        } else {
