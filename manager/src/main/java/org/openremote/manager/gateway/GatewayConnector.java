...
public class GatewayConnector {
  ...
  protected GatewayConnector(
    ...
    publishAttributeEvent(
        new AttributeEvent(gatewayId, GatewayAsset.STATUS, ConnectionStatus.DISCONNECTED));
  }
  ...
  protected void connected(
    ...
    publishAttributeEvent(
        new AttributeEvent(gatewayId, GatewayAsset.STATUS, ConnectionStatus.CONNECTING));
    startSync();
  }
    ...
    synchronized (eventConsumerMap) {
      if (eventConsumerMap.containsKey(GatewayCapabilitiesResponseEvent.class)) {
        return CompletableFuture.failedFuture(
            new IllegalArgumentException("A capabilities request is already pending"));
      }
  ...
  protected CompletableFuture<Void> startTunnel(GatewayTunnelInfo tunnelInfo) {
    ...
    if (!isConnected() || isInitialSyncInProgress()) {
      String msg =
          "Gateway is not connected or initial sync in progress so cannot start tunnel: " + this;
      LOG.info(msg);
  ...
  protected synchronized void onSyncAssetsResponse(AssetsEvent e) {
    ...
    if (!expectedSyncResponseName.equalsIgnoreCase(messageId)) {
      LOG.info(
          "Unexpected response from gateway so ignoring (expected="
    ...
    if (isInitialResponse) {
      ...
      if (returnedAssets.size() != requestedAssetIds.size()
        ...
        LOG.warning(
            "Retrieved gateway asset batch count or ID mismatch, attempting to re-send the request: "
                + this);
        syncErrors++;
        requestAssets();
        return;
  ...
  protected void deleteObsoleteLocalAssets() {
    ...
    if (!obsoleteLocalAssetIds.isEmpty()) {
      ...
      if (!deleted) {
        LOG.warning(
            "Failed to delete obsolete local gateway assets; assets are not correctly synced: "
                + this);
      }
    }
  }
    ...
    getCapabilities()
        .whenComplete(
            (response, error) -> {
              if (error != null) {
                LOG.warning(
                    "An error occurred whilst getting the gateway capabilities, assuming no support: "
                        + this);
              }
              ...
              publishAttributeEvent(
                  new AttributeEvent(gatewayId, GatewayAsset.STATUS, ConnectionStatus.CONNECTED));
            });
  }
  ...
  protected <T extends Asset<?>> T saveAssetLocally(T asset) {
    ...
    LOG.fine(
        "Creating/updating gateway asset: Asset ID="
            + assetId
            + ", Asset ID Mapped="
            + asset.getId()
            + ": "
  ...
  protected boolean deleteAssetsLocally(List<String> assetIds) {
    LOG.fine(
        "Removing gateway asset: Asset IDs=" + Arrays.toString(assetIds.toArray()) + ": " + this);
