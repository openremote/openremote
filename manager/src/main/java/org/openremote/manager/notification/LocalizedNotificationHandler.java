...
/**
 * This {@link LocalizedNotificationHandler} handles notification differently compared to other
 * handlers. When a {@link LocalizedNotificationMessage} is processed, that normally contains a
 * configuration for multiple languages, we repeat the {@link #isMessageValid} and {@link
 * #getTargets} methods for each individual locale. (using the {@link #notificationHandlerMap})
 */
public class LocalizedNotificationHandler extends RouteBuilder implements NotificationHandler {
  ...
  public boolean isMessageValid(AbstractNotificationMessage message) {
    ...
    return localizedMessage.getMessages().values().stream()
        .allMatch(
            msg -> {
  ...
  public List<Notification.Target> getTargets(
    ...
    try {
      ...
      Map<String, List<Notification.Target>> targetsMap =
          localizedMessage.getMessages().entrySet().stream()
              .map(
                  entry -> {
                    ...
                    if (!notificationHandlerMap.containsKey(entry.getValue().getType())) {
                      return Map.entry(
                          entry.getKey(), Collections.<Notification.Target>emptyList());
                    }
                    // Get valid targets for this language
                    return Map.entry(
                        entry.getKey(),
                        notificationHandlerMap
                            .get(entry.getValue().getType())
      ...
      targetsMap.forEach(
          (language, targets) -> {
            for (Notification.Target target : targets) {
              mergedTargetsMap
                  .computeIfAbsent(target.getId(), k -> target)
  ...
  public void sendMessage(
    ...
    if (target.getAllowedLocales() == null || target.getAllowedLocales().isEmpty()) {
      throw new NotificationProcessingException(
          SEND_FAILURE, "No localized message could not be sent to target " + target.getId());
    }

    String targetLocale = target.getLocale();
    if (TextUtil.isNullOrEmpty(targetLocale)) {

      // If locale of the target cannot be found, we try to get the user preferred language through
      ...
      if (target.getType().equals(Notification.TargetType.USER)) {
        ...
        List<UserAssetLink> links =
            assetStorageService.findUserAssetLinks(null, null, target.getId());
        if (!links.isEmpty()) {
          user =
              identityService.getIdentityProvider().getUser(links.getFirst().getId().getUserId());
        }
      }

      // If user has configured their locale (through the user attribute), update the targetLocale
      if (user != null
          && user.getAttributeMap() != null
    ...
    if (TextUtil.isNullOrEmpty(targetLocale)
        || !localizedMessage.getMessages().containsKey(targetLocale)) {
    ...
    if (!target.getAllowedLocales().contains(targetLocale)) {
      throw new NotificationProcessingException(
          SEND_FAILURE,
          "The localized message could not be sent to target '"
              + target.getId()
              + "' ("
              + targetLocale
              + "), because only "
              + target.getAllowedLocales()
              + " were allowed.");
    }
    ...
    if (!notificationHandlerMap.containsKey(targetMsg.getType())) {
      throw new NotificationProcessingException(
          SEND_FAILURE,
          "Could not find the handler of type '"
              + targetMsg.getType()
              + "' for the message of target "
              + target.getId());
    }

    // Send message
    notificationHandlerMap
        .get(targetMsg.getType())
        .sendMessage(id, source, sourceId, target, targetMsg);
  }
}
