...
public interface NotificationHandler extends ContainerService {
    ...
    /**
     * The unique type name; it must correspond with the value set in {@link AbstractNotificationMessage#getType} so
     * that
    ...
    /**
     * Map the requested targets to one or more targets that are compatible with this handler; if there are no
     * compatible
     * targets then return null. If the requested target is already compatible then it can just be returned; handlers
     * are free to determine how the requested target should be mapped (e.g. if requested
     * {@link Notification.TargetType}
     ...
     */
    List<Notification.Target> getTargets(Notification.Source source, String sourceId,
     ...
     * <p>
     * The ID can be used by the {@link NotificationHandler} to update the delivered and/or acknowledged status of the
     * notification
     * by calling {@link NotificationService#setNotificationDelivered} or
     * {@link NotificationService#setNotificationAcknowledged}
     */
    void sendMessage(long id, Notification.Source source, String sourceId, Notification.Target target,
