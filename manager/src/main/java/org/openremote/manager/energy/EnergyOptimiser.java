...
public class EnergyOptimiser {
     ...
     * time.
     */
    public void applyEnergySchedule(double[] energyLevelMins, double[] energyLevelMaxs, double energyCapacity,
            int[][] energyLevelSchedule, LocalDateTime currentTime) {

        if (energyLevelSchedule == null) {
            return;
        }

        // Extract the schedule for the next 24 hour period starting at current hour plus 1 (need to attain energy level
        // by the time the hour starts)
        ...
        if (intervalSize <= 1d) {
            ...
            for (i = 0; i < schedule.length; i++) {
                ...
                energyLevelMins[(hourIntervals * i)] = Math.min(energyLevelMaxs[hourIntervals * i],
            ...
            for (i = 0; i < hourIntervals; i++) {
                ...
                energyLevelMins[i] = Math.min(energyLevelMaxs[i], Math.max(energyLevelMins[i],
                        java.util.Arrays.stream(schedule, (i * takeSize), (i * takeSize) + takeSize).max().orElse(0)));
            }
        }
    }
     ...
     * rates).
     */
    public void normaliseEnergyMinRequirements(double[] energyLevelMins,
            Function<Integer, Double> powerImportMaxCalculator, Function<Integer, Double> powerExportMaxCalculator,
            double energyLevel) {
        ...
        IntStream.range(0, intervalCount).forEach(i -> {
            ...
            if (energyDelta > 0) {
                ...
                for (int j = i; j < intervalCount; j++) {
                    ...
                    double min = previousEnergyLevelCalculator.apply(j)
     ...
     */
    public void applyEnergyMinImports(double[][] importCostAndPower, double[] energyLevelMins, double[] powerSetpoints,
            Function<Integer, Double> energyLevelCalculator, BiFunction<Integer, double[], double[]> importOptimiser,
        ...
        IntStream.range(0, get24HourIntervalCount()).forEach(i -> {
            ...
            if (energyDeficit > 0) {
                ...
                if (energyDeficit > 0) {
                    retrospectiveEnergyAllocator(importCostAndPower, energyLevelMins, powerSetpoints, importOptimiser,
                            powerImportMaxCalculator, energyDeficit, fromInterval.getAndSet(i), i);
                }
            }
        });
    }
     ...
     */
    public void retrospectiveEnergyAllocator(double[][] importCostAndPower, double[] energyLevelMins,
            double[] powerSetpoints, BiFunction<Integer, double[], double[]> importOptimiser,
            Function<Integer, Double> powerImportMaxCalculator, double energyLevel, int fromInterval, int toInterval) {
        ...
        boolean canMeetDeficit = IntStream.range(fromInterval, toInterval)
                .mapToDouble(i -> Math.min(powerImportMaxCalculator.apply(i), importCostAndPower[i][2]))
                .sum() >= energyDeficit;
        boolean morePowerAvailable = !canMeetDeficit && IntStream.range(fromInterval, toInterval)
                .mapToObj(i -> importCostAndPower[i][2] < powerImportMaxCalculator.apply(i)).anyMatch(b -> b);
        ...
        List<Pair<Integer, double[]>> sortedImportCostAndPower = IntStream.range(fromInterval, toInterval)
                ...
                .sorted(Comparator.comparingDouble(pair -> pair.value[0])).collect(Collectors.toList());
     ...
     * updated as a result.
     */
    public void applyEarningOpportunities(double[][] importCostAndPower, double[][] exportCostAndPower,
            double[] energyLevelMins, double[] energyLevelMaxs, double[] powerSetpoints,
            Function<Integer, Double> energyLevelCalculator, Function<Integer, Double> powerImportMaxCalculator,
        ...
        if (LOG.isLoggable(Level.FINEST)) {
            earningOpportunities.forEach(op -> LOG.finest("Earning opportunity: interval=" + op.key + ", cost="
                    + op.value[0] + ", powerMin=" + op.value[1] + ", powerMax=" + op.value[2]));
        }
        ...
        for (Pair<Integer, double[]> earningOpportunity : earningOpportunities) {
            ...
            if (isImportOpportunity(costAndPower, powerSetpoints[interval], interval, powerImportMaxCalculator)) {
                ...
                applyImportOpportunity(importCostAndPower, exportCostAndPower, energyLevelMins, energyLevelMaxs,
                        powerSetpoints, energyLevelCalculator, powerImportMaxCalculator, powerExportMaxCalculator,
                        interval);
            } else if (isExportOpportunity(costAndPower, powerSetpoints[interval], interval,
                    powerExportMaxCalculator)) {
                // export opportunity and interval still available to export power
                applyExportOpportunity(importCostAndPower, exportCostAndPower, energyLevelMins, energyLevelMaxs,
                        powerSetpoints, energyLevelCalculator, powerImportMaxCalculator, powerExportMaxCalculator,
                        interval);
            }
        }
    ...
    protected boolean isImportOpportunity(double[] costAndPower, double powerSetpoint, int interval,
            Function<Integer, Double> powerImportMaxCalculator) {
        return costAndPower[2] > 0 && powerSetpoint >= 0
                && powerSetpoint < Math.min(powerImportMaxCalculator.apply(interval), costAndPower[2]);
    }

    protected boolean isExportOpportunity(double[] costAndPower, double powerSetpoint, int interval,
            Function<Integer, Double> powerExportMaxCalculator) {
        return costAndPower[1] < 0 && powerSetpoint <= 0
     ...
     */
    public void applyImportOpportunity(double[][] importCostAndPower, double[][] exportCostAndPower,
            double[] energyLevelMins, double[] energyLevelMaxs, double[] powerSetpoints,
            Function<Integer, Double> energyLevelCalculator, Function<Integer, Double> powerImportMaxCalculator,
            Function<Integer, Double> powerExportMaxCalculator, int interval) {
        ...
        if (energySpace < energySpaceMax && exportCostAndPower != null) {
            ...
            LOG.finest("Looking for earlier export opportunities to maximise on this import opportunity: space="
                    + energySpace + ", max=" + energySpaceMax);
            int i = interval - 1;
            ...
            while (energySpace < energySpaceMax && j < pastOpportunities.size()) {
                ...
                while (k < powerSetpoints.length && energySurplus < 0 && energySurplus <= energySurplusMin) {
                    ...
                    if (energySurplus <= 0) {
                        LOG.finest("Earlier export opportunity would violate future energy min level: interval=" + j
                                + ", futureInterval=" + k);
                    }
                    k++;
                }
                ...
                if (expPowerCapacity < 0 && expPowerCapacity < pastCostAndPower[2]) {
                    ...
                    LOG.finest("Earlier export opportunity identified: interval=" + pastInterval + ", power="
                            + expPowerCapacity);
                }
        ...
        if (energySpace > 0 && energySpace >= energySpaceMin) {
            ...
            pastIntervalPowerDeltas
                    .forEach(intervalAndDelta -> powerSetpoints[intervalAndDelta.key] += intervalAndDelta.value);
            ...
            LOG.finest("Applied import earning opportunity: set point=" + powerSetpoints[interval] + " (delta: "
                    + powerCapacity + ")");
        }
    }
     ...
     * powerSetpoints will be updated as a result.
     */
    public void applyExportOpportunity(double[][] importCostAndPower, double[][] exportCostAndPower,
            double[] energyLevelMins, double[] energyLevelMaxs, double[] powerSetpoints,
            Function<Integer, Double> energyLevelCalculator, Function<Integer, Double> powerImportMaxCalculator,
            Function<Integer, Double> powerExportMaxCalculator, int interval) {
        ...
        while (k < powerSetpoints.length && energySurplus > 0 && energySurplus >= energySurplusMin) {
            ...
            // // Try and allocate an import between this future point and the earning opportunity to prevent this
            // deficit
            // int l = k - 1;
            // while (futureEnergySurplus < energySurplusMax && l > interval) {
            // int finalL = l;
            // if (powerSetpoints[l] >= 0 && pastAndFutureIntervalPowerDeltas.stream().noneMatch(delta ->
            // delta.key.equals(finalL))) {
            //
            ...
            // intermediatePowerCapacity = Math.min(intermediatePowerCapacity, intermediateEnergyCapacity /
            // intervalSize);
            //
            // if (intermediatePowerCapacity > 0 && powerSetpoints[l] + intermediatePowerCapacity >
            // importCostAndPower[l][1] && costAndPower[0] + importCostAndPower[l][0] < 0) {
        ...
        if (energySurplus < energySurplusMax && importCostAndPower != null) {
            ...
            // in a cost effective way
            LOG.finest("Looking for earlier import opportunities to maximise on this export opportunity: surplus="
                    + energySurplus + ", max=" + energySurplusMax);
            int i = interval - 1;
            List<Pair<Integer, Double>> pastOpportunities = new ArrayList<>();
            ...
            while (energySurplus < energySurplusMax && j < pastOpportunities.size()) {
                ...
                while (k < powerSetpoints.length && energySpace > 0 && energySpace >= energySpaceMin) {
                    ...
                    energySpace = Math.min(energySpace, futureEnergySpace);
                    if (energySpace <= 0) {
                        LOG.finest("Earlier import opportunity would violate future energy max level: interval=" + j
                                + ", futureInterval=" + k);
                    }
                    k++;
                ...
                if (impPowerCapacity > 0 && impPowerCapacity > pastCostAndPower[1]) {
                    ...
                    LOG.finest("Earlier import opportunity identified: interval=" + pastInterval + ", power="
                            + impPowerCapacity);
                }
        ...
        if (energySurplus > 0 && energySurplus >= energySurplusMin) {
            ...
            // Adjust past interval set points as required
            pastAndFutureIntervalPowerDeltas
                    .forEach(intervalAndDelta -> powerSetpoints[intervalAndDelta.key] += intervalAndDelta.value);
            ...
            powerSetpoints[interval] = powerSetpoints[interval] + powerCapacity;
            LOG.finest("Applied export earning opportunity: interval=" + interval + ", set point="
                    + powerSetpoints[interval] + " (delta: " + powerCapacity + ")");
        }
    }
     ...
     */
    public BiFunction<Integer, Double, double[]> getExportOptimiser(double[] powerNets, double[] powerNetLimits,
            double[] tariffImports, double[] tariffExports, double assetExportCost) {
     ...
     */
    public BiFunction<Integer, double[], double[]> getImportOptimiser(double[] powerNets, double[] powerNetLimits,
            double[] tariffImports, double[] tariffExports, double assetImportCost) {
