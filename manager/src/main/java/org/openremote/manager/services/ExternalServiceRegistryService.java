...
/**
 * Manages {@link org.openremote.model.services.ExternalService} discovery and registration with
 * lease-based lifecycle management; supports both global (multi-tenant) and realm-bound services,
 * allowing multiple instances per service ID for horizontal scaling and high availability.
 *
 * <p>The service automatically manages external service health through scheduled tasks that mark
 * expired instances as unavailable and eventually deregister long-expired instances. It registers a
 * REST resource ({@link ExternalServiceResourceImpl}) for external access to the registry and adds
 * event subscription authorization to the {@link ClientEventService} to filter external service
 * events based on realm and user roles, publishing external service lifecycle events for external
 * consumption.
 */
public class ExternalServiceRegistryService implements ContainerService {

  private static final Logger LOG =
      Logger.getLogger(ExternalServiceRegistryService.class.getName());
  ...
  public void init(Container container) throws Exception {
    ...
    container
        ...
        .addApiSingleton(new ExternalServiceResourceImpl(timerService, identityService, this));
  ...
  protected void addExternalServiceEventSubscriptionAuthorizer() {
    clientEventService.addSubscriptionAuthorizer(
        (realm, authContext, eventSubscription) -> {
          ...
          EventSubscription<ExternalServiceEvent> subscription =
              (EventSubscription<ExternalServiceEvent>) eventSubscription;
          subscription.setFilter(
              event -> {
                ...
                boolean realmMatches =
                    Objects.equals(
                        eventExternalService.getRealm(), authContext.getAuthenticatedRealmName());

                return isGlobalService || realmMatches ? event : null;
              });

          return authContext.hasResourceRole(
              ClientRole.READ_SERVICES.getValue(), Constants.KEYCLOAK_CLIENT_ID);
        });
  }

  @Override
  public void start(Container container) throws Exception {
    markExpiredInstancesAsUnavailableFuture =
        scheduledExecutorService.scheduleAtFixedRate(
            this::markExpiredInstancesAsUnavailable,
            MARK_EXPIRED_INSTANCES_INTERVAL_MS,
            MARK_EXPIRED_INSTANCES_INTERVAL_MS,
            TimeUnit.MILLISECONDS);

    deregisterExpiredUnavailableInstancesFuture =
        scheduledExecutorService.scheduleAtFixedRate(
            this::deregisterLongExpiredInstances,
            DEREGISTER_UNAVAILABLE_INTERVAL_MS,
            DEREGISTER_UNAVAILABLE_INTERVAL_MS,
            TimeUnit.MILLISECONDS);
  }
  ...
  public void registerService(String registrarUserId, ExternalService externalService) {
    ...
    int nextInstanceId =
        serviceIdCounters
            .computeIfAbsent(externalService.getServiceId(), k -> new AtomicInteger(0))
            .incrementAndGet();
    ...
    LOG.info(
        "Registering external service: "
            ...
            + ", isGlobal: "
            + externalService.getIsGlobal()
            + ", registrarUserId: "
            + registrarUserId);

    long now = timerService.getCurrentTimeMillis();
    externalService.setLeaseInfo(
        new ExternalServiceLeaseInfo(registrarUserId, now + DEFAULT_LEASE_DURATION_MS, now, now));
    ...
    registry.merge(
        ...
        (existingMap, newMap) -> {
          ExternalService existing =
              existingMap.putIfAbsent(externalService.getInstanceId(), externalService);
          if (existing != null) {
            throw new IllegalStateException(
                "ExternalService already registered: "
                    + externalService.getServiceId()
                    + ", instanceId: "
                    + externalService.getInstanceId());
          }
    ...
    clientEventService.publishEvent(
        new ExternalServiceEvent(externalService, ExternalServiceEvent.Cause.REGISTER));

    LOG.info(
        "Successfully registered external service: "
            + externalService.getServiceId()
            + ", instanceId: "
            + externalService.getInstanceId()
            + ", isGlobal: "
  ...
  /**
   * Update the active registration lease info for the specified external service. This is used to
   * indicate that the external service is still running and available. If the external service is
   * not found, a {@link NoSuchElementException} is thrown.
   *
        ...
        (id, entry) -> {
          if (entry == null) {
            throw new NoSuchElementException(
                "Instance not found: " + serviceId + ", instanceId: " + instanceId);
          }
          ...
          clientEventService.publishEvent(
              new ExternalServiceEvent(entry, ExternalServiceEvent.Cause.UPDATE));

          LOG.fine(
              "Successfully refreshed lease info for external service: "
                  + serviceId
                  + ", instanceId: "
                  + instanceId);
          return entry;
   ...
   *
   * <p>If the external service is not found, a {@link NoSuchElementException} is thrown.
   *
  ...
  public void deregisterService(String serviceId, int instanceId) {
    registry.compute(
        ...
        (unused, instanceMap) -> {
          ...
          LOG.info(
              "Successfully de-registered external service: "
                  + serviceId
                  + ", instanceId: "
                  + instanceId);
  ...
  /**
   * Get all registered services and external services and their instances for the given realm.
   *
  ...
  /**
   * Get all globally registered services and external services and their instances. A service is
   * considered global if it is registered with the master realm and, it has the isGlobal flag set
   * to true.
   *
  ...
  /**
   * Check for expired registrations and mark them as unavailable if the instance lease has not been
   * renewed within the configured {@link #DEFAULT_LEASE_DURATION_MS} lease duration.
   */
  protected void markExpiredInstancesAsUnavailable() {
    ...
    registry.entrySet().stream()
        ...
        .filter(
            ms ->
                ms.getLeaseInfo().isExpired(now)
                    && ms.getStatus() == ExternalServiceStatus.AVAILABLE)
        .forEach(
            ms -> {
              ms.setStatus(ExternalServiceStatus.UNAVAILABLE);
              clientEventService.publishEvent(
                  new ExternalServiceEvent(ms, ExternalServiceEvent.Cause.UPDATE));
              LOG.fine(
                  "Marked external service as unavailable: "
                      + ms.getServiceId()
                      + ", instanceId: "
                      + ms.getInstanceId());
            });
  ...
  /**
   * Deregister long expired external services. This is used to remove external service instances
   * that have been unavailable for longer than the configured {@link
   * #DEFAULT_DEREGISTER_UNAVAILABLE_MS} threshold.
   */
