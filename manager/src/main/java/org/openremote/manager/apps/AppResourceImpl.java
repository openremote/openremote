...
public class AppResourceImpl extends WebResource implements AppResource {
    ...
    public Response getAppInfos(RequestParams requestParams) {
        ...
        if (consoleAppInfoMap == null) {
            try {
                consoleAppInfoMap = Files
                        .find(consoleAppService.consoleAppDocRoot, 2,
                                (filePath, fileAttr) -> filePath.getFileName().toString().endsWith("info.json")
    ...
    public Response getConsoleConfig(RequestParams requestParams) {
        ...
        if (consoleConfig == null) {
            try {
                consoleConfig = ValueUtil.JSON.readValue(
                        new File(consoleAppService.consoleAppDocRoot.resolve("console_config.json").toString()),
                        Object.class);
            } catch (IOException e) {
                throw new WebApplicationException(e);
            }
    ...
    protected ConsoleAppConfig getAppConfig(String realm) {
        try {
            ...
            return Files.list(consoleAppService.consoleAppDocRoot)
                    .filter(dir -> dir.getFileName().toString().startsWith(realm)).map(dir -> {
                        try {
