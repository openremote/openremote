  ...
  public Response getAppInfos(RequestParams requestParams) {
    ...
    if (consoleAppInfoMap == null) {
      try {
        consoleAppInfoMap =
            Files.find(
                    consoleAppService.consoleAppDocRoot,
                    2,
                    (filePath, fileAttr) ->
                        filePath.getFileName().toString().endsWith("info.json")
                            && fileAttr.isRegularFile())
                .collect(
                    Collectors.toMap(
                        dir -> dir.getName(dir.getNameCount() - 2).toString(),
                        dir -> {
                          try {
      ...
      try {
        consoleConfig =
            ValueUtil.JSON.readValue(
                new File(
                    consoleAppService.consoleAppDocRoot.resolve("console_config.json").toString()),
                Object.class);
      } catch (IOException e) {
        throw new WebApplicationException(e);
      }
