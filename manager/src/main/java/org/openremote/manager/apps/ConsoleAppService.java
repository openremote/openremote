  ...
  public void init(Container container) throws Exception {
    ...
    if (Files.isDirectory(consoleAppDocRoot)) {
      HttpHandler customBaseFileHandler =
          ManagerWebService.createFileHandler(
              container, new PathResourceManager(consoleAppDocRoot), null);

      HttpHandler pathHandler = new PathHandler().addPrefixPath("info", customBaseFileHandler);
      managerWebService
          .getRequestHandlers()
          .add(0, pathStartsWithHandler("Console app info files", "info", pathHandler));
    }
  }
