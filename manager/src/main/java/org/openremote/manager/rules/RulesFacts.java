...
public class RulesFacts extends Facts implements RuleListener {
  ...
  // Loop detection
  // TODO Better way than tracking rule trigger count? Max trigger could be a configurable multiple
  // of facts count?
  ...
  protected List<RulesEngine.AssetLocationPredicates> stopTrackingLocationRules() {
    ...
    Map<String, Set<GeofencePredicate>> assetStateLocationPredicateMap =
        this.assetStateLocationPredicateMap;
    this.assetStateLocationPredicateMap = null;
    return assetStateLocationPredicateMap == null
        ? null
        : assetStateLocationPredicateMap.entrySet().stream()
            .map(
                assetStateSetEntry ->
                    new RulesEngine.AssetLocationPredicates(
                        assetStateSetEntry.getKey(), assetStateSetEntry.getValue()))
  ...
  public Stream<Object> getAllFacts() {
    return Stream.concat(
            ...
            Stream.concat(
                getAnonymousFacts().stream().parallel(), getAssetStates().stream().parallel()))
        .parallel();
  }
  ...
  public RulesFacts putTemporary(long expires, Object value) {
    getAnonymousFacts()
        .add(new TemporaryFact<>(timerService.getCurrentTimeMillis(), expires, value));
    return this;
  }
  ...
  public Stream<TemporaryFact<?>> getTemporaryFacts() {
    return Stream.concat(
            getNamedFacts().values().stream()
                .parallel()
                .filter(fact -> fact instanceof TemporaryFact<?>)
  ...
  public void onFailure(Rule rule, Facts facts, Exception exception) {
    throw new RuntimeException(
        "Error executing action of rule '" + rule.getName() + "': " + exception.getMessage(),
        exception);
  }
  ...
  public Stream<AttributeInfo> matchAssetState(AssetQuery assetQuery) {
    ...
    Predicate<AttributeInfo> p =
        new AssetQueryPredicate(timerService, assetStorageService, assetQuery);
  ...
  protected void storeLocationPredicates(List<GeofencePredicate> foundLocationPredicates) {
    ...
    if (foundLocationPredicates != null && !foundLocationPredicates.isEmpty()) {
      ...
      // Collect asset states only where the attribute is location (location predicates only make
      // sense when the location
      // attribute is exposed to rules)
      Collection<AttributeInfo> locationAssetStates =
      ...
      assetStateLocationPredicateMap.forEach(
          (assetState, locationPredicates) -> locationPredicates.addAll(foundLocationPredicates));
    }
  }
  ...
  protected void logRule(Rule rule, String msg, boolean logFacts, boolean logVars) {
    ...
    if (ruleName.startsWith("--") && LOG.isLoggable(Level.INFO)) {
      boolean haveLog = false;
      if (logFacts) haveLog = logFacts(LOG, Level.INFO);
      if (logVars) haveLog = haveLog || logVars(LOG);
      if (haveLog) {
        LOG.info("------------------------------------------------------------------------------");
      }
    }
