...
public class RulesetStorageService implements ContainerService {
  ...
  public <T extends Ruleset> T find(Class<T> rulesetType, Long id, boolean loadComplete) {
    if (id == null) throw new IllegalArgumentException("Can't query null ruleset identifier");
    ...
    return persistenceService.doReturningTransaction(
        em -> {
          ...
          return em.unwrap(Session.class)
              .doReturningWork(
                  new AbstractReturningWork<List<T>>() {
                    @Override
  ...
  public <T extends Ruleset> void delete(Class<T> rulesetType, Long id) {
    persistenceService.doTransaction(
        entityManager -> {
          ...
          if (ruleset != null) entityManager.remove(ruleset);
        });
  }

  protected <T extends Ruleset> void appendSelectString(
      StringBuilder sb, Class<T> rulesetType, RulesetQuery query) {
    sb.append(
        "SELECT R.ID, R.VERSION, R.RULES_LANG, R.ENABLED, R.LAST_MODIFIED, R.CREATED_ON, R.NAME, R.META");
  ...
  protected <T extends Ruleset> void appendFromString(
      StringBuilder sb, Class<T> rulesetType, RulesetQuery query) {
  ...
  protected <T extends Ruleset> void appendWhereString(
      StringBuilder sb, Class<T> rulesetType, RulesetQuery query, List<ParameterBinder> binders) {
    sb.append(" WHERE 1=1");
    ...
    if (query.realm != null
        && (rulesetType == RealmRuleset.class || rulesetType == AssetRuleset.class)) {
      sb.append(" AND REALM = ?");
      final int pos = binders.size() + 1;
  ...
  protected <T extends Ruleset> T mapResultTuple(
      Class<T> rulesetType, RulesetQuery query, ResultSet rs) throws SQLException {
    T ruleset;
