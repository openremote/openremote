...
public class RulesetDeployment {
    ...
    static {
        ...
        /*
         * TODO Sharing a static GroovyShell doesn't work, redeploying a ruleset which defines classes (e.g. Flight) is
         * broken:
         * java.lang.RuntimeException: Error evaluating condition of rule '-Update flight facts when estimated landing
         * time of flight asset is updated':
         * No signature of method:
         * org.openremote.manager.setup.database.Script1$_run_closure2$_closure14$_closure17.doCall() is applicable for
         * argument types: (org.openremote.manager.setup.database.Flight) values: [...]
         * Possible solutions: doCall(org.openremote.manager.setup.database.Flight), findAll(), findAll(),
         * isCase(java.lang.Object), isCase(java.lang.Object)
         * The following classes appear as argument class and as parameter class, but are defined by different class
         * loader:
         * org.openremote.manager.setup.database.Flight (defined by 'groovy.lang.GroovyClassLoader$InnerLoader@2cc34cd5'
         * and 'groovy.lang.GroovyClassLoader$InnerLoader@1af957bc')
    ...
    public RulesetDeployment(Ruleset ruleset, RulesEngine<?> rulesEngine, TimerService timerService,
            AssetStorageService assetStorageService, ExecutorService executorService,
            ScheduledExecutorService scheduledExecutorService, Assets assetsFacade, Users usersFacade,
    ...
    protected void init() throws IllegalStateException {
        if (ruleset.getMeta().containsKey(Ruleset.VALIDITY)) {
            ...
            if (validity == null) {
                LOG.log(Level.WARNING, "Ruleset '" + ruleset.getName() + "' has invalid validity value: "
                        + ruleset.getMeta().get(Ruleset.VALIDITY));
        ...
        switch (ruleset.getLang()) {
            case JAVASCRIPT:
                return compileRulesJavascript(ruleset, assetsFacade, usersFacade, notificationsFacade,
                        historicDatapointsFacade, predictedDatapointsFacade);
            case GROOVY:
    ...
    protected boolean compileRulesJson(Ruleset ruleset) {
        ...
        try {
            jsonRulesBuilder = new JsonRulesBuilder(LOG, ruleset, rulesEngine, timerService, assetStorageService,
                    assetsFacade, usersFacade, notificationsFacade, webhooksFacade, alarmsFacade,
                    historicDatapointsFacade, predictedDatapointsFacade, this::scheduleRuleAction);
    ...
    protected boolean compileRulesJavascript(Ruleset ruleset, Assets assetsFacade, Users usersFacade,
            Notifications notificationsFacade, HistoricDatapoints historicDatapointsFacade,
            PredictedDatapoints predictedDatapointsFacade) {
        // TODO
        // https://github.com/pfisterer/scripting-sandbox/blob/master/src/main/java/de/farberg/scripting/sandbox/ScriptingSandbox.java
    ...
    protected boolean compileRulesGroovy(Ruleset ruleset, Assets assetsFacade, Users usersFacade,
            Notifications notificationFacade, HistoricDatapoints historicDatapointsFacade,
            PredictedDatapoints predictedDatapointsFacade) {
        try {
    ...
    protected boolean compileRulesFlow(Ruleset ruleset, Assets assetsFacade, Users usersFacade,
            Notifications notificationsFacade, HistoricDatapoints historicDatapointsFacade,
            PredictedDatapoints predictedDatapointsFacade) {
        try {
            flowRulesBuilder = new FlowRulesBuilder(LOG, timerService, assetStorageService, assetsFacade, usersFacade,
                    notificationsFacade, historicDatapointsFacade, predictedDatapointsFacade);
    ...
    public boolean isError() {
        return status == RulesetStatus.LOOP_ERROR || status == VALIDITY_PERIOD_ERROR
                || ((status == RulesetStatus.EXECUTION_ERROR || status == RulesetStatus.COMPILATION_ERROR)
                        && !isContinueOnError());
    }
    ...
    public String toString() {
        return getClass().getSimpleName() + "{" + "id=" + getId() + ", name='" + getName() + '\'' + ", version="
                + getVersion() + ", status=" + getStatus() + '}';
    }
}
