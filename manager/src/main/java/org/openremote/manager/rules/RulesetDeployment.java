...
public class RulesetDeployment {
    ...
    protected boolean compileRulesJson(Ruleset ruleset) {
        ...
        try {
            jsonRulesBuilder = new JsonRulesBuilder(Logger.getLogger(""),ruleset, timerService, assetStorageService, executorService, assetsFacade, usersFacade, notificationsFacade, webhooksFacade, alarmsFacade, historicDatapointsFacade, predictedDatapointsFacade,this::scheduleRuleAction);

            for (Rule rule : jsonRulesBuilder.build()) {
                RulesEngine.STATS_LOG.finest("Registering JSON rule: " + rule.getName());
                rules.register(rule);
            }
