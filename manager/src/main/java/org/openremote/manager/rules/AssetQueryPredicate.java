...
public class AssetQueryPredicate implements Predicate<AttributeInfo> {
  ...
  public AssetQueryPredicate(
      TimerService timerService, AssetStorageService assetStorageService, AssetQuery query) {
  ...
  public boolean test(AttributeInfo assetState) {
    ...
    if (query.attributes != null) {
      ...
      Set<AttributeInfo> matches =
          asAttributeMatcher(timerService::getCurrentTimeMillis, query.attributes)
  ...
  public static Predicate<NameValueHolder<?>> asPredicate(
      Supplier<Long> currentMillisSupplier, NameValuePredicate predicate) {

    Predicate<Object> namePredicate =
        predicate.name != null ? predicate.name.asPredicate(currentMillisSupplier) : str -> true;
    ...
    AtomicReference<Function<NameValueHolder<?>, Object>> valueExtractor =
        new AtomicReference<>(nameValueHolder -> nameValueHolder.getValue().orElse(null));
    ...
    if (predicate.path != null && predicate.path.getPaths().length > 0) {
      valueExtractor.set(
          nameValueHolder -> {
            ...
            if (!ValueUtil.isArray(rawValue.getClass())
                && !ValueUtil.isObject(rawValue.getClass())) {
              return null;
            }
    ...
    return nameValueHolder ->
        namePredicate.test(nameValueHolder.getName())
            && valuePredicate.test(valueExtractor.get().apply(nameValueHolder));
  }

  /**
   * A function for matching {@link AttributeInfo}s of an asset; the infos must be related to the
   * same asset to allow {@link LogicGroup.Operator#AND} to be applied.
   *
  ...
  public static Function<Collection<AttributeInfo>, Set<AttributeInfo>> asAttributeMatcher(
    ...
    LogicGroup.Operator operator =
        condition.operator == null ? LogicGroup.Operator.AND : condition.operator;
    List<Function<Collection<AttributeInfo>, Set<AttributeInfo>>> assetStateMatchers =
        new ArrayList<>();
    ...
    if (condition.getItems().size() > 0) {
      ...
      condition.getItems().stream()
          .forEach(
              p -> {
                Predicate<AttributeInfo> wrappedPredicate =
                    attributeInfo -> {
                      ...
                      Predicate<NameValueHolder<?>> basePredicates =
                          asPredicate(currentMillisProducer, p);
                      return basePredicates.test(attributeInfo);
                    };
                attributePredicates.add(wrappedPredicate);

                AtomicReference<Predicate<AttributeInfo>> metaPredicate =
                    new AtomicReference<>(nameValueHolder -> true);
                AtomicReference<Predicate<AttributeInfo>> oldValuePredicate =
                    new AtomicReference<>(value -> true);
    ...
    if (condition.groups != null && condition.groups.size() > 0) {
      assetStateMatchers.addAll(
          condition.groups.stream()
              .map(c -> asAttributeMatcher(currentMillisProducer, c))
              .toList());
    }
