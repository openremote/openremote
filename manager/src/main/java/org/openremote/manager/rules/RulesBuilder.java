...
public class RulesBuilder {
    ...
    public Rule[] build() {
        ...
        for (Builder builder : builders) {
            ...
            int priority = (builder.priority < Integer.MAX_VALUE - 1) ? builder.priority
                    : Integer.MAX_VALUE - 10000 + i;
            i++;

            Rule rule = new RuleBuilder().name(builder.name).description(builder.description).priority(priority)
                    .when(facts -> {
                        Object result;
                        try {
                            ...
                            throw new RuntimeException(
                                    "Error evaluating condition of rule '" + builder.name + "': " + ex.getMessage());
                        }
                        if (result instanceof Boolean) {
                            return (boolean) result;
                        } else {
                            throw new IllegalArgumentException("Error evaluating condition of rule '" + builder.name
                                    + "': result is not boolean but " + result);
                        }
