...
public class MapService implements ContainerService {
  ...
  protected static ObjectNode loadMapSettingsJson(Path mapSettingsPath) {
    ...
    try {
      ...
      LOG.log(
          Level.SEVERE,
          "Failed to extract map config from: " + mapSettingsPath.toAbsolutePath(),
          ex);
    }
    return mapSettings;
  }
  ...
  public void init(Container container) throws Exception {
    ...
    mapTilesPath =
        Paths.get(getString(container.getConfig(), OR_MAP_TILES_PATH, OR_MAP_TILES_PATH_DEFAULT));
    if (!Files.isRegularFile(mapTilesPath)) {
      LOG.warning(
          "Map tiles data file not found '"
    ...
    mapSettingsPath =
        Paths.get(
            getString(container.getConfig(), OR_MAP_SETTINGS_PATH, OR_MAP_SETTINGS_PATH_DEFAULT));
    if (!Files.isRegularFile(mapSettingsPath)) {
      LOG.warning(
          "Map settings file not found '"
              + mapSettingsPath.toAbsolutePath()
  ...
  public void setData() throws ClassNotFoundException, SQLException, NullPointerException {
    ...
    if (metadata.isValid()) {
      ...
      if (mapConfig == null) {
        LOG.warning(
            "Map config could not be loaded from '"
                + mapSettingsPath.toAbsolutePath()
  ...
  public ObjectNode getMapSettings(String realm, URI host) {
    ...
    Optional.ofNullable(
            settings.has("sprite") && settings.get("sprite").isTextual()
                ? settings.get("sprite").asText()
                : null)
        .ifPresent(
            sprite -> {
              String spriteUri =
    ...
    // add /fonts here
    Optional.ofNullable(
            settings.has("glyphs") && settings.get("glyphs").isTextual()
                ? settings.get("glyphs").asText()
                : null)
        .ifPresent(
            glyphs -> {
              String glyphsUri =
                  UriBuilder.fromUri(host).replacePath(MAP_SHARED_DATA_BASE_URI).build().toString()
                      + "/fonts/"
                      + glyphs;
