 ...
 *
 * <p>Checks if attribute has {@link MetaItemType#RULE_STATE} and/or {@link MetaItemType#RULE_EVENT}
 * {@link MetaItem}s, and if so the message is passed through the rule engines that are in scope for
 * the asset.
 *
 * <p>
 *
  ...
  public void init(Container container) throws Exception {
    ...
    clientEventService.addEventAuthorizer(
        (requestedRealm, authContext, event) -> {
          ...
          if (authContext != null
              && !authContext.hasResourceRoleOrIsSuperUser(
        ...
        () -> {
          ...
          return persistenceService.doReturningTransaction(
              em -> {
                ...
                if (!validationFailures.isEmpty()) {
                  String msg =
                      "Event processing failed value failed constraint validation: realm="
                          ...
                          + ", event value type="
                          + enrichedEvent.getValue().map(v -> v.getClass().getName()).orElse("null")
                          + ", attribute value type="
                ...
                //            // For executable attributes, non-sensor sources can set a writable
