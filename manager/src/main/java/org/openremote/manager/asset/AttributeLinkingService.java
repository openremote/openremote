 ...
 *
 * <p>See {@link AttributeLink} for capabilities.
 */
public class AttributeLinkingService implements ContainerService {
  ...
  public void onAttributeEvent(AttributeEvent event) {
    if (getClass().getSimpleName().equals(event.getSource())) {
      LOG.finest(
          "Attribute update came from this service so ignoring to avoid infinite loops: ref="
              + event.getRef());
      return;
    }
    ...
    event
        .getMetaValue(MetaItemType.ATTRIBUTE_LINKS)
        .ifPresent(
            attributeLinks ->
                Arrays.stream(attributeLinks)
  ...
  protected void processLinkedAttributeUpdate(
      AttributeInfo attributeInfo, AttributeLink attributeLink) {
    ...
    Pair<Boolean, Object> sendConvertedValue =
        convertValueForLinkedAttribute(assetStorageService, attributeInfo, attributeLink);
    ...
    getAttribute(assetStorageService, attributeInfo.getRealm(), attributeLink.getAttributeRef())
        .ifPresent(
            attr -> {
              if (value[0] != null) {
                ...
                if (!attr.getTypeClass().isAssignableFrom(value[0].getClass())) {
                  ...
                  if (val == null) {
                    LOG.warning(
                        "Failed to convert value into attribute value type ("
                            + value[0].getClass()
                            + " -> "
                            + attr.getTypeClass()
                            + "): "
                            + attributeInfo.getRef());
                    return;
                  }
                  value[0] = val;
                }
  ...
  protected Pair<Boolean, Object> convertValueForLinkedAttribute(
    ...
    return attributeLink
        ...
        .map(
            converter -> {
              Pair<Boolean, Object> converterValue =
                  ProtocolUtil.applyValueConverter(finalOriginalValue, converter);
  ...
  protected static Optional<AttributeLink.ConverterType> getSpecialConverter(Object value) {
    if (value != null && ValueUtil.isString(value.getClass())) {
      return AttributeLink.ConverterType.fromValue(
          ValueUtil.getValue(value, String.class).orElse(""));
    }
    return Optional.empty();
  }

  protected static Pair<Boolean, Object> doSpecialConversion(
      AssetStorageService assetStorageService,
      AttributeInfo attributeInfo,
      AttributeLink.ConverterType converter,
      AttributeRef linkedAttributeRef)
    ...
    switch (converter) {
      case TOGGLE -> {
        ...
        try {
          Attribute<?> currentAttribute =
              getAttribute(assetStorageService, attributeInfo.getRealm(), linkedAttributeRef)
                  .orElseThrow(
                      () ->
                          new RuntimeException(
                              "Cannot toggle value as attribute cannot be found: "
                                  + linkedAttributeRef));
          ...
          return new Pair<>(
              false, !(Boolean) currentAttribute.getValue(Boolean.class).orElse(false));
        } catch (NoSuchElementException e) {
          LOG.fine(
              "The attribute doesn't exist so ignoring toggle value request: "
                  + linkedAttributeRef);
      ...
      case INCREMENT, DECREMENT -> {
        ...
        try {
          ...
          LOG.fine(
              "The attribute doesn't exist so ignoring increment/decrement value request: "
                  + linkedAttributeRef);
          return new Pair<>(true, null);
        }
      }
      default ->
          throw new RuntimeException(
              "Converter is not supported ref=" + linkedAttributeRef + ": " + converter);
    }
  }

  protected static Optional<Attribute<?>> getAttribute(
      AssetStorageService assetStorageService, String realm, AttributeRef attributeRef) {
    // Get the full asset as shared em
    Asset<?> asset =
        assetStorageService.find(
            new AssetQuery().realm(new RealmPredicate(realm)).ids(attributeRef.getId()));

    Attribute<?> attribute =
        asset != null ? asset.getAttributes().get(attributeRef.getName()).orElse(null) : null;
