...
public class ForecastService extends RouteBuilder implements ContainerService {
  ...
  public void start(Container container) throws Exception {
    ...
    Set<ForecastAttribute> forecastAttributes =
        assets.stream()
            .flatMap(
                asset ->
                        ...
                        .filter(
                            attr -> {
                              if (attr.hasMeta(FORECAST)) {
                                Optional<ForecastConfiguration> forecastConfig =
                                    attr.getMetaValue(FORECAST);
                                return forecastConfig.isPresent()
                                    && ForecastConfigurationWeightedExponentialAverage.TYPE.equals(
                                        forecastConfig.get().getType());
                              }
                              return false;
                            })
  ...
  protected void processAssetChange(PersistenceEvent<Asset<?>> persistenceEvent) {
    ...
    switch (persistenceEvent.getCause()) {
      ...
      case UPDATE:
        ...
        List<Attribute<?>> newOrModifiedAttributes =
            getAddedOrModifiedAttributes(oldAttributes, newAttributes).collect(toList());

        Set<ForecastAttribute> attributesToDelete =
            newOrModifiedAttributes.stream()
                .map(attr -> new ForecastAttribute(asset, attr))
                .filter(attr -> forecastTaskManager.containsAttribute(attr))
                .collect(Collectors.toSet());

        attributesToDelete.addAll(
  ...
  private class ForecastTaskManager {
    ...
    public synchronized void add(Set<ForecastAttribute> attributes) {
      ...
      attributes.forEach(
          attr -> {
            if (attr.isValidConfig()) {
              LOG.fine(
                  "Adding asset attribute to forecast calculation service: "
                      + attr.getAttributeRef());
              forecastAttributes.add(attr);
            }
          });
      long now = timerService.getCurrentTimeMillis();
    ...
    public synchronized void delete(ForecastAttribute attribute) {
      LOG.fine(
          "Removing asset attribute from forecast calculation service: "
              + attribute.getAttributeRef());

      nextForecastCalculationMap.remove(attribute);
      forecastAttributes.remove(attribute);

      assetPredictedDatapointService.purgeValues(
          attribute.getAttributeRef().getId(), attribute.getAttributeRef().getName());
    }
    ...
    public synchronized boolean containsAttribute(AttributeRef attributeRef) {
      return forecastAttributes.stream()
    ...
    private void calculateForecasts() {
      ...
      try {
        synchronized (ForecastTaskManager.this) {
          ...
          nextForecastCalculationMap.forEach(
              (attribute, nextForecastCalculationTimestamp) -> {
        ...
        attributesToCalculate.forEach(
            attr -> {
              ...
              ForecastConfigurationWeightedExponentialAverage weaConfig =
                  (ForecastConfigurationWeightedExponentialAverage) attr.getConfig();

              LOG.fine("Calculating forecast values for attribute: " + attr.getAttributeRef());

              Long offset =
                  forecastTimestamps.getFirst() - (now + weaConfig.getForecastPeriod().toMillis());
              List<List<Long>> allSampleTimestamps = calculateSampleTimestamps(weaConfig, offset);
              List<DatapointBucket> historyDatapointBuckets =
                  getHistoryDataFromDb(attr.getAttributeRef(), weaConfig, offset);

              List<Optional<Number>> forecastValues =
                  allSampleTimestamps.stream()
                      .map(
                          sampleTimestamps -> {
                            List<AssetDatapoint> sampleDatapoints =
                                findSampleDatapoints(historyDatapointBuckets, sampleTimestamps);

                            if (sampleDatapoints.size() == weaConfig.getPastCount()) {
                              return calculateWeightedExponentialAverage(
                                  attr.getAttribute(), sampleDatapoints);
                            } else {
              ...
              if (forecastTimestamps.size() >= forecastValues.size()) {
                List<ValueDatapoint<?>> datapoints =
                    IntStream.range(0, forecastValues.size())
                        .filter(i -> forecastValues.get(i).isPresent())
                        .mapToObj(
                            i ->
                                new ValueDatapoint<>(
                                    forecastTimestamps.get(i), forecastValues.get(i).get()))
                ...
                if (datapoints.size() > 0) {
                  ...
                  assetPredictedDatapointService.updateValues(
                      attr.getId(), attr.getName(), datapoints);
                }
              }
            });
        ...
        scheduleForecastCalculation(
            timerService.getCurrentTimeMillis(), Optional.of(DEFAULT_SCHEDULE_DELAY));
      }
    }
      ...
      if (delay.isPresent()) {
        ...
        scheduledFuture =
            scheduledExecutorService.schedule(
                () -> calculateForecasts(), delay.get(), TimeUnit.MILLISECONDS);
      } else {
        scheduledFuture = null;
        if (!forecastAttributes.isEmpty()) {
          LOG.fine(
              "Scheduling next forecast calculation in '" + DEFAULT_SCHEDULE_DELAY + " [ms]'.");
          scheduleForecastCalculation(now, Optional.of(DEFAULT_SCHEDULE_DELAY));
        }
      }
    }

    private List<List<Long>> calculateSampleTimestamps(
      ...
      for (int forecastIndex = 1; forecastIndex <= config.getForecastCount(); forecastIndex++) {
        ...
        for (int pastPeriodIndex = config.getPastCount(); pastPeriodIndex > 0; pastPeriodIndex--) {
          timestamps.add(
              now - (pastPeriod * pastPeriodIndex) + (forecastPeriod * forecastIndex) + offset);
        }
    ...
    private List<Long> calculateForecastTimestamps(
        long now, ForecastConfigurationWeightedExponentialAverage config) {
    ...
    private List<AssetDatapoint> findSampleDatapoints(
        List<DatapointBucket> datapointBuckets, List<Long> sampleTimestamps) {
    ...
    private Optional<Number> calculateWeightedExponentialAverage(
        Attribute<?> attribute, List<AssetDatapoint> datapoints) {
      // a = 2 / (R + 1)
      // p: past period
      // Attr(t) = Attr(t-p) * a + Attr(t-2p) * (1 - a)
      List<Object> values =
          datapoints.stream().map(Datapoint::getValue).collect(Collectors.toList());
        ...
        Optional<Number> value =
            values.stream()
                ...
                .reduce(
                    (olderValue, oldValue) ->
                        Double.valueOf(
                            oldValue.doubleValue() * a + olderValue.doubleValue() * (1 - a)));
        if (value.isPresent()) {
          if (clazz == Long.class) {
    ...
    private Optional<Long> calculateScheduleDelay(long now) {
      OptionalLong calculateForecastTimestamp =
          nextForecastCalculationMap.values().stream().mapToLong(v -> v).min();
      ...
      forecastAttributes.forEach(
          attr -> {
            ...
            if (oldTimestamps == null || oldTimestamps.size() == 0) {
              ...
              List<Long> newShiftedTimestamps =
                  newTimestamps.stream()
                      .map(timestamp -> timestamp + offset)
                      .collect(Collectors.toList());
              while (true) {
                if (newShiftedTimestamps.getFirst() < now) {
                  newShiftedTimestamps =
                      newShiftedTimestamps.stream()
                          .map(
                              timestamp ->
                                  timestamp
              ...
              if (isServerRestart
                  && (oldTimestamps.getFirst() < now
                      || newTimestamps.size() > oldTimestamps.size())) {
                // force immediate forecast calculation
    ...
    private List<DatapointBucket> getHistoryDataFromDb(
      ...
      sb.append(
          "select dp from "
              + AssetDatapoint.class.getSimpleName()
              + " dp "
      ...
      for (int i = 1; i <= config.getPastCount(); i++) {
        ...
        sb.append(
            "(dp.timestamp >= :timestampMin" + i + " and dp.timestamp <= :timestampMax" + i + ")");
      ...
      List<AssetDatapoint> datapoints =
          persistenceService.doReturningTransaction(
              entityManager -> {
                TypedQuery<AssetDatapoint> query =
                    entityManager
                        .createQuery(sb.toString(), AssetDatapoint.class)
                        .setParameter("assetId", attributeRef.getId())
                        .setParameter("attributeName", attributeRef.getName());
                long now = timerService.getCurrentTimeMillis();
                long pastPeriod = config.getPastPeriod().toMillis();
                long forecastPeriod = config.getForecastPeriod().toMillis();
                long totalForecastPeriod =
                    Math.min(forecastPeriod * config.getForecastCount(), pastPeriod);
