...
public class AlarmResourceImpl extends ManagerWebResource implements AlarmResource {
  ...
  public SentAlarm[] getAlarms(
    ...
    try {
      ...
      throwIfNotRealmActiveAndAccessible(filterRealm);
      return alarmService
  ...
  public void removeAlarms(RequestParams requestParams, List<Long> alarmIds) {
    try {
      ...
      for (SentAlarm alarm : alarms) {
        throwIfNotRealmActiveAndAccessible(alarm.getRealm());
      }
  ...
  public void updateAlarm(RequestParams requestParams, Long alarmId, SentAlarm newAlarm) {
    try {
      ...
      throwIfNotRealmActiveAndAccessible(oldAlarm.getRealm());
    ...
    try {
      ...
      if (alarmIds.size() > 1) {
        throw new IllegalArgumentException(
            "AlarmAssetLink objects must all have the same alarm ID");
      }
