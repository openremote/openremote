...
public class AlarmService extends RouteBuilder implements ContainerService {
  ...
  /**
   * Validates if the {@code alarms} are non-null, match the alarmIds length and the user has access
   * to all alarm realms.
   */
  protected void validateAlarmsExistAndAccessible(
      List<Long> alarmIds, List<SentAlarm> alarms, String userId) {
  ...
  private Map<String, String> getAlarmNotificationContent(SentAlarm alarm, String url) {
    ...
    List<String> assetLinks =
        getAssetLinks(alarm.getId(), null, alarm.getRealm()).stream()
  ...
  public void updateAlarm(Long alarmId, String userId, SentAlarm alarm) {
    ...
    clientEventService.publishEvent(
        new AlarmEvent(alarm.getRealm(), PersistenceEvent.Cause.UPDATE));

    if (alarm.getSeverity() == Alarm.Severity.HIGH) {
      Set<String> excludeUserIds =
          Stream.of(userId, oldAssigneeId).filter(Objects::nonNull).collect(Collectors.toSet());
  ...
  public void linkAssets(List<String> assetIds, String realm, Long alarmId) {
    linkAssets(
        assetIds.stream().map(assetId -> new AlarmAssetLink(realm, alarmId, assetId)).toList(),
        null);
  }
  ...
  public void linkAssets(List<AlarmAssetLink> links, String userId) {
    ...
    Set<Long> alarmIds =
        links.stream().map(link -> link.getId().getAlarmId()).collect(Collectors.toSet());
    validateAlarmIds(alarmIds);

    Set<String> assetIds =
        links.stream().map(link -> link.getId().getAssetId()).collect(Collectors.toSet());
    validateAssetIds(assetIds);

    if (userId != null) {
      Set<String> realms =
          links.stream().map(link -> link.getId().getRealm()).collect(Collectors.toSet());
  ...
  public List<SentAlarm> getAlarms(List<Long> alarmIds, String userId)
      throws IllegalArgumentException {
    validateAlarmIds(alarmIds);
