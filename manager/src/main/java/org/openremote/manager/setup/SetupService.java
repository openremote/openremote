 ...
 *
 * <p>This service is disabled when {@link PersistenceService#isCleanInstall} is <code>false</code>.
 *
 * <p>First, this service will load an implementation of {@link SetupTasks} from the classpath using
 * {@link ServiceLoader}. If multiple providers are found, an error is raised. If a provider is
 * found, its tasks will be used on top of {@link KeycloakCleanSetup} and {@link KeycloakInitSetup}
 * which are auto-loaded if keycloak is enabled.
 */
public class SetupService implements ContainerService {
  ...
  public void init(Container container) throws Exception {
    ...
    boolean keycloakEnabled =
        container.getService(ManagerIdentityService.class).isKeycloakEnabled();
    ...
    tasks.addAll(
        ServiceLoader.load(SetupTasks.class).stream()
            .map(ServiceLoader.Provider::get)
            .flatMap(
                discoveredSetupTasks -> {
                  LOG.info(
                      "Found custom SetupTasks provider on classpath: "
                          + discoveredSetupTasks.getClass().getName());
                  List<Setup> tasks =
                      discoveredSetupTasks.createTasks(container, setupType, keycloakEnabled);
  ...
  public <S extends Setup> S getTaskOfType(Class<S> setupType) {
    for (Setup task : tasks) {
      if (setupType.isAssignableFrom(task.getClass())) return (S) task;
    }
