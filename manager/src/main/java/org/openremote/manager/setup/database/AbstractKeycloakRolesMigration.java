...
/**
 * Base class for Flyway migrations that add client roles to the {@code openremote} client of every
 * realm and wire them into the existing {@code read}/{@code write} composite roles (read → {@link
 * #getReadRoles()}, write → {@link #getReadRoles()} + {@link #getWriteRoles()}). See {@link
 * AbstractKeycloakMigration} for credential handling, the identity-provider guard, and why
 * subclasses must stay Java migrations.
 */
public abstract class AbstractKeycloakRolesMigration extends AbstractKeycloakMigration {
    ...
    try (Keycloak keycloak = openKeycloak()) {
      ...
      for (String realmName : realmNames) {
        ...
        List<ClientRepresentation> clients =
            realm.clients().findByClientId(Constants.KEYCLOAK_CLIENT_ID);
        if (clients.isEmpty()) {
          LOG.warning(
              "Client '"
  ...
  private void addToComposite(
      RolesResource roles, String compositeName, List<RoleRepresentation> children) {
    try {
