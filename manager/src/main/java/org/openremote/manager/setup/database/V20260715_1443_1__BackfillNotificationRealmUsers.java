...
/**
 * Companion to {@code V20260715_1443__AddNotificationRealm.sql}: backfills the {@code realm} column
 * for USER-targeted and CLIENT-sourced notifications by looking up each referenced user's realm via
 * the Keycloak Admin API (the stable public interface, not the internal schema). The Keycloak
 * lookup is skipped entirely when no rows reference a user (e.g. clean installs) or on non-Keycloak
 * deployments. Any rows still unresolved after the backfill (CUSTOM targets, deleted users/assets,
 * INTERNAL/GLOBAL_RULESET sources) fall back to {@code master}. Also applies the master fallback
 * and {@code NOT NULL} constraint that the SQL migration deferred until all resolvable rows were
 * filled.
 */
public class V20260715_1443_1__BackfillNotificationRealmUsers extends AbstractKeycloakMigration {
  ...
  private Map<String, List<String>> buildRealmUserIdsMap(Set<String> unresolvedUserIds) {
    ...
    try (Keycloak keycloak = openKeycloak()) {
      List<String> realmNames =
          keycloak.realms().findAll().stream().map(RealmRepresentation::getRealm).toList();
    ...
    LOG.info(
        "Resolved realm for "
            + (unresolvedUserIds.size() - remaining.size())
            + " of "
            + unresolvedUserIds.size()
            + " users referenced by notifications");
    return realmUserIds;
  }

  /**
   * Pages through the realm's users and returns the ids present in {@code wanted}, stopping as soon
   * as all of them have been found.
   */
  private List<String> findUserIdsInRealm(Keycloak keycloak, String realmName, Set<String> wanted) {
    ...
    do {
      ...
      page.stream().map(UserRepresentation::getId).filter(wanted::contains).forEach(matched::add);
      first += page.size();
    } while (page.size() == USER_PAGE_SIZE && matched.size() < wanted.size());
    return matched;
  }

  private void applyRealmUserIds(Context context, Map<String, List<String>> realmUserIds)
      throws Exception {
    // USER-targeted notifications: targetId is the user id
    updateRealmByUserIds(
        context,
        realmUserIds,
  ...
  private void updateRealmByUserIds(
      Context context, Map<String, List<String>> realmUserIds, String sql) throws Exception {
    try (PreparedStatement ps = context.getConnection().prepareStatement(sql)) {
      for (Map.Entry<String, List<String>> entry : realmUserIds.entrySet()) {
        ...
        ps.setArray(
            2, context.getConnection().createArrayOf("varchar", entry.getValue().toArray()));
        ps.addBatch();
      }
      ps.executeBatch();
    }
