...
public abstract class AbstractKeycloakSetup implements Setup {
    ...
    public static final ClientRole[] PUBLIC_USER_ROLES = new ClientRole[] { ClientRole.READ_ASSETS, ClientRole.READ_MAP,
            ClientRole.READ_INSIGHTS };
    public static final ClientRole[] REGULAR_USER_ROLES = new ClientRole[] { ClientRole.WRITE_USER, ClientRole.READ_MAP,
            ClientRole.READ_ASSETS, ClientRole.READ_USERS, ClientRole.READ_RULES, ClientRole.READ_INSIGHTS,
            ClientRole.READ_ALARMS, ClientRole.WRITE_ASSETS, ClientRole.WRITE_ATTRIBUTES, ClientRole.WRITE_RULES,
            ClientRole.WRITE_INSIGHTS, ClientRole.WRITE_ALARMS };
    ...
    public User createUser(String realm, String username, String password, String firstName, String lastName,
            String email, boolean enabled, ClientRole[] roles) {
        return createUser(realm, username, password, firstName, lastName, email, enabled, false, false, roles);
    }

    public User createUser(String realm, String username, String password, String firstName, String lastName,
            String email, boolean enabled, boolean emailNotificationsDisabled, boolean pushNotificationsDisabled,
            ClientRole[] roles) {
        User user = new User();
        ...
        if (roles != null && roles.length > 0) {
            keycloakProvider.updateUserRoles(realm, user.getId(), KEYCLOAK_CLIENT_ID,
                    Arrays.stream(roles).map(ClientRole::getValue).toArray(String[]::new));
        }
        return user;
    }

    /**
     * Default realm roles will assign manage-account role to account client so we have to remove this role from the
     * composite default roles
     * This is a temporary thing and when/if we move to groups we should look at explicit default roles on realm
     * creation
     */
    public void removeManageAccount(String realm) {
        keycloakProvider.<Void> getRealms(realmsResource -> {
            ...
            roleResource.getClientRoleComposites(clientRepresentation.getId()).stream()
                    .filter(role -> role.getName().equals("manage-account")).findFirst()
                    .ifPresent(manageAccountRole -> roleResource
                            .deleteComposites(Collections.singletonList(manageAccountRole)));
            return null;
        });
    }
}
