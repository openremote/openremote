...
public class RulesService implements ContainerService, AttributeStateChangeConsumer {
    ...
    private Map<AttributeRef, FactHandle> stateSources = new HashMap<>();
    ...
    public AttributeStateConsumerResult consumeAttributeStateChange(AttributeStateChange attributeStateChange) {
        if (kb == null) {
            LOG.fine("No knowledge base configured, skipping processing of: " + attributeStateChange);
        ...
        try {
            ...
            if (!knowledgeSession.getObjects().contains(attributeStateChange)) {

                // ... update or new insert?
                boolean isUpdate = true;

                // If there already is a fact in wm with the same entity ID and attribute name
                if (stateSources.keySet().contains(attributeStateChange.getAttributeRef())) {
                    try {
                        // ... retract it from working memory ...
                        knowledgeSession.delete(stateSources.get(attributeStateChange.getAttributeRef()));
                    } finally {
                        // ... and make sure we don't keep a reference to a fact
                        stateSources.remove(attributeStateChange.getAttributeRef());
                    }
                } else {
                    isUpdate = false;
                }
                ...
                FactHandle handle = knowledgeSession.insert(attributeStateChange);
                stateSources.put(attributeStateChange.getAttributeRef(), handle);

                if (isUpdate) {
                    LOG.finest("Inserted: " + attributeStateChange);
                } else {
                    LOG.finest("Updated: " + attributeStateChange);
                }
                ...
                if (newFactCount != currentFactCount) {
                    LOG.finest("Fact count changed from " + currentFactCount + " to " + newFactCount + " on '" + attributeStateChange + "'");
                }
            ...
            LOG.log(Level.SEVERE, "Error executing rules for: " + attributeStateChange, t);
            if (t.getCause() != null) {
