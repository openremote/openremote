 ...
 * <li><code>manufacturer - customer</code> - (Password: customer) A user in the "manufacturer" realm with restricted
 * access to his assets</li>
 *
 * </ul>
 */
public class KeycloakDemoSetup extends AbstractKeycloakSetup {
    ...
    public void onStart() throws Exception {
        ...
        // Users
        User smartCityUser = createUser(realmCity.getName(), "smartcity", "smartcity", "Smart", "City", null, true,
                demoUserRoles);
        ...
        keycloakProvider.updateUserRoles(realmCity.getName(), smartCityUserId, "account"); // Remove all roles for
                                                                                           // account client
        User manufacturerUser = createUser(realmManufacturer.getName(), "manufacturer", "manufacturer", "Agri", "Tech",
                null, true, demoUserRoles);
        manufacturerUserId = manufacturerUser.getId();
        keycloakProvider.updateUserRoles(realmManufacturer.getName(), manufacturerUserId, "account"); // Remove all
                                                                                                      // roles for
                                                                                                      // account client
        User customerUser = createUser(realmManufacturer.getName(), "customer", "customer", "Bert", "Frederiks", null,
                true, demoUserRoles);
        customerUserId = customerUser.getId();
        keycloakProvider.updateUserRoles(realmManufacturer.getName(), customerUserId, "account"); // Remove all roles
                                                                                                  // for account client
    }
}
