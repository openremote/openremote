...
public class ManagerDateTimeSetup extends ManagerSetup {
  ...
  public void onStart() throws Exception {
    ...
    // Future constraints need a value after now
    long future = Instant.now().plus(30, ChronoUnit.DAYS).toEpochMilli();
    asset.getAttributes().getOrCreate(DateTimeTestAsset.FUTURE).setValue(future);
    asset.getAttributes().getOrCreate(DateTimeTestAsset.FUTURE_OR_PRESENT).setValue(future);
    asset
        .getAttributes()
        .getOrCreate(DateTimeTestAsset.MIN_BETWEEN_MINUTES)
        .setValue(VALUE.plus(30, ChronoUnit.MINUTES).toEpochMilli());
