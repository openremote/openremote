...
public class ManagerSetup extends org.openremote.manager.setup.ManagerSetup {
    ...
    public void onStart() throws Exception {
        ...
        WeatherAsset templateAsset = new WeatherAsset(UNIQUE_ID_PLACEHOLDER).addAttributes(
                new Attribute<>("calculated", NUMBER).addMeta(new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_READ),
        ...
        templateAsset.getAttribute(WeatherAsset.RAINFALL)
                .ifPresent(attr -> attr.addMeta(new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_READ),
                        new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_WRITE), new MetaItem<>(MetaItemType.RULE_STATE),
                        new MetaItem<>(MetaItemType.STORE_DATA_POINTS)));
        templateAsset.getAttribute(WeatherAsset.TEMPERATURE)
                .ifPresent(attr -> attr.addMeta(new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_READ),
                        new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_WRITE), new MetaItem<>(MetaItemType.RULE_STATE),
                        new MetaItem<>(MetaItemType.STORE_DATA_POINTS)));

        X509ProvisioningConfig provisioningConfig = new X509ProvisioningConfig("OEM Device",
                new X509ProvisioningData().setCACertPEM(caCert))
                .setAssetTemplate(ValueUtil.asJSON(templateAsset).orElse("")).setRealm("master").setRestrictedUser(true)
                .setUserRoles(new ClientRole[] { ClientRole.WRITE_ASSETS, ClientRole.WRITE_ATTRIBUTES,
                        ClientRole.READ_ASSETS });

        provisioningConfig = provisioningService.merge(provisioningConfig);

        RealmRuleset ruleset = new RealmRuleset(Constants.MASTER_REALM, "Weather calculations", GROOVY,
                IOUtils.toString(getClass().getResource("/org/openremote/setup/load1/WeatherAssetCalculations.groovy"),
            ...
            IntStream.rangeClosed(1, devices).forEach(i -> {
                if (i % deviceSkipMod > 0) {
                    executor.execute(() -> {
                        ...
                        User user = UserAssetProvisioningMQTTHandler.getCreateClientServiceUser(Constants.MASTER_REALM,
                                ((ManagerKeycloakIdentityProvider) identityService.getIdentityProvider()), uniqueId,
                                finalProvisioningConfig);
                        UserAssetProvisioningMQTTHandler.getCreateClientAsset(assetStorageService,
                                Constants.MASTER_REALM, uniqueId, user, finalProvisioningConfig);
