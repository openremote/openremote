...
public class ManagerSetup extends org.openremote.manager.setup.ManagerSetup {
  ...
  public void onStart() throws Exception {
    ...
    WeatherAsset templateAsset =
        new WeatherAsset(UNIQUE_ID_PLACEHOLDER)
            .addAttributes(
                new Attribute<>("calculated", NUMBER)
                    .addMeta(
                        new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_READ),
                        new MetaItem<>(MetaItemType.ACCESS_RESTRICTED_WRITE)));

    try (InputStream inputStream =
        getClass().getResourceAsStream("/org/openremote/setup/load1/ca.pem")) {
    ...
    X509ProvisioningConfig provisioningConfig =
        new X509ProvisioningConfig("OEM Device", new X509ProvisioningData().setCACertPEM(caCert))
            .setAssetTemplate(ValueUtil.asJSON(templateAsset).orElse(""))
            .setRealm("master")
            .setRestrictedUser(true)
            .setUserRoles(
                new ClientRole[] {
                  ClientRole.WRITE_ASSETS, ClientRole.WRITE_ATTRIBUTES, ClientRole.READ_ASSETS
                });
    ...
    RealmRuleset ruleset =
        new RealmRuleset(
            ...
            IOUtils.toString(
                getClass()
                    .getResource("/org/openremote/setup/load1/WeatherAssetCalculations.groovy"),
    ...
    if (devices > 1) {
      ...
      IntStream.rangeClosed(1, devices)
          .forEach(
              i -> {
                if (i % deviceSkipMod > 0) {
                  executor.execute(
                      () -> {
                        ...
                        User user =
                            UserAssetProvisioningMQTTHandler.getCreateClientServiceUser(
                                Constants.MASTER_REALM,
