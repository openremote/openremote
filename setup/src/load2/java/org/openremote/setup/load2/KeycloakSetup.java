...
public class KeycloakSetup extends AbstractKeycloakSetup {
    ...
    public void onStart() throws Exception {
        ...
        if (accounts > 0) {
            ...
            IntStream.rangeClosed(1, accounts).forEach(i ->
                executor.submit(() -> {
                    User user = createUser(Constants.MASTER_REALM, User.SERVICE_ACCOUNT_PREFIX + "serviceuser" + i, "serviceuser" + i, null, null, null, true, CLIENT_ROLES);
                    keycloakProvider.updateUserRealmRoles(MASTER_REALM, user.getId(), RESTRICTED_USER_REALM_ROLE);
                    createdUsers.incrementAndGet();
                })
            );
        }
        ...
        int waitCounter = 0;
        int waitCounterLimit = Math.max(300, accounts / 2);
