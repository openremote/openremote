...
class ResidenceSmartSwitchTest extends Specification implements ManagerContainerTrait {
    ...
    def "Set mode ON_AT and begin/end cycle time in future"() {
        ...
        conditions.eventually {
            ...
            assert kitchen.getAttribute("smartSwitchStartTimeA").get().getValueAsNumber().get() == oneMinuteInFutureMillis/1000
            assert kitchen.getAttribute("smartSwitchStopTimeA").get().getValueAsNumber().get() == (oneMinuteInFutureMillis/1000) + (2.5 * 60 * 60) + 60
    ...
    def "Set mode READY_AT and begin/end cycle time in past"() {
        ...
        then: "mode should be READY_AT and actuator not enabled with correct start/stop time"
        conditions.eventually {
            def expectedTimeMillis = getClockTimeOf(container) + ((2.5 * 60 * 60) + 60) * 1000
            def kitchen = assetStorageService.find(managerDemoSetup.apartment1KitchenId, true)
            assert kitchen.getAttribute("smartSwitchModeA").get().getValueAsString().get() == "READY_AT"
            assert kitchen.getAttribute("smartSwitchBeginEndA").get().getValueAsNumber().get() == (double)expectedTimeMillis
            assert kitchen.getAttribute("smartSwitchStartTimeA").get().getValueAsNumber().get() == (double)getClockTimeOf(container)/1000
            assert kitchen.getAttribute("smartSwitchStopTimeA").get().getValueAsNumber().get() == (double)(expectedTimeMillis/1000)
            assert kitchen.getAttribute("smartSwitchEnabledA").get().getValueAsNumber().get() == 1
            assert !kitchen.getAttribute("smartSwitchModeB").get().getValue().isPresent()
            assert !kitchen.getAttribute("smartSwitchStartTimeB").get().getValue().isPresent()
            assert !kitchen.getAttribute("smartSwitchStopTimeB").get().getValue().isPresent()
