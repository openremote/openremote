...
export class OrDashboardTree extends translate(i18next)(OrElement) {
  ...
  private createDashboard(size: DashboardSizeOption) {
    DashboardService.create(undefined, size, this.realm)
      .then((d) => {
        if (!this.dashboards) {
  ...
  protected _doDuplicateDashboard(dashboard: Dashboard) {
    ...
    DashboardService.create(newDashboard, undefined, this.realm)
      .then((d) => {
        if (!this.dashboards) {
          this.dashboards = [d] as Dashboard[];
        } else {
  ...
  private deleteDashboard(dashboard: Dashboard) {
    if (dashboard.id != null) {
      DashboardService.delete(dashboard.id, this.realm)
        .then(() => {
          this.getAllDashboards();
        })
        .catch((reason) => {
                  ...
                  ${when(
                    ...
                    () => html`
                      <or-vaadin-button
                        ...
                        @click=${() => this.selectDashboard(undefined)}
                      >
                      ...
                      ${when(
                        ...
                        () => html`
                          <or-vaadin-button
                            ...
                            @click=${() => this.duplicateDashboard(this.selected!)}
                          >
                                  ...
                                  html`
                                    <or-vaadin-confirm-dialog @confirm=${() => this.deleteDashboard(this.selected!)}>
                                      ${getConfirmDialogContent(
                                        ...
                                        "areYouSure",
                                        html`<or-translate
                                          value="dashboard.deletePermanentWarning"
                                          .options=${{ dashboard: this.selected.displayName }}
                                        ></or-translate>`,
                                        "delete",
                                        "cancel"
                                      )}
                  ...
                  ${when(
                    ...
                    () => html`
                      <or-vaadin-button
                        theme="icon"
                        class="hideMobile"
                        title=${i18next.t("addInsights")}
                        @click=${() => this.createDashboard(DashboardSizeOption.DESKTOP)}
                      >
          ...
          <ol id="list">
            ${dashboardItems.map(
              (listItem: ListItem) => html`
                <li
                  ?data-selected="${listItem.value === this.selected?.id}"
                  @click="${(_evt: MouseEvent) => this.onDashboardClick(listItem.value)}"
                >
