...
export class OrMapWidget implements OrWidgetEntity {
    ...
    getWidgetHTML(widget: DashboardWidget, editMode: boolean, realm: string): TemplateResult {
        return html`<or-map-widget .widget="${widget}" .editMode="${editMode}" .realm="${realm}" style="overflow: auto; height: 100%;"></or-map-widget>`;
    }

    getSettingsHTML(widget: DashboardWidget, realm: string): TemplateResult {
        return html`<or-map-widgetsettings .widget="${widget}" .realm="${realm}"></or-map-widgetsettings>`;
    }
}
...
export class OrMapWidgetContent extends LitElement {
            ...
            ${(this.assets) ?
                ...
                .map(asset => {
                    if(this.markers){
                        ...
                        this.markers[asset.type!] = {attributeName: this.widget!.widgetConfig.attribute as string};
                        ...
                        if(this.widget!.widgetConfig.valueType == 'boolean') {
                            ...
                            (this.widget!.widgetConfig.thresholds as [number, string][]).sort((x, y) => (x[0] > y[0]) ? -1 : 1).map((threshold, index) => {
                                var range: AttributeMarkerColoursRange = { min: threshold[0], colour: threshold[1].replace('#', '') }
                                ranges.push(range);
                            })
                            var colorsNum: RangeAttributeMarkerColours = { type: 'range', ranges: ranges};
    ...
    async fetchAssets(config: OrWidgetConfig | any): Promise<Asset[] | undefined> {
        if(config.assetType && config.attribute){
            let assets: Asset[] = [];
            await manager.rest.api.AssetResource.queryAssets({
                select: {
                    attributes: [config.attribute, 'location']
                },
        ...
        return html`
            ...
            <div>
                ${this.expandedPanels.includes(i18next.t('display')) ? html`
                    <or-dashboard-settingspanel .type="${SettingsPanelType.ASSETTYPES}" .widgetConfig="${this.widget?.widgetConfig}"
