...
import { OrWidget, type WidgetManifest } from "../util/or-widget";
...
export class GatewayWidget extends OrWidget {
  ...
  protected firstUpdated(changedProps: PropertyValues) {
    if (this.widgetConfig) {
      ...
      setTimeout(() => {
        ...
        this._getActiveTunnel(tunnelInfo)
          .then((info) => {
            if (info) {
              ...
              if (this._activeTunnel) {
                return html`
                  <div>
                    ...
                    ${when(
                      this.widgetConfig.type === GatewayTunnelInfoType.TCP,
                      () => html`
                        <or-vaadin-button
                          ?disabled=${disabled}
                          @click=${(ev: Event) => this._onCopyTunnelAddressClick(ev)}
                        >
                      ...
                      () => html`
                        <or-vaadin-button
                          ?disabled=${disabled}
                          @click=${(ev: Event) => this._onTunnelNavigateClick(ev, this._activeTunnel)}
                        >
                  ...
                  ${when(
                    this._activeTunnel?.autoCloseTime,
                    () => html`
                      <div>
                        <or-translate value="gatewayTunnels.closesAt"></or-translate>:
                        ${moment(this._activeTunnel?.autoCloseTime).format("lll")}
                      </div>
                    `
                  )}
                `;
  ...
  protected async _getActiveTunnel(info: GatewayTunnelInfo): Promise<GatewayTunnelInfo | undefined> {
    ...
    const response = await manager.rest.api.GatewayServiceResource.getActiveTunnelInfo(
      info.realm,
      info.gatewayId,
      info.target,
      info.targetPort
