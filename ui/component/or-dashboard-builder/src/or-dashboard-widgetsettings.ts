...
import { type WidgetSettings, WidgetSettingsChangedEvent } from "./util/widget-settings";
...
export class OrDashboardWidgetsettings extends translate(i18next)(LitElement) {
  ...
  forceParentUpdate(changes: Map<string, any>, force: boolean = false) {
    this.requestUpdate();
    this.dispatchEvent(new CustomEvent("update", { detail: { changes, force } }));
  }
    ...
    return html`
      <div style="padding: 12px; border-bottom: 1px solid #E0E0E0">
        <or-vaadin-text-field
          value=${this.selectedWidget?.displayName}
          required
          minlength="1"
          @change=${(ev: Event) => {
      ...
      <div>
        ${guard(
          [this.selectedWidget],
          () => html` ${until(this.generateContent(this.selectedWidget!.widgetTypeId!), html`Loading...`)} `
        )}
      </div>
    `;
  }
