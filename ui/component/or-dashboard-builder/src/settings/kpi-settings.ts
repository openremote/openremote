...
import type { KpiWidgetConfig } from "../widgets/kpi-widget";
import type { AttributesSelectEvent } from "../panels/attributes-panel";
import type { Attribute } from "@openremote/model";
      ...
      <div>
        ...
        <settings-panel displayName="display" expanded="${true}">
          <div style="display: flex; flex-direction: column; gap: 8px;">
            <or-vaadin-select
              .items=${["year", "month", "week", "day", "hour"].map((i) => ({ value: i, label: i18next.t(i) }))}
              value=${this.widgetConfig.period}
              @change=${(ev: Event) => this.onTimeframeSelect(ev)}
            >
        ...
        <settings-panel displayName="values" expanded="${true}">
          <div style="display: flex; flex-direction: column; gap: 8px;">
            <or-vaadin-select
              .items=${["absolute", "percentage"].map((i) => ({ value: i, label: i18next.t(i) }))}
              value=${this.widgetConfig.deltaFormat}
              @change=${(ev: Event) => this.onDeltaFormatSelect(ev)}
            >
