...
import type {
  AssetIdsSelectEvent,
  AssetTypeSelectEvent,
  AssetAllOfTypeSwitchEvent,
  AssetTypesFilterConfig,
  AttributeNamesSelectEvent,
} from "../panels/assettypes-panel";
import manager from "@openremote/core";
import { showSnackbar } from "@openremote/or-mwc-components/or-mwc-snackbar";
import type { BoolColorsChangeEvent, TextColorsChangeEvent, ThresholdChangeEvent } from "../panels/thresholds-panel";
...
export class MapSettings extends AssetWidgetSettings {
  protected static _allowedValueTypes = [
    "boolean",
    "number",
    "integer",
    "positiveInteger",
    ...
    return html`
      <div>
        ...
        <settings-panel displayName="configuration.mapSettings" expanded="${true}">
          <div style="display: flex; flex-direction: column; gap: 8px;">
            ...
            <or-vaadin-number-field
              value=${this.widgetConfig.zoom}
              min="0"
              @change=${(ev: Event) => this.onZoomUpdate(ev)}
            >
        ...
        <settings-panel displayName="attributes" expanded="${true}">
          <assettypes-panel
            .assetType="${this.widgetConfig.assetType}"
            .attributeNames="${[this.widgetConfig.attributeName]}"
            .config="${MapSettings._config}"
            .allOfType="${this.widgetConfig.allOfType}"
            .assetIds="${this.widgetConfig.assetIds}"
        ...
        ${when(
          this.widgetConfig.attributeName,
          () => html`
            <settings-panel displayName="thresholds" expanded="${true}">
              <thresholds-panel
                .thresholds="${this.widgetConfig.thresholds}"
                .boolColors="${this.widgetConfig.boolColors}"
                .textColors="${this.widgetConfig.textColors}"
                .valueType="${this.widgetConfig.valueType}"
                style="padding-bottom: 12px;"
                .min="${this.widgetConfig.min}"
                .max="${this.widgetConfig.max}"
  ...
  protected onAssetTypeSelect(ev: AssetTypeSelectEvent) {
    if (this.widgetConfig.assetType !== ev.detail) {
      ...
      this.widgetConfig.textColors = [
        ["example1", "#4caf50"],
        ["example2", "#ff9800"],
      ];
      this.widgetConfig.thresholds = [
        [0, "#4caf50"],
        [75, "#ff9800"],
        [90, "#ef5350"],
      ];
