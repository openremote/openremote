...
export class OrAttributeCard extends LitElement {
  ...
  updated(changedProperties: PropertyValues) {
    ...
    const reloadData =
      ...
      changedProperties.has("compareTimestamp") ||
    ...
    if (!this._chart) {
      ...
      if (changedProperties.has("data")) {
        ...
        this._chart.update();
        this.delta = this.getFormattedDelta(
          this.getFirstKnownMeasurement(this.data),
          this.getLastKnownMeasurement(this.data)
        );
      }
    }
  ...
  shouldShowControls(): boolean {
    // Checking for string input as well since that was not working
        ...
        <div class="panel panel-empty">
          <div class="panel-content-wrapper">
            <div class="panel-content">
              ${
                !this.shouldHideAttributePicker()
                  ? html`
                    ...
                    `
                  : html` <or-translate style="text-align: center;" value="noAttributeConnected"></or-translate> `
              }
            </div>
          </div>
        </div>
          ...
          <div class="panel-content">
            <div class="mainvalue-wrapper">
              ${when(
                !this._loading,
                () => html`
                  <span class="main-number-icon"
                    >${this.assets && this.assets.length === 1 ? getAssetDescriptorIconTemplate(AssetModelUtil.getAssetDescriptor(this.assets[0].type!)) : ""}</span
                  >
                  <span class="main-number ${this.mainValueSize}"
                    >${this.formattedMainValue ? this.formattedMainValue.value : ""}</span
                  >
                  <span class="main-number-unit ${this.mainValueSize}"
                    >${this.formattedMainValue ? this.formattedMainValue.unit : ""}</span
                  >
                `
              )}
            </div>
            <div class="graph-wrapper">
              ...
              <div class="controls-wrapper">
                <div class="delta-wrapper">
                  <span class="delta"
                    >${this.delta ? this.deltaPlus + (this.delta.val !== undefined && this.delta.val !== null ? this.delta.val : "") + (this.delta.unit || "") : ""}</span
                  >
                </div>
                <div class="period-selector-wrapper">
                  <or-vaadin-menu-bar
                    ...
                    .items=${periodItems}
                    ?disabled=${!this.shouldShowControls()}
  ...
  protected _openDialog() {
    const dialog = new OrAssetAttributePicker().setHeading(i18next.t("selectAttribute"));
  ...
  protected async loadSettings(reset: boolean = false) {
    ...
    const view =
      config && config.views && config.views[viewSelector] ? config.views[viewSelector][this.panelName] : undefined;
  ...
  protected getFirstKnownMeasurement(data: ValueDatapoint<any>[]): number {
    for (let i = 0; i <= data.length; i++) {
      if (data[i] && data[i].y !== undefined) return data[i].y;
    }
    return 0;
  }
