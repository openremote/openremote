...
class OrSurvey extends LitElement {
    ...
    checkButtons() {
        if(!this.survey || !this.questions  || !this.questionIndex) {
            return;
        }
        ...
        if(this.questionIndex === this.questions.length && this.questions.length !== 0) {
            ...
            if (this.survey.attributes && this.survey.attributes.published.value && this.saveanswers) {
                ...
                const url = ORCore.manager.config.managerUrl ? ORCore.manager.config.managerUrl+"/rest/survey/" : window.location.origin+"/rest/survey/";
