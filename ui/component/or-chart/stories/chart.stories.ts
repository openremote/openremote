...
export const AssetsExample: Story = {
  ...
  parameters: {
    ...
    summary:
      "You can show historic asset data in the chart using the `assets`, `assetAttributes`, `attributeColors`, `datapointQuery` and a `timeframe`.",
  },
  args: {
    datapointQuery: JSON.stringify({
      type: "lttb",
      amountOfPoints: 100,
      fromTimestamp: Date.now() - 120000 * 60,
  ...
  render: (storyArgs, { loaded }) => {
    const query: AssetQuery = {
      limit: 1,
      attributes: {
        items: [{ meta: [{ name: { predicateType: "string", value: WellknownMetaItems.STOREDATAPOINTS } }] }],
      },
    };
    loaded.orManager.rest.api.AssetResource.queryAssets(query).then((response: any) => {
      ...
      const datapointAttr = Object.values(data[0].attributes!).find((attr) =>
        Object.keys(attr.meta!).includes(WellknownMetaItems.STOREDATAPOINTS)
      );
      component.assets = data;
      component.assetAttributes = [[0, datapointAttr!]];
      component.attributeColors = [
        [{ id: component.assets[0].id, name: datapointAttr!.name }, OrChart.DEFAULT_COLORS[0]],
      ];
      component.timeframe = [new Date(Date.now() - 60000 * 60), new Date()];
    });
    return html`
...
async function loadOrChart(storyArgs: any) {
  const newArgs = Object.fromEntries(
    Object.entries(storyArgs).filter(([_key, value]) => value != null && String(value).length > 0)
  );
