...
import { type Node, type NodeInternal, NodeInternalBreakType } from "@openremote/model";
...
export class FlowNode extends SelectableElement {
  ...
  protected async firstUpdated() {
    ...
    if (newIds.has(this.node.id!)) {
      // node centering has to keep track of which nodes were created by the user
  ...
  protected render() {
    ...
    const title = this.minimal
      ? html`<div class="title minimal" ?singlechar="${this.node.displayCharacter!.length === 1}">
          ${i18next.t("flow." + this.node.displayCharacter!, this.node.displayCharacter!)}
        </div>`
      : html`<div class="title ${this.node.type!.toLowerCase()}" @mousedown="${this.startDrag}">
          ${i18next.t("flow." + this.node.name!) || "invalid"}
        </div>`;

    const inputSide = html`<div class="socket-side inputs">
      ${this.node.inputs!.map((i) => html`<flow-node-socket ?renderlabel="${!this.minimal}" .socket="${i}" side="input"></flow-node-socket>`)}
    </div>`;
    const outputSide = html`<div class="socket-side outputs">
      ${this.node.outputs!.map((i) => html`<flow-node-socket ?renderlabel="${!this.minimal}" .socket="${i}" side="output"></flow-node-socket>`)}
    </div>`;
    ...
    return html`
      ${title} ${this.node.inputs!.length > 0 ? inputSide : spacer}
      ${
        this.minimal
          ? null
          : html` <div class="internal-container" style="padding-top: 8px">
              ${this.node
                .internals!.reduce(
                  (acc, i, index, array) => {
                ...
                .map(
                  (group) => html`
                    <div
                      ...
                      style="max-width: fit-content; min-width: fit-content; padding-bottom: 8px;"
                    >
                      ${group.map(
                        (i) => html`
                          <div class="internal-item" style="display: inline-block">
                            <internal-picker
                              style="pointer-events: ${this.frozen ? "none" : "normal"};"
                              @picked="${async () => {
                                this.forceUpdate();
                                await this.updateComplete;
                              ...
                              .node="${this.node}"
                              .internalIndex="${this.node.internals!.indexOf(i)}"
                            ></internal-picker>
                          </div>
                        `
                      )}
