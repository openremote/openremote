...
import type { FlowEditor } from "./flow-editor";
...
export class NodePanel extends translate(i18next)(LitElement) {
  ...
  static get styles() {
    return css`
      ...
      .input-node:hover {
        background-color: var(--input-color-h);
      }
      .processor-node:hover {
        background-color: var(--processor-color-h);
      }
      .output-node:hover {
        background-color: var(--output-color-h);
      }
    `;
  }
  ...
  private nodeTemplate(node: Node) {
    return html`<node-menu-item
      class="node-item"
      .node="${node}"
      .workspace="${this.application.editorWorkspace}"
    ></node-menu-item>`;
  }
