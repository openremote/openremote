...
import type { NodePanel } from "./node-panel";
import type { EditorWorkspace } from "./editor-workspace";
import type { TopBar } from "./top-bar";
import { type RuleView, OrRulesRuleChangedEvent } from "../..";
import type { RulesetUnion } from "@openremote/model";
...
export class FlowEditor extends translate(i18next)(LitElement) implements RuleView {
    ...
    return [
      css`
        :host {
          ...
          grid-template-areas: "workspace node-panel";
  ...
  protected render() {
    ...
    return html`
      ...
      <node-panel
        .application="${this}"
        style="grid-area: node-panel; overflow:auto;"
  ...
  private serialiseRule() {
    this._ruleset.rules = exporter.flowToJson(
      project.toNodeCollection(this._ruleset.name!, project.existingFlowRuleDesc!)
    );
  }
}
