...
export class InternalPicker extends translate(i18next)(LitElement) {
  ...
  private async setSocketTypeDynamically(value: AttributeInternalValue) {
    const results = (
      await rest.api.AssetResource.queryAssets({
        ids: [value.assetId!],
        select: {
  ...
  private get assetAttributeInput(): TemplateResult {
    ...
    return html`<div>
      ${
        this.selectedAsset
          ? html`<div
              class="attribute-label attribute-label-white selected-asset-container"
              @click="${() => openDialog()}"
            >
          ...
          : html`<or-mwc-input
              class="attribute-label-white"
              .type="${InputType.BUTTON}"
              label="attribute"
              icon="plus"
              @or-mwc-input-changed="${() => openDialog()}"
            ></or-mwc-input>`
      }
    </div>`;
  }
  ...
  private get dropdownInput(): TemplateResult {
    return html`<writable-dropdown
      @input="${(e: any) => this.setValue(e.target.value)}"
      .value="${this.internal.value}"
      .options="${this.internal.picker!.options}"
    >
    </writable-dropdown>`;
  }
  ...
  private get checkBoxInput(): TemplateResult {
    return html`<input
      type="checkbox"
      ?checked="${this.internal.value || false}"
      @input="${(e: any) => this.setValue(e.target.checked)}"
    />`;
  ...
  private get multilineInput(): TemplateResult {
    ...
    return html`<textarea
      ...
      style="${sizeString}"
      @input="${(e: any) => this.setValue(e.target.value)}"
      placeholder="${this.internal.name!}"
    >
${this.internal.value || ""}</textarea>`;
  }
