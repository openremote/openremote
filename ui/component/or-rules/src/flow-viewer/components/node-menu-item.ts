...
export class NodeMenuItem extends translate(i18next)(OrElement) {
  ...
  private get dragNodeTemplate() {
    return html`<div
      class="node-drag-item ${this.node.displayCharacter ? "small" : null}"
      style="top: ${this.y - this.yOffset}px; left: ${this.x - this.xOffset}px"
    >
      <div class="label">${this.flowNodeName}</div>
    </div>`;
  }
  ...
  private get flowNodeName() {
    return i18next.t(
      this.node.displayCharacter ? "flow." + this.node.displayCharacter : "flow." + this.node.name!,
  ...
  private stopDrag = (e: MouseEvent) => {
    ...
    if (
      Utilities.isPointInsideBox(e.offsetX, e.offsetY, {
      ...
      newIds.add(copy.id!);
      copy.position = this.workspace.offsetToWorld({
        x: e.offsetX - this.workspace.clientRect.left,
        y: e.offsetY - this.workspace.clientRect.top,
      });
