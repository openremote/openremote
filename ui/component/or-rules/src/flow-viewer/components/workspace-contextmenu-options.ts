...
import type { ContextMenuButton, ContextMenuSeparator } from "../models/context-menu-button";
...
export const createContextMenuButtons = (workspace: EditorWorkspace, e: MouseEvent) => {
  ...
  const selectedConnections = input.selected.filter(
    (s) => s instanceof ConnectionLine && s.selected
  ) as ConnectionLine[];
    ...
    let nB: (ContextMenuButton | ContextMenuSeparator)[] = [
      {
        ...
        icon: "arrow-left",
        action: () => {
          workspace.dispatchEvent(new MouseEvent("contextmenu", e));
        },
      },
    ];
    nB = nB.concat(
        ...
        .map((node) => {
          const b: ContextMenuButton = {
            ...
            action: () => {
              ...
              copy.position = workspace.offsetToWorld({
                x: e.offsetX - workspace.offsetLeft,
                y: e.offsetY - workspace.offsetTop,
              });
  ...
  let buttons: (ContextMenuButton | ContextMenuSeparator)[] = [
    {
      ...
      action: () => {
        ContextMenu.open(e.pageX, e.pageY, workspace, createNodeButtons(NodeType.INPUT));
      },
    },
    {
      type: "button",
      ...
      icon: "cog",
      action: () => {
        ContextMenu.open(e.pageX, e.pageY, workspace, createNodeButtons(NodeType.PROCESSOR));
      },
    },
    {
      type: "button",
      ...
      action: () => {
        ContextMenu.open(e.pageX, e.pageY, workspace, createNodeButtons(NodeType.OUTPUT));
      },
    },
  ];
  ...
  buttons = buttons.concat([
    ...
    },
    {
      ...
      disabled:
        ...
        (selectedNodes.length !== 0 && selectedNodes.every((n) => n.frozen)),
    },
    { type: "separator" },
    {
      type: "button",
    ...
    { type: "separator" },
    {
      ...
      action: () => {
        modal.anything(
          ...
          html`
            <div
              style="user-select: all;font-family: monospace; padding: 5px; overflow: auto; background: whitesmoke; max-width:50vw; max-height:50vh"
            >
