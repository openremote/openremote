...
import type { FlowNodeSocket } from "./flow-node-socket";
import type { LightNodeCollection } from "../models/light-node-collection";
import { EditorWorkspaceStyle } from "../styles/editor-workspace-style";
import { i18next, translate } from "@openremote/or-translate";
import type { FlowNode } from "./flow-node";
import { ContextMenu } from "./context-menu";
import { project, input, copyPasteManager, type FlowEditor } from "./flow-editor";
...
export class EditorWorkspace extends translate(i18next)(LitElement) {
  ...
  private get isCameraInDefaultPosition() {
    ...
    return (
      Math.abs(this.camera.x) < errorMargin &&
      Math.abs(this.camera.y) < errorMargin &&
      this.camera.zoom > 1 - errorMargin &&
      this.camera.zoom < 1 + errorMargin
    );
  }
  ...
  @property({ attribute: false }) public zoomLowerBound = 0.2;
  ...
  public fitCamera(nodes: Node[], padding = 25) {
    ...
    const XoutermostNode = enumerable
      .OrderByDescending((a: Node) => a!.position!.x! + (IdentityDomLink.map[a.id!] as FlowNode).scrollWidth)
      .First() as Node;
    const YoutermostNode = enumerable
      .OrderByDescending((a: Node) => a!.position!.y! + (IdentityDomLink.map[a.id!] as FlowNode).scrollHeight)
      .First() as Node;
