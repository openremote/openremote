 ...
 */
import { OrTreeMenu, TreeMenuSelection, type TreeMenuSorting, type TreeNode } from "@openremote/or-tree-menu";
import { customElement, property, state } from "lit/decorators.js";
import { type RealmRuleset, RulesetLang, RulesetStatus, type RulesetUnion } from "@openremote/model";
import {
  createMenuBarItem,
  type MenuBarItem,
  type SubMenuItem,
...
export class OrRuleTree extends OrTreeMenu {
  ...
  sortOptions: any[] = [
    RuleTreeSorting.NAME,
    RuleTreeSorting.CREATED_ON,
    RuleTreeSorting.STATUS,
    RuleTreeSorting.LANGUAGE,
  ];
  ...
  protected willUpdate(changedProps: PropertyValues) {
    if (changedProps.has("rules")) {
      if (!this.rules) {
        ...
        const emptyGroups = this._cachedEmptyGroupNodes.filter((cachedNode) => !ruleNodeIds.has(cachedNode.id));
  ...
  protected _getSingleNodeSlotTemplate(node: RuleTreeNode): TemplateResult {
    return html`
      ...
      <or-icon
        slot="suffix"
        icon=${ifDefined(this._getRulesetStatusIcon(node.ruleset?.status))}
  ...
  protected _getGroupNodeSlotTemplate(node: RuleTreeNode): TemplateResult {
    let icon;
    const childrenStatus = (node.children as RuleTreeNode[] | undefined)
      ?.map((child) => child.ruleset?.status)
      .filter((status) => status) as RulesetStatus[];
    const hasError =
      childrenStatus?.find((c) =>
        [RulesetStatus.LOOP_ERROR, RulesetStatus.EXECUTION_ERROR, RulesetStatus.COMPILATION_ERROR].includes(c)
      ) || false;
    if (hasError) {
      const errorCounts = [
        RulesetStatus.LOOP_ERROR,
        RulesetStatus.EXECUTION_ERROR,
        RulesetStatus.COMPILATION_ERROR,
      ].map((err) => [err, childrenStatus.filter((s) => s === err).length] as [RulesetStatus, number]);
    ...
    return html`
      ...
      <div id="tree-header">
        ...
        <div id="tree-header-actions">
          ${when(
            this._selectedTypes.size === 1 && this._selectedTypes.has("ruleset") && !this.readonly,
            () => html`
              ${when(
                this._findSelectedTreeNodes().length === 1 && !this._isSelectedLegacyJavascriptRuleset(),
                () => html`
          ...
          ${when(
            this._selectedTypes.size === 1 && this._selectedTypes.has("group") && !this.readonly,
            () => html`
              <or-vaadin-button theme="icon" @click=${(ev: Event) => this._onDeleteClicked(ev)}>
                <or-icon icon="delete"></or-icon>
              </or-vaadin-button>
          ...
          ${this._getAddActionTemplate()} ${this._getSortActionTemplate(this.sortBy, this.sortOptions)}
        </div>
      </div>
    `;
  }
  ...
  protected _onDeleteClicked(_ev: Event) {
    ...
    if (selected.length > 0) {
      ...
      const selectedRuleNodes = selectedRules.map((ruleset) => ({ ruleset, selected: true }) as RulesetNode);
      ...
      Util.dispatchCancellableEvent(this, new OrRulesRequestDeleteEvent(selectedRuleNodes)).then((detail) => {
        if (detail.allow) {
  ...
  protected _dispatchSelectEvent(nodes?: RuleTreeNode[]): boolean {
    const lastSelected = this._lastSelectedNode
      ? (this._getTreeNodeFromTree(this._lastSelectedNode) as RuleTreeNode)
      : undefined;
    ...
    const oldNodes = lastSelected
      ? isGroupNode(lastSelected)
        ? [{ type: "group", groupId: lastSelected.id } as RulesetGroupNode]
        : [{ type: "rule", ruleset: lastSelected.ruleset!, selected: false } as RulesetNode]
      : [];
    const newNodes = newSelected
      ? isGroupNode(newSelected)
        ? [{ type: "group", groupId: newSelected.id } as RulesetGroupNode]
        : [{ type: "rule", ruleset: newSelected.ruleset!, selected: true } as RulesetNode]
      : [];

    // Ask for consumers if selection is allowed. If event.preventDefault() is called, we cancel the selection
    const success = this.dispatchEvent(new OrRulesRequestSelectionEvent({ oldNodes, newNodes }));
    if (!success) return false;

    // If succeeded, actually sent the "select" event.
    this.dispatchEvent(new OrRulesSelectionEvent({ oldNodes, newNodes }));
  ...
  protected _getRuleNodes(rules: RulesetUnion[]): RuleTreeNode[] {
    ...
    const groupedNodes = Array.from(groupList).map(
      (group) =>
        ({
          id: group,
          label: group,
          children: rules
            .filter((r) => (r.meta as any)?.groupId === group)
            .map(
              (rule) =>
                ({
                  id: rule.id?.toString(),
    ...
    const ungroupedNodes = ungroupedRules.map(
      (rule) =>
        ({
          id: rule.id?.toString(),
          ruleset: rule,
    ...
    const menuItems: MenuBarItem[] = [
      {
        ...
        children: [
          ...
          { component: "hr" },
          {
            component: createMenuBarItem(html`<or-translate value="group"></or-translate>`),
            value: "group",
          } as SubMenuItem,
        ],
      },
  ...
  protected _onRulesetAddClick(lang: RulesetLang, global = this.global) {
    ...
    Util.dispatchCancellableEvent(this, new OrRulesRequestAddEvent(detail)).then((detail) => {
      if (detail.allow) {
        this.dispatchEvent(new OrRulesAddEvent(detail.detail));
      }
    });
  ...
  protected _getAllowedLanguages(config = this.config, global = this.global): RulesetLang[] | undefined {
    const languages = (
      config?.controls?.allowedLanguages
        ? [...config.controls.allowedLanguages]
        : [...OrRuleTree.DEFAULT_ALLOWED_LANGUAGES]
