...
import { ClientRole, RulesetLang, type RulesetUnion } from "@openremote/model";
...
import { INTUITIVE_NOT_APPLICABLE, type OrSchedulerChangedEvent, type RRulePartKeys } from "@openremote/or-scheduler";
...
export class OrRuleViewer extends translate(i18next)(OrElement) {
  ...
  public get valid() {
    return (
      ...
      this._ruleValid &&
      this.ruleset.name &&
      this.ruleset.name.length >= 1 &&
      this.ruleset.name.length < 255
    );
  }
  ...
  protected render(): TemplateResult | void {
    if (!this.ruleset) {
      return html`<div class="wrapper" style="justify-content: center">
    ...
    const viewer = RuleViewInfoMap[this.ruleset!.lang!].viewTemplateProvider(this.ruleset, this.config, readonly);
      ...
      <div id="main-wrapper" class="wrapper">
        <div id="rule-header">
          ...
          <div style="display: flex; align-items: center;">
            <or-icon
              class="${statusClass}"
              title="${i18next.t("rulesetStatus." + statusText)}"
              icon="${statusIcon}"
            ></or-icon>
          ...
          <div id="rule-header-controls">
            <span id="active-wrapper">
              ...
              <or-vaadin-checkbox
                ?checked=${this.ruleset?.enabled}
                ?disabled=${readonly || !this.ruleset.id}
  ...
  protected _onSaveClicked() {
    ...
    Util.dispatchCancellableEvent(this, new OrRulesRequestSaveEvent(this.ruleset)).then((detail) => {
      if (detail.allow) {
        this._doSave();
      }
    });
