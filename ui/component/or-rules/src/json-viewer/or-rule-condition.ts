...
import { ConditionType, getAssetTypeFromQuery, type RulesConfig } from "../index";
...
import type { OrRuleAssetQuery } from "./or-rule-asset-query";
import {
  createMenuBarItem,
  type MenuBarItem,
  type SubMenuItem,
...
export function getWhenTypesMenu(
  config?: RulesConfig,
...
export function updateRuleConditionType(
  ruleCondition: RuleCondition,
  value: string | ConditionType | undefined,
  config?: RulesConfig
) {
  if (!value) {
    ...
    ruleCondition.assets = undefined;
    const date = new Date();
    ruleCondition.cron = Util.formatCronString(
      undefined,
      undefined,
      undefined,
...
class OrRuleCondition extends translate(i18next)(OrElement) {
  ...
  protected render() {
    const showTypeSelect =
      !this.config || !this.config.controls || this.config.controls.hideConditionTypeOptions !== true;
    ...
    if (type) {
      switch (type) {
        case ConditionType.TIME:
          template = html`<or-rule-trigger-query
            id="asset-query"
            .condition="${this.ruleCondition}"
          ></or-rule-trigger-query>`;
          break;
        default:
          template = html`<or-rule-asset-query
            id="asset-query"
            .config="${this.config}"
    ...
    return html` ${typeTemplate} ${template} `;
  }
