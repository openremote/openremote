...
import {
  ...
  AssetQueryMatch,
  type AssetTypeInfo,
  type RuleActionUpdateAttribute,
  type RuleActionWriteAttribute,
  WellknownValueTypes,
...
export class OrRuleActionAttribute extends translate(i18next)(LitElement) {
  ...
  protected _getAssetType() {
    ...
    const query = this.action.target.assets
      ? this.action.target.assets
      : this.action.target.matchedAssets
        ? this.action.target.matchedAssets
        : undefined;
  ...
  protected _assetDataProvider = debounce(
    (params: ComboBoxDataProviderParams, callback: ComboBoxDataProviderCallback<{ value: any; label: string }>) => {
      const query = this.action.target?.assets ?? this.action?.target?.matchedAssets;
      const assetType = getAssetTypeFromQuery(query);
  ...
  protected render() {
    ...
    const idOptions: { value: any; label: string }[] = [{ value: "*", label: i18next.t("matched") }];
      ...
      ${when(
        ...
        () => {
          ...
          const descriptors = AssetModelUtil.getAttributeAndValueDescriptors(
            ...
            asset && asset.attributes && this.action.attributeName
              ? asset.attributes[this.action.attributeName]
              : undefined
          );
          ...
          if (asset && asset.attributes) {
            attributes = Object.values(asset.attributes).map((attr) => {
          ...
          if (this.action.attributeName) {
            const label =
              descriptors[1] && descriptors[1].name === WellknownValueTypes.BOOLEAN ? "" : i18next.t("value");
            let inputType;
            if (descriptors[0]?.format?.asSlider) inputType = InputType.NUMBER;
            attributeInput = html`<or-attribute-input
              ?compact=${descriptors[1] && descriptors[1].name === WellknownValueTypes.GEOJSONPOINT}
              .inputType="${ifDefined(inputType)}"
              @or-attribute-input-changed="${(ev: OrAttributeInputChangedEvent) => this.setActionAttributeValue(ev.detail.value)}"
            ...
            ${when(
              ...
              () => html`
                <or-vaadin-combo-box
                  ...
                  ?readonly=${this.readonly}
                  .items=${attributes}
                  value=${this.action.attributeName}
                ...
                ${attributeInput}
              `,
              () => html` <or-translate value="No attributes with write permission"></or-translate> `
            )}
          `;
        }
      )}
