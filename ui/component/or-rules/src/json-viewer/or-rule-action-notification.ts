            ...
            if (targetType === NotificationTargetType.USER) {
                ...
                targetValuesGenerator = manager.rest.api.UserResource.query(query).then(
                    async (usersResponse) => {
                        ...
                        const realmRoleOpts: [string, string][] = realm.data.realmRoles!.filter(r => Util.realmRoleFilter(r)).map(r => ["linked-" + r.name!,  linkedLabel + ": " + i18next.t("realmRole." + r.name, Util.camelCaseToSentenceCase(r.name!.replace("_", " ").replace("-", " ")))]);
