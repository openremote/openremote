...
import { ActionType, OrRulesRuleUnsupportedEvent, type RulesConfig } from "../index";
import {
  type AssetQuery,
...
export class OrRuleActionNotification extends OrElement {
  static get styles() {
    return style;
  }
  ...
  protected _onJsonRuleChanged() {
    ...
    if (this.action.notification) {
      ...
      if (message?.type === "localized") {
        const locale =
          this.config?.notifications?.[manager.displayRealm]?.defaultLanguage ||
  ...
  protected static getActionTargetTemplate(
    targetTypeMap: [string, string?][],
    action: RuleAction,
    actionType: ActionType,
    readonly: boolean,
    config: RulesConfig | undefined,
    baseAssetQuery: AssetQuery | undefined,
    onTargetTypeChangedCallback: (type: NotificationTargetType) => void,
    onTargetChangedCallback: (type: NotificationTargetType, value: string | undefined) => void
  ): PromiseLike<TemplateResult> | undefined {
    ...
    if (action.target) {
      if (
        action.target.users &&
        !action.target.conditionAssets &&
        !action.target.matchedAssets &&
        ...
        action.target.custom !== undefined &&
        !action.target.conditionAssets &&
        !action.target.matchedAssets &&
        !action.target.assets
      ) {
    ...
    if (targetType === NotificationTargetType.CUSTOM) {
      ...
      if (targetType === NotificationTargetType.USER) {
        ...
        targetValuesGenerator = manager.rest.api.UserResource.query(query).then(async (usersResponse) => {
          ...
          const realmRoleOpts: { value: any; label: string }[] = realm.data.realmRoles!.map((r) => ({
            value: `linked-${r.name!}`,
            label:
              linkedLabel +
              ": " +
              i18next.t(
                "realmRole." + r.name,
                Util.camelCaseToSentenceCase(r.name!.replace("_", " ").replace("-", " "))
              ),
          }));
          const values: { value: any; label: string }[] = usersResponse.data.map((user) => ({
            value: user.id!,
            label: user.username!,
          }));
          return [
            { value: "linkedUsers", label: linkedLabel },
            ...realmRoleOpts,
            ...values.sort(Util.sortByString((user) => user.label)),
          ];
        });
        ...
        if (action.target!.linkedUsers) {
          ...
          if (
            (userQuery.ids && userQuery.ids.length > 1) ||
            userQuery.usernames ||
            userQuery.assets ||
            userQuery.limit ||
        ...
        targetValuesGenerator = manager.rest.api.AssetResource.queryAssets(assetQuery).then((response) => {
          const values: { value: any; label: string }[] = response.data.map((asset) => ({
            value: asset.id!,
            label: asset.name! + " (" + asset.id! + ")",
          }));

          // Add additional options for assets
          const additionalValues: { value: any; label: string }[] = [
            { value: "allMatched", label: i18next.t("matched") },
          ];
            ...
            targetTypeMap.forEach((typeAndTag) => {
              if (!additionalValues.find((av) => av.value === typeAndTag[0])) {
                additionalValues.push({
                  value: typeAndTag[0],
                  label: i18next.t("matchedOfType", { type: Util.getAssetTypeLabel(typeAndTag[0]) }),
                });
              }
            });
          }
        ...
        if (!action.target) {
          ...
          if (action.target.conditionAssets) {
            console.warn(
              "Rule action asset target, conditionAssets is unsupported: " + JSON.stringify(action.target, null, 2)
            );
            return;
          }
          if (action.target.matchedAssets) {
            if (action.target.matchedAssets.types && action.target.matchedAssets.types.length > 1) {
              console.warn(
                "Rule action asset target, matchedAssets query unsupported: " + JSON.stringify(action.target, null, 2)
              );
              return;
            }
            if (action.target.matchedAssets.types && action.target.matchedAssets.types.length === 1) {
            ...
            if (action.target.assets.ids && action.target.assets.ids.length > 1) {
              console.warn(
                "Rule action asset target, assets query unsupported: " + JSON.stringify(action.target, null, 2)
              );
              return;
            }
            if (action.target.assets.ids && action.target.assets.ids.length === 1) {
  ...
  protected render() {
    ...
    if (messageType === "push") {
      baseAssetQuery = {
        types: [WellknownAssets.CONSOLEASSET],
      };
    } else {
      baseAssetQuery = {
        attributes: {
    ...
    let targetTemplate = OrRuleActionNotification.getActionTargetTemplate(
      getTargetTypeMap(this.rule),
      this.action,
      this.actionType,
      !!this.readonly,
      this.config,
      baseAssetQuery,
      (type) => this._onTargetTypeChanged(type),
      (type, value) => this._onTargetChanged(type, value)
    );
    ...
    if (message) {
      if (messageType === "push") {
        modalTemplate = html`
          <or-rule-action-dialog ?readonly=${this.readonly} @cancel="${onModalCancel}" @ok="${onModalOk}">
        ...
        const notificationConfig =
          this.config?.notifications?.[manager.displayRealm] || this.config?.notifications?.default;
        const languages = [
          ...new Set([
    ...
    targetTemplate = targetTemplate.then((targetTemplate) => html` ${targetTemplate} ${modalTemplate} `);
