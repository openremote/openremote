...
import { ActionType, getAssetTypeFromQuery, type RulesConfig } from "../index";
...
function getActionTypesMenu(config?: RulesConfig, assetInfos?: AssetTypeInfo[], selectedType?: string): SubMenuItem[] {
  ...
  if (config?.notifications) {
    multiLanguage =
      Object.entries(config.notifications).filter(
        (x) => (x[0] === manager.displayRealm || x[0] === "default") && x[1] !== undefined
      ).length > 0;
  }
  ...
  if (addEmailNotification) {
    const value: ActionType =
      multiLanguage && addEmailLocalizedNotification ? ActionType.EMAIL_LOCALIZED : ActionType.EMAIL;
    menu.push({
  ...
  if (addPushNotification) {
    const value: ActionType =
      multiLanguage && addPushLocalizedNotification ? ActionType.PUSH_LOCALIZED : ActionType.PUSH_NOTIFICATION;
    menu.push({
      checked: selectedType && selectedType === value,
      component: getMenuBarItem(
        "cellphone-message",
        html`<or-translate value="push-notification"></or-translate>`,
        NOTIFICATION_COLOR
      ),
      value,
    } as SubMenuItem);
...
class OrRuleThenOtherwise extends translate(i18next)(OrElement) {
  ...
  protected ruleRecurrenceTemplate(reset: RuleRecurrence | undefined, readonly?: boolean) {
    ...
    if (readonly) {
      ...
      recurrenceTemplate = html`
        <div style="margin-right: 6px; min-width: 180px;">
          <or-vaadin-select
            .items=${getRecurrenceOptions(this.config)}
            value=${value}
    ...
    return html` <div class="rule-recurrence">${recurrenceTemplate}</div> `;
  }
  ...
  protected ruleActionTemplate(actions: RuleActionUnion[], action: RuleActionUnion, readonly?: boolean) {
    ...
    if (type) {
      switch (type) {
        ...
        case ActionType.EMAIL_LOCALIZED:
          ...
          template = html`<or-rule-action-notification
            id="${id}"
            .rule="${this.rule}"
            .action="${action}"
            .actionType="${type}"
          ...
          ></or-rule-action-notification>`;
          break;
        case ActionType.WEBHOOK:
          template = html`<or-rule-action-webhook
            .rule="${this.rule}"
            .action="${action}"
          ...
          break;
        case ActionType.ALARM:
          template = html`<or-rule-action-alarm
            .rule="${this.rule}"
            .action="${action}"
          ...
          break;
        default:
          template = html`<or-rule-action-attribute
            .action="${action}"
            .targetTypeMap="${this.targetTypeMap}"
            .config="${this.config}"
            .assetProvider="${this.assetProvider}"
            .assetInfos="${this.assetInfos}"
            .readonly="${this.readonly}"
          ></or-rule-action-attribute>`;
          break;
      }
    }
    ...
    return html`
      <div class="rule-action">
        <div class="rule-action-wrapper">${typeTemplate} ${template}</div>
        ${
          thenAllowRemove
            ? html`
  ...
  protected getActionType(action: RuleActionUnion): string | undefined {
    switch (action.action) {
      ...
      case "notification":
        const type =
          action.notification && action.notification.message && action.notification.message.type
            ? action.notification.message.type
            : action.action;
        if (type === "localized") {
