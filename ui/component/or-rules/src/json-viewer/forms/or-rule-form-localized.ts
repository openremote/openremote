...
export class OrRuleFormLocalized extends translate(i18next)(OrElement) {
  ...
  protected render() {
    return html`
      <div>
        ...
        ${when(
          this.languages?.length && this._validLanguages && this._validLanguages.length < this.languages.length,
   ...
   */
  protected async _getLanguageSelectForm(
    selected: string,
    languageCodes: string[] = [this._selectedLanguage],
    divider = true
    ...
    return html`
      ...
      ${when(divider, () => html` <div class="divider"></div>`)}
    `;
  }
  ...
  protected async _getLanguageErrorTemplate(languages: string[], validLanguages: string[]): Promise<TemplateResult> {
    const invalidLocalesStr = languages
      .filter((l) => !validLanguages.includes(l))
      .map((l) => ISO6391.getName(l))
      .join(", ");
    return html`
      ...
      const languageEntries = Object.entries(this.message.languages).filter(([lang, msg]) => {
        const userDefinedFields = Object.entries(msg).filter((entry) => {
          ...
          if (Array.isArray(entry[1])) {
            ...
            const arrayHasValue = entry[1].some(
              (obj) => Object.keys(obj).length > 0 && Object.values(obj).some((value) => value)
            );
