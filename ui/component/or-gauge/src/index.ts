...
export class OrGauge extends LitElement {
  ...
  render() {
    ...
    return html`
                ...
                ? html`
                    <div class="mainvalue-wrapper">
                      <span class="main-number-icon"
                        >${this.asset ? getAssetDescriptorIconTemplate(AssetModelUtil.getAssetDescriptor(this.asset.type)) : ""}</span
                      >
      ...
      ${this.loading ? html` <span>${i18next.t("loading")}</span> ` : undefined}
    `;
  }
