...
const style = css`
  :host {
    --internal-or-attribute-history-background-color: var(
      --or-attribute-history-background-color,
      var(--or-app-color2, ${unsafeCSS(DefaultColor2)})
    );
    --internal-or-attribute-history-text-color: var(
      --or-attribute-history-text-color,
    ...
    --internal-or-attribute-history-controls-justify-content: var(
      --or-attribute-history-controls-justify-content,
      flex-end
    );
    --internal-or-attribute-history-graph-fill-color: var(
      --or-attribute-history-graph-fill-color,
    ...
    --internal-or-attribute-history-graph-line-color: var(
      --or-attribute-history-graph-line-color,
      var(--or-app-color4, ${unsafeCSS(DefaultColor4)})
    );
    --internal-or-attribute-history-graph-point-color: var(
      --or-attribute-history-graph-point-color,
      var(--or-app-color4, ${unsafeCSS(DefaultColor4)})
    );
    --internal-or-attribute-history-graph-point-border-color: var(
      --or-attribute-history-graph-point-border-color,
      var(--or-app-color4, ${unsafeCSS(DefaultColor4)})
    );
    ...
    --internal-or-attribute-history-graph-point-hover-color: var(
      --or-attribute-history-graph-point-hover-color,
      var(--or-app-color4, ${unsafeCSS(DefaultColor4)})
    );
    --internal-or-attribute-history-graph-point-hover-border-color: var(
      --or-attribute-history-graph-point-hover-border-color,
      var(--or-app-color4, ${unsafeCSS(DefaultColor4)})
    );
    --internal-or-attribute-history-graph-point-hover-radius: var(--or-attribute-history-graph-point-hover-radius, 4);
    --internal-or-attribute-history-graph-point-hover-border-width: var(
      --or-attribute-history-graph-point-hover-border-width,
      2
    );
    display: block;
  }
...
export class OrAttributeHistory extends translate(i18next)(LitElement) {
  ...
  shouldUpdate(_changedProperties: PropertyValues): boolean {
    const reloadData =
      _changedProperties.has("period") || _changedProperties.has("toTimestamp") || _changedProperties.has("attribute");
        ...
        <div id="controls">
          ...
          <div id="ending-controls">
            <or-vaadin-date-time-picker
              id="ending-date"
              value=${ifDefined(OrVaadinDateTimePicker.getLocalizedISOString(this.toTimestamp))}
            ...
            <or-vaadin-button
              ...
              theme="icon"
              ?disabled=${disabled}
              @click=${() => this._updateTimestamp(this.toTimestamp!, false, 0)}
            >
            ...
            <or-vaadin-button
              theme="icon"
              ?disabled=${disabled}
              @click=${() => this._updateTimestamp(this.toTimestamp!, true, 0)}
            >
            ...
            <or-vaadin-button
              theme="icon"
              ?disabled=${disabled}
              @click=${() => this._updateTimestamp(this._getDefaultDate())}
            >
    ...
    if (isChart) {
      ...
      if (!this._chart) {
        ...
        const opacity = Number(
          this._style.getPropertyValue("--internal-or-attribute-history-graph-fill-opacity").trim()
        );
        if (!isNaN(opacity)) {
        ...
        this._chartOptions = {
          ...
          xAxis: {
            ...
            axisLine: {
              onZero: false,
              lineStyle: { color: this._style.getPropertyValue("--internal-or-attribute-history-text-color") },
            },
            splitLine: { show: true },
  ...
  protected _getTableTemplate(): TemplateResult {
    ...
    const attributeName = this.attribute
      ? this.attribute.name!
      : this.attributeRef
    ...
    if (this.config && this.config.table) {
      if (
        ...
        ((this.config.table.assetTypes[assetType].attributeNames &&
          this.config.table.assetTypes[assetType].attributeNames![attributeName]) ||
          (attributeType &&
            this.config.table.assetTypes[assetType].attributeValueTypes &&
            this.config.table.assetTypes[assetType].attributeValueTypes![attributeType]))
      ) {
        config =
          (this.config.table.assetTypes[assetType].attributeNames &&
            this.config.table.assetTypes[assetType].attributeNames![attributeName]) ||
        ...
        attributeType &&
        this.config.table.attributeValueTypes &&
        this.config.table.attributeValueTypes[attributeType]
      ) {
    ...
    if (config.columns && config.columns.length > 0) {
      return html`
        <div id="table" class="mdc-data-table">
          <table
            style="${config.styles ? styleMap(config.styles) : ""}"
                ...
                return html`
                  <tr class="mdc-data-table__row">
                    ${config.columns!.map((c) => {
                      ...
                      return html`<td
                        style="${c.styles ? styleMap(c.styles) : ""}"
  ...
  protected async _loadData() {
    if (
      ...
      !this.assetType ||
      !this.assetId ||
      (!this.attribute && !this.attributeRef) ||
      !this.period ||
      !this.toTimestamp
    ) {
      return;
    ...
    try {
      ...
      if (!this._zoomChanged) {
        ...
        this._startOfPeriod = moment(this.toTimestamp)
          .subtract(1, this.period)
          .startOf(lowerCaseInterval as moment.unitOfTime.StartOf)
          .add(1, lowerCaseInterval as moment.unitOfTime.Base)
          .toDate()
          .getTime();
        this._endOfPeriod = moment(this.toTimestamp)
          .startOf(lowerCaseInterval as moment.unitOfTime.StartOf)
          .add(1, lowerCaseInterval as moment.unitOfTime.Base)
          .toDate()
          .getTime();
