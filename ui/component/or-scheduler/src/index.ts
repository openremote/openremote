...
import type { CalendarEvent } from "@openremote/model";
...
import { Frequency as FrequencyValue, RRule, type Weekday, type WeekdayStr } from "rrule";
import {
  BY_RRULE_PARTS,
  EventTypes,
  FREQUENCIES,
...
export class OrScheduler extends translate(i18next)(OrElement) {
  ...
  protected get _timeLabel(): TemplateResult | undefined {
    if (this._eventType === EventTypes.default) {
      return html`<or-translate
        value="${this.defaultEventTypeLabel}"
        .options="${this._getDefaultMessageOptions()}"
      ></or-translate>`;
    }
    ...
    if (this._scheduleWithOffset) {
      ...
      if (calendarEvent.recurrence) {
        ...
        const fromTo = {
          start: moment(calendarEvent.start).format("HH:mm"),
          end: moment(calendarEvent.end).format("HH:mm"),
        };
        ...
        if (this.isAllDay && diff > 0) {
          ...
          template = html`<or-translate
            value="fromToDays"
            .options="${{ ...fromTo, days: diff, count: diff }}"
          ></or-translate>`;
        } else if (diff === 0) {
  ...
  shouldUpdate(changedProps: PropertyValues) {
    ...
    if (changedProps.has("_rrule") && this._rrule) {
      this._byRRuleParts = BY_RRULE_PARTS.filter(
        (p) => !this.disabledByPartCombinations[FrequencyValue[this._rrule!.options.freq] as Frequency]?.includes(p)
      ).filter((p) => !this.disabledRRuleParts.includes(p));
      this._ends =
        (this._rrule?.origOptions?.until && "until") || (this._rrule?.origOptions?.count && "count") || "never";
    }
  ...
  protected render() {
    return html`
      <or-vaadin-button style="max-width: 100%" @click="${() => this._dialog!.open()}"
        >${this._timeLabel}</or-vaadin-button
      >
      <or-vaadin-dialog
        id="scheduler"
        header-title="${i18next.t(this.header)}"
        ?opened="${this.open}"
  ...
  protected _getDialogFooter(): TemplateResult {
    return html`
      ${when(
        this.removable,
        () =>
          html` <or-vaadin-button
            style="background-color: unset; margin-right: auto"
            theme="error"
      ...
      <or-vaadin-button theme="primary" @click="${this._onSave}"
        ><or-translate value="schedule.save"></or-translate
      ></or-vaadin-button>
    `;
  }
  ...
  protected _getRepeatTemplate(): TemplateResult {
    ...
    return html` <div id="recurrence" class="section">
      <h4><or-translate value="schedule.repeat"></or-translate></h4>
      <div style="display: flex; gap: 8px">
        ${when(
          !this.disabledRRuleParts?.includes("interval"),
          () => html`
      ...
      ${when(
        BY_RRULE_PARTS.some((p) => this._byRRuleParts?.includes(p)),
        () => html`
          <h4 style="margin-top: var(--lumo-space-l)">
            <or-translate value="schedule.repeatOn"></or-translate>
          </h4>
   ...
   */
  protected _getByRulePart<T extends number | [string, string]>(
    part: ByRRulePartsKeys,
    type: InputType,
    options: T[]
    ...
    const value = part === "byweekday" ? (opts as Weekday[])?.map(String) : Array.isArray(opts) ? opts : [opts];

    return type === InputType.CHECKBOX_LIST
      ? html`
  ...
  protected _getPeriodTemplate(): TemplateResult {
    ...
    return html` <div id="period" class="section">
      <h4><or-translate value="period"></or-translate></h4>
      <div style="display: flex; gap: 8px">
        <div class="period">
          <or-vaadin-date-picker
            style="text-transform: capitalize"
            ?combined="${!this.isAllDay}"
            .value="${moment(calendar.start).format("YYYY-MM-DD")}"
            @change="${this._onPartChange("start", "value")}"
            label="${i18next.t("from")}"
            .max="${moment(calendar.end).format("YYYY-MM-DD")}"
          >
          </or-vaadin-date-picker>
          <or-vaadin-time-picker
            style="margin-top: auto"
            ?hidden=${this.isAllDay}
            .value="${moment(calendar.start).format("HH:mm")}"
        ...
        <div class="period">
          <or-vaadin-date-picker
            style="text-transform: capitalize"
            ?combined="${!this.isAllDay}"
            .value="${moment(calendar.end).format("YYYY-MM-DD")}"
            @change="${this._onPartChange("end", "value")}"
            label="${i18next.t("to")}"
            .min="${moment(calendar.start).format("YYYY-MM-DD")}"
          >
          </or-vaadin-date-picker>
          <or-vaadin-time-picker
            style="margin-top: auto"
            ?hidden=${this.isAllDay}
            .value="${moment(calendar.end).format("HH:mm")}"
      ...
      <or-vaadin-checkbox
        ...
        @change="${this._onPartChange("all-day", "checked")}"
        .label="${i18next.t("allDay")}"
      ></or-vaadin-checkbox>
    </div>`;
  }
  ...
  protected _getEndsTemplate(): TemplateResult {
    return html` <div id="recurrence-ends" class="section">
      <h4><or-translate value="schedule._ends"></or-translate></h4>
      <div style="display: flex; gap: 8px;">
        <or-vaadin-radio-group
          style="padding-right: 10px; width: unset"
          .value="${this._ends}"
          theme="vertical"
        ...
        <div style="display: flex; flex-direction: column; flex: 1; ">
          ${when(
            !this.disabledRRuleParts.includes("until"),
            () =>
              html` <or-vaadin-date-time-picker
                style="margin-top: auto; padding: var(--lumo-space-s) 0"
                ?disabled="${this._ends !== "until"}"
          ...
          ${when(
            !this.disabledRRuleParts.includes("count"),
            () =>
              html`<div style="display: inline-flex">
                <or-vaadin-number-field
                  ?disabled="${this._ends !== "count"}"
                  min="1"
                  style="width: 120px"
                  step-buttons-visible
                  .value="${this._count}"
