 ...
 */
import type { Locator } from "@openremote/test";
import { ct, expect } from "./fixtures";

import {
  type ByRRulePartsKeys,
  OrScheduler,
  type OrSchedulerChangedEvent,
  type OrSchedulerRemovedEvent,
...
import type { Frequency } from "../src";
...
ct.describe("Recurrence event type should", () => {
  ct("show recurrence inputs", async ({ mount, vaadinDialog, vaadinInput, shared }) => {
    ...
    const monthsSelector = dialog.locator("or-vaadin-checkbox-group").first().getByRole("checkbox", { name: months });
