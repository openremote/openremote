...
export class OrClusterMarker extends OrElement {
  ...
  protected _createDonutChart(slices: Slice[]): TemplateResult {
    ...
    return html`
      ...
      >
        ${(slices as [] as SliceWithOffset[]).map(([, color, count, offset]) =>
          this._donutSegment(offset / total, (offset + count) / total, r2, innerRadius, color, dR)
        )}
        <circle cx="${r}" cy="${r}" r="${innerRadius}" fill="white" />
        <text dominant-baseline="central" transform="translate(${r}, ${r})">${formatCount(total)}</text>
      </svg>
    `;
  }
