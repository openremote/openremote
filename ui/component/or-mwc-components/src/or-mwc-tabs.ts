...
export class OrMwcTabs extends LitElement {
  static get styles() {
    return [
      unsafeCSS(tabStyle),
      unsafeCSS(tabBarStyle),
      unsafeCSS(tabIndicatorStyle),
      unsafeCSS(tabScrollerStyle),
      tabStyling,
    ];
  }
  ...
  constructor() {
    ...
    this.updateComplete.then(() => {
      ...
      if (tabBarElement != null) {
        ...
        if (this.iconPosition === "top") {
          if (
            this.items?.find((item) => {
              return item.name != null && item.icon != null;
            }) != undefined
          ) {
            tabBarElement.querySelectorAll(".mdc-tab").forEach((value: Element) => {
              value.classList.add("mdc-tab-vertical");
            });
            tabBarElement.querySelectorAll(".mdc-tab__content").forEach((value: Element) => {
              value.classList.add("mdc-tab-vertical__content");
            });
            tabBarElement.querySelectorAll(".mdc-tab__text-label").forEach((value: Element) => {
              value.classList.add("mdc-tab-vertical__text-label");
            });
          }
        }
        ...
        if (this.color != null) {
          tabBarElement.querySelectorAll(".mdc-tab__text-label").forEach((value: Element) => {
            (value as HTMLElement).style.color = this.color as string;
          });
          tabBarElement.querySelectorAll(".mdc-tab__icon").forEach((value: Element) => {
            (value as HTMLElement).style.color = this.color as string;
          });
          tabBarElement.querySelectorAll(".mdc-tab-indicator__content--underline").forEach((value: Element) => {
            (value as HTMLElement).style.borderColor = this.color as string;
          });
          tabBarElement.querySelectorAll(".mdc-tab__ripple").forEach((value: Element) => {
            (value as HTMLElement).style.setProperty("--ripplecolor", this.color as string);
          });
        }
      }
    });
                ...
                ${this.items?.map((menuItem) => {
                  return html`
                    <button
                      ...
                      role="tab"
                      aria-selected="false"
                      tabindex="${this.items?.indexOf(menuItem)}"
                    >
