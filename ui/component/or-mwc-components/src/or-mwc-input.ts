...
import { MDCSlider, type MDCSliderChangeEventDetail } from "@material/slider";
import { MDCSelect, type MDCSelectEvent } from "@material/select";
import { MDCList, type MDCListActionEvent } from "@material/list";

import { MDCFormField, type MDCFormFieldInput } from "@material/form-field";
import { MDCIconButtonToggle, type MDCIconButtonToggleEventDetail } from "@material/icon-button";
...
import { getItemTemplate, getListTemplate, type ListItem, ListType } from "./or-mwc-list";
...
/** @deprecated Please use `or-vaadin-input` instead. */
export type ValueInputTemplateFunction =
  | ((
      value: any,
      focused: boolean,
      loading: boolean,
    ...
    ) => TemplateResult | PromiseLike<TemplateResult>)
  | undefined;

/** @deprecated Please use `or-vaadin-input` instead */
export type ValueInputProviderGenerator = (
  assetDescriptor: AssetDescriptor | string,
  valueHolder: (NameHolder & ValueHolder<any>) | undefined,
  valueHolderDescriptor: ValueDescriptorHolder | undefined,
  valueDescriptor: ValueDescriptor,
  valueChangeNotifier: (value: (OrInputChangedEventDetail & { errors?: boolean }) | undefined) => void,
  options: ValueInputProviderOptions
) => ValueInputProvider;
...
const style = css`
  :host {
    ...
    --internal-or-mwc-input-text-color: var(
      --or-mwc-input-text-color,
      var(--or-app-color8, ${unsafeCSS(DefaultColor8)})
    );
  ...
  .mdc-text-field.mdc-text-field--invalid:not(.mdc-text-field--disabled)
...
export class OrMwcInput extends OrElement {
  static get styles() {
    return [
      ...
      `,
      css`
        ${unsafeCSS(buttonFabStyle)}
      `,
      css`
        ${unsafeCSS(textfieldStyle)}
      `,
      css`
        ${unsafeCSS(rippleStyle)}
      `,
      css`
        ${unsafeCSS(lineRippleStyle)}
      `,
      css`
        ${unsafeCSS(floatingLabelStyle)}
      `,
      css`
        ${unsafeCSS(formFieldStyle)}
      `,
      css`
        ${unsafeCSS(checkboxStyle)}
      `,
      css`
        ${unsafeCSS(radioStyle)}
      `,
      css`
      ...
      `,
      css`
        ${unsafeCSS(menuSurfaceStyle)}
      `,
      css`
        ${unsafeCSS(sliderStyle)}
      `,
      style,
    ];
  }
  ...
  public focus() {
    if (
      ...
      this._mdcComponent2 &&
      typeof (this._mdcComponent2 as any).focus === "function"
    ) {
  ...
  protected render() {
    if (this.type) {
      ...
      const labelTemplate = showLabel
        ? html`<span class="mdc-floating-label ${hasValue ? "mdc-floating-label--float-above" : ""}" id="label"
            >${this.label}</span
          >`
        : undefined;
      ...
      switch (this.type) {
        case InputType.RADIO:
          ...
          return html`
            <div class="mdc-radio-container">
              ${
                optsRadio
                  ? optsRadio.map(([optValue, optDisplay]) => {
                      return html` <div id="field" class="mdc-form-field">
                        <div class="mdc-radio">
                          <input
                            type="radio"
                            id="elem-${optValue}"
        ...
        case InputType.LIST:
          ...
          return html`
            <div
              id="component"
              class="mdc-list mdc-select ${classMap(classesList)}"
              @MDCList:action="${(e: MDCListActionEvent) => this.onValueChange(undefined, e.detail.index === -1 ? undefined : Array.isArray(this.options![e.detail.index]) ? this.options![e.detail.index][0] : this.options![e.detail.index])}"
            >
              <ul class="mdc-list">
                ${
                  optsList
                    ? optsList.map(([optValue, optDisplay], index) => {
                        ...
                        return html`<li
                          class="${classMap({ "mdc-list-item": true, "mdc-list-item--selected": this.value === optValue })}"
                          role="option"
                          data-value="${optValue}"
                        >
                          <or-translate value="${optDisplay}"></or-translate>
                        </li>`;
                      })
                    : ``
                }
        ...
        case InputType.SELECT:
          ...
          const itemClickHandler: (ev: MouseEvent, item: ListItem) => void = (ev, item) => {
            ...
            if (this.multiple) {
              ...
              const inputValue =
                this._tempValue ??
                (Array.isArray(this.value) ? [...this.value] : this.value !== undefined ? [this.value] : []);
          ...
          const listTemplate = (options: [any, string][]) => {
            if (this.searchProvider != undefined && (!options || options.length == 0)) {
              return html`<span class="mdc-text-field-helper-line" style="margin: 8px 8px 8px 0;"
                >${i18next.t("noResults")}</span
              >`;
            } else {
            ...
            <div
              ...
              @MDCSelect:change="${async (e: MDCSelectEvent) => {
                ...
                this.onValueChange(
                  ...
                  e.detail.index === -1
                    ? undefined
                    : Array.isArray(options[e.detail.index])
                      ? options[e.detail.index][0]
                      : options[e.detail.index]
                );
              }}"
            >
              <div
              ...
              <div
                id="mdc-select-menu"
                class="mdc-select__menu mdc-menu mdc-menu-surface mdc-menu-surface--fixed ${this.searchProvider != undefined ? "mdc-menu__searchable" : undefined}"
                @MDCMenuSurface:closed="${menuCloseHandler}"
              >
                ${when(
                  ...
                  () => html`
                    <label id="select-searchable" class="mdc-text-field mdc-text-field--filled">
                      <span
                        ...
                        style="color: rgba(0, 0, 0, 0.6); text-transform: capitalize; visibility: ${this.searchableValue ? "hidden" : "visible"}"
                        id="my-label-id"
                      >
                ...
                ${when(
                  ...
                  () => {
                    return until(
                      ...
                      html`<span class="mdc-text-field-helper-line" style="margin: 8px 8px 8px 0;"
                        >${i18next.t("loading")}</span
                      >`
                    );
                  }
                )}
              ...
              ${
                hasHelper || showValidationMessage
                  ? html` <p
                      id="component-helper-text"
                      class="mdc-select-helper-text ${classMap(helperClasses)}"
                      aria-hidden="true"
                    >
        ...
        case InputType.BUTTON_TOGGLE:
          return html`
            <button
              id="component"
              class="mdc-icon-button ${this.value ? "mdc-icon-button--on" : ""}"
        ...
        case InputType.CHECKBOX_LIST:
          ...
          return html`
            <div class="mdc-checkbox-list">
              ${
                optsCheckboxList
                  ? optsCheckboxList.map(([optValue, optDisplay], index) => {
                      if (this.value === optValue) {
                        this._selectedIndex = index;
                      }
                      return html`
          ...
          return html`
            <div id="component" style="width: 100%; display: inline-flex; align-items: center; padding: 8px 0;">
              <input
                type="color"
                id="elem"
                style="border: none; height: 31px; width: 31px; padding: 1px 3px; min-height: 22px; min-width: 30px;cursor: pointer"
                value="${this.value}"
                ?disabled="${this.disabled || this.readonly}"
                ?required="${this.required}"
        ...
        case InputType.JSON_OBJECT: {
          ...
          let valMinMax: [any, any, any] = [
            this.value === undefined || this.value === null ? undefined : this.value,
            this.min,
            this.max,
          ];
          ...
          if (valMinMax.some((v) => typeof v !== "string")) {
            if (this.type === InputType.JSON || this.type === InputType.JSON_OBJECT) {
                ...
                case InputType.TIME:
                  ...
                  format.timeStyle =
                    this.step && this.step < 60
                      ? ValueFormatStyleRepresentation.MEDIUM
                      : ValueFormatStyleRepresentation.SHORT;
                  break;
                case InputType.DATE:
                  format.asDate = true;
              ...
              valMinMax = valMinMax.map((val) =>
                val !== undefined ? Util.getValueAsString(val, () => format, "en-GB") : undefined
              ) as [any, any, any];
            }
          }
          ...
          if (!(this.type === InputType.RANGE && this.disableSliderNumberInput)) {
            ...
            const classes = {
              ...
              "mdc-text-field--textarea":
                type === InputType.TEXTAREA || type === InputType.JSON || type === InputType.JSON_OBJECT,
            ...
            inputElem =
              type === InputType.TEXTAREA || type === InputType.JSON || type === InputType.JSON_OBJECT
                ? html` <textarea
                    id="elem"
                    class="mdc-text-field__input ${this.resizeVertical ? "resize-vertical" : ""}"
                    ?required="${this.required}"
                    ?readonly="${this.readonly}"
                    ?disabled="${this.disabled}"
                  ...
                  >
${valMinMax[0] ? valMinMax[0] : ""}</textarea>`
                : html` <input
                    type="${type}"
                    id="elem"
                    aria-labelledby="${ifDefined(label ? "label" : undefined)}"
                    ...
                    @blur="${(e: Event) => {
                      if ((e.target as HTMLInputElement).value === "") this.reportValidity();
                    }}"
            ...
            inputElem = html`
              <label id="${componentId}" class="${classMap(classes)}">
                ...
                ${outlined ? `` : html`<span class="mdc-text-field__ripple"></span>`} ${inputElem}
                ${outlined ? this.renderOutlined(labelTemplate) : labelTemplate}
                ...
                hasHelper || showValidationMessage
                  ? html`
                      <div class="mdc-text-field-helper-line">
                        <div class="mdc-text-field-helper-text ${classMap(helperClasses)}">
                          ${showValidationMessage ? this.errorMessage || this.validationMessage : this.helperText}
                        </div>
            ...
            inputElem = html`
              <span id="wrapper">
                ...
                <div
                  id="component"
                  class="${classMap(classes)}"
                  @MDCSlider:change="${(ev: CustomEvent<MDCSliderChangeEventDetail>) => this.onValueChange(undefined, ev.detail.value)}"
                >
                  <input
                    id="elem"
                    ...
                    min="${ifDefined(valMinMax[1])}"
                    max="${ifDefined(valMinMax[2])}"
                    value="${valMinMax[0] || valMinMax[1] || 0}"
                    name="slider"
                    step="${this.step || 1}"
                    ?readonly="${this.readonly}"
                    ?disabled="${this.disabled}"
                  ...
                  <div class="mdc-slider__thumb">
                    ${
                      !this.continuous
                        ? html`<div class="mdc-slider__value-indicator-container" aria-hidden="true">
  ...
  protected updated(_changedProperties: PropertyValues): void {
    ...
    if (_changedProperties.has("type")) {
      ...
      if (component && this.type) {
        switch (this.type) {
          ...
          case InputType.SELECT:
            ...
            mdcSelect.valid =
              !this.required ||
              (!this.multiple && mdcSelect.valid) ||
              (this.multiple && Array.isArray(this.value) && (this.value as []).length > 0);
            ...
            if (!this._menuObserver) {
              this._menuObserver = new IntersectionObserver((entries, observer) => {
                if (
                  (entries[0].target as HTMLElement).style.minWidth !=
                  entries[0].target.parentElement?.clientWidth + "px"
                ) {
                  (entries[0].target as HTMLElement).style.minWidth =
                    entries[0].target.parentElement?.clientWidth + "px";
                }
      ...
      if (this.type === InputType.SELECT && this._mdcComponent) {
        ...
        (this._mdcComponent as MDCSelect).valid =
          !this.required ||
          (!this.multiple && (this._mdcComponent as MDCSelect).valid) ||
          (this.multiple && Array.isArray(this.value) && (this.value as []).length > 0);
        const selectedText = this.getSelectedTextValue();
        (this._mdcComponent as any).foundation.adapter.setSelectedText(selectedText);
        (this._mdcComponent as any).foundation.adapter.floatLabel(!!selectedText);
  ...
  protected renderOutlined(labelTemplate: TemplateResult | undefined) {
    return html`
      <span class="mdc-notched-outline">
        ...
        ${labelTemplate ? html` <span class="mdc-notched-outline__notch"> ${labelTemplate} </span> ` : ``}
  ...
  protected onValueChange(
    elem: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | undefined,
        ...
        case InputType.RANGE:
          if (newValue === "") {
            ...
            try {
              ...
              newValue = this.value;
              errorMsg =
                this.type === InputType.JSON || this.type == InputType.JSON_OBJECT
                  ? i18next.t("validation.invalidJSON")
                  : i18next.t("validation.invalidNumber");
            }
          }
          break;
        case InputType.DATETIME:
  ...
  protected getSelectedTextValue(options?: [string, string][] | undefined): string {
    ...
    return !opts || !values
      ? ""
      : values
          .map((v) => opts.find(([optValue, optDisplay], index) => v === optValue))
          .map((opt) => (opt ? opt[1] : ""))
          .join(", ");
  }
}
