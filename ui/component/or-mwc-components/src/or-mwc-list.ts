...
export function getListTemplate(
  ...
  isTwoLine?: boolean,
  role?: string,
  actionHandler?: (ev: MDCListActionEvent) => void
): TemplateResult {
  role = role || "listbox";
  ...
  return html`
    <ul
      id="list"
      class="mdc-list${isTwoLine ? " mdc-list--two-line" : ""}"
      role="${ifDefined(role)}"
      @MDCList:action="${(ev: MDCListActionEvent) => actionHandler && actionHandler(ev)}"
      aria-hidden="true"
...
export function getItemTemplate(
  ...
  type: ListType,
  translate?: boolean,
  itemClickCallback?: (e: MouseEvent, item: ListItem) => void
): TemplateResult {
  if (item === null) {
    // Divider
      ...
      textTemplate = html`
        <span class="mdc-list-item__text">
          <span class="mdc-list-item__primary-text"
            >${translate && !!text ? html`<or-translate value="${text}"></or-translate>` : text}</span
          >
          <span class="mdc-list-item__secondary-text"
            >${translate && !!secondaryText ? html`<or-translate value="${secondaryText}"></or-translate>` : secondaryText}</span
          >
        </span>
      `;
    } else {
      if (type === ListType.RADIO) {
        textTemplate = html`<label class="mdc-list-item__text" for="radio-item-${index + 1}"
          >${translate && !!text ? html`<or-translate value="${text}"></or-translate>` : text}</label
        >`;
      } else {
        textTemplate = html`<span class="mdc-list-item__text" title="${translate && !!text ? i18next.t(text) : text}"
          >${translate && !!text ? html`<or-translate value="${text}"></or-translate>` : text}</span
        >`;
      }
    }
...
export class OrMwcList extends LitElement {
    ...
    if (this._mdcComponent && _changedProperties.has("values")) {
      if (!Util.objectsEqual(this.values, _changedProperties.get("values"))) {
        ...
        this.setSelectedItems(
          this.values && this.listItems
            ? (this.listItems.filter((li) => li && vals?.includes(li.value)) as ListItem[])
            : undefined
        );
      }
    }
  ...
  protected render() {
    const content = !this.listItems
      ? html``
      : html`${this.listItems.map((listItem, index) => getItemTemplate(listItem, index, Array.isArray(this.values) ? this.values : this.values ? [this.values] : [], this.type))}`;
  ...
  public get selectedItems(): ListItem[] {
    ...
    const selectedIndexes = Array.isArray(this._mdcComponent.selectedIndex)
      ? this._mdcComponent.selectedIndex
      : [this._mdcComponent.selectedIndex];
  ...
  public setSelectedItems(items: ListItem | ListItem[] | string | string[] | undefined) {
    ...
    const itemArr = (!Array.isArray(items) ? [items] : items).map((item) =>
    ...
    this._mdcComponent.selectedIndex =
      this.type === ListType.MULTI_CHECKBOX ? indexes : indexes.length >= 1 ? indexes[0] : -1;
  }
