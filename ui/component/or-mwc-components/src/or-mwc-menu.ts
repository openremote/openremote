...
import { getItemTemplate, getListTemplate, type ListItem, ListType, MDCListActionEvent } from "./or-mwc-list";
...
export function positionMenuAtElement<T extends OrMwcMenu>(menu: T, hostElement?: HTMLElement): T {
  if (!hostElement) {
...
export function getContentWithMenuTemplate(
  ...
  valueChangedCallback: (values: string[] | string) => void,
  closedCallback?: () => void,
  multiSelect = false,
      ...
      ${
        menuItems
          ? html`<or-mwc-menu
              ...
              @or-mwc-menu-changed="${(evt: OrMwcMenuChangedEvent) => {
              ...
              .fullWidth="${fullWidth}"
              id="${menuId}"
              ${ref((el) => (menuRef = el as OrMwcMenu))}
            ></or-mwc-menu>`
          : ``
      }
    </span>
...
export class OrMwcMenu extends OrElement {
  static get styles() {
    return [
      css`
        ${unsafeCSS(listStyle)}
  ...
  protected render() {
    ...
    const isTwoLine =
      this.menuItems && this.menuItems.some((item) => item && !Array.isArray(item) && !!item.secondaryText);
    ...
    return html`
      <div id="wrapper" class="mdc-menu-surface--anchor">
        <div
          class="mdc-menu mdc-menu-surface ${classMap(classes)}"
          id="menu"
          @MDCMenuSurface:closed="${this._onMenuClosed}"
        >
  ...
  protected getItemsTemplate(items: (ListItem | ListItem[] | null)[], translate?: boolean): TemplateResult {
    ...
    return html` ${items.map((item, index) => {
      ...
      return getItemTemplate(
        ...
        type,
        translate,
        (e, item) => this._itemClicked(e, item)
      );
    })}`;
  }
