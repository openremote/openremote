 ...
 */
import { type EventProviderFactory, EventProviderStatus } from "./event";
import { objectsEqual } from "./util";
import type { AttributeRef, SharedEvent } from "@openremote/model";
import { Util } from ".";
  ...
  <T extends Constructor<CustomElement>>(base: T): T & Constructor<SubscribedElement> =>
    class extends base {
      ...
      public get eventsConnected() {
        return (
          this._connectRequested && eventProviderFactory.getEventProvider()!.status === EventProviderStatus.CONNECTED
        );
      }
      ...
      public async _addEventSubscriptions(): Promise<void> {
        ...
        if (ids && ids.length > 0) {
          ...
          if (isAsset) {
            const assetSubscriptionId = await eventProviderFactory
              .getEventProvider()!
              .subscribeAssetEvents(ids, true, (event) => this._onEvent(event));
            // Check if the same IDs are in place
            const currentIds: string[] | AttributeRef[] | undefined = this._attributeRefs
              ? this._attributeRefs
              : this._assetIds;
          ...
          const subscriptionId = await eventProviderFactory
            .getEventProvider()!
            .subscribeAttributeEvents(ids, isAttributes, (event) => this._onEvent(event));
          // Check if the same IDs are in place
          const currentIds: string[] | AttributeRef[] | undefined = this._attributeRefs
            ? this._attributeRefs
            : this._assetIds;
