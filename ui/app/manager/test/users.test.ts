...
import { adminStatePath, test, userStatePath } from "./fixtures/manager";
...
setup.describe(async () => {
   ...
   */
  setup(`Create regular users with tags and search by tag`, async ({ page, manager, usersPage }) => {
    await manager.goToRealmStartPage("master");
   ...
   */
  setup(`Create service users with tags and search by tag`, async ({ page, manager, usersPage }) => {
    await manager.goToRealmStartPage("master");
    await manager.switchToRealmByRealmPicker("smartcity");
    await manager.navigateToMenuItem("Users");
   ...
   */
  setup(`Verify gateway service user is read-only`, async ({ page, manager, usersPage, assetsPage }) => {
    await manager.goToRealmStartPage("master");
    await manager.switchToRealmByRealmPicker("smartcity");
