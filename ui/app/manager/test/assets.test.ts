...
assets.forEach(({ type, name, attributes }) => {
   ...
   */
  test(`Add new ${name} asset and configure its attributes`, async ({ page, manager, assetsPage, components }) => {
   ...
   */
  test(`Update a ${name} asset's attributes and location`, async ({ page, manager, components }) => {
    await manager.setup("smartcity", { assets });
    await manager.goToRealmStartPage("smartcity");
    await manager.navigateToTab("asset");
    await page.click(`text=${name}`);
   ...
   */
  test(`Toggle read-only for two attributes on a ${name} asset`, async ({ page, manager, components }) => {
    await manager.setup("smartcity", { assets });
    await manager.goToRealmStartPage("smartcity");
    await manager.navigateToTab("asset");
    await page.click(`text=${name}`);
   ...
   */
  test(`Set "ruleState" and "storeDataPoints" for ${name} asset attributes`, async ({ page, manager, components }) => {
    await manager.setup("smartcity", { assets });
    await manager.goToRealmStartPage("smartcity");
    await manager.navigateToTab("asset");
    await page.click(`text=${name}`);
