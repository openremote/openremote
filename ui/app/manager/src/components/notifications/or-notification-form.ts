...
import type { OrAssetTree, OrAssetTreeSelectionEvent } from "@openremote/or-asset-tree";
...
export class OrNotificationForm extends OrElement {
  ...
  protected _renderPropertiesContainer() {
    ...
    const status = this.notification.error
      ? i18next.t("error")
      : this.notification.deliveredOn
        ? i18next.t("delivered")
        : i18next.t("sent");
    ...
    return html`
      <div class="propContainer">
        ...
        ${this._renderReadOnlyField("source", source)} ${this._renderReadOnlyField("status", status)}
        ${this._renderReadOnlyField("sent", sent)} ${this._renderReadOnlyField("delivered", delivered)}
      </div>
    `;
  }
  ...
  protected _renderTargetContainer(inputDisabled: boolean) {
    ...
    if (manager.isSuperUser()) {
      allowedTargetTypes.push({
        label: i18next.t("notifications.targetTypes.REALM"),
        value: NotificationTargetType.REALM,
      });
    }
    ...
    return html`
      <div class="targetContainer">
        ...
        <div class="target-area">
          ${when(
            this._targetType === NotificationTargetType.ASSET,
            () => html`
              <or-asset-tree
                id="asset-selector"
            ...
            () => html`
              ...
              >
                <or-translate
                  slot="label"
                  value="${this._targetType === NotificationTargetType.REALM ? "notifications.targetListLabel.REALM" : "notifications.targetListLabel.USER"}"
                ></or-translate>
                ${this._targetOptions
