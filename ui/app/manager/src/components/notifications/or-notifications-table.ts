...
import type { NotificationService } from "../../pages/page-notifications";
...
export class OrNotificationsSortChangedEvent extends CustomEvent<{
...
export class OrNotificationsTable extends OrMwcTable {
  ...
  protected getTableRows(notifications: SentNotification[]): TableRow[] | undefined {
    return notifications.map((notification) => {
      ...
      const title = isEmail
        ? (message as EmailNotificationMessage).subject
        : (message as PushNotificationMessage)?.title;
  ...
  protected getTargetContent(notification: SentNotification): TemplateResult {
    ...
    if (!details) {
      ...
      return html`
        <span class="target-loading"> ${notification.targetId ? html`<or-translate value="loading" />` : "-"} </span>
      `;
    }
    ...
    return html`
      <div class="target-wrapper">
        <or-icon class="target-icon" icon="${iconMap[details.type] || "help-circle-outline"}"> </or-icon>
        ${
          details.link
  ...
  protected async resolveAllTargetDetails(notifications: SentNotification[]) {
    ...
    notifications.forEach((n) => {
      ...
      if (n.target === NotificationTargetType.ASSET) assetIds.add(n.targetId);
        ...
        this.targetDetailsMap.set(this.getTargetMapKey(n.targetId, n.target), {
          name: n.targetId,
          type: "realm",
          link: "",
        });
    ...
    await Promise.all([
      this.resolveTargetDetails(
        assetIds,
        NotificationTargetType.ASSET,
        "asset",
