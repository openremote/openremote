...
import { AlarmSeverity, AlarmStatus, type Asset, type SentAlarm } from "@openremote/model";
  ...
  protected getSeverityContent(severity?: AlarmSeverity): TemplateResult | string | number {
    ...
    return html`
      <or-translate
        ...
        value="${severity ? `alarm.severity_${severity}` : "error"}"
      ></or-translate>
    `;
  }
  ...
  protected sortTemplateRows(cellA: any, cellB: any, cIndex: number, sortDirection: "ASC" | "DESC"): number {
    ...
    if (valueA !== undefined && valueB !== undefined) {
      if (valueA.includes("alarm.status") && valueB.includes("alarm.status")) {
        const sortingArr = [
          "alarm.status_OPEN",
          "alarm.status_ACKNOWLEDGED",
          "alarm.status_IN_PROGRESS",
