...
import type { ClientRole, ProvisioningConfig, X509ProvisioningData } from "@openremote/model";
...
export class PageProvisioning extends Page<AppStateKeyed> {
  ...
  @state()
  protected _configFilter: (configs: ProvisioningConfig<any, any>[]) => ProvisioningConfig<any, any>[] = (configs) =>
    configs;
          ...
          <div id="table-users" class="mdc-data-table">
            <table class="mdc-data-table__table" aria-label="attribute list">
              ...
              <tbody class="mdc-data-table__content">
                ${this._configFilter(this._configs).map((config, index) =>
                  this._getConfigTemplate(() => {
                    this._configs.pop();
                ...
                ${
                  ...
                  (this._configs.length > 0 && !!this._configs[this._configs.length - 1].id)
                    ? html`
                        <tr
                        ...
                        >
                          <td colspan="100%">
                            <a class="button"
                              ><or-icon icon="plus"></or-icon>${i18next.t("add")} ${i18next.t("provisioningConfig")}</a
                            >
                          </td>
                        </tr>
                      `
  ...
  protected async _loadConfigs() {
    ...
    const responseAndStateOK: <T extends GenericAxiosResponse<any[]>>(response: T, errorMsg: string) => boolean = (
      response,
      errorMsg
    ) => {
    ...
    const rolesResponse = await manager.rest.api.UserResource.getClientRoles(
    ...
    this._roleOptions = rolesResponse.data
  ...
  protected _getConfigTemplate(addCancel: () => void, config: ProvisioningConfig<any, any>): TemplateResult {
    ...
    if (config.type === "x509") {
      ...
      typeColumnContents = html`
        <or-vaadin-text-area
          ...
          @change=${(ev: Event) => {
            data.CACertPEM = (ev.currentTarget as HTMLInputElement).value;
            this.requestUpdate();
          }}
        >
      ...
      typeColumnContents = html` <or-translate .value="notSupported"></or-translate> `;
    }
    ...
    return html`
      <tr class="mdc-data-table__row" @click="${(ev) => this._toggleConfigExpand(ev)}">
        ...
        <td class="padded-cell mdc-data-table__cell">${!config.id ? "" : config.realm}</td>
        <td class="padded-cell mdc-data-table__cell">${!config.id ? "" : config.type}</td>
        <td class="padded-cell mdc-data-table__cell">
            ...
            <div class="row">
              <div class="column">
                <or-vaadin-text-field
                  ...
                  value=${config.name}
                  @change=${(ev: Event) => {
                    config.name = (ev.currentTarget as HTMLInputElement).value;
                    this.requestUpdate();
                  }}
                >
                ...
                <or-vaadin-select
                  required
                  ?readonly=${!!config.id}
                  .items=${this._realmOptions}
                  value=${config.realm}
                  @change=${(ev: Event) => {
                    config.realm = (ev.currentTarget as OrVaadinSelect).value;
                    this.requestUpdate();
                  }}
                >
                ...
                <or-vaadin-multi-select-combo-box
                  ...
                  @change=${(ev: Event) => {
                    config.userRoles = (ev.currentTarget as OrVaadinMultiSelectComboBox).selectedItems.map(
                      (i) => i.value as ClientRole
                    );
                  }}
                >
                ...
                <or-mwc-input
                  label="${i18next.t("assetTemplate")}"
                  .type="${InputType.JSON}"
                  .value="${config.assetTemplate ? JSON.stringify(JSON.parse(config.assetTemplate), null, 2) : undefined}"
                  resizeVertical
                  @or-mwc-input-changed="${(e: OrInputChangedEvent) => (config.assetTemplate = e.detail.value ? JSON.stringify(e.detail.value) : undefined)}"
                ></or-mwc-input>
                <or-vaadin-checkbox
                  ?checked=${config.restrictedUser}
              ...
              <div class="column">${typeColumnContents}</div>
            </div>
