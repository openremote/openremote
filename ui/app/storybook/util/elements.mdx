...
export const DocsBadge = (props) => {
  ...
  switch (props?.type) {
    case "NPM": {
      return (
        <a href={`https://npmjs.com/package/${props.packageJson.name}`}>
          <img alt="NPM Version" src={`https://img.shields.io/npm/v/${props.packageJson.name}`} height="24"></img>
        </a>
      );
    }
    case "License": {
      if (!props.packageJson.repository) {
        return <i>GitHub repository was not specified in package.json file.</i>;
      }
      return (
        <a href={props.packageJson.repository}>
