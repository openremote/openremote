...
import type { Dashboard } from "@openremote/model";
import { i18next } from "@openremote/or-translate";
import type { ListItem } from "@openremote/or-mwc-components/or-mwc-list";
import type { OrMwcDrawer } from "@openremote/or-mwc-components/or-mwc-drawer";
...
export class DashboardMenu extends OrElement {
  ...
  protected promptRealmSwitch() {
    showDialog(
      new OrMwcDialog()
        ...
        .setActions(
          Object.entries(this.realms).map(([key, value]) => ({
  ...
  protected render(): TemplateResult {
    ...
    const headerTemplate = html`
      <div
        style="display: flex; align-items: center; gap: 16px; padding: 13px 16px; border-bottom: 1px solid var(--or-app-color5, ${unsafeCSS(DefaultColor5)});"
      >
        ${
          this.loading
            ? html`
            ...
            : html`
                <img
                  ...
                  width="34"
                  height="34"
                  alt="${curRealm.displayName} Logo"
                  src="${this.logoMobileSrc}"
                />
    ...
    return html`
            <or-mwc-drawer id="drawer" .header="${headerTemplate}" .dismissible="${true}">
                <div id="drawer-wrapper">
                    ...
                    <div id="drawer-actions-container">
                        <or-mwc-list .listItems="${actionItems}" .values="${this.selectedActions}" @or-mwc-list-changed="${(
                          ev: CustomEvent
                        ) => {
                          switch (ev.detail[0].value) {
                            case "realm": {
                              this.promptRealmSwitch();
                              break;
                            }
                            case "logout": {
                              this.logout();
            ...
            ${when(
              this.isDrawerOpen(),
              () => html` <div id="drawer-custom-scrim" @click="${() => this.toggleDrawer(false)}"></div> `
            )}
        `;
  }
}
...
function getDashboardListTemplate(
        ...
        return html`
          <div style="padding: 16px 0;">
            <div id="list-container">
              ${when(
                menuItems[0] == undefined,
                () => html` <span style="width: 100%; text-align: center;">${i18next.t("noDashboardFound")}</span> `,
                () => html`
                  <div>
                    <or-mwc-list
                      .listItems="${menuItems}"
