...
import { type Dashboard, DashboardRefreshInterval, type Realm } from "@openremote/model";
import { registerWidgetTypes, intervalToMillis, type OrDashboardPreview } from "@openremote/or-dashboard-builder";
import manager from "@openremote/core";
import type { DashboardMenu } from "../components/dashboard-menu";
...
export function pageViewProvider(
  store: EnhancedStore<AppStateKeyed>,
  realmConfigs: { [p: string]: RealmAppConfig }
): PageProvider<AppStateKeyed> {
  return {
    name: "view",
    routes: ["view", "view/:id", "view/:id/:showMenu"],
    pageCreator: () => {
...
export class PageView extends Page<AppStateKeyed> {
  ...
  constructor(props) {
    ...
    if (manager.authenticated) {
      const userPromise = manager.rest.api.UserResource.getCurrent().then((response: any) => {
    ...
    const realmPromise = manager.rest.api.RealmResource.getAccessible().then((response: any) => {
      this._loadedRealms = response.data;
    });
  ...
  stateChanged(state: AppStateKeyed): void {
    ...
    this.viewDashboardOnly =
      state.app.params?.showMenu != undefined ? state.app.params?.showMenu.toLowerCase() != "true" : false;
  ...
  willUpdate(changedProps: Map<string, any>) {
    ...
    if (
      this._loadedDashboards == undefined &&
      !this.getPromise("dashboard/all") &&
      !this.getPromise("dashboard/" + this._selectedId)
    ) {
  ...
  protected async fetchAllDashboards(realmName?: string): Promise<Dashboard[]> {
    ...
    if (promise === undefined) {
      promise = this.registerPromise(
        "dashboard/all",
        manager.rest.api.DashboardResource.getAllRealmDashboards(realmName),
        true,
        false
      );
    }
  ...
  protected async fetchDashboard(id: string, loginRedirect: boolean = true): Promise<Dashboard | undefined> {
    ...
    if (promise == undefined) {
      promise = this.registerPromise(
        "dashboard/" + id,
        manager.rest.api.DashboardResource.get(this._realm, id),
        true,
        false
      );
    }
    ...
    return html`
      ${when(!this.viewDashboardOnly, () => {
        ...
        return html`
          <dashboard-menu
            id="dashboard-menu"
            .dashboards="${this._loadedDashboards}"
            .realms="${realms}"
            .realm="${this._realm}"
            .selectedId="${this._selectedId}"
            .userId="${this._userId}"
            .loading="${this.isLoading()}"
            .logoMobileSrc="${logoMobile}"
      ...
      <div style="flex: 1; ${styleMap(pageStyles)}">
        ${getDashboardHeaderTemplate(
          !this.viewDashboardOnly,
          selected,
          () => this.dashboardMenu?.toggleDrawer(),
          () => this.dashboardPreview?.refreshWidgets()
        )}
        <div style="flex: 1;">
          ${guard(
            [this._selectedId, this._loadedDashboards, this.rerenderPending],
            () => html`
              ${when(
                this._selectedId && selected !== undefined,
                () => html`
                  <or-dashboard-preview
                    style="background: transparent;"
                    .rerenderPending="${this.rerenderPending}"
  ...
  registerPromise(
    ...
    promise.finally(() => {
      this.completePromise(topic, updateOnComplete);
    });
    if (doUpdate) {
      this.requestUpdate("_activePromises");
    }
    return promise;
  }
...
function getDashboardHeaderTemplate(
  showMenu = true,
  selected?: Dashboard,
      ...
      <div id="fullscreen-header-wrapper">
        ...
        <div id="fullscreen-header-actions">
          <div id="fullscreen-header-actions-content">
            ...
            ></or-mwc-input>
            <dashboard-refresh-controls .interval="${refreshInterval}" .readonly="${true}"></dashboard-refresh-controls>
          </div>
        </div>
      </div>
    </div>
