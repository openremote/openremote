...
public class TokenVerifierImpl implements TokenVerifier {
  protected static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.API, TokenVerifierImpl.class.getName());
  ...
  protected final Map<String, JWTProcessor<SecurityContext>> processorCache =
      new ConcurrentHashMap<>();
  protected final boolean disableIssuerValidation;

  public TokenVerifierImpl(
      String keycloakUrl, String keycloakPublicUrl, boolean disableIssuerValidation) {
  ...
  protected JWTProcessor<SecurityContext> initialiseProcessor(String realm) {
    ...
    // 3. Configure the processor with a key selector for the appropriate JWS algorithm (e.g.,
    // RS256)
    ...
    final JWTClaimsSetVerifier<SecurityContext> defaultClaimsVerifier =
    ...
    JWTClaimsSetVerifier<SecurityContext> verifier =
        (claims, context) -> {
          ...
          // Keycloak uses azp to store client ID by default but we support aud for non keycloak
          // auth servers
          String azp = null;
          try {
          ...
          if (claims.getAudience() == null || !claims.getAudience().contains(expectedClientId)) {
            ...
            try {
              ...
              if (preferredUsername == null
                  || !preferredUsername.startsWith(User.SERVICE_ACCOUNT_PREFIX)) {
  ...
  public TokenPrincipal verify(String realm, String token) throws AuthenticationException {
    ...
    try {
      ...
      if (!Objects.equals(authRealm, realm)) {
        if (!Constants.MASTER_REALM.equals(authRealm)
            || !principal.hasRealmRole(Constants.SUPER_USER_REALM_ROLE)) {
