...
public abstract class BasicIdentityProvider implements IdentityProvider {
  ...
  /**
   * This is a fix for {@link BasicAuthenticationMechanism} which doesn't conform to RFC2617. see:
   * https://issues.redhat.com/browse/UNDERTOW-1727
   *
   * <p>When no {@link io.undertow.util.Headers#AUTHORIZATION} header is supplied then a 401 is
   * returned with {@link io.undertow.util.Headers#WWW_AUTHENTICATE} header unless {@link #silent}
   * is true in which case a 403 will be returned.
   *
   * <p>When an {@link io.undertow.util.Headers#AUTHORIZATION} header is supplied and is valid then
   * the request can proceed otherwise a 403 is returned.
   */
  protected static class BasicFixAuthenticationMechanism extends BasicAuthenticationMechanism {
    ...
    protected static class Factory implements AuthenticationMechanismFactory {
      ...
      public AuthenticationMechanism create(
        ...
        Charset charset =
            charsetString == null ? StandardCharsets.UTF_8 : Charset.forName(charsetString);
        ...
        if (userAgentString != null) {
          ...
          if (parts.length % 2 != 0) {
            throw UndertowMessages.MESSAGES.userAgentCharsetMustHaveEvenNumberOfItems(
                userAgentString);
          }
        ...
        return new BasicFixAuthenticationMechanism(
            realm,
            mechanismName,
            silent != null && silent.equals("true"),
            identityManager,
            charset,
            userAgentCharsets);
      }
    ...
    public BasicFixAuthenticationMechanism(
        String realmName,
        String mechanismName,
        boolean silent,
        IdentityManager identityManager,
  ...
  public void init(Container container) {
    ...
    this.persistenceService
        ...
        .add("classpath:org/openremote/container/persistence/schema/basicidentityprovider");
  ...
  public void secureDeployment(DeploymentInfo deploymentInfo) {
    ...
    loginConfig.addFirstAuthMethod(
        new AuthMethodConfig("BASIC-FIX", Collections.singletonMap("silent", "true")));
          ...
          public Account verify(String id, Credential credential) {
            if (credential instanceof PasswordCredential) {
              ...
              LOG.fine(
                  "Verification of '"
                      + id
                      + "' failed, no password credentials found, but: "
                      + credential);
              return null;
            }
          }
  ...
  protected Account verifyAccount(String username, char[] password) {
    ...
    final BasicAuthContext principal =
        new BasicAuthContext(Constants.MASTER_REALM, idAndPassword[0].toString(), username);
    return new Account() {
      @Override
