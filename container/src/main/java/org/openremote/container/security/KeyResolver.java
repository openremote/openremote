...
public class KeyResolver {
  ...
  private final ConcurrentMap<String, JWKSource<SecurityContext>> jwkSources =
  ...
  /**
   * Returns a JWKSource for the given Keycloak realm. The source handles caching of the JSON Web
   * Key Set (JWKS).
   */
  public JWKSource<SecurityContext> getJwkSource(String realm) {
    ...
    return jwkSources.computeIfAbsent(
        realm,
        r -> {
          try {
            // The standard URL for the JWKS endpoint in Keycloak
            URL jwksUrl =
                URI.create(keycloakBaseUrl + "/realms/" + r + "/protocol/openid-connect/certs")
                    .toURL();
