...
public abstract class KeycloakIdentityProvider implements IdentityProvider {
  ...
  public static final String OR_KEYCLOAK_HOST_DEFAULT =
      "127.0.0.1"; // Bug in keycloak default hostname provider means localhost causes problems with
  // dev-proxy profile
  ...
  // Cache Keycloak deployment per realm/client so we don't have to access Keycloak for every token
  // validation
  protected LoadingCache<KeycloakRealmClient, KeycloakDeployment> keycloakDeploymentCache;
  // The configuration for the Keycloak servlet extension, looks up the openremote client
  // application per realm
    ...
    keycloakConfigResolver =
        request -> {
          ...
          if (realm == null || realm.length() == 0) {
            LOG.finest(
                "No realm in request, no authentication will be attempted: " + request.getURI());
          ...
          if (keycloakDeployment == null) {
            LOG.fine(
                "No Keycloak deployment available for realm, no authentication will be attempted: "
                    + request.getURI());
            return notAuthenticatedKeycloakDeployment;
          }
          ...
          ProxyHandler.builder()
              .setProxyClient(
                  new LoadBalancingProxyClient()
                      .addHost(keycloakServiceUri.clone().replacePath("").build()))
              .setMaxRequestTime(
                  getInteger(
                      container.getConfig(),
                      KEYCLOAK_REQUEST_TIMEOUT,
  ...
  public void secureDeployment(DeploymentInfo deploymentInfo) {
    LoginConfig loginConfig =
        new LoginConfig(SimpleKeycloakServletExtension.AUTH_MECHANISM, "OpenRemote");
  ...
  public synchronized KeycloakDeployment getKeycloakDeployment(String realm, String clientId) {
    ...
    try {
      ...
      if (ex.getCause() != null && ex.getCause() instanceof NotFoundException) {
        LOG.fine(
            "Client '" + clientId + "' for realm '" + realm + "' not found on identity provider");
      } else {
        LOG.log(
            Level.WARNING,
  ...
  public synchronized boolean setActiveCredentials(OAuthGrant grant) {
    ...
    WebTargetBuilder targetBuilder =
        new WebTargetBuilder(httpClient, proxyURI).setOAuthAuthentication(grant);
    ...
    CacheLoader<KeycloakRealmClient, KeycloakDeployment> loader =
        new CacheLoader<KeycloakRealmClient, KeycloakDeployment>() {
          public KeycloakDeployment load(KeycloakRealmClient keycloakRealmClient) {
            LOG.finest(
                "Loading adapter config for client '"
                    + keycloakRealmClient.clientId
                    + "' in realm '"
            ...
            KeycloakResource keycloak =
                getTarget(httpClient, keycloakServiceUri.build(), null, null, null)
                    .proxy(KeycloakResource.class);

            // Can't get adapter for client in another realm
            AdapterConfig adapterConfig =
                keycloak.getAdapterConfig(
                    keycloakRealmClient.realm, KEYCLOAK_CLIENT_ID // keycloakRealmClient.clientId
                    );

            // The auth-server-url in the adapter config must be reachable by this manager it will
            // be the frontend URL by default
  ...
  protected void enableAuthProxy(WebService webService, String keycloakPath) {
    ...
    webService
        .getRequestHandlers()
        .add(0, pathStartsWithHandler("Keycloak auth proxy", "/" + keycloakPath, authProxyHandler));
  }
  ...
  public static KeycloakSecurityContext getSecurityContext(Subject subject) {
    ...
    return subject.getPrincipals().stream()
        ...
        .map(
            keycloakPrincipal ->
                ((KeycloakPrincipal<?>) keycloakPrincipal).getKeycloakSecurityContext())
        .orElse(null);
  }
