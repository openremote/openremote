...
public class TokenPrincipal implements Principal, AuthContext {
  ...
  public String getUsername() {
    try {
      return Optional.ofNullable(claimsSet.getClaimAsString(PREFERRED_USERNAME_CLAIM))
          .orElseGet(claimsSet::getSubject);
  ...
  public boolean isUserInRole(String role) {
    return realmRoles.contains(role)
        || resourceRoles.values().stream().anyMatch(roles -> roles.contains(role));
  }
  ...
  protected static Map<String, List<String>> parseResourceAccessRoles(JWTClaimsSet claimsSet)
