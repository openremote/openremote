  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (realm == null) {
      httpResponse.sendError(
          HttpServletResponse.SC_BAD_REQUEST, Constants.REALM_PARAM_NAME + " header is missing");
      return;
    }

    String authHeader = httpRequest.getHeader(HttpHeaders.AUTHORIZATION);
    if (authHeader == null || !authHeader.startsWith(BEARER_AUTH_PREFIX)) {
      // Anonymous - If the resource is protected, the container or application logic will handle
      // the 401/403.
      chain.doFilter(request, response);
      return;
    }
    ...
    try {
      ...
      HttpServletRequestWrapper authenticatedRequest =
      ...
      throw new NotAuthorizedException(
          "Authentication has failed: " + e.getMessage(),
