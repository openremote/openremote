...
/**
 * If system property <code>java.util.logging.config.file</code> or <code>
 * java.util.logging.config.file</code> has been set then the normal {@link
 * LogManager#readConfiguration()} will be used, otherwise try to load the logging configuration
 * specified in environment variable <code>OR_LOGGING_CONFIG_FILE</code> as a file. If this wasn't
 * set, try to find the file <code>/deployment/manager/logging.properties</code>. If this also
 * wasn't found, load the given default logging configuration from the classpath
 * (logging-dev.properties when OR_DEV_MODE=true otherwise logging.properties).
 *
 * <p>This method should be called in a <code>static { ... }</code> block in the "first" class of
 * your application (typically where your <code>main()</code> method is located).
 */
  ...
  public static void initialiseJUL() throws ExceptionInInitializerError {
    ...
    if (!TextUtil.isNullOrEmpty(System.getProperty("java.util.logging.config.class"))) {
      LOG.log(
          ...
          "Using specified java.util.logging.config.class system property: "
    ...
    if (!TextUtil.isNullOrEmpty(System.getProperty("java.util.logging.config.file"))) {
      LOG.log(
          System.Logger.Level.INFO,
          "Using specified java.util.logging.config.file system property: "
  ...
  protected static InputStream getConfigInputStream() {
    ...
    if (envConfigFileSet) {
      ...
      if (configFile != null) {
        LOG.log(
            System.Logger.Level.INFO,
            "Using logging configuration: " + System.getenv(OR_LOGGING_CONFIG_FILE));
        return configFile;
      }
      ...
      configFile =
          Thread.currentThread()
              ...
              .getResourceAsStream(System.getenv(OR_LOGGING_CONFIG_FILE));
      if (configFile != null) {
        LOG.log(
            System.Logger.Level.INFO,
            "Using logging configuration from classpath: " + System.getenv(OR_LOGGING_CONFIG_FILE));
        return configFile;
      }
    }
    ...
    InputStream configFile =
        org.openremote.model.Container.class.getClassLoader().getResourceAsStream(loggingFile);

    if (configFile != null) {
      LOG.log(
          System.Logger.Level.INFO,
          "Using built in logging configuration from classpath: " + loggingFile);
      return configFile;
    }
