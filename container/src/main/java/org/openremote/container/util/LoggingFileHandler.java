...
public class LoggingFileHandler extends FileHandler {
  ...
  private static String getProperty(String propertyName) {
    return LogManager.getLogManager()
        .getProperty(LoggingFileHandler.class.getName() + propertyName);
  }
  ...
  private static String pattern() {
    return getProperty(".pattern")
        .replace(
            "${" + LogUtil.OR_LOGGING_PROPERTY_NAME + "}",
            System.getProperty(LogUtil.OR_LOGGING_PROPERTY_NAME));
  }

  public LoggingFileHandler() throws IOException {
    // The FileHandler(String) constructor cannot be used because this constructor sets the limit
    // and count after configuration.
