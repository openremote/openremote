...
/**
 * A filter that once a HTTP status code is received that matches any in the {@link
 * #failureResponses} will result in all future requests being blocked (i.e. the request will not
 * reach the server) and instead a {@link Response.Status#METHOD_NOT_ALLOWED} status will be
 * returned.
 */
public class PermanentFailureFilter implements ClientRequestFilter, ClientResponseFilter {

  private static final Logger LOG =
      SyslogCategory.getLogger(PROTOCOL, PermanentFailureFilter.class);
