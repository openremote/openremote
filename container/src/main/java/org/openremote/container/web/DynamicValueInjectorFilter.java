 ...
 * <p>
 * Dynamic time replacement has been moved to {@link DynamicTimeInjectorFilter#filter(ClientRequestContext)}.
 */
@Provider
public class DynamicValueInjectorFilter implements ClientRequestFilter {
    ...
    public void filter(ClientRequestContext requestContext) throws IOException {
        ...
        if (!requestContext.getPropertyNames().contains(DYNAMIC_VALUE_PROPERTY))
            return;

        String dynamicValue = requestContext.getProperty(DYNAMIC_VALUE_PROPERTY) != null
        ...
        try {
            ...
            requestContext.setUri(new URI(oldURI.getScheme(), oldURI.getUserInfo(), oldURI.getHost(), oldURI.getPort(),
                    path, query, oldURI.getFragment()));
