 ...
 *
 * <p>This is naturally quite messy, our goal is to disable all the other default exception handling
 * and do it here in a "simple" way.
 *
 * <p>In production we want to be INFOrmed of exceptions but only log a stacktrace if FINE debug
 * logging is enabled. We never want to send a stacktrace or some crappy HTML to a client in
 * production.
 *
 * <p>TODO: Fine-grained logging for exception, filter which should be logged at INFO or WARN in
 * production TODO: Websocket session errors
 */
public class WebServiceExceptions {
  ...
  // bburke hardcoded a response entity in "new ForbiddenException", which means we can't handle it
  // anymore,
  // according to JAX-RS rules. however, he then checks if there is an exact matching handler, so
  // it's
  // still possible...
  @Provider
  public static class ForbiddenResteasyExceptionMapper
    ...
    public Response toResponse(ForbiddenException exception) {
      return handleResteasyException(
          devMode, "RESTEasy Role Security", request, uriInfo, exception);
    }
  }
  ...
  public static class ServletUndertowExceptionHandler implements ExceptionHandler {
    ...
    public boolean handleThrowable(
      ...
      handleUndertowException(
          devMode, "Undertow Servlet Dispatch", response.isCommitted(), exchange, throwable);
      return true;
    }
  }

  public static Response handleResteasyException(
      boolean devMode, String origin, Request request, UriInfo uriInfo, Throwable throwable) {
    ...
    if (throwable instanceof OptimisticLockException) {
      // We get here if JPA entity id is set on merge but it doesn't already exist (unless ID
      // generator is configured to allow ID to be assigned)
    ...
    try {
      if (devMode) {
        return Response.status(statusCode)
            .entity(renderDevModeError(statusCode, throwable))
            .type(TEXT_PLAIN_TYPE)
            .build();
      } else {
        return Response.status(statusCode)
            .entity(renderProductionError(statusCode, throwable))
            .type(TEXT_PLAIN_TYPE)
            .build();
      }
    } catch (Exception ex) {
  ...
  public static void handleUndertowException(
      boolean devMode,
      String origin,
      boolean logOnly,
      HttpServerExchange exchange,
    ...
    if (!logOnly && exchange.isResponseChannelAvailable()) {
      exchange
          .getResponseHeaders()
          .put(HttpString.tryFromString(HttpHeaders.CONTENT_TYPE), "text/plain");
      try {
        if (devMode) {
          exchange
              .getResponseSender()
              .send(renderDevModeError(exchange.getStatusCode(), throwable));
        } else {
          exchange
              .getResponseSender()
              .send(renderProductionError(exchange.getStatusCode(), throwable));
        }
      } catch (Exception ex2) {
  ...
  public static void logException(Throwable throwable, String origin, String info) {
    ...
    if ("java.io.IOException: Broken pipe".equals(getRootCause(throwable).toString())) return;
    ...
    if (throwable instanceof WebApplicationException) {
      ...
      if (statusCode == 404) {
        ...
        LOG.log(
            Level.FINE,
            "Web service response (" + statusCode + ") in '" + origin + "' for '" + info + "'");
        return;
      }
    }
