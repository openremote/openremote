...
public class LoggingFilter implements Filter {
  ...
  public static final String FILTER_APPLIED_ATTR =
      LoggingFilter.class.getName() + ".FILTER_APPLIED";
  protected static final System.Logger LOG =
      System.getLogger(LoggingFilter.class.getName() + "." + SyslogCategory.API.name());
  ...
  public static void logException(
      boolean devMode, String source, System.Logger.Level level, Throwable t) {
  ...
  /**
   * Minimal response wrapper that tees response body into a buffer while still writing to the real
   * response.
   */
  ...
  static final class TeeWriter extends java.io.Writer {
    ...
    TeeWriter(
        PrintWriter downstream, ByteArrayOutputStream capture, Charset charset, int maxBytes) {
    ...
    try {
      ...
      if (traceEnabled || status >= 400 || thrown != null) {
        ...
        if (LOG.isLoggable(level)) {
          ...
          String message =
              buildHttpLogMessage(
                  req, status, contentType, traceEnabled ? body : null, tookMillis, thrown, null);

          if (thrown != null
              && (devMode || level.getSeverity() >= System.Logger.Level.INFO.getSeverity())) {
