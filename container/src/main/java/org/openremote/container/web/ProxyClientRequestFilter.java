...
public class ProxyClientRequestFilter implements ClientRequestFilter {
  ...
  public void filter(ClientRequestContext requestContext) throws IOException {
    String forwardedFor =
        (String) requestContext.getConfiguration().getProperty(REQUEST_PROPERTY_X_FORWARDED_FOR);
    if (forwardedFor != null) {
      requestContext.getHeaders().add("X-Forwarded-For", forwardedFor);
    }
    String forwardedHost =
        (String) requestContext.getConfiguration().getProperty(REQUEST_PROPERTY_X_FORWARDED_HOST);
    if (forwardedHost != null) {
      requestContext.getHeaders().add("X-Forwarded-Host", forwardedHost);
    }
    String forwardedProto =
        (String) requestContext.getConfiguration().getProperty(REQUEST_PROPERTY_X_FORWARDED_PROTO);
    if (forwardedProto != null) {
      requestContext.getHeaders().add("X-Forwarded-Proto", forwardedProto);
    }
    Integer forwardedPort =
        (Integer) requestContext.getConfiguration().getProperty(REQUEST_PROPERTY_X_FORWARDED_PORT);
