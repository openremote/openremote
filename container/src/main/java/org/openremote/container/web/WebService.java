...
public abstract class WebService implements ContainerService {
        ...
        new ServletContextListener() {
          ...
          public void contextInitialized(ServletContextEvent sce) {
            ...
            if (secure) {
              ...
              // servlet.setInitParameter(ResteasyContextParameters.RESTEASY_ROLE_BASED_SECURITY,
              // "true");
            }
          }
        };
  ...
  public void deployFileServlet(
    ...
    FilterInfo alreadyGzippedFilterInfo =
        Servlets.filter(
                "Already GZipped Filter",
                AlreadyGZippedFilter.class,
                () -> new ImmediateInstanceHandle<>(alreadyGzippedFilter))
    ...
    if (requiredRoles != null && requiredRoles.length > 0) {
      ...
      FilterInfo securityFilterInfo =
          Servlets.filter(
                  "Security Filter",
                  SecurityFilter.class,
                  () -> new ImmediateInstanceHandle<>(securityFilter))
              .setAsyncSupported(true);
    ...
    configureDeploymentInfo(
        deploymentInfo, null, requiredRoles != null && requiredRoles.length > 0, corsOverride);
  ...
  public void configureDeploymentInfo(
      DeploymentInfo deploymentInfo, Integer realmIndex, boolean secure, CORSConfig corsOverride) {
    ...
    // TODO: Remove this handler wrapper once JAX-RS RealmPathExtractorFilter can be utilised before
    // security is applied
    if (realmIndex != null) {
            ...
            return exchange -> {
              ...
              if (realm != null) {
                ...
                String newRelativePath =
                    !newRelativePathBuilder.isEmpty() ? newRelativePathBuilder.toString() : "/";
    ...
    FilterInfo corsFilterInfo =
        Servlets.filter(
                "CORS Filter", CORSFilter.class, () -> new ImmediateInstanceHandle<>(corsFilter))
            .setAsyncSupported(true);
    getCORSConfiguration(corsOverride)
        .forEach((k, v) -> corsFilterInfo.addInitParam(k.toString(), v.toString()));
