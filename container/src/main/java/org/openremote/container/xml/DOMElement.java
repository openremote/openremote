 ...
 * Wraps a W3C element.
 *
 * <p>Using XPath for simple queries like "get all children with attribute value X" is actually
 * slower than iterating with the W3C API. We still use XPath because there is a small chance that
 * the authors of the XML implementation in the JDK will not get forced by Larry to fix their ugly
 * code and APIs. Yes, it' s unlikely but one can dream...
 */
...
public abstract class DOMElement<CHILD extends DOMElement, PARENT extends DOMElement> {
  ...
  public CHILD getRequiredChild(String name) throws ParserException {
    ...
    if (children.length != 1) {
      throw new ParserException(
          "Required single child element of '" + getElementName() + "' not found: " + name);
    }
    return children[0];
  }
  ...
  public CHILD findChildWithIdentifier(final String id) {
    Collection<CHILD> list =
        getXPathChildElements(CHILD_BUILDER, "descendant::" + prefix("*") + "[@id=\"" + id + "\"]");
  ...
  public Object getXPathResult(Node context, String expr, QName result) {
    try {
      // System.out.println("#### XPATH: " + expr + " CONTEXT: " +
      // context.getAttributes().getNamedItem("id") + " EXPECTING: " + result);
      if (result == null) {
