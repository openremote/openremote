 ...
 * names and whose value is the enum value as an instance of the interface that the enum implements
 * (only supports a single interface at the moment).
 *
 * <blockquote>
 *
 * <pre>{@code
 ...
 *
 * <a
 * href="https://github.com/vojtechhabarta/typescript-generator/issues/299">https://github.com/vojtechhabarta/typescript-generator/issues/299</a>
 */
...
public class EnumWithInterfacesExtension extends Extension {
  ...
  public static class EnumInterfaceModel {
    ...
    public EnumInterfaceModel(
        List<EnumInterfaceMemberModel> memberModels, Map<String, String> fieldTypeMap) {
  ...
  protected static final List<String> ENUM_CLASS_HEADER_TEMPLATE =
      Arrays.asList("", "/*export*/ class $$EnumName$$ implements $$EnumImplements$$ {");
  ...
  protected static final List<String> ENUM_CLASS_FOOTER_TEMPLATE =
      Arrays.asList(
          "",
  ...
  public static final Predicate<Settings> supportsNullValuesPredicate =
      settings ->
          !(settings.optionalPropertiesDeclaration == null
              || settings.optionalPropertiesDeclaration
  ...
  protected JacksonAnnotationIntrospector ignoreJsonFormatIntrospector =
      new JacksonAnnotationIntrospector() {
        ...
        protected <A extends Annotation> A _findAnnotation(Annotated ann, Class<A> annoClass) {
          if (ann.getRawType().isEnum()
              ...
              && matchedEnums.stream()
                  .anyMatch(matchedEnum -> matchedEnum.getOrigin() == ann.getRawType())) {
            return null;
          }
  ...
  public void setConfiguration(Map<String, String> configuration) throws RuntimeException {
    enumPatterns =
        configuration.entrySet().stream()
            ...
            .map(es -> globToRegexp(es.getValue()))
  ...
  public void emitElements(Writer writer, Settings settings, boolean exportKeyword, TsModel model) {
    ...
    for (TsEnumModel tsEnum : matchedEnums) {
      ...
      String implementsStr =
          Arrays.stream(tsEnum.getOrigin().getInterfaces())
              .map(
                  i -> {
                    TsBeanModel beanModel = model.getBean(i);
                    return beanModel != null
                        ? beanModel.getName().getFullName()
                        : i.getSimpleName();
                  })
              .collect(Collectors.joining(" | "));

      String fieldsStr =
          enumInterfaceModel.getFieldTypeMap().entrySet().stream()
      ...
      enumInterfaceModel
          .getMemberModels()
          .forEach(
              enumInterfaceMemberModel -> {
                String memberFieldValues =
                    enumInterfaceMemberModel.getMemberFieldValues().entrySet().stream()
                        .map(
                            es ->
                                getMemberValue(
                                    tsEnum.getName().getFullName(),
                                    es.getKey(),
                                    es.getValue(),
                                    settings,
                                    objectMapper))
  ...
  protected boolean enumOrInterfacesMatch(TsEnumModel tsEnumModel) {
    return tsEnumModel.getOrigin().getInterfaces().length > 0
        && (hasNoConfig
            || enumMatches(tsEnumModel.getName().getFullName())
  ...
  protected boolean enumInterfacesMatch(Type[] interfaces) {
    return enumInterfacePatterns != null
  ...
  protected String getMemberValue(
      String enumName,
      String memberName,
      Object value,
      Settings settings,
  ...
  protected EnumInterfaceModel parseEnum(TsEnumModel enm, TsModel model, Settings settings) {
    ...
    List<TsPropertyModel> interfacePropertyModels =
        Arrays.stream(enm.getOrigin().getInterfaces())
            .map(model::getBean)
    ...
    List<Pair<Field, Optional<TsPropertyModel>>> enumFieldInfos =
        Arrays.stream(enm.getOrigin().getDeclaredFields())
            .filter(interfaceFieldPredicate)
            .map(
                field -> {
                  // Ignore case in comparison as TsPropertyModel has the first two letters as lower
                  // case for some reason might be coming from jackson - not investigated (e.g.
                  // getAArray() -> aarray)
                  Optional<TsPropertyModel> matchingPropertyModel =
                      interfacePropertyModels.stream()
                          .filter(ipm -> ipm.getName().equalsIgnoreCase(field.getName()))
                          .findFirst();
                  return Pair.of(field, matchingPropertyModel);
                })
            .sorted(
                (pair1, pair2) -> {
                  boolean pair1IsOptional =
                      pair1
                          .getValue2()
                          .map(mpm -> mpm.getTsType() instanceof TsType.OptionalType)
                          .orElse(false);
                  boolean pair2IsOptional =
                      pair2
                          .getValue2()
                          .map(mpm -> mpm.getTsType() instanceof TsType.OptionalType)
                          .orElse(false);
        ...
        enumFieldInfos.stream()
            .collect(
                ...
                (map, enumFieldInfo) -> {
                  ...
                  // TODO: Need access to the DefaultTypeProcessor known types here but it is
                  // private
    ...
    List<EnumInterfaceMemberModel> memberModels =
        Arrays.stream(enm.getOrigin().getEnumConstants())
            .map(
                m -> {
                  String memberName = ((Enum<?>) m).name();
                  Map<String, Object> memberFieldValues =
                      enumFieldInfos.stream()
                          .collect(
                              LinkedHashMap::new,
                              (map, enumFieldInfo) -> {
  ...
  public static void sortEnums(List<TsEnumModel> enumModels) {
    ...
    BiFunction<Integer, List<Integer>, Boolean> hasDependency =
        (r, t) -> {
          for (Integer c : t) {
            if (adjacency[r][c]) return true;
          }
          return false;
        };
                      ...
                      .filter(
                          em -> {
                            ...
                            return em.getOrigin() == t
                                || Arrays.stream(em.getOrigin().getInterfaces())
                                    .anyMatch(intrface -> intrface == t);
                          })
                      .findFirst())
