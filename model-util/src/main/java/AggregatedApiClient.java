...
/**
 * Creates a wrapper class around all of the clients the generator creates so that they can easily be exported as a
 * single
 * entity.
 */
public class AggregatedApiClient extends EmitterExtension {
    ...
    public void emitElements(Writer writer, Settings settings, boolean exportKeyword, TsModel model) {
        ...
        List<TsBeanModel> clients = model.getBeans().stream().filter(TsBeanModel::isJaxrsApplicationClientBean)
    ...
    private FieldAndGetter getOutputs(TsBeanModel client) {
        String getterName = client.getName().getSimpleName().substring(0,
                client.getName().getSimpleName().length() - 6);
