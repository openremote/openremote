  ...
  public Result processType(Type javaType, Context context) {
    ...
    if (javaType instanceof ParameterizedType parameterizedType) {
      ...
      if (rawClass == Map.class
          && Utils.getRawClassOrNull(parameterizedType.getActualTypeArguments()[0]) == String.class
          && Utils.getRawClassOrNull(parameterizedType.getActualTypeArguments()[1])
              == Object.class) {
      ...
      if (ignoreTypeParams != null) {
        ...
        List<Integer> removeIndexes =
            Arrays.stream(ignoreTypeParams.paramIndexes()).boxed().toList();
        ...
        if (parameterizedType.getRawType() instanceof final Class<?> javaClass) {
          ...
          return new Result(
              new TsType.GenericReferenceType(context.getSymbol(javaClass), tsTypeArguments),
              discoveredClasses);
        }
      }
    }
