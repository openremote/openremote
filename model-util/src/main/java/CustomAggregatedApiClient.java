 ...
 * be exported as a single entity; this is specifically for custom projects and creates a reference
 * to the main rest client.
 */
public class CustomAggregatedApiClient extends EmitterExtension {
  ...
  private static final String getter3FormatString =
      "this.%1$s = new Axios%2$sClient(rest.baseUrl, rest.axiosInstance);";
  ...
  private void emitGetter(Writer writer, FieldAndGetter fieldAndGetter) {
    ...
    writer.writeIndentedLine(
        String.format(getter3FormatString, fieldAndGetter.field, fieldAndGetter.getter));
