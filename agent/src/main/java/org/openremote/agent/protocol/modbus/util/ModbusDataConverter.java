...
public final class ModbusDataConverter {
   ...
   */
  public static Object extractValueFromBatchResponse(
      byte[] data,
      int offset,
      int registerCount,
    ...
    try {
      ...
      if (byteOffset + (registerCount * 2) > data.length) {
        LOG.warning(
            "Not enough data in response: need "
                + (byteOffset + registerCount * 2)
                + " bytes, have "
                + data.length);
        return null;
      }
      ...
      LOG.log(
          Level.WARNING,
          "Failed to extract value from batch response at offset " + offset + ": " + e.getMessage(),
          e);
      return null;
    }
  }
   ...
   */
  public static byte[] convertValueToRegisterBytes(
      Object value,
      int registerCount,
      ModbusAgentLink.ModbusDataType dataType,
      ModbusAgent.EndianFormat endianFormat) {
    ...
    // Apply byte order
    buffer.order(
        endianFormat == ModbusAgent.EndianFormat.BIG_ENDIAN
   ...
   */
  public static byte[] applyWordOrder(
      byte[] data, int registerCount, ModbusAgent.EndianFormat format) {
    // No swap for single register or non-byte-swap formats
    if (registerCount <= 1
        || format == ModbusAgent.EndianFormat.BIG_ENDIAN
        || format == ModbusAgent.EndianFormat.LITTLE_ENDIAN) {
      return data;
    }
   ...
   */
  public static Object parseMultiRegisterValue(
      byte[] dataBytes,
      int registerCount,
      ModbusAgentLink.ModbusDataType dataType,
      ModbusAgent.EndianFormat endianFormat) {
  ...
  public static ByteOrder getJavaByteOrder(ModbusAgent.EndianFormat format) {
    return (format == ModbusAgent.EndianFormat.BIG_ENDIAN
            || format == ModbusAgent.EndianFormat.BIG_ENDIAN_BYTE_SWAP)
        ? ByteOrder.BIG_ENDIAN
        : ByteOrder.LITTLE_ENDIAN;
  }
}
