...
public class ProgramsProcessor extends ChannelProcessor {
    ...
    protected static final List<PropertyDescriptor> STATIC_PROPERTIES = Arrays.asList(
            new PropertyDescriptor("allProgramStepsEnabled", "All Program Steps Enabled",
                    "ALL" + PROGRAM_STEPS_ENABLED_SUFFIX, ValueType.BOOLEAN),
            new PropertyDescriptor("allProgramStepsDisableSeconds", "All Program Steps Disable (s)",
                    "ALL" + PROGRAM_STEPS_DISABLED_SECONDS_SUFFIX, ValueType.NUMBER),
    ...
    public List<PropertyDescriptor> getPropertyDescriptors(VelbusDeviceType deviceType) {
        ...
        for (int i = 1; i <= maxChannelNumber; i++) {
            properties.add(new PropertyDescriptor("ch" + i + "ProgramStepsEnabled", "CH" + i + " Program Steps Enabled",
                    "CH" + i + PROGRAM_STEPS_ENABLED_SUFFIX, ValueType.BOOLEAN));
            properties.add(new PropertyDescriptor("ch" + i + "ProgramStepsDisableSeconds",
                    "CH" + i + " Program Steps Disable (s)", "CH" + i + PROGRAM_STEPS_DISABLED_SECONDS_SUFFIX,
                    ValueType.NUMBER));
        }
    ...
    public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
        ...
        if (alarm1 != null && alarm2 != null) {
            return Arrays.asList(
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.READ_MEMORY_BLOCK.getCode(),
                            (byte) (alarm1 >> 8), alarm1.byteValue()),
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.READ_MEMORY_BLOCK.getCode(),
                            (byte) (alarm2 >> 8), alarm2.byteValue()));
        }
    ...
    public List<VelbusPacket> getPropertyWritePackets(VelbusDevice device, String property, Object value) {
        ...
        Optional<Pair<Integer, String>> channelNumberAndPropertySuffix = getChannelNumberAndPropertySuffix(device,
                CHANNEL_REGEX, property);
        ...
        if (channelNumberAndPropertySuffix.isPresent()) {
            switch (channelNumberAndPropertySuffix.get().value) {
                case PROGRAM_STEPS_ENABLED_SUFFIX:
                    return ValueUtil.getBooleanCoerced(value).map(enabled -> getProgramStepsPackets(device,
                            channelNumberAndPropertySuffix.get().key, enabled, 0xFFFFF)).orElse(null);
                case PROGRAM_STEPS_DISABLED_SECONDS_SUFFIX:
                    return ValueUtil.getIntegerCoerced(value).map(durationSeconds -> getProgramStepsPackets(device,
                            channelNumberAndPropertySuffix.get().key, false, durationSeconds)).orElse(null);
            }
            return null;
        }
        ...
        if (property.equals("ALL" + PROGRAM_STEPS_ENABLED_SUFFIX)) {
            return ValueUtil.getBooleanCoerced(value)
                    .map(enabled -> getProgramStepsPackets(device, 0xFF, enabled, 0xFFFFF)).orElse(null);
        }

        if (property.equals("ALL" + PROGRAM_STEPS_DISABLED_SECONDS_SUFFIX)) {
            return ValueUtil.getIntegerCoerced(value)
                    .map(durationSeconds -> getProgramStepsPackets(device, 0xFF, false, durationSeconds)).orElse(null);
        }

        if (property.equals("PROGRAM")) {
            return EnumUtil.enumFromValue(Program.class, value)
                    .map(program -> Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                            VelbusPacket.OutboundCommand.PROGRAM_SELECT.getCode(), VelbusPacket.PacketPriority.LOW,
                            (byte) program.getCode())))
                    .orElse(null);
        }

        if (property.equals("SUNRISE_ENABLED") || property.equals("SUNSET_ENABLED")) {
            return ValueUtil.getBooleanCoerced(value).map(enabled -> {
                boolean sunriseEnabled = device.getPropertyValue("SUNRISE_ENABLED") != null
                        && ((Boolean) device.getPropertyValue("SUNRISE_ENABLED"));
                boolean sunsetEnabled = device.getPropertyValue("SUNSET_ENABLED") != null
                ...
                return Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                        VelbusPacket.OutboundCommand.SET_SUNRISE_SUNSET.getCode(), VelbusPacket.PacketPriority.LOW,
                        (byte) 0xFF, (byte) sunriseSunsetByte));
            }).orElse(null);
        }
        ...
        if (property.startsWith("ALARM")) {
            ...
            switch (alarmProperty) {
                ...
                case "BED_TIME":
                    ...
                    boolean enabled = Optional.ofNullable((Boolean) device.getPropertyValue(alarmEnabledProperty))
                            .orElse(false);
                    ...
                    boolean isGlobal = Optional.ofNullable((Boolean) device.getPropertyValue(alarmMasterProperty))
                            .orElse(false);
                    ...
                    try {
                        ...
                        if (wakeHours < 0 || wakeHours > 23 || wakeMins < 0 || wakeMins > 59 || bedHours < 0
                                || bedHours > 23 || bedMins < 0 || bedMins > 59) {
                    ...
                    return Collections.singletonList(new VelbusPacket(isGlobal ? 0x00 : device.getBaseAddress(),
                            VelbusPacket.OutboundCommand.SET_ALARM.getCode(), VelbusPacket.PacketPriority.LOW,
                            (byte) alarmNumber, (byte) wakeHours, (byte) wakeMins, (byte) bedHours, (byte) bedMins,
                            (byte) (enabled ? 1 : 0)));
            }
        }
    ...
    public boolean processReceivedPacket(VelbusDevice device, VelbusPacket packet) {
        ...
        switch (packetCommand) {
            case MODULE_STATUS:
                ...
                for (int i = 1; i <= maxChannelNumber; i++) {
                    boolean isChannelEnabled = Optional
                            .ofNullable((Boolean) device.getPropertyValue("CH" + i + PROGRAM_STEPS_ENABLED_SUFFIX))
                            .orElse(true);
                    if (!isChannelEnabled) {
                        allEnabled = false;
                        break;
    ...
    protected static List<VelbusPacket> getProgramStepsPackets(VelbusDevice device, int channelNumber, boolean enabled,
            int durationSeconds) {
        ...
        VelbusPacket.OutboundCommand command = enabled ? VelbusPacket.OutboundCommand.PROGRAM_STEPS_ENABLE
                : VelbusPacket.OutboundCommand.PROGRAM_STEPS_DISABLE;

        return Collections.singletonList(new VelbusPacket(device.getBaseAddress(), command.getCode(),
                VelbusPacket.PacketPriority.LOW, packetBytes));
    }
    ...
    protected static Integer getAlarmMemoryLocation(int alarmNumber, VelbusDeviceType deviceType) {
        ...
        } else if (deviceType == VelbusDeviceType.VMB6PBN || deviceType == VelbusDeviceType.VMB8PBU
                || deviceType == VelbusDeviceType.VMB7IN) {
            location = alarmNumber == 2 ? 0x0098 : 0x0094;
        } else if (deviceType == VelbusDeviceType.VMBGP1 || deviceType == VelbusDeviceType.VMBGP2
                || deviceType == VelbusDeviceType.VMBGP4 || deviceType == VelbusDeviceType.VMBGP4PIR) {
            location = alarmNumber == 2 ? 0x00A9 : 0x00A5;
        } else if (deviceType == VelbusDeviceType.VMBPIRC || deviceType == VelbusDeviceType.VMBPIRO
