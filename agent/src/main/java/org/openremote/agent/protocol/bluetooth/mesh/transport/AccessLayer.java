...
/**
 * AccessLayer implementation of the mesh network architecture as per the mesh profile
 * specification.
 *
 * <p>AccessLayer class generates/parses a raw mesh message containing the specific OpCode and
 * Parameters.
 */
abstract class AccessLayer {
   ...
   *
   * @param message Access message containing the required opcodes and parameters to create access
   *     message pdu.
   */
   ...
   * Creates an access message
   *
   * @param accessMessage Access message containing the required opcodes and parameters to create
   *     access message pdu.
   */
  final synchronized void createAccessMessage(final AccessMessage accessMessage) {
   ...
   *     access message pdu.
   */
  final synchronized void createCustomAccessMessage(final AccessMessage accessMessage) {
    final byte[] parameters = accessMessage.getParameters();
    final byte[] vendorOpcode =
        createVendorOpCode(accessMessage.getOpCode(), accessMessage.getCompanyIdentifier());
    final ByteBuffer accessMessageBuffer;
    if (parameters != null) {
   ...
   */
  final synchronized void parseAccessLayerPDU(final AccessMessage message) {
