...
public class BluetoothMeshProtocol
  ...
  public static final String REGEXP_INDEX_AND_KEY =
      "^(\\s*(0|([1-9]+[0-9]*))\\s*:)?(\\s*[0-9A-Fa-f]{32}\\s*)";
  public static final String REGEXP_PROXY_ADDRESS = "^(?:[0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$";

  // Class Members --------------------------------------------------------------------------------

  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, BluetoothMeshProtocol.class.getName());
  ...
  private static final BluetoothCentralManagerCallback bluetoothManagerCallback =
      new BluetoothCentralManagerCallback() {
        @Override
        public void onConnectedPeripheral(BluetoothPeripheral peripheral) {
          LOG.info(
              "BluetoothCentralManager::onConnectedPeripheral: [Name="
                  + peripheral.getName()
                  + ", Address="
        ...
        public void onConnectionFailed(
            BluetoothPeripheral peripheral, BluetoothCommandStatus status) {
          LOG.info(
              "BluetoothCentralManager::onConnectionFailed: [Name="
                  + peripheral.getName()
                  + ", Address="
                  + peripheral.getAddress()
                  + ", Status="
        ...
        public void onDisconnectedPeripheral(
            BluetoothPeripheral peripheral, BluetoothCommandStatus status) {
          LOG.info(
              "BluetoothCentralManager::onDisconnectedPeripheral: [Name="
                  + peripheral.getName()
                  + ", Address="
                  + peripheral.getAddress()
                  + ", Status="
        ...
        public void onDiscoveredPeripheral(BluetoothPeripheral peripheral, ScanResult scanResult) {
          LOG.info(
              "BluetoothCentralManager::onDiscoveredPeripheral: [Name="
                  + peripheral.getName()
                  + ", Address="
                  + peripheral.getAddress()
                  + ", ScanResult="
  ...
  private static final BluetoothCentralManager bluetoothCentral =
      new BluetoothCentralManager(bluetoothManagerCallback);
  ...
  public static synchronized void initMainThread(
  ...
  public String getProtocolInstanceUri() {
    return "bluetoothmesh://"
        + ((meshNetwork != null && meshNetwork.getNetworkKey() != null)
  ...
  protected synchronized void doStart(Container container) throws Exception {
    ...
    synchronized (BluetoothMeshProtocol.mainThread) {
      if (BluetoothMeshProtocol.sequenceNumberManager == null) {
        ...
        BluetoothMeshProtocol.sequenceNumberManager =
            new SequenceNumberPersistencyManager(storagePath.resolve("bluetoothmesh"));
        BluetoothMeshProtocol.sequenceNumberManager.load();
      }
    }

    LOG.info("Starting Bluetooth Mesh protocol.");
    String meshNetKeyParam =
        agent
            .getNetworkKey()
            .orElseThrow(
                () -> {
    ...
    if (netKeyIndex == null || netKeyAsString == null) {
      String msg =
          "Format of network key '" + meshNetKeyParam + "' is invalid for protocol: " + this;
      LOG.warning(msg);
      throw new IllegalArgumentException(msg);
    }
    NetworkKey networkKey =
        new NetworkKey(netKeyIndex, MeshParserUtils.toByteArray(netKeyAsString));

    String meshAppKeyParam =
        agent
            .getApplicationKey()
            .orElseThrow(
                () -> {
    ...
    if (appKeyIndex == null || appKeyAsString == null) {
      String msg =
          "Format of application key '" + meshAppKeyParam + "' is invalid for protocol: " + this;
      LOG.warning(msg);
      throw new IllegalArgumentException(msg);
    }
    ApplicationKey applicationKey =
        new ApplicationKey(appKeyIndex, MeshParserUtils.toByteArray(appKeyAsString));
    ...
    if (proxyAddress != null && !proxyAddress.matches(REGEXP_PROXY_ADDRESS)) {
      String msg =
          "Format of proxy address '" + proxyAddress + "' is invalid for protocol: " + this;
      LOG.warning(msg);
      throw new IllegalArgumentException(msg);
    }

    String sourceAddressParam =
        agent
            .getSourceAddress()
            .orElseThrow(
                () -> {
                  String msg =
                      "No Bluetooth Mesh unicast source address provided for protocol: " + this;
                  LOG.warning(msg);
                  return new IllegalArgumentException(msg);
                });
    ...
    if (sourceAddress == null) {
      String msg =
          "Format of Bluetooth Mesh unicast source address '"
              + sourceAddressParam
              + "' is invalid for protocol: "
              + this;
      throw new IllegalArgumentException(msg);
    }
    ...
    Integer oldSequenceNumber =
        BluetoothMeshProtocol.sequenceNumberManager.getSequenceNumber(networkKey, sourceAddress);
    if (oldSequenceNumber == null) {
      oldSequenceNumber = sequenceNumberParam;
      BluetoothMeshProtocol.sequenceNumberManager.save(
          networkKey, sourceAddress, oldSequenceNumber);
    }
    BluetoothMeshProtocol.initMainThread(scheduledExecutorService);
    meshNetwork =
        new BluetoothMeshNetwork(
            BluetoothMeshProtocol.bluetoothCentral,
            BluetoothMeshProtocol.sequenceNumberManager,
            BluetoothMeshProtocol.mainThread,
            proxyAddress,
            sourceAddress,
            networkKey,
            applicationKeyMap,
  ...
  @Override
  protected synchronized void doLinkAttribute(
      String assetId, Attribute<?> attribute, BluetoothMeshAgentLink agentLink)
    ...
    Integer appKeyIndex =
        getOrThrowAgentLinkProperty(
            agentLink.getAppKeyIndex(), "Bluetooth Mesh Application Key Index");
    String modelName =
        getOrThrowAgentLinkProperty(agentLink.getModelName(), "Bluetooth Mesh Model Name");
    Integer modelId = toModelId(modelName, attributeRef);
    if (modelId == null) {
      return;
    }
    String addressAsString =
        getOrThrowAgentLinkProperty(agentLink.getAddress(), "Bluetooth Mesh Address");
    Integer address = toIntegerAddress(addressAsString, attributeRef);
    if (address == null) {
      return;
    }
    LOG.info(
        "Linking Bluetooth Mesh attribute: [address: '"
    ...
    Consumer<Object> sensorValueConsumer =
        value -> updateLinkedAttribute(attributeRef, toAttributeValue(value, clazz));
  ...
  @Override
  protected synchronized void doUnlinkAttribute(
      String assetId, Attribute<?> attribute, BluetoothMeshAgentLink agentLink) {
    if (meshNetwork == null) {
      return;
    }
    ...
    LOG.info(
        "Unlinking Bluetooth Mesh attribute: [address: '"
            + String.format("0x%04X", address)
            + "', model: '"
            + modelName
            + "', appKeyIndex: '"
  ...
  @Override
  protected synchronized void doLinkedAttributeWrite(
      BluetoothMeshAgentLink agentLink, AttributeEvent event, Object processedValue) {
    if (meshNetwork == null) {
      return;
    }
    Integer appKeyIndex =
    ...
    LOG.info(
        "Writing Bluetooth Mesh attribute: [address: '"
            + String.format("0x%04X", address)
            + "', model: '"
            + modelName
            + "', appKeyIndex: '"
  ...
  private Integer toIntegerAddress(String addressAsString, AttributeRef attributeRef) {
    ...
    if (address == null) {
      if (attributeRef != null) {
        LOG.warning(
            "Format of Bluetooth Mesh unicast address value '"
                + addressAsString
                + "' is invalid for protocol attribute: "
                + attributeRef);
      } else {
        LOG.warning(
            "Format of Bluetooth Mesh unicast address value '" + addressAsString + "' is invalid.");
      }
    }
    return address;
  }
  ...
  private Integer toModelId(String modelName, AttributeRef attributeRef) {
    ...
    if (modelId == null) {
      if (attributeRef != null) {
        LOG.warning(
            "Unknown or unsupported Bluetooth Mesh model name '"
                + modelName
