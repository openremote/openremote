...
/**
 * UDP in Netty is crap; everything goes through the single server channel as it is connectionless
 * but there is no way of tracking packet origin across decoders. From reading each channel is
 * single threaded so making an assumption that we can store packet origin for processing in {@link
 * #onMessageReceived}. For outbound messages we wrap each decoder in a {@link
 * DatagramPacketEncoder} which unfortunately only supports {@link MessageToMessageEncoder}s.
 */
public abstract class AbstractUDPServer<T>
  ...
  public void sendMessage(T message, InetSocketAddress destination) {
    try {
      channelFuture
          .channel()
          .writeAndFlush(new DefaultAddressedEnvelope<>(message, destination, localAddress));
