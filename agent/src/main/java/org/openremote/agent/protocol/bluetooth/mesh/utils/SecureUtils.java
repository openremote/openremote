...
public class SecureUtils {
  ...
  public static byte[] encryptCCM(
    ...
    final AEADParameters aeadParameters =
        new AEADParameters(new KeyParameter(key), micSize * 8, nonce);
  ...
  public static byte[] encryptCCM(
    ...
    final CCMModeCipher ccmBlockCipher = CCMBlockCipher.newInstance(AESEngine.newInstance());
    final AEADParameters aeadParameters =
        new AEADParameters(new KeyParameter(key), micSize * 8, nonce, additionalData);
    ccmBlockCipher.init(true, aeadParameters);
    ccmBlockCipher.processBytes(data, 0, data.length, ccm, data.length);
    try {
      ccmBlockCipher.doFinal(ccm, 0);
  ...
  public static byte[] calculateK1(
      final byte[] ecdh, final byte[] confirmationSalt, final byte[] text) {
   ...
   */
  public static byte[] calculateAuthValueSecureNetBeacon(
      final byte[] n, final int flags, final byte[] networkId, final int ivIndex) {
   ...
   *
   * @param n network key
   * @param flags network flags, this represents the current state of hte network if key refresh/iv
   *     update is ongoing or complete
   * @param networkId unique id of the network
   * @param ivIndex iv index of the network
   */
  public static SecureNetworkBeacon createSecureNetworkBeacon(
      final byte[] n, final int flags, final byte[] networkId, final int ivIndex) {
   ...
   * @param ivIndex iv index of the network
   */
  public static byte[] calculateSecureNetworkBeacon(
      final byte[] n,
  ...
  public static byte[] calculateHash(
      final byte[] identityKey, final byte[] random, final byte[] src) {
