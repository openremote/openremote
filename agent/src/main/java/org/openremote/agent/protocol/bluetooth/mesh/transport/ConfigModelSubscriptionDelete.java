...
public final class ConfigModelSubscriptionDelete extends ConfigMessage {
   ...
   * @param modelIdentifier identifier of the model, 16-bit for Sig model and 32-bit model id for
   *     vendor models.
   * @throws IllegalArgumentException if any illegal arguments are passed
   */
  public ConfigModelSubscriptionDelete(
      final int elementAddress, final int subscriptionAddress, final int modelIdentifier)
    ...
    if (isValidUnassignedAddress(subscriptionAddress)
        || isValidUnicastAddress(subscriptionAddress)
        || isValidVirtualAddress(subscriptionAddress)
        || subscriptionAddress == 0xFFFF)
      throw new IllegalArgumentException(
  ...
  void assembleMessageParameters() {
    ...
    if (mModelIdentifier >= Short.MIN_VALUE && mModelIdentifier <= Short.MAX_VALUE) {
      paramsBuffer =
          ByteBuffer.allocate(SIG_MODEL_APP_KEY_BIND_PARAMS_LENGTH).order(ByteOrder.LITTLE_ENDIAN);
      ...
      paramsBuffer =
          ByteBuffer.allocate(VENDOR_MODEL_APP_KEY_BIND_PARAMS_LENGTH)
