...
public class AnalogOutputProcessor extends OutputChannelProcessor {
  ...
  protected static final List<Pair<String, ValueDescriptor<?>>> CHANNEL_PROPERTIES =
      Arrays.asList(
          // RW - ChannelState
  ...
  public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
    byte channelRequest =
        device.getDeviceType() == VelbusDeviceType.VMB4DC ? (byte) 0x0F : (byte) 0x01;
  ...
  public List<PropertyDescriptor> getPropertyDescriptors(VelbusDeviceType deviceType) {
    ...
    int channelCount =
        deviceType == VelbusDeviceType.VMB4AN
            ? 4
            : ChannelProcessor.getMaxChannelNumber(deviceType);

    return IntStream.rangeClosed(1, channelCount)
        .mapToObj(Integer::toString)
        .flatMap(
            chNumber ->
                propertySuffixes.stream()
                    .map(
                        propSuffix ->
                            new PropertyDescriptor(
                                toLowerCamelCase(chPrefix).trim()
                                    + chNumber
                                    + toUpperCamelCase(propSuffix.key),
                                (chPrefix + chNumber + " " + toProperCase(propSuffix.key, true))
                                    .trim(),
  ...
  public List<VelbusPacket> getPropertyWritePackets(
    ...
    return getChannelNumberAndPropertySuffix(
            device,
            device.getDeviceType() == VelbusDeviceType.VMB4AN ? IO_CHANNEL_REGEX : CHANNEL_REGEX,
            property)
        .map(
            channelNumberAndPropertySuffix -> {
              ...
              switch (channelNumberAndPropertySuffix.value) {
                ...
                case "_LEVEL_AND_SPEED":
                  command =
                      getLevelAndSpeedFromValue(value)
                          .map(
                              levelAndSpeed -> {
                                params[0] = levelAndSpeed.key;
                                params[1] = levelAndSpeed.value;
                                return levelAndSpeed.key < 0
                                    ? SET_LEVEL_HALT
                                    : channelNumberAndPropertySuffix.value.equals(
                                            "_VALUE_AND_SPEED")
                                        ? SET_VALUE
                                        : SET_LEVEL;
                              })
                      ...
                      ValueUtil.getIntegerCoerced(value)
                          .map(
                              level -> {
                                ...
                                return level < 0
                                    ? SET_LEVEL_HALT
                                    : channelNumberAndPropertySuffix.value.equals("_VALUE")
                                        ? SET_VALUE
                                        : SET_LEVEL;
                              })
                          .orElse(null);
  ...
  public boolean processReceivedPacket(VelbusDevice device, VelbusPacket packet) {
    ...
    switch (packetCommand) {
      ...
      case OUT_LEVEL_STATUS:
        ...
        LedState ledState =
            packetCommand == DIMMER_STATUS
                ? LedState.fromCode(packet.getByte(3) & 0xFF)
  ...
  protected List<VelbusPacket> getPackets(
    ...
    switch (command) {
      ...
      case INHIBIT:
        ...
        break;
      case SET_LEVEL_HALT:
        // for some reason the module doesn't reliable update the LED status when a halt command is
        // sent
