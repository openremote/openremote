...
public abstract class AbstractVelbusProtocol<
        S extends AbstractVelbusProtocol<S, T>, T extends VelbusAgent<T, S>>
  ...
  protected void doStart(Container container) throws Exception {
    ...
    try {
      ...
      int timeInjectionSeconds =
          agent.getTimeInjectionInterval().orElse(DEFAULT_TIME_INJECTION_INTERVAL_SECONDS);
  ...
  protected void doLinkAttribute(
      String assetId, Attribute<?> attribute, VelbusAgentLink agentLink) {
    ...
    Consumer<Object> propertyValueConsumer =
        propertyValue -> updateLinkedAttribute(attributeRef, propertyValue);
  ...
  protected void doLinkedAttributeWrite(
      VelbusAgentLink agentLink, AttributeEvent event, Object processedValue) {
    ...
    return executorService.submit(
        () -> {
          ...
          for (int i = 0; i < modules.getLength(); i++) {
            ...
            Optional<VelbusDeviceType> deviceType =
                EnumUtil.enumFromString(
                    VelbusDeviceType.class, module.getAttribute("type").replaceAll("-", ""));

            if (!deviceType.isPresent()) {
              LOG.info(
                  "Module device type '"
                      + module.getAttribute("type")
                      + "' is not supported so ignoring");
              continue;
            }
            ...
            device.addAttributes(
                deviceType
                    .flatMap(
                        type ->
                            Optional.ofNullable(type.getFeatureProcessors())
                                .map(
                                    processors ->
                                        Arrays.stream(processors)
                                            .flatMap(
                                                processor ->
                                                    processor.getPropertyDescriptors(type).stream()
                                                        .map(
                                                            descriptor -> {
                                                              VelbusAgentLink agentLink =
                                                                  new VelbusAgentLink(
                                                                      agent.getId(),
                                                                      baseAddress,
          ...
          assetConsumer.accept(
              devices.stream().map(AssetTreeNode::new).toArray(AssetTreeNode[]::new));
        },
        null);
  }
}
