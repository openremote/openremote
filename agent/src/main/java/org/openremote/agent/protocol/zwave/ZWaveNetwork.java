...
public class ZWaveNetwork {
    ...
    public synchronized void addSensorValueConsumer(int nodeId, int endpoint, String channelName,
    ...
    public synchronized AssetTreeNode[] discoverDevices(ZWaveAgent agent) {
        ...
        List<ZWaveNode> nodes = controller.getNodes().stream()
                .filter(node -> node.getState() == INITIALIZATION_FINISHED).filter(node -> {
                    ...
                    return cmdClasses.stream().map(ZWCommandClass::getChannels).flatMap(List<Channel>::stream)
                            .noneMatch(channel -> consumerLinkMap.values().stream()
                                    .anyMatch(link -> link.getChannel() == channel));
                }).collect(toList());
        ...
        List<AssetTreeNode> assetNodes = nodes.stream().map(node -> {
            ...
            AssetTreeNode deviceNode = createDeviceNode(agent.getId(), node.getSupportedCommandClasses(),
            ...
            for (ZWEndPoint curEndpoint : node.getEndPoints()) {
                String subDeviceName = curEndpoint.getGenericDeviceClassID().getDisplayName(
                        curEndpoint.getSpecificDeviceClassID()) + " - " + curEndpoint.getEndPointNumber();
                AssetTreeNode subDeviceNode = createDeviceNode(agent.getId(), curEndpoint.getCmdClasses(),
                        subDeviceName);
            ...
            if (parameters.size() > 0) {
                ...
                List<AssetTreeNode> parameterNodes = parameters.stream()
                        .filter(parameter -> parameter.getChannels().size() > 0).map(parameter -> {
                            ...
                            List<Attribute<?>> attributes = parameter.getChannels().stream().map(channel -> {
                                Attribute<?> attribute = TypeMapper.createAttribute(channel.getName(),
                            ...
                            if (description != null && description.length() > 0) {
                                Attribute<String> descriptionAttribute = new Attribute<>("description",
                                        org.openremote.model.value.ValueType.TEXT, description);
                                descriptionAttribute.addMeta(new MetaItem<>(MetaItemType.LABEL, "Description"),
        ...
        List<Attribute<?>> attributes = controller.getSystemCommandManager().getChannels().stream()
                .filter(channel -> consumerLinkMap.values().stream().noneMatch(link -> link.getChannel() == channel))
                .map(channel -> {
    ...
    private AssetTreeNode createDeviceNode(String agentId, List<ZWCommandClass> cmdClasses, String name) {
        ...
        List<Attribute<?>> attributes = cmdClasses.stream().filter(
                commandClass -> commandClass.getID().toRaw() != ZWCommandClassID.COMMAND_CLASS_CONFIGURATION.toRaw()
                ...
                .map(ZWCommandClass::getChannels).flatMap(List<Channel>::stream).map(channel -> {
    ...
    private static List<Attribute<?>> createNodeInfoAttributes(String agentId, ZWaveNode node) {
        ...
        Attribute<?> nodeIdAttrib = new Attribute<>("nodeId", org.openremote.model.value.ValueType.INTEGER,
                node.getNodeID())
                .addMeta(new MetaItem<>(MetaItemType.LABEL, "Node ID"), new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(nodeIdAttrib);

        Attribute<?> manufacturerIdAttrib = new Attribute<>("manufacturerId", org.openremote.model.value.ValueType.TEXT,
                String.format("0x%04X", node.getManufacturerId()));
        manufacturerIdAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Manufacturer ID"),
                new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(manufacturerIdAttrib);

        Attribute<?> manufacturerAttrib = new Attribute<>("manufacturerName", org.openremote.model.value.ValueType.TEXT,
                ZWManufacturerID.fromRaw(node.getManufacturerId()).getName());
        manufacturerAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Manufacturer"),
                new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(manufacturerAttrib);

        Attribute<?> flirsAttrib = new Attribute<>("isFlirs", org.openremote.model.value.ValueType.BOOLEAN,
                node.getNodeInfo().isFLIRS());
        flirsAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "FLIRS"), new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(flirsAttrib);

        Attribute<?> routingAttrib = new Attribute<>("isRouting", org.openremote.model.value.ValueType.BOOLEAN,
                node.getNodeInfo().isRouting());
        routingAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Routing"),
                new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(routingAttrib);

        Attribute<?> listeningAttrib = new Attribute<>("isListening", org.openremote.model.value.ValueType.BOOLEAN,
                node.getNodeInfo().isListening());
        listeningAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Listening"),
                new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(listeningAttrib);

        Attribute<?> productTypeIdAttrib = new Attribute<>("productTypeId", org.openremote.model.value.ValueType.TEXT,
                String.format("0x%04X", node.getProductTypeID()));
        productTypeIdAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Product Type ID"),
                new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(productTypeIdAttrib);

        Attribute<?> productIdAttrib = new Attribute<>("productId", org.openremote.model.value.ValueType.TEXT,
                String.format("0x%04X", node.getProductID()));
        productIdAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Product ID"),
                new MetaItem<>(MetaItemType.READ_ONLY, true));
        attributes.add(productIdAttrib);

        String modelName = node.getModelName();
        if (modelName != null) {
            Attribute<?> modelNameAttrib = new Attribute<>("modelName", org.openremote.model.value.ValueType.TEXT,
                    modelName);
            modelNameAttrib.addMeta(new MetaItem<>(MetaItemType.LABEL, "Model"),
                    new MetaItem<>(MetaItemType.READ_ONLY, true));
        ...
        List<Attribute<?>> zwPlusAttributes = node.getSupportedCommandClasses().stream().filter(
                commandClass -> commandClass.getID().toRaw() == ZWCommandClassID.COMMAND_CLASS_ZWAVEPLUS_INFO.toRaw())
                .map(ZWCommandClass::getChannels).flatMap(List<Channel>::stream).map(channel -> {
