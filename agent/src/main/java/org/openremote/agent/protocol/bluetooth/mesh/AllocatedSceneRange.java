...
public class AllocatedSceneRange extends Range {
  ...
  public boolean overlaps(final Range otherRange) {
    if (otherRange instanceof AllocatedSceneRange) {
      ...
      return overlaps(
          firstScene, lastScene, otherSceneRange.getFirstScene(), otherSceneRange.getLastScene());
    }
    return false;
  }
   ...
   */
  public static List<AllocatedSceneRange> minus(
      final List<AllocatedSceneRange> ranges, final AllocatedSceneRange other) {
  ...
  private List<AllocatedSceneRange> minus(final AllocatedSceneRange other) {
    ...
    if (other.firstScene > firstScene) {
      final AllocatedSceneRange leftSlice =
          new AllocatedSceneRange(firstScene, (Math.min(lastScene, other.firstScene - 1)));
    ...
    if (other.lastScene < lastScene) {
      final AllocatedSceneRange rightSlice =
          new AllocatedSceneRange(Math.max(other.lastScene + 1, firstScene), lastScene);
