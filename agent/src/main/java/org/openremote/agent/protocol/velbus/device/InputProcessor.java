...
public class InputProcessor extends ChannelProcessor {
        ...
        for (int i = 1; i <= maxChannelNumber; i++) {
            for (PropertyType propertyType : PropertyType.values) {
                switch (propertyType) {
                    ...
                    case LED_STATE:
                        if (deviceType != VelbusDeviceType.VMB4AN) {
                            properties.add(new PropertyDescriptor("ch" + i + "LedState", "CH" + i + "LED State",
                                    "CH" + i + propertyType.getPropertySuffix(), ValueType.TEXT));
                        }
                        break;
                    case LOCK_STATE:
                        properties.add(new PropertyDescriptor("ch" + i + "Locked", "CH" + i + " Locked",
                                "CH" + i + propertyType.getPropertySuffix(), ValueType.BOOLEAN));
                        break;
                    case LOCK_DURATION:
                        properties
                                .add(new PropertyDescriptor("ch" + i + "LockDuration", "CH" + i + " Lock Duration (s)",
                                        "CH" + i + propertyType.getPropertySuffix(), ValueType.NUMBER));
                        break;
                    case ENABLED_STATE:
                        properties.add(new PropertyDescriptor("ch" + i + "Enabled", "CH" + i + " Enabled",
                                "CH" + i + propertyType.getPropertySuffix(), ValueType.BOOLEAN, true));
                        break;
                    case INVERTED_STATE:
                        properties.add(new PropertyDescriptor("ch" + i + "Inverted", "CH" + i + " Inverted",
                                "CH" + i + propertyType.getPropertySuffix(), ValueType.BOOLEAN, true));
                        break;
                }
    ...
    public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
        ...
        if (device.getDeviceType() == VelbusDeviceType.VMB4AN) {
            return Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                    VelbusPacket.OutboundCommand.MODULE_STATUS.getCode(), (byte) 0xFF));
        }

        return Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                VelbusPacket.OutboundCommand.MODULE_STATUS.getCode(), (byte) 0x00));
    }

    @Override
    public List<VelbusPacket> getPropertyWritePackets(VelbusDevice device, String property, Object value) {
        return getChannelNumberAndPropertyType(device, property).map(channelNumberAndPropertyType -> {
            ...
            switch (channelNumberAndPropertyType.value) {
                case CHANNEL_STATE:
                    ...
                    ChannelState.fromValue(value).ifPresent(channelState -> {
                        packets.addAll(getChannelStatePackets(device,
                                ChannelProcessor.getAddressForChannel(device, channelNumber), channelNumber,
                                channelState));
                ...
                case LED_STATE:
                    ...
                    EnumUtil.enumFromValue(LedState.class, value).ifPresent(ledState -> {
                        packets.addAll(getLedStatePackets(device,
                                ChannelProcessor.getAddressForChannel(device, channelNumber), channelNumber, ledState));
                        // Push value into device cache as well
                        device.setProperty(property, ledState);
                    });
                    break;
                case LOCK_STATE:
                    ValueUtil.getBoolean(value).ifPresent(
                            locked -> packets.addAll(getLockStatePackets(device, channelNumber, locked ? -1 : 0)));
                    break;
                case LOCK_DURATION:
                    ValueUtil.getIntegerCoerced(value).ifPresent(duration -> {
    ...
    public boolean processReceivedPacket(VelbusDevice device, VelbusPacket packet) {
        ...
        switch (packetCommand) {
            ...
            case PUSH_BUTTON_STATUS:
                ...
                if (startChannel < 0 || startChannel > Math.max(1,
                        ChannelProcessor.getMaxChannelNumber(device.getDeviceType()) - 7)) {
                    return false;
                }
    ...
    protected Optional<Pair<Integer, PropertyType>> getChannelNumberAndPropertyType(VelbusDevice device,
            String propertyName) {

        return getChannelNumberAndPropertySuffix(device, CHANNEL_REGEX, propertyName).map(pair -> PropertyType
                .fromPropertySuffix(pair.value).map(propertyType -> new Pair<>(pair.key, propertyType)).orElse(null));
    }

    protected static List<VelbusPacket> getChannelStatePackets(VelbusDevice velbusDevice, int address,
            int channelNumber, ChannelState channelState) {
        ...
        return Collections.singletonList(new VelbusPacket(address, VelbusPacket.OutboundCommand.BUTTON_STATUS.getCode(),
                VelbusPacket.PacketPriority.HIGH, packetBytes));
    }

    protected static List<VelbusPacket> getLedStatePackets(VelbusDevice velbusDevice, int address, int channelNumber,
            LedState ledState) {
    ...
    static List<VelbusPacket> getLockStatePackets(VelbusDevice velbusDevice, int channelNumber,
            int lockDurationSeconds) {
        boolean locked = lockDurationSeconds != 0;
        byte[] packetBytes = locked ? new byte[4] : new byte[1];
        packetBytes[0] = velbusDevice.getDeviceType() == VelbusDeviceType.VMB4AN ? (byte) (channelNumber - 1)
                : (byte) channelNumber;
        ...
        VelbusPacket.OutboundCommand command = locked ? VelbusPacket.OutboundCommand.LOCK
                : VelbusPacket.OutboundCommand.LOCK_CANCEL;
