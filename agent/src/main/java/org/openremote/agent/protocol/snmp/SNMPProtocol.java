 ...
 *
 * <p>To use this protocol create a {@link SNMPAgent}.
 */
public class SNMPProtocol extends AbstractProtocol<SNMPAgent, SNMPAgentLink> {
  ...
  public String getProtocolInstanceUri() {
    return String.format(
        "snmp:%s:%d?protocol=udp&type=TRAP&snmpVersion=%s",
  ...
  protected void doStart(Container container) throws Exception {
    ...
    String snmpUri =
        String.format(
            "snmp:%s:%d?protocol=udp&type=TRAP&snmpVersion=%d",
            snmpBindHost, snmpBindPort, snmpVersion.getVersion());

    messageBrokerContext.addRoutes(
        new RouteBuilder() {
          @Override
                          ...
                          .forEach(
                              variableBinding -> {
                                AttributeRef attributeRef =
                                    oidMap.get(variableBinding.getOid().format());
                                if (attributeRef != null) {
                                  updateLinkedAttribute(
                                      attributeRef, variableBinding.toValueString());
                                }
                              });
                    });
          }
  ...
  protected void doUnlinkAttribute(
      String assetId, Attribute<?> attribute, SNMPAgentLink agentLink) {
    agentLink.getOID().ifPresent(oidMap::remove);
  }

  @Override
  protected void doLinkedAttributeWrite(
      SNMPAgentLink agentLink, AttributeEvent event, Object processedValue) {
    // Nothing to do here
  }
}
