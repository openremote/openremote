...
public final class ProvisionedMeshNode extends ProvisionedBaseMeshNode {
  ...
  public ProvisionedMeshNode(
    ...
    nodeFeatures =
        new Features(
            Features.UNSUPPORTED, Features.UNSUPPORTED, Features.UNSUPPORTED, Features.UNSUPPORTED);
  }
  ...
  public final boolean hasUnicastAddress(final int unicastAddress) {
    if (unicastAddress == getUnicastAddress()) return true;
    for (Element element : mElements.values()) {
      if (element.getElementAddress() == unicastAddress) return true;
    }
    return false;
  }
   ...
   *
   * <p>This is only meant to be used internally within the library. However this is open now for
   * users to set the sequence number manually in provisioner node.
   *
   ...
   */
  protected final void updateAppKeyList(
      final int netKeyIndex, final List<Integer> indexes, final List<ApplicationKey> keyIndexes) {
  ...
  protected final void setCompositionData(
      final ConfigCompositionDataStatus configCompositionDataStatus) {
    ...
    final boolean lowPowerFeatureSupported =
        configCompositionDataStatus.isLowPowerFeatureSupported();
    nodeFeatures =
        new Features(
            friendFeatureSupported ? Features.ENABLED : Features.UNSUPPORTED,
