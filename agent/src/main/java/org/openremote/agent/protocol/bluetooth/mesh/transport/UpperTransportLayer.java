...
/**
 * UpperTransportLayer implementation of the mesh network architecture as per the mesh profile
 * specification.
 *
 * <p>UpperTransportLayer class encrypts/decrypts Access PDUs created in the access layer.
 */
public abstract class UpperTransportLayer extends AccessLayer {
  ...
  public static final int APPLICATION_KEY_IDENTIFIER =
      0; // Identifies that the device key is to be used
  ...
  /**
   * Removes the lower transport layer header and reassembles a segmented lower transport access pdu
   * in to one message
   *
  ...
  /**
   * Removes the lower transport layer header and reassembles a segmented lower transport control
   * pdu in to one message
   *
  ...
  synchronized void createMeshMessage(final Message message) { // Access message
    if (message instanceof AccessMessage) {
      ...
      LOG.info(
          "Encrypted upper transport pdu: "
              + MeshParserUtils.bytesToHex(encryptedTransportPDU, false));
  ...
  synchronized void createUpperTransportPDU(final Message message) {
    if (message instanceof AccessMessage) {
      ...
      if (parameters != null) {
        accessMessageBuffer =
            ByteBuffer.allocate(PROXY_CONFIG_OPCODE_LENGTH + parameters.length)
   ...
   */
  final synchronized void parseUpperTransportPDU(final Message message)
    ...
    try {
      switch (message.getPduType()) {
        case MeshManagerApi.PDU_TYPE_NETWORK:
          if (message instanceof AccessMessage) { // Access message
            ...
            final byte[] decryptedUpperTransportControlPdu =
                decryptUpperTransportPDU(accessMessage);
            accessMessage.setAccessPdu(decryptedUpperTransportControlPdu);
          } else {
            // TODO
            // this where control messages such as heartbeat and friendship messages are to be
            // implemented
          }
          break;
        case MeshManagerApi.PDU_TYPE_PROXY_CONFIGURATION:
          ...
          if (controlMessage.getLowerTransportControlPdu().size() == 1) {
            final byte[] lowerTransportControlPdu =
                controlMessage.getLowerTransportControlPdu().get(0);
            final ByteBuffer buffer =
                ByteBuffer.wrap(lowerTransportControlPdu).order(ByteOrder.BIG_ENDIAN);
      ...
      throw new ExtendedInvalidCipherTextException(
          ex.getMessage(), ex.getCause(), UpperTransportLayer.class.getSimpleName());
    }
  }
  ...
  private byte[] encryptUpperTransportPDU(final AccessMessage message) {
    ...
    final int aszmic =
        message
            .getAszmic(); // upper transport layer will always have the aszmic as 0 because the mic
    // is always 32bit
    ...
    if (MeshAddress.isValidVirtualAddress(dst)) {
      return SecureUtils.encryptCCM(
          accessPDU, key, nonce, MeshParserUtils.uuidToBytes(message.getLabel()), transMicLength);
    } else {
  ...
  private byte[] decryptUpperTransportPDU(final AccessMessage accessMessage)
    ...
    byte[] key;
    final int transportMicLength =
        accessMessage.getAszmic() == SZMIC ? MAXIMUM_TRANSMIC_LENGTH : MINIMUM_TRANSMIC_LENGTH;
    ...
    if (APPLICATION_KEY_IDENTIFIER == accessMessage.getAkf()) {
      ...
      // If its a device key that was used to encrypt the message we need to create a device nonce
      // to decrypt it
      nonce =
          createDeviceNonce(
              accessMessage.getAszmic(),
              accessMessage.getSequenceNumber(),
              accessMessage.getSrc(),
              accessMessage.getDst(),
              accessMessage.getIvIndex());
      decryptedUpperTransportPDU =
          SecureUtils.decryptCCM(
              accessMessage.getUpperTransportPdu(), key, nonce, transportMicLength);
    } else {
      final List<ApplicationKey> keys =
          mUpperTransportLayerCallbacks.getApplicationKeys(
              accessMessage.getNetworkKey().getKeyIndex());
      if (keys.isEmpty())
        throw new IllegalArgumentException("Unable to find the app key to decrypt the message");

      nonce =
          createApplicationNonce(
              accessMessage.getAszmic(),
              accessMessage.getSequenceNumber(),
              accessMessage.getSrc(),
              accessMessage.getDst(),
      ...
      if (MeshAddress.isValidVirtualAddress(accessMessage.getDst())) {
        decryptedUpperTransportPDU =
            decrypt(
                accessMessage,
                mUpperTransportLayerCallbacks.gerVirtualGroups(),
                keys,
                nonce,
                transportMicLength);
      } else {
    ...
    if (decryptedUpperTransportPDU == null)
      throw new IllegalArgumentException(
          "Unable to decrypt the message, invalid application key identifier!");
  ...
  private byte[] decrypt(
      final AccessMessage accessMessage,
      ...
      for (Group group : groups) {
        if (group.getAddressLabel() != null) {
          if (key.getAid() == accessMessage.getAid()) {
            try {
              return SecureUtils.decryptCCM(
                  accessMessage.getUpperTransportPdu(),
                  key.getKey(),
                  nonce,
                  MeshParserUtils.uuidToBytes(group.getAddressLabel()),
                  transportMicLength);
            } catch (Exception ex) {
              // Retrying decryption
            }
  ...
  private byte[] decrypt(
    ...
    for (ApplicationKey key : keys) {
      if (key.getAid() == accessMessage.getAid()) {
        try {
          return SecureUtils.decryptCCM(
              accessMessage.getUpperTransportPdu(), key.getKey(), nonce, transportMicLength);
        } catch (Exception ex) {
          // Retrying decryption.
        }
      }
   ...
   */
  private byte[] createApplicationNonce(
      final int aszmic,
    ...
    applicationNonceBuffer.put(
        (byte)
            ((aszmic << 7)
                | PAD_APPLICATION_DEVICE_NONCE)); // ASZMIC (SZMIC if a segmented access message)
    // and PAD
   ...
   * @return Device nonce
   */
  private byte[] createDeviceNonce(
      final int aszmic,
      final byte[] sequenceNumber,
      final int src,
      final int dst,
  ...
  /**
   * Derives the original transport layer sequence number from the network layer sequence number
   * that was received with every segment
   *
   * @param networkLayerSequenceNumber sequence number on network layer which is a part of the
   *     original pdu received
   * @param seqZero the lower 13 bits of the sequence number. This is a part of the lower transport
   *     pdu header and is the same value for all segments
   * @return original transport layer sequence number that was used to encrypt the transport layer
   *     pdu
   */
  final int getTransportLayerSequenceNumber(
      final int networkLayerSequenceNumber, final int seqZero) {
    if ((networkLayerSequenceNumber & TRANSPORT_SAR_SEQZERO_MASK) < seqZero) {
      return ((networkLayerSequenceNumber
          - ((networkLayerSequenceNumber & TRANSPORT_SAR_SEQZERO_MASK) - seqZero)
          - (TRANSPORT_SAR_SEQZERO_MASK + 1)));
    } else {
      return ((networkLayerSequenceNumber
          - ((networkLayerSequenceNumber & TRANSPORT_SAR_SEQZERO_MASK) - seqZero)));
    }
