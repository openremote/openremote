...
public abstract class UpperTransportLayer extends AccessLayer {
    ...
    /**
     * Removes the lower transport layer header and reassembles a segmented lower transport control pdu in to one
     * message
     *
        ...
        if (message instanceof AccessMessage) {
            ...
            if (parameters != null) {
                accessMessageBuffer = ByteBuffer.allocate(PROXY_CONFIG_OPCODE_LENGTH + parameters.length)
                        .order(ByteOrder.BIG_ENDIAN).put((byte) opCode).put(parameters);
            } else {
    ...
    synchronized final void parseUpperTransportPDU(final Message message) throws ExtendedInvalidCipherTextException {
        try {
            ...
            throw new ExtendedInvalidCipherTextException(ex.getMessage(), ex.getCause(),
                    UpperTransportLayer.class.getSimpleName());
        }
    }
    ...
    private byte[] encryptUpperTransportPDU(final AccessMessage message) {
        ...
        final int aszmic = message.getAszmic(); // upper transport layer will always have the aszmic as 0 because the
                                                // mic is always 32bit
        ...
        if (MeshAddress.isValidVirtualAddress(dst)) {
            return SecureUtils.encryptCCM(accessPDU, key, nonce, MeshParserUtils.uuidToBytes(message.getLabel()),
                    transMicLength);
        } else {
    ...
    private byte[] decryptUpperTransportPDU(final AccessMessage accessMessage) throws InvalidCipherTextException {
        ...
        byte[] key;
        final int transportMicLength = accessMessage.getAszmic() == SZMIC ? MAXIMUM_TRANSMIC_LENGTH
                : MINIMUM_TRANSMIC_LENGTH;
        ...
        if (APPLICATION_KEY_IDENTIFIER == accessMessage.getAkf()) {
            ...
            nonce = createDeviceNonce(accessMessage.getAszmic(), accessMessage.getSequenceNumber(),
                    accessMessage.getSrc(), accessMessage.getDst(), accessMessage.getIvIndex());
            decryptedUpperTransportPDU = SecureUtils.decryptCCM(accessMessage.getUpperTransportPdu(), key, nonce,
                    transportMicLength);
        } else {
            final List<ApplicationKey> keys = mUpperTransportLayerCallbacks
                    .getApplicationKeys(accessMessage.getNetworkKey().getKeyIndex());
            if (keys.isEmpty())
                throw new IllegalArgumentException("Unable to find the app key to decrypt the message");

            nonce = createApplicationNonce(accessMessage.getAszmic(), accessMessage.getSequenceNumber(),
                    accessMessage.getSrc(), accessMessage.getDst(), accessMessage.getIvIndex());

            if (MeshAddress.isValidVirtualAddress(accessMessage.getDst())) {
                decryptedUpperTransportPDU = decrypt(accessMessage, mUpperTransportLayerCallbacks.gerVirtualGroups(),
                        keys, nonce, transportMicLength);
            } else {
    ...
    private byte[] decrypt(final AccessMessage accessMessage, final List<Group> groups, List<ApplicationKey> keys,
            final byte[] nonce, final int transportMicLength) {
        for (ApplicationKey key : keys) {
            for (Group group : groups) {
                if (group.getAddressLabel() != null) {
                    if (key.getAid() == accessMessage.getAid()) {
                        try {
                            return SecureUtils.decryptCCM(accessMessage.getUpperTransportPdu(), key.getKey(), nonce,
                                    MeshParserUtils.uuidToBytes(group.getAddressLabel()), transportMicLength);
                        } catch (Exception ex) {
                            // Retrying decryption
                        }
                    }
                    if (key.getOldAid() == accessMessage.getAid()) {
                        ...
                        try {
                            return SecureUtils.decryptCCM(accessMessage.getUpperTransportPdu(), key.getOldKey(), nonce,
                                    MeshParserUtils.uuidToBytes(group.getAddressLabel()), transportMicLength);
                        } catch (Exception ex) {
                            // Retrying decryption
                        }
    ...
    private byte[] decrypt(final AccessMessage accessMessage, List<ApplicationKey> keys, final byte[] nonce,
     ...
     */
    private byte[] createApplicationNonce(final int aszmic, final byte[] sequenceNumber, final int src, final int dst,
            final byte[] ivIndex) {
        ...
        applicationNonceBuffer.put((byte) ((aszmic << 7) | PAD_APPLICATION_DEVICE_NONCE)); // ASZMIC (SZMIC if a
     ...
     * @return Device nonce
     */
    private byte[] createDeviceNonce(final int aszmic, final byte[] sequenceNumber, final int src, final int dst,
            final byte[] ivIndex) {
        final ByteBuffer deviceNonceBuffer = ByteBuffer.allocate(13);
        deviceNonceBuffer.put((byte) NONCE_TYPE_DEVICE); // Nonce type
        deviceNonceBuffer.put((byte) ((aszmic << 7) | PAD_APPLICATION_DEVICE_NONCE)); // ASZMIC (SZMIC if a segmented
                                                                                      // access message) and PAD
    ...
    /**
     * Derives the original transport layer sequence number from the network layer sequence number that was received
     * with every segment
     *
     * @param networkLayerSequenceNumber sequence number on network layer which is a part of the original pdu received
     * @param seqZero the lower 13 bits of the sequence number. This is a part of the lower transport pdu header and is
     *            the same value for all segments
