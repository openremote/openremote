...
public class ModbusAgentLink extends AgentLink<ModbusAgentLink> {
  ...
  @NotNull() @JsonPropertyDescription("Poll interval in milliseconds")
  private Long pollingMillis;

  @NotNull() @JsonPropertyDescription("Memory area to read from during read request")
  private ReadMemoryArea readMemoryArea;

  @NotNull() @JsonPropertyDescription(
      "Type to convert the returned data to. As specified by the PLC4X Modbus data types.")
  private ModbusDataType readValueType;

  @NotNull() @JsonPropertyDescription("Zero based address from which the value is read from")
