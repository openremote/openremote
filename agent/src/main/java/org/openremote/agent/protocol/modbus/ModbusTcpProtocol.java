...
/**
 * Modbus TCP protocol implementation. Handles TCP-specific transport: transaction ID correlation
 * and full-duplex communication.
 */
public class ModbusTcpProtocol
    extends AbstractModbusProtocol<ModbusTcpProtocol, ModbusTcpAgent, ModbusTcpFrame> {

  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, ModbusTcpProtocol.class);

  // Request/response correlation via transaction ID
  private final Map<Integer, CompletableFuture<ModbusTcpFrame>> pendingRequests =
  ...
  protected NettyIOClient<ModbusTcpFrame> doCreateIoClient() throws Exception {
    String host =
        agent.getHost().orElseThrow(() -> new IllegalStateException("Host not configured"));
  ...
  public ModbusTcpFrame sendModbusRequest(int unitId, byte[] pdu, long timeoutMs) throws Exception {
    ...
    try {
      ...
      LOG.finest(
          () ->
              "Received Modbus TCP response - TxID: "
