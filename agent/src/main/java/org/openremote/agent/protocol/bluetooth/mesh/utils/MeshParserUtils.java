...
public class MeshParserUtils {
  ...
  private static final int[] M_NUMERIC_MAX = {
  ...
  public static String bytesToHex(
      final byte[] bytes, final int start, final int length, final boolean add0x) {
    if (bytes == null || bytes.length <= start || length <= 0) return "";
    ...
    if (!add0x) return new String(hexChars);
   ...
   *
   * <p>The IV Index is a 32-bit value that is a shared network resource (i.e., all nodes in a mesh
   * network share the same value of the IV Index and use it for all subnets they belong to). The IV
   * Index starts at 0x00000000
   *
  ...
  public static byte[] addKeyIndexPadding(final Integer keyIndex) {
    return ByteBuffer.allocate(2)
        ...
        .putShort((short) (keyIndex & 0x0FFF))
        .array();
  }
   ...
   * @throws IllegalArgumentException in case of an invalid was entered as an input and the message
   *     containing the error
   */
  public static boolean validateNetworkKeyInput(@NotNull final String networkKey)
   ...
   *     containing the error
   */
  public static boolean validateIvIndexInput(final Integer ivIndex)
      throws IllegalArgumentException {
    if (ivIndex == null) {
      throw new IllegalArgumentException("IV Index cannot be empty!");
    }
   ...
   *     containing the error
   */
  public static boolean validateKeyInput(@NotNull final String key)
      throws IllegalArgumentException {
  ...
  public static int getSequenceNumberFromPDU(final byte[] pdu) {
    return convert24BitsToInt(
        new byte[] {pdu[3], pdu[4], pdu[5]}); // get sequence number array from pdu
  }

  public static int calculateSeqZero(final byte[] sequenceNumber) {
    return ((sequenceNumber[1] & 0x1F) << 8)
        | (sequenceNumber[2] & 0xFF); // 13 least significant bits
  }
  ...
  */
  ...
  /**
   * Returns an array of opcodes. If the MSB = 0 then the length is 1 If the MSB = 1 then the length
   * is 2 If the MSB = 2 then the length is 3
   *
  ...
  /**
   * Returns the length of the opcode. If the MSB = 0 then the length is 1 If the MSB = 1 then the
   * length is 2 If the MSB = 2 then the length is 3
   *
  ...
  public static int getOpCodeLength(final int mostSignificantOpcodeByte) {
    ...
    /*mostSignificantOpcodeByte >= 0x80 &&*/ mostSignificantOpcodeByte <= 0xBF) {
      return 2;
    } else {
      return 3;
    }
  ...
  /**
   * Returns the vendor opcode packed with company identifier If the MSB = 0 then the length is 1 If
   * the MSB = 1 then the length is 2 If the MSB = 2 then the length is 3
   *
   * @param opCode operation code
   * @return length of opcodes
   */
   ...
   *
   * <p>0x00, 0x02 - 0x7F are the Default TTL states. 0x01, 0x80 and 0xFF are Prohibited.
   *
   * @param ttl ttl
   * @return true if valid and false otherwise
   */
   ...
   * @return remaining time as string.
   */
  public static String getRemainingTransitionTime(
      final int stepResolution, final int numberOfSteps) {
    switch (stepResolution) {
      case RESOLUTION_100_MS:
  ...
  public static int bytesToInt(@NotNull byte[] b) {
    ...
    return b.length == 4
        ? ByteBuffer.wrap(b).order(ByteOrder.BIG_ENDIAN).getInt()
  ...
  /**
   * Generates an oob count to be entered on the device when blink,beep,vibrate,push or twist action
   * is selected
   *
   * <p>Based on mesh profile specification v1.0.1 section 5.4.2.4 page 254
   *
  ...
  public static boolean isValidHeartbeatCountLog(final int countLog) {
    if ((countLog >= 0x00 && countLog < 0x12) || countLog == 0xFF) return true;
