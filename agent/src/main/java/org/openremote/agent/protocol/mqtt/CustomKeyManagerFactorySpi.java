...
public class CustomKeyManagerFactorySpi extends KeyManagerFactorySpi {
  ...
  protected void engineInit(KeyStore keyStore, char[] password) {
    ...
    for (KeyManager keyManager : factory.getKeyManagers()) {
      if (keyManager instanceof X509ExtendedKeyManager) {
        this.keyManager =
            new CustomX509KeyManager((X509ExtendedKeyManager) keyManager, realmPrefixedAlias);
        return;
      }
    }
  ...
  protected void engineInit(ManagerFactoryParameters parameters) {
    throw new UnsupportedOperationException(
        "engineInit(KeyManagerFactorySpi.Parameters) not supported");
  }
  ...
  private static class CustomX509KeyManager extends X509ExtendedKeyManager {
    ...
    public String chooseEngineClientAlias(String[] keyType, Principal[] issuers, SSLEngine engine) {
      ...
      // TODO: Not sure how this should be handled. This would mean that the keypair with
      // realmPrefixedAlias
      //      either wasn't found, or didn't match up with the KeyType, or does not have the correct
      // issuer.
