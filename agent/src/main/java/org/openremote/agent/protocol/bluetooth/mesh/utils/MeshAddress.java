...
public class MeshAddress {
  ...
  public static boolean isValidUnicastAddress(final int address) {
    return isAddressInRange(address)
        && (address >= START_UNICAST_ADDRESS && address <= END_UNICAST_ADDRESS);
  }
  ...
  public static boolean isValidGroupAddress(final byte[] address) {
    if (!isAddressInRange(address)) return false;
  ...
  /**
   * Returns true if the its a valid fixed group address. Fixed group addresses include all proxies,
   * all friends, all relays and all nodes address.
   *
   * @param address 16-bit address
   * @return true if the address is valid and false otherwise
   */
  ...
  public static UUID getLabelUuid(final List<UUID> uuids, final int address) {
    if (MeshAddress.isValidVirtualAddress(address)) {
      for (UUID uuid : uuids) {
        ...
        final byte[] encryptedUuid =
            SecureUtils.calculateCMAC(MeshParserUtils.uuidToBytes(uuid), salt);
        ByteBuffer buffer = ByteBuffer.wrap(encryptedUuid);
        buffer.position(12); // Move the position to 12
   ...
   *
   * <p>The hash stored in a virtual address is derived from the label UUID. In a virtual address
   * bits 13 to 0 are set to the value of a hash of the corresponding Label UUID.
   *
  ...
  /**
   * Validates if the address is a valid Heartbeat publicaiton destination. A valid address shall be
   * an unassigned address, a unicast address, or a group address, all other values are Prohibited.
   *
   * @param address address
  ...
  /**
   * Validates if the address is a valid Heartbeat subscription source. A valid address shall be an
   * unassigned address, a unicast address. All other values are Prohibited.
   *
   * @param address address
   * @return true if valid or false otherwise.
   * @throws IllegalArgumentException if the address is not the unassigned address or a unicast
   *     address.
   */
  public static boolean isValidHeartbeatSubscriptionSource(final int address) {
    if (isValidUnassignedAddress(address) || isValidUnicastAddress(address)) return true;
    else
      throw new IllegalArgumentException(
          "Source address must be an unassigned address or a unicast address!");
  }

  /**
   * Validates if the address is a valid Heartbeat subscription source. A valid address shall be an
   * unassigned address, a primary unicast address of the node, or a group address, All other values
   * are Prohibited.
   *
   * @param address address
   * @return true if valid or false otherwise.
  ...
  public static boolean isValidHeartbeatSubscriptionDestination(final int address) {
    if (isValidUnassignedAddress(address)
        || isValidUnicastAddress(address)
        || isValidGroupAddress(address)) return true;
    else
      throw new IllegalArgumentException(
          "Destination address must be an unassigned address, "
