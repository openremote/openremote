...
public class HeartbeatPublication extends Heartbeat {
   ...
   *
   * @param dst Destination address identifies the Heartbeat Publication destination where the
   *     address can only be an unassigned address, unicast address or a group address. All other
  ...
  public String toString() {
    return "Destination address: "
                + Integer.toHexString(dst)
                + "\nCount Log: "
                + Integer.toHexString(countLog)
                + "\nPeriod Log: "
                + Integer.toHexString(periodLog)
                + "\nTTL: "
                + ttl
                + "\nFeatures: "
                + features
  ...
  public String getPeriodLogDescription() {
    if (periodLog == 0x00) return "Disabled";
    else if (periodLog >= 0x01 || periodLog <= 0x11) return (int) Math.pow(2, periodLog - 1) + " s";
  ...
  public String getCountLogDescription() {
    if (countLog == 0x00) return "Disabled";
    else if (countLog == 0xFF) return "Indefinite";
    else if (countLog == 0x01 || countLog == 0x02) return String.valueOf(countLog);
