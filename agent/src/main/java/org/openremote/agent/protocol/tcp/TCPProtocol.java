...
/**
 * This is a generic TCP client protocol for communicating with TCP servers; it uses the {@link
 * TCPIOClient} to handle the communication and all messages are processed as strings; if you
 * require custom message type handling or more specific handling then sub class the {@link
 * AbstractTCPClientProtocol}).
 *
 * <p>To use this protocol create a {@link TCPAgent}.
 */
public class TCPProtocol
    extends AbstractTCPClientProtocol<
        TCPProtocol, TCPAgent, DefaultAgentLink, String, TCPIOClient<String>> {
  ...
  protected void doLinkAttribute(
    ...
    if (messageConsumer != null) {
      protocolMessageConsumers.add(
          new Pair<>(new AttributeRef(assetId, attribute.getName()), messageConsumer));
    }
  }
  ...
  protected void doUnlinkAttribute(
    ...
    protocolMessageConsumers.removeIf(
        attRefConsumerPair -> attRefConsumerPair.key.equals(attributeRef));
  }
