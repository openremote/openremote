 ...
 *
 * <p>Implementors are responsible for handling the low level communication and for providing
 * consumers with a mechanism for encoding/decoding messages of type &lt;T&gt; received from source
 * &lt;U&gt; with an address &lt;V&gt;.
 *
 * @param <T> Defines the message type that the server can send/receive
 * @param <U> Defines the client identifier type that can be used to uniquely identify clients
...
public interface IOServer<T, U, V extends SocketAddress> {
  ...
  /**
   * Get current connection status of a client (either {@link ConnectionStatus#CONNECTED} or {@link
