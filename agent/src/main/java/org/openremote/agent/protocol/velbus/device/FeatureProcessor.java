 ...
 *
 * <p>A {@link FeatureProcessor} will be used as a singleton and must therefore not store any device
 * specific state.
 */
public abstract class FeatureProcessor {
  ...
  /**
   * Get the packets that need to be sent to the device to get the state data of this feature's
   * properties
   */
  ...
  /**
   * Asks the processor to generate the packets required to write the requested value to this
   * property.
   *
   * <p>Once a processor returns a non null value then no other processors will be asked to handle
   * the write
   */
  public abstract List<VelbusPacket> getPropertyWritePackets(
      VelbusDevice device, String property, Object value);

  /**
   * Allows this feature processor to handle the inbound packet. If a processor handles a packet
   * then it should set the packets {@link VelbusPacket#isHandled()} flag.
   *
   * <p>The processor can prevent other processors from processing the packet by returning true
   * (processors should only do this if they are certain no other processor is interested in the
   * packet).
   *
