 ...
 *
 * <p>This protocol periodically fetches weather data from OpenWeatherMap for all linked asset
 * attributes, grouped by the asset that contains them. Each asset’s location (latitude and
 * longitude) is used to query the API once per asset.
 *
 * <p>For each linked attribute:
 *
 * <ul>
 *   <li>The <b>current value</b> is updated using the latest weather data.
 *   <li><b>Predicted datapoints</b> (hourly and daily forecasts) are written to enable time-series
 *       analysis and visualization in Insights.
 * </ul>
 *
 * <p>A helper action can provision a default {@link WeatherAsset} preconfigured with common weather
 * attributes (temperature, humidity, wind, etc.), each linked to a corresponding {@link
 * OpenWeatherMapProperty}.
 *
 * <p>The agent requires a valid OpenWeatherMap API key
 */
public class OpenWeatherMapProtocol
    extends AbstractProtocol<OpenWeatherMapAgent, OpenWeatherMapAgentLink> {

  private static final Logger LOG =
      SyslogCategory.getLogger(PROTOCOL, OpenWeatherMapProtocol.class);
  ...
  protected void doStart(Container container) throws Exception {
    ...
    if (!healthCheck()) {
      setConnectionStatus(ConnectionStatus.ERROR);
      LOG.warning(
          "Could not reach OpenWeatherMap API, either API is unavailable or API key is invalid");
      return;
    }
  ...
  public boolean onAgentAttributeChanged(AttributeEvent event) {
    ...
    if (event.getName().equals(OpenWeatherMapAgent.PROVISION_WEATHER_ASSET.getName())) {
      ...
      if (provisionWeatherAsset) {
        ...
        sendAttributeEvent(
            new AttributeEvent(
                agent.getId(), OpenWeatherMapAgent.PROVISION_WEATHER_ASSET.getName(), false));
      }
  ...
  protected void doLinkAttribute(
      String assetId, Attribute<?> attribute, OpenWeatherMapAgentLink agentLink)
  ...
  protected void doLinkedAttributeWrite(
      OpenWeatherMapAgentLink agentLink, AttributeEvent event, Object processedValue) {
    // Do nothing
  }

  /**
   * Update the agent's linked attributes with weather data from the OpenWeatherMap API
   *
   * <p>This method groups all linked attributes by their Asset ID and then updates the weather data
   * for each asset based on its location
   */
    ...
    getLinkedAttributes()
        .forEach(
            (attributeRef, attribute) ->
                assetGroups
                    .computeIfAbsent(attributeRef.getId(), k -> new ArrayList<>())
                    .add(attributeRef));
  ...
  protected void updateAssetWeatherData(String assetId, List<AttributeRef> attributeRefs) {
    ...
    OpenWeatherMapResponse weatherData =
        fetchWeatherData(buildApiUrl(location.getY(), location.getX()));
    ...
    if (weatherData != null) {
      ...
      try {
        updatePredictedDatapoints(
            attributeRefs, weatherData.getDaily()); // write daily forecast, up to 8 days
        updatePredictedDatapoints(
            attributeRefs, weatherData.getHourly()); // write hourly forecast, up to 48 hours
      } catch (Exception e) {
    ...
    try (Response response = WebTargetBuilder.getClient().target(apiUrl).request().get()) {
      if (response.getStatus() == 200) {
        return ValueUtil.JSON.readValue(
            response.readEntity(String.class), OpenWeatherMapResponse.class);
      } else if (response.getStatus() == 401) {
        LOG.warning(
            "API request was unauthorized, either the API key is invalid or the key does not have access to the One Call 3.0 API");
        return null;
      } else {
  ...
  protected void updateCurrentValues(
      List<AttributeRef> attributeRefs,
      OpenWeatherMapResponse.WeatherDatapoint currentWeatherDatapoint) {
    for (AttributeRef attributeRef : attributeRefs) {
      ...
      if (value == null) {
        LOG.log(
            Level.WARNING,
            () -> "Weather property or value is null for attribute: " + attributeRef);
        continue;
      }
  ...
  /**
   * Update the predicted data points for the given attributes based on the provided list of weather
   * datapoints
   *
   * @param attributeRefs list of attribute references
   * @param weatherDatapoints list of weather datapoints
   */
  protected void updatePredictedDatapoints(
      List<AttributeRef> attributeRefs,
      List<OpenWeatherMapResponse.WeatherDatapoint> weatherDatapoints) {
    for (AttributeRef attributeRef : attributeRefs) {

      if (weatherDatapoints == null || weatherDatapoints.isEmpty()) {
        LOG.log(
            Level.WARNING,
            () -> "Weather data points are null or empty for attribute: " + attributeRef);
        continue;
      }

      // Build the predicted datapoints
      List<ValueDatapoint<?>> predictedDatapoints =
          buildPredictedDatapoints(attributeRef, weatherDatapoints);
  ...
  /**
   * Build a list of predicted datapoints based on the provided weather data and attribute reference
   *
   * @param attributeRef the attribute reference
   * @param weatherData the weather data
   * @return a list of ValueDatapoints
   */
  protected List<ValueDatapoint<?>> buildPredictedDatapoints(
      AttributeRef attributeRef, List<OpenWeatherMapResponse.WeatherDatapoint> weatherData) {
  ...
  /**
   * Extract the corresponding `weatherProperty` value from the weather data object
   *
   ...
   */
  protected Object extractWeatherPropertyValue(
      OpenWeatherMapResponse.WeatherDatapoint weatherData, OpenWeatherMapProperty weatherProperty) {
  ...
  /**
   * Build the OpenWeatherMap API URL for the given latitude and longitude (One Call 3.0 API)
   *
  ...
  protected String buildApiUrl(double latitude, double longitude) {
    String apiKey =
        agent.getAPIKey().orElseThrow(() -> new IllegalStateException("API key is not set"));
    return "https://api.openweathermap.org/data/3.0/onecall?lat="
        + latitude
        + "&lon="
        + longitude
        + "&units=metric&exclude=alerts,minutely&appid="
        + apiKey;
  }
  ...
  /**
   * Provision a weather asset with the appropriate agent links, location needs to be manually
   * configured after provisioning
   */
  protected WeatherAsset provisionWeatherAsset() {
    ...
    // Temperature
    weatherAsset
        .getAttribute(WeatherAsset.TEMPERATURE)
        .ifPresent(
            attribute ->
                attribute.addOrReplaceMeta(
                    new MetaItem<>(
                        AGENT_LINK,
                        new OpenWeatherMapAgentLink(agent.getId())
                            .setWeatherProperty(OpenWeatherMapProperty.TEMPERATURE)),
                    new MetaItem<>(HAS_PREDICTED_DATA_POINTS)));

    // Humidity
    weatherAsset
        .getAttribute(WeatherAsset.HUMIDITY)
        .ifPresent(
            attribute ->
                attribute.addOrReplaceMeta(
                    new MetaItem<>(
                        AGENT_LINK,
                        new OpenWeatherMapAgentLink(agent.getId())
                            .setWeatherProperty(OpenWeatherMapProperty.HUMIDITY)),
                    new MetaItem<>(HAS_PREDICTED_DATA_POINTS)));

    // Wind Speed
    weatherAsset
        .getAttribute(WeatherAsset.WIND_SPEED)
        .ifPresent(
            attribute ->
                attribute.addOrReplaceMeta(
                    new MetaItem<>(
                        AGENT_LINK,
                        new OpenWeatherMapAgentLink(agent.getId())
                            .setWeatherProperty(OpenWeatherMapProperty.WIND_SPEED)),
                    new MetaItem<>(HAS_PREDICTED_DATA_POINTS)));

    // Wind Direction
    weatherAsset
        .getAttribute(WeatherAsset.WIND_DIRECTION)
        .ifPresent(
            attribute ->
                attribute.addOrReplaceMeta(
                    new MetaItem<>(
                        AGENT_LINK,
                        new OpenWeatherMapAgentLink(agent.getId())
                            .setWeatherProperty(OpenWeatherMapProperty.WIND_DIRECTION)),
                    new MetaItem<>(HAS_PREDICTED_DATA_POINTS)));

    // Rain Amount
    weatherAsset
        .getAttribute(WeatherAsset.RAINFALL)
        .ifPresent(
            attribute ->
                attribute.addOrReplaceMeta(
                    new MetaItem<>(
                        AGENT_LINK,
                        new OpenWeatherMapAgentLink(agent.getId())
                            .setWeatherProperty(OpenWeatherMapProperty.RAIN_AMOUNT)),
                    new MetaItem<>(HAS_PREDICTED_DATA_POINTS)));

    // Ultraviolet Index
    weatherAsset
        .getAttribute(WeatherAsset.UV_INDEX)
        .ifPresent(
            attribute ->
                attribute.addOrReplaceMeta(
                    new MetaItem<>(
                        AGENT_LINK,
                        new OpenWeatherMapAgentLink(agent.getId())
                            .setWeatherProperty(OpenWeatherMapProperty.ULTRAVIOLET_INDEX)),
                    new MetaItem<>(HAS_PREDICTED_DATA_POINTS)));

    // Atmospheric Pressure (optional attribute)
    weatherAsset
        .getAttributes()
        .getOrCreate(WeatherAsset.ATMOSPHERIC_PRESSURE)
        .addOrReplaceMeta(
            new MetaItem<>(
                AGENT_LINK,
                new OpenWeatherMapAgentLink(agent.getId())
                    .setWeatherProperty(OpenWeatherMapProperty.ATMOSPHERIC_PRESSURE)),
            new MetaItem<>(HAS_PREDICTED_DATA_POINTS));

    // Wind Gust Speed (optional attribute)
    weatherAsset
        .getAttributes()
        .getOrCreate(WeatherAsset.WIND_GUST_SPEED)
        .addOrReplaceMeta(
            new MetaItem<>(
                AGENT_LINK,
                new OpenWeatherMapAgentLink(agent.getId())
                    .setWeatherProperty(OpenWeatherMapProperty.WIND_GUST_SPEED)),
            new MetaItem<>(HAS_PREDICTED_DATA_POINTS));

    // Cloud Coverage (optional attribute)
    weatherAsset
        .getAttributes()
        .getOrCreate(WeatherAsset.CLOUD_COVERAGE)
        .addOrReplaceMeta(
            new MetaItem<>(
                AGENT_LINK,
                new OpenWeatherMapAgentLink(agent.getId())
                    .setWeatherProperty(OpenWeatherMapProperty.CLOUD_COVERAGE)),
            new MetaItem<>(HAS_PREDICTED_DATA_POINTS));

    // Probability of Precipitation (optional attribute)
    weatherAsset
        .getAttributes()
        .getOrCreate(WeatherAsset.PROBABILITY_OF_PRECIPITATION)
        .addOrReplaceMeta(
            new MetaItem<>(
                AGENT_LINK,
                new OpenWeatherMapAgentLink(agent.getId())
                    .setWeatherProperty(OpenWeatherMapProperty.PROBABILITY_OF_PRECIPITATION)),
            new MetaItem<>(HAS_PREDICTED_DATA_POINTS));
  ...
  protected void setOpenWeatherMapAttribution() {
    sendAttributeEvent(
        new AttributeEvent(
            agent.getId(),
            OpenWeatherMapAgent.ATTRIBUTION.getName(),
