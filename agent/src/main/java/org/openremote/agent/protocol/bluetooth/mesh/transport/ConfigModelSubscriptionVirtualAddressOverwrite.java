 ...
 *
 * <p>his message will clear the existing subscription list in a model and add a new one
 */
@SuppressWarnings({"unused"})
public final class ConfigModelSubscriptionVirtualAddressOverwrite extends ConfigMessage {

  public static final Logger LOG =
      Logger.getLogger(ConfigModelSubscriptionVirtualAddressOverwrite.class.getName());
  private static final int OP_CODE =
      ConfigMessageOpCodes.CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_OVERWRITE;

  // Length in bytes
  private static final int CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_OVERWRITE_LENGTH = 20;
  private static final int VENDOR_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_OVERWRITE_LENGTH =
      CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_OVERWRITE_LENGTH + 2;
   ...
   * @param modelIdentifier Model identifier, 16-bit for Sig model and 32-bit model id for vendor
   *     models.
   * @throws IllegalArgumentException if any illegal arguments are passed
   */
  public ConfigModelSubscriptionVirtualAddressOverwrite(
      final int elementAddress, /* @NonNull */ final UUID labelUuid, final int modelIdentifier)
  ...
  void assembleMessageParameters() {
    ...
    // We check if the model identifier value is within the range of a 16-bit value here. If it is
    // then it is a sigmodel
    ...
    if (mModelIdentifier >= Short.MIN_VALUE && mModelIdentifier <= Short.MAX_VALUE) {
      paramsBuffer =
          ByteBuffer.allocate(CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_OVERWRITE_LENGTH)
      ...
      paramsBuffer =
          ByteBuffer.allocate(VENDOR_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_OVERWRITE_LENGTH)
              .order(ByteOrder.LITTLE_ENDIAN);
      paramsBuffer.put(elementAddress[1]);
      paramsBuffer.put(elementAddress[0]);
      paramsBuffer.put(subscriptionAddress);
      final byte[] modelIdentifier =
          new byte[] {
            (byte) ((mModelIdentifier >> 24) & 0xFF),
            (byte) ((mModelIdentifier >> 16) & 0xFF),
            (byte) ((mModelIdentifier >> 8) & 0xFF),
