...
public class ShadowGenericOnOffModel extends ShadowMeshModel {
  ...
  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, ShadowGenericOnOffModel.class.getName());

  public ShadowGenericOnOffModel(
      ExecutorService executorService,
      BluetoothMeshNetwork network,
  ...
  public synchronized void onMeshMessageReceived(MeshMessage meshMessage) {
    if (meshMessage.getOpCode() == GENERIC_ON_OFF_STATUS
        && meshMessage instanceof GenericOnOffStatus) {
      final GenericOnOffStatus status = (GenericOnOffStatus) meshMessage;
      Boolean isOn = status.getPresentState();
      LOG.info(
          "Received model status update: [model=GenericOnOffServer, address="
              + element.getAddress()
              + ", state="
              + (isOn ? "ON" : "OFF")
              + "]");
