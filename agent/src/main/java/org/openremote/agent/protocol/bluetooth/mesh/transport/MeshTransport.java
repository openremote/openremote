...
/**
 * MeshTransport class is responsible for building the configuration and application layer mesh
 * messages.
 */
final class MeshTransport extends NetworkLayer {
  ...
  @Override
  protected final synchronized void setLowerTransportLayerCallbacks(
      final LowerTransportLayerCallbacks callbacks) {
  ...
  @Override
  final synchronized void setUpperTransportLayerCallbacks(
      final UpperTransportLayerCallbacks callbacks) {
   ...
   *
   * @param controlMessage Control message containing the required opcodes and parameters to create
   *     the message
   * @return Control message containing the acknowledgement message pdu
   */
  final synchronized ControlMessage createSegmentBlockAcknowledgementMessage(
   ...
   *
   * <p>This method will create the access message and propagate the message through the transport
   * layers to create the final mesh pdu.
   *
   * @param src Source address of the provisioner/configurator.
   * @param dst Destination address to be sent to
   * @param key Device Key
   * @param akf Application key flag defines which key to be used to decrypt the message i.e device
   *     key or application key.
   * @param aid Identifier of the application key.
   * @param aszmic Defines the length of the transport mic length where 1 will encrypt with 64 bit
   *     and 0 with 32 bit encryption.
   ...
   */
  final synchronized AccessMessage createMeshMessage(
      final int src,
      final int dst,
    ...
    LOG.info(
        "Access message parameters: " + MeshParserUtils.bytesToHex(accessMessageParameters, false));
   ...
   *     key or application key.
   * @param aid Identifier of the application key.
   * @param aszmic Defines the length of the transport mic length where 1 will encrypt within 64 bit
   *     and 0 with 32 bit encryption.
   * @param accessOpCode Operation code for the access message.
   * @param accessMessageParameters Parameters for the access message.
   * @return access message containing the mesh pdu
   */
  final synchronized AccessMessage createVendorMeshMessage(
   ...
   */
  final synchronized ControlMessage createProxyConfigurationMessage(
      final int src, final int dst, final int opcode, final byte[] parameters) {
    final ProvisionedMeshNode node = mUpperTransportLayerCallbacks.getNode(src);
    final int sequenceNumber = node.incrementSequenceNumber();
    final byte[] sequenceNum = MeshParserUtils.getSequenceNumberBytes(sequenceNumber);
