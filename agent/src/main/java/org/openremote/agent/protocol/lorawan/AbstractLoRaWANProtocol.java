...
public abstract class AbstractLoRaWANProtocol<
    ...
    implements Protocol<T>, ProtocolAssetImport {

  private static final Logger LOG =
      SyslogCategory.getLogger(PROTOCOL, AbstractLoRaWANProtocol.class);
  ...
  public AbstractLoRaWANProtocol(T agent) {
    ...
    agent
        .getCertificateAlias()
        .ifPresent(certificateAlias -> mqttAgent.setCertificateAlias(certificateAlias));
    ...
    agent
        .getWebsocketQuery()
        .ifPresent(websocketQuery -> mqttAgent.setWebsocketQuery(websocketQuery));
    agent.getLastWillTopic().ifPresent(lastWillTopic -> mqttAgent.setLastWillTopic(lastWillTopic));
    agent
        .getLastWillPayload()
        .ifPresent(lastWillPayload -> mqttAgent.setLastWillPayload(lastWillPayload));
    agent
        .isLastWillRetain()
        .ifPresent(lastWillRetain -> mqttAgent.setLastWillRetain(lastWillRetain));
    agent
        .getUsernamePassword()
        .ifPresent(usernamePassword -> mqttAgent.setUsernamePassword(usernamePassword));
    mqttAgent.setWildcardSubscriptionTopics(
        createWildcardSubscriptionTopicList().toArray(new String[0]));
    ...
    return executorService.submit(
        () -> {
          ...
          try (ByteArrayInputStream inputStream = new ByteArrayInputStream(fileData)) {
            List<CsvRecord> csvEntries =
                csvMapper
                    .readerFor(CsvRecord.class)
                    .with(schema)
                    .<CsvRecord>readValues(inputStream)
                    .readAll();

            AssetTreeNode[] assetTreeNodes =
                csvEntries.stream()
  ...
  protected abstract boolean configureMQTTSubscriptionTopic(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord);

  protected abstract boolean configureMQTTPublishTopic(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord);

  protected abstract boolean configureMQTTMessageMatchFilterAndPredicate(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord);

  protected abstract boolean configureMQTTWriteValueTemplate(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord);

  protected boolean configureMQTTValueFilter(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord) {
    if (attribute == null || agentLink == null || csvRecord == null) {
      return false;
    }
    getAgentConfigJsonPath(attribute)
        .ifPresent(
            jsonPath ->
                agentLink.setValueFilters(
                    new ValueFilter[] {new JsonPathFilter(jsonPath, true, false)}));
    getAgentConfigRegex(attribute)
        .ifPresent(
            pattern ->
                agentLink.setValueFilters(
                    new ValueFilter[] {new RegexValueFilter(pattern, false, false, 1, 0)}));
    return true;
  }

  protected boolean configureMQTTValueConverter(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord) {
    if (attribute == null || agentLink == null || csvRecord == null) {
      return false;
    }
    getAgentConfigValueConverter(attribute)
        .ifPresent(converter -> agentLink.setValueConverter(converter));
    return true;
  }

  protected boolean configureMQTTWriteValueConverter(
      Attribute<?> attribute, MQTTAgentLink agentLink, CsvRecord csvRecord) {
    if (attribute == null || agentLink == null || csvRecord == null) {
      return false;
    }
    getAgentConfigWriteValueConverter(attribute)
        .ifPresent(converter -> agentLink.setWriteValueConverter(converter));
    return true;
  }
  ...
  protected Optional<ValueType.ObjectMap> getAgentConfigWriteValueConverter(
      Attribute<?> attribute) {
    return getAgentConfig(attribute)
  ...
  private boolean duplicateAssetCheck(CsvRecord record) {
    ...
    if (!duplicateAssetCheck(record.getDevEUI())) {
      LOG.log(
          Level.INFO,
          "CSV import skipped a CSV record because an asset already existed: " + record);
      return false;
    }
  ...
  private Optional<Class<? extends Asset<?>>> resolveAssetClass(
      String simpleClassName, CsvRecord csvRecord) {
    ...
    return ValueUtil.getAssetClass(simpleClassName)
        .or(
            () -> {
              LOG.log(
                  Level.WARNING,
                  "CSV import skipped a CSV record because of an invalid asset type name: "
                      + csvRecord);
              return Optional.empty();
            });
  }
  ...
  private Optional<Asset<?>> configureAsset(Asset<?> asset, CsvRecord csvRecord) {
    ...
    Optional.ofNullable(csvRecord.getDevEUI())
        .ifPresent(
            devEUI ->
                asset
                    .getAttribute(ATTRIBUTE_NAME_DEV_EUI)
                    .ifPresent(attribute -> attribute.setValue(devEUI.toUpperCase())));
    Optional.ofNullable(csvRecord.getVendorId())
        .ifPresent(
            vendorId ->
                asset
                    .getAttribute(ATTRIBUTE_NAME_VENDOR_ID)
                    .ifPresent(attribute -> attribute.setValue(vendorId)));
    Optional.ofNullable(csvRecord.getModelId())
        .ifPresent(
            modelId ->
                asset
                    .getAttribute(ATTRIBUTE_NAME_MODEL_ID)
                    .ifPresent(attribute -> attribute.setValue(modelId)));
    Optional.ofNullable(csvRecord.getFirmwareVersion())
        .ifPresent(
            version ->
                asset
                    .getAttribute(ATTRIBUTE_NAME_FIRMWARE_VERSION)
                    .ifPresent(attribute -> attribute.setValue(version)));
    ...
    for (Map.Entry<String, Attribute<?>> entry : asset.getAttributes().entrySet()) {
      ...
      if (getAgentConfig(attribute).isPresent()
          || ATTRIBUTE_NAME_DEV_EUI.equals(attribute.getName())) {
