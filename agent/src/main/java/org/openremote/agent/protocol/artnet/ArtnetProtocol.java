...
public class ArtnetProtocol extends
        AbstractNettyIOClientProtocol<ArtnetProtocol, ArtnetAgent, ArtnetPacket, UDPIOClient<ArtnetPacket>, DefaultAgentLink>
    ...
    protected UDPIOClient<ArtnetPacket> doCreateIoClient() throws Exception {
        String host = agent.getHost()
                .orElseThrow(() -> new IllegalArgumentException("Required host attribute is null or missing"));

        int port = agent.getPort().orElseThrow(() -> new IllegalArgumentException("Port must be in the range 1-65536"));
    ...
    protected void doLinkAttribute(String assetId, Attribute<?> attribute, DefaultAgentLink agentLink) {
        ...
        // if(lightAttribute != null && groupAttribute != null && universeAttribute != null && amountOfLedsAttribute !=
        // null && requiredValuesAttribute != null)
        // {
        ...
        // ArtnetLight lightToCreate = new ArtnetLight(lightId, groupId, universe, amountOfLeds, requiredKeys, state,
        // null);
    ...
    protected void doUnlinkAttribute(String assetId, Attribute<?> attribute, DefaultAgentLink agentLink) {
        ...
        // artnetLightMemory.stream().filter(light -> light.getLightId() == lightId).findFirst().ifPresent(artnetLight
        // -> artnetLightMemory.remove(artnetLight));
        // }
        // }
        // }
        // }
    ...
    protected ArtnetPacket createWriteMessage(DefaultAgentLink agentLink, AttributeEvent event, Object processedValue) {
        ...
        // ArtnetLight updatedLight = artnetLightMemory.stream().filter(light -> light.getLightId() ==
        // lightId).findFirst().orElse(null);
        ...
        // }
        // }
        // updateLightStateInMemory(lightId, new ArtnetLightState(lightId, valuesToUpdate, oldLightState.getDim(),
        // oldLightState.isEnabled()));
        // }
        // //UPDATE DIM
        ...
        // updateLightStateInMemory(lightId, new ArtnetLightState(lightId, oldLightState.getReceivedValues(), dimValue,
        // oldLightState.isEnabled()));
        // } catch (JsonProcessingException e) {
        // e.printStackTrace();
        // }
        ...
        // updateLightStateInMemory(lightId, new ArtnetLightState(lightId, oldLightState.getReceivedValues(),
        // oldLightState.getDim(), enabled));
        // } catch (JsonProcessingException e) {
        // e.printStackTrace();
        // }
        // }
        // //SEND ALL LIGHTS TO UPDATE TO THE ENCODER
        // List<ArtnetLight> lightsToSend = artnetLightMemory.stream().filter(light -> light.getUniverse() ==
        // universeId).collect(Collectors.toList());
    ...
    // ArtnetLight foundLight = (ArtnetLight) artnetLightMemory.stream().filter(l -> l.getLightId() ==
    // light.getLightId()).findFirst().orElse(null);
    // if(foundLight != null) {
    ...
    // }
    //
    // private AssetTreeNode[] syncLightsToAssets(List<ArtnetLight> lights, Attribute<?> protocolConfiguration) throws
    // Exception {
    // List<AssetTreeNode> output = new ArrayList<AssetTreeNode>();
    //
    // //Fetch all the assets that're connected to the ArtNet agent.
    // List<Asset<?>> assetsUnderProtocol = assetService.findAssets(protocolConfiguration.getAssetId().orElse(null), new
    // AssetQuery());
    ...
    // new Attribute<>("Id", NUMBER, ValueUtil.create(updatedLight.getLightId())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("GroupId", NUMBER, ValueUtil.create(updatedLight.getGroupId())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("Universe", NUMBER, ValueUtil.create(updatedLight.getUniverse())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("AmountOfLeds", NUMBER, ValueUtil.create(updatedLight.getAmountOfLeds())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("RequiredValues", STRING, ValueUtil.create(String.join(",",
    // updatedLight.getRequiredValues()))).addMeta(new Meta(new MetaItem<>(READ_ONLY, true))),
    ...
    // values.put(key, 0);
    // List<Attribute<?>> artNetLightAttributes = Arrays.asList(
    // new Attribute<>("Id", NUMBER, ValueUtil.create(light.getLightId())).addMeta(new Meta(new MetaItem<>(READ_ONLY,
    // true))),
    // new Attribute<>("GroupId", NUMBER, ValueUtil.create(light.getGroupId())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("Universe", NUMBER, ValueUtil.create(light.getUniverse())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("AmountOfLeds", NUMBER, ValueUtil.create(light.getAmountOfLeds())).addMeta(new Meta(new
    // MetaItem<>(READ_ONLY, true))),
    // new Attribute<>("RequiredValues", STRING, ValueUtil.create(String.join(",",
