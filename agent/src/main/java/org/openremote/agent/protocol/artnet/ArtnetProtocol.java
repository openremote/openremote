...
public class ArtnetProtocol
    extends AbstractNettyIOClientProtocol<
        ArtnetProtocol, ArtnetAgent, ArtnetPacket, UDPIOClient<ArtnetPacket>, DefaultAgentLink>
  ...
  protected UDPIOClient<ArtnetPacket> doCreateIoClient() throws Exception {
    ...
    int port =
        agent
            .getPort()
            .orElseThrow(() -> new IllegalArgumentException("Port must be in the range 1-65536"));
    ...
    return () ->
        new ChannelHandler[] {
          new AbstractNettyIOClient.MessageToByteEncoder<>(
              ArtnetPacket.class, client, ArtnetPacket::toByteBuf)
        };
  }
  ...
  protected void doLinkAttribute(
    ...
    //        Attribute<?> requiredValuesAttribute =
    // asset.getAttribute("RequiredValues").orElse(null);
    //        if(lightAttribute != null && groupAttribute != null && universeAttribute != null &&
    // amountOfLedsAttribute != null && requiredValuesAttribute != null)
    //        {
    ...
    //            String requiredKeysField =
    // requiredValuesAttribute.getValueAsString().orElse(null);
    //
    //            if(lightId != -1 && groupId != -1 && universe != -1 && amountOfLeds != -1 &&
    // requiredKeysField != null)
    //            {
    //                String[] requiredKeys = requiredKeysField.split(",");
    //                ArtnetLightState state = new ArtnetLightState(lightId, new HashMap<String,
    // Integer>(), 100, true);
    //                for(String key : requiredKeys)
    //                    state.getReceivedValues().put(key, 0);
    //                ArtnetLight lightToCreate = new ArtnetLight(lightId, groupId, universe,
    // amountOfLeds, requiredKeys, state, null);
    //                if(artnetLightMemory.stream().noneMatch(light -> light.getLightId() ==
    // lightToCreate.getLightId()))
  ...
  protected void doUnlinkAttribute(
    ...
    // getLinkedAttribute(attribute.getReference().orElse(null));
    ...
    //                Attribute<?> universeAttribute =
    // parentAsset.getAttribute("Universe").orElse(null);
    //                if(lightAttribute != null && universeAttribute != null) {
    //                    int lightId = lightAttribute.getValueAsInteger().orElse(-1);
    //                    int universe = universeAttribute.getValueAsInteger().orElse(-1);
    //                    if(lightId != -1 && universe != -1) {
    //                        if(artnetLightMemory.stream().anyMatch(light -> light.getLightId() ==
    // lightId)) {
    //                            artnetLightMemory.stream().filter(light -> light.getLightId() ==
    // lightId).findFirst().ifPresent(artnetLight -> artnetLightMemory.remove(artnetLight));
    //                        }
    //                    }
    //                }
    //            }
  ...
  protected ArtnetPacket createWriteMessage(
    ...
    //                ArtnetLight updatedLight = artnetLightMemory.stream().filter(light ->
    // light.getLightId() == lightId).findFirst().orElse(null);
    ...
    //                            try {
    //                                JsonNode node =
    // ValueUtil.JSON.readTree(processedValue.toJson());
    ...
    // valuesToUpdate, oldLightState.getDim(), oldLightState.isEnabled()));
    //                    }
    //                    //UPDATE DIM
    ...
    //                    List<ArtnetLight> lightsToSend = artnetLightMemory.stream().filter(light
    // -> light.getUniverse() == universeId).collect(Collectors.toList());
  ...
  //            ArtnetLight light = new ArtnetLight(lightId, groupId, universe, amountOfLeds,
  // requiredValues, null, null);
  ...
  //            ArtnetLightState state = new ArtnetLightState(light.getLightId(), new
  // HashMap<String, Integer>(), 100, true);
  ...
  //                ArtnetLight foundLight = (ArtnetLight) artnetLightMemory.stream().filter(l ->
  // l.getLightId() == light.getLightId()).findFirst().orElse(null);
  //                if(foundLight != null) {
  //                    artnetLightMemory.set(artnetLightMemory.indexOf(foundLight), light);
  //                }
  //            }
  //            //No light is found with the current id in the import file which is present
  // in-memory
  ...
  //    }
  //
  //    private AssetTreeNode[] syncLightsToAssets(List<ArtnetLight> lights, Attribute<?>
  // protocolConfiguration) throws Exception {
  //        List<AssetTreeNode> output = new ArrayList<AssetTreeNode>();
  //
  //        //Fetch all the assets that're connected to the ArtNet agent.
  //        List<Asset<?>> assetsUnderProtocol =
  // assetService.findAssets(protocolConfiguration.getAssetId().orElse(null), new AssetQuery());
  //        //Get the instance of the ArtNet agent itself.
  //        Asset<?> parentAgent = assetsUnderProtocol.stream().filter(a -> a.getWellKnownType() ==
  // AssetType.AGENT).findFirst().orElse(null);
  //        if(parentAgent != null) {
  ...
  //                            ArtnetLight updatedLight = lights.stream().filter(l ->
  // l.getLightId() == lightId).findFirst().orElse(null);
  //                            if(updatedLight != null) {
  //                                Map<String, Integer> values = new HashMap<>();
  //                                for(String key : updatedLight.getRequiredValues())
  //                                    values.put(key, 0);
  //                                List<Attribute<?>> artNetLightAttributes = Arrays.asList(
  //                                        new Attribute<>("Id", NUMBER,
  // ValueUtil.create(updatedLight.getLightId())).addMeta(new Meta(new MetaItem<>(READ_ONLY,
  // true))),
  //                                        new Attribute<>("GroupId", NUMBER,
  // ValueUtil.create(updatedLight.getGroupId())).addMeta(new Meta(new MetaItem<>(READ_ONLY,
  // true))),
  //                                        new Attribute<>("Universe", NUMBER,
  // ValueUtil.create(updatedLight.getUniverse())).addMeta(new Meta(new MetaItem<>(READ_ONLY,
  // true))),
  //                                        new Attribute<>("AmountOfLeds", NUMBER,
  // ValueUtil.create(updatedLight.getAmountOfLeds())).addMeta(new Meta(new MetaItem<>(READ_ONLY,
  // true))),
  //                                        new Attribute<>("RequiredValues", STRING,
  // ValueUtil.create(String.join(",", updatedLight.getRequiredValues()))).addMeta(new Meta(new
  // MetaItem<>(READ_ONLY, true))),
  //                                        new Attribute<>("ValueUtil", OBJECT,
  // ValueUtil.parseOrNull(ValueUtil.JSON.writeValueAsString(values))).addMeta(
  //                                                new MetaItem<>(AGENT_LINK, new
  // AttributeRef(parentAgent.getId(), agentProtocolConfigName).toArrayValue())
  //                                        ),
  ...
  //            values.put(key, 0);
  //        List<Attribute<?>> artNetLightAttributes = Arrays.asList(
  //                new Attribute<>("Id", NUMBER, ValueUtil.create(light.getLightId())).addMeta(new
  // Meta(new MetaItem<>(READ_ONLY, true))),
  //                new Attribute<>("GroupId", NUMBER,
  // ValueUtil.create(light.getGroupId())).addMeta(new Meta(new MetaItem<>(READ_ONLY, true))),
  //                new Attribute<>("Universe", NUMBER,
  // ValueUtil.create(light.getUniverse())).addMeta(new Meta(new MetaItem<>(READ_ONLY, true))),
  //                new Attribute<>("AmountOfLeds", NUMBER,
  // ValueUtil.create(light.getAmountOfLeds())).addMeta(new Meta(new MetaItem<>(READ_ONLY, true))),
  //                new Attribute<>("RequiredValues", STRING, ValueUtil.create(String.join(",",
  // light.getRequiredValues()))).addMeta(new Meta(new MetaItem<>(READ_ONLY, true))),
  //                new Attribute<>("Values", OBJECT,
  // Values.parseOrNull(Values.JSON.writeValueAsString(values))).addMeta(
  //                        new MetaItem<>(AGENT_LINK, new AttributeRef(parentAgent.getId(),
  // agentProtocolConfigName).toArrayValue())
  //                ),
  //                new Attribute<>("Switch", BOOLEAN, true).addMeta(
  //                        new MetaItem<>(AGENT_LINK, new AttributeRef(parentAgent.getId(),
  // agentProtocolConfigName).toArrayValue())
  ...
  //        return UniqueIdentifierGenerator.generateId("ArtnetLight" + groupId + lightId +
  // universe);
  //    }
}
