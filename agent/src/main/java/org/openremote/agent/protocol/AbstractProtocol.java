...
public abstract class AbstractProtocol<T extends Agent<T, ?, U>, U extends AgentLink<?>>
  ...
  private static final System.Logger LOG =
      System.getLogger(AbstractProtocol.class.getName() + "." + PROTOCOL.name());
  protected final Map<AttributeRef, Attribute<?>> linkedAttributes = new ConcurrentHashMap<>();
  protected final Set<AttributeRef> dynamicAttributes =
  ...
  /**
   * Send an arbitrary {@link AttributeState} through the processing chain using the current system
   * time as the timestamp. Use {@link #updateLinkedAttribute} to publish new sensor values, which
  ...
  /**
   * Send an arbitrary {@link AttributeEvent} through the processing chain. Use {@link
   * #updateLinkedAttribute} to publish new sensor values, which performs additional verification
   * and uses a different messaging queue.
   */
  ...
  @Override
  public final void updateLinkedAttribute(
      final AttributeRef attributeRef, final Object value, long timestamp) {
    ...
    Pair<Boolean, Object> ignoreAndConverted =
        ProtocolUtil.doInboundValueProcessing(
            attributeRef.getId(), attribute, agent.getAgentLink(attribute), value);
    ...
    AttributeEvent attributeEvent =
        new AttributeEvent(attributeRef, ignoreAndConverted.value, timestamp);
  ...
  @Override
  public final void updateLinkedAttribute(final AttributeRef attributeRef, final Object value) {
  ...
  protected abstract void doLinkAttribute(String assetId, Attribute<?> attribute, U agentLink)
      throws RuntimeException;

  /** Unlink an {@link Attribute} from its linked {@link Agent}. */
  protected abstract void doUnlinkAttribute(String assetId, Attribute<?> attribute, U agentLink);

  /**
   * An Attribute event (write) has been requested for an attribute linked to this protocol. The
   * processedValue is the resulting value after applying standard outbound value processing (see
   * {@link ProtocolUtil#doOutboundValueProcessing}). Protocol implementations should generally use
   * the processedValue but may also choose to use the original value for some purpose if required.
   */
  protected abstract void doLinkedAttributeWrite(
      U agentLink, AttributeEvent event, Object processedValue);
}
