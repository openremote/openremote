...
public class ModbusTcpDecoder extends io.netty.handler.codec.ByteToMessageDecoder {
  ...
  public static final Logger LOG = SyslogCategory.getLogger(PROTOCOL, ModbusTcpDecoder.class);
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
    ...
    LOG.finest(
        () ->
            String.format(
                "Decoded Modbus TCP frame: TxID=%d, UnitID=%d, FC=0x%02X, PDU length=%d, Exception=%b",
                frame.getTransactionId(),
                frame.getUnitId(),
                frame.getFunctionCode(),
                frame.getPdu().length,
                frame.isException()));
