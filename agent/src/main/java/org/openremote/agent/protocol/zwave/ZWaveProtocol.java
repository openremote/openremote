...
public class ZWaveProtocol extends AbstractProtocol<ZWaveAgent, ZWaveAgentLink>
  ...
  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, ZWaveProtocol.class.getName());
  ...
  protected synchronized void doLinkAttribute(
      String assetId, Attribute<?> attribute, ZWaveAgentLink agentLink) {
    if (network == null) {
      return;
    }
    ...
    Consumer<Value> sensorValueConsumer =
  ...
  protected synchronized void doLinkedAttributeWrite(
      ZWaveAgentLink agentLink, AttributeEvent event, Object processedValue) {
    if (network == null) {
      return;
    }
  ...
  private Object toAttributeValue(
      org.openremote.protocol.zwave.model.commandclasses.channel.value.Value value,
      Class<?> clazz) {
    ...
    if (clazz == String.class) {
      ...
      retValue = value.getInteger();
        ...
        && value
            instanceof
            org.openremote.protocol.zwave.model.commandclasses.channel.value.ArrayValue zwArray) {
      if (zwArray.length() >= 3) {
        ...
        retValue =
            new ColourRGB(
                (Integer) values.get(offset),
                (Integer) values.get(1 + offset),
                (Integer) values.get(2 + offset));
      }
    } else {
      LOG.warning(
          "Couldn't update Z-Wave sensor value because of unexpected attribute type: " + clazz);
    }
    return retValue;
  }
}
