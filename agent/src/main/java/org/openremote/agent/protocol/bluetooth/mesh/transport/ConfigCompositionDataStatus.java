...
public class ConfigCompositionDataStatus extends ConfigStatusMessage {
    ...
    while (tempOffset < accessPayload.length) {
        ...
        for (int i = 0; i < numSigModelIds; i++) {
          final int modelId =
              MeshParserUtils.unsignedBytesToInt(
                  accessPayload[tempOffset], accessPayload[tempOffset + 1]);
      ...
      if (numVendorModelIds > 0) {
        for (int i = 0; i < numVendorModelIds; i++) {
          // vendor models are 32-bit that contains a 16-bit company identifier and a 16-bit model
          // identifier
          ...
          final int modelIdentifier =
              MeshParserUtils.unsignedBytesToInt(
                  accessPayload[tempOffset + 2], accessPayload[tempOffset + 3]);
          ...
          LOG.info(
              "Vendor - model ID "
                  + i
                  + " : "
                  + String.format(Locale.US, "%08X", vendorModelIdentifier));
  ...
  /**
   * Returns a 16-bit value representing the minimum number of replay protection list entries in a
   * device.
   *
   * @return crpl
   */
