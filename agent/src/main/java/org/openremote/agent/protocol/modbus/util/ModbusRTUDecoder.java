...
/**
 * Decodes Modbus RTU frames from bytes. Frame format: [Unit ID (1 byte)][PDU (variable)][CRC16 (2
 * bytes)]
 */
public class ModbusRTUDecoder extends ByteToMessageDecoder {
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
    ...
    if (!ModbusSerialFrame.isValidCRC(frameBytes)) {
      LOG.warning(
          "Invalid CRC in Modbus RTU frame, discarding: UnitId="
              + unitId
              + ", FunctionCode=0x"
              + Integer.toHexString(functionCode));
      return;
    }
