...
public class SimulatorProtocol extends AbstractProtocol<SimulatorAgent, SimulatorAgentLink> {
  ...
  protected ScheduledFuture<?> scheduleReplay(
    ...
    long timeSinceOccurrenceStart =
        schedule
            .map(s -> now - s.tryAdvanceActive(now, tzOffset))
        ...
        Arrays.stream(simulatorReplayDatapoints)
            .filter(
                replaySimulatorDatapoint ->
                    replaySimulatorDatapoint.timestamp * 1000 > timeSinceOccurrenceStart)
            .findFirst()
    ...
    OptionalLong nextRun =
        Schedule.getDelay(nextDatapoint.timestamp, timeSinceOccurrenceStart, schedule.orElse(null));
    if (nextRun.isEmpty()
        || nextRun.getAsLong() < 0
  ...
  public List<ValueDatapoint<?>> calculatePredictedDatapoints(
    ...
    if (status.equals(PredictedDatapointWindow.BOTH)) {
      for (SimulatorReplayDatapoint datapoint : simulatorReplayDatapoints) {
        OptionalLong delay =
            Schedule.getDelay(datapoint.timestamp, timeSinceOccurrenceStart, schedule.orElse(null));
        if (delay.isEmpty()) {
          return predictedDatapoints;
        }
  ...
  public static class Schedule implements Serializable {
    ...
    public static OptionalLong getDelay(
        long offset, long timeSinceOccurrenceStart, Schedule schedule) {
      long offsetInMillis = offset * 1000;
      if (offsetInMillis <= timeSinceOccurrenceStart) {
        return getTimeUntilNextOccurrence(timeSinceOccurrenceStart, schedule).stream()
    ...
    public static OptionalLong getTimeUntilNextOccurrence(
        long timeSinceOccurrenceStart, Schedule schedule) {
      if (schedule != null) {
        if (schedule.recurrence != null) {
          ...
          long duration =
              schedule.upcoming.toEpochSecond(ZoneOffset.UTC)
                  - schedule.current.toEpochSecond(ZoneOffset.UTC);
