...
class GenericMessageState extends MeshMessageState {
  ...
  GenericMessageState(
    ...
    if (!MeshAddress.isAddressInRange(src)) {
      throw new IllegalArgumentException(
          "Invalid address, a source address must be a valid 16-bit value!");
    }
    this.mDst = dst;
    if (!MeshAddress.isAddressInRange(dst)) {
      throw new IllegalArgumentException(
          "Invalid address, a destination address must be a valid 16-bit value");
    }
    mLabel = label;
    createAccessMessage();
  }
  ...
  protected synchronized void createAccessMessage() {
    ...
    message =
        mMeshTransport.createMeshMessage(
            mSrc,
            mDst,
            mLabel,
            genericMessage.messageTtl,
            key,
            akf,
            aid,
            aszmic,
