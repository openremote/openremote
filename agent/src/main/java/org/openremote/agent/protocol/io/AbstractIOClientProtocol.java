...
public abstract class AbstractIOClientProtocol<T extends AbstractIOClientProtocol<T, U, V, W, X>, U extends IOAgent<U, T, X>, V, W extends IOClient<V>, X extends AgentLink<?>>
    ...
    /**
     * Supplies a set of encoders/decoders that convert from/to {@link String} to/from {@link ByteBuf} based on the
     * generic protocol {@link Attribute}s
     */
    public static Supplier<ChannelHandler[]> getGenericStringEncodersAndDecoders(
        ...
        return () -> {
            ...
            if (hexMode || binaryMode) {
                encodersDecoders
                        .add(new AbstractNettyIOClient.MessageToByteEncoder<>(String.class, client, (msg, out) -> {
                            byte[] bytes = hexMode ? ValueUtil.bytesFromHexString(msg)
                                    : ValueUtil.bytesFromBinaryString(msg);
                            out.writeBytes(bytes);
                        }));

                if (delimiters.length > 0) {
                    ByteBuf[] byteDelimiters = Arrays.stream(delimiters).map(delim -> Unpooled.wrappedBuffer(
                            hexMode ? ValueUtil.bytesFromHexString(delim) : ValueUtil.bytesFromBinaryString(delim)))
                            .toArray(ByteBuf[]::new);
                ...
                if (agent.getMessageMaxLength().isPresent()) {
                    ...
                    if (delimiters.length > 0) {
                        byteDelimiters = Arrays.stream(delimiters)
                                .map(delim -> Unpooled.wrappedBuffer(delim.getBytes(charset))).toArray(ByteBuf[]::new);
                    } else {
    ...
    protected void doLinkedAttributeWrite(X agentLink, AttributeEvent event, Object processedValue) {
        ...
        if (message == null) {
            LOG.fine("No message produced for attribute event so not sending to IO client '" + client.getClientUri()
                    + "': " + event);
            return;
        }
