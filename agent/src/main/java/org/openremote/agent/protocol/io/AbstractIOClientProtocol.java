...
public abstract class AbstractIOClientProtocol<
        T extends AbstractIOClientProtocol<T, U, V, W, X>,
        U extends IOAgent<U, T, X>,
        V,
        W extends IOClient<V>,
        X extends AgentLink<?>>
    extends AbstractProtocol<U, X> {

  /**
   * Supplies a set of encoders/decoders that convert from/to {@link String} to/from {@link ByteBuf}
   * based on the generic protocol {@link Attribute}s
   */
  public static Supplier<ChannelHandler[]> getGenericStringEncodersAndDecoders(
      AbstractNettyIOClient<String, ?> client, IOAgent<?, ?, ?> agent) {
        ...
        encodersDecoders.add(
            new AbstractNettyIOClient.MessageToByteEncoder<>(
                ...
                (msg, out) -> {
                  byte[] bytes =
                      hexMode
                          ? ValueUtil.bytesFromHexString(msg)
                          : ValueUtil.bytesFromBinaryString(msg);
                  ...
                  .map(
                      delim ->
                          Unpooled.wrappedBuffer(
                              hexMode
                                  ? ValueUtil.bytesFromHexString(delim)
                                  : ValueUtil.bytesFromBinaryString(delim)))
                  .toArray(ByteBuf[]::new);
          encodersDecoders.add(
              new DelimiterBasedFrameDecoder(maxLength, stripDelimiter, byteDelimiters));
        } else {
        ...
        encodersDecoders.add(
            new AbstractNettyIOClient.ByteToMessageDecoder<>(
                ...
                (byteBuf, messages) -> {
                  ...
                  String msg =
                      hexMode
                          ? ValueUtil.bytesToHexString(bytes)
                          : ValueUtil.bytesToBinaryString(bytes);
                  messages.add(msg);
                }));
      } else {
  ...
  public static final Logger LOG =
      SyslogCategory.getLogger(PROTOCOL, AbstractIOClientProtocol.class);
  protected W client;
  ...
  protected void doLinkedAttributeWrite(X agentLink, AttributeEvent event, Object processedValue) {
    ...
    if (message == null) {
      LOG.fine(
          "No message produced for attribute event so not sending to IO client '"
              + client.getClientUri()
              + "': "
              + event);
      return;
