...
public class MeshManagerApi implements MeshMngrApi {
  ...
  private static final long PROXY_SAR_TRANSFER_TIME_OUT =
      20 * 1000; // According to the spec the proxy protocol must contain an SAR timeout of 20
  // seconds.
  ...
  public MeshManagerApi(ScheduledExecutorService scheduledExecutorService) {
    ...
    mMeshProvisioningHandler =
        new MeshProvisioningHandler(internalTransportCallbacks, internalMeshMgrCallbacks);
    mMeshMessageHandler =
        new MeshMessageHandler(
            internalTransportCallbacks, networkLayerCallbacks, upperTransportLayerCallbacks);
  ...
  @Override
  public final synchronized void handleWriteCallbacks(final int mtuSize, final byte[] data) {
    byte[] unsegmentedPdu;
    if (!shouldWaitForMoreData(data)) {
      unsegmentedPdu = data;
    } else {
      final byte[] combinedPdu = appendWritePdu(mtuSize, data);
      if (combinedPdu == null) return;
      else {
  ...
  @Override
  public final synchronized void handleNotifications(final int mtuSize, final byte[] data) {
    byte[] unsegmentedPdu;
    if (!shouldWaitForMoreData(data)) {
      unsegmentedPdu = data;
    } else {
  ...
  /**
   * Toggles the Segmentation and Reassembly timeout for proxy configuration messages received via
   * proxy protocol
   *
   * @param data pdu
   */
  private void toggleProxyProtocolSarTimeOut(final byte[] data) {
    ...
    if (pduType
        == ((GATT_SAR_START << SAR_BIT_OFFSET) | MeshManagerApi.PDU_TYPE_PROXY_CONFIGURATION)) {
      // mHandler.postDelayed(mProxyProtocolTimeoutRunnable, PROXY_SAR_TRANSFER_TIME_OUT);
      scheduleTimeoutHandler();
    } else if (pduType
        == ((GATT_SAR_END << SAR_BIT_OFFSET) | MeshManagerApi.PDU_TYPE_PROXY_CONFIGURATION)) {
  ...
  private void scheduleTimeoutHandler() {
    ...
    scheduledFuture =
        scheduledExecutorService.schedule(
            mProxyProtocolTimeoutRunnable, PROXY_SAR_TRANSFER_TIME_OUT, TimeUnit.MILLISECONDS);
  }
      ...
      switch (unsegmentedPdu[0]) {
        ...
        case PDU_TYPE_MESH_BEACON:
          ...
          for (int i = 0; i < mMeshNetwork.getNetKeys().size(); i++) {
            ...
            final SecureNetworkBeacon localSecureNetworkBeacon =
                SecureUtils.createSecureNetworkBeacon(n, flags, networkId, ivIndex);
            // Check the the beacon received is a valid by matching the authentication values
            if (Arrays.equals(
                receivedBeacon.getAuthenticationValue(),
              ...
              if (mMeshNetwork.getPrimaryNetworkKey() != null && networkKey.keyIndex != 0) {
                LOG.info(
                    "Discarding beacon for secondary subnet with network key index: "
                        + networkKey.keyIndex);
                return;
              }
              ...
              /// The node shall not execute more than one IV Index Recovery within a period of 192
              // hours.
              ...
              if (!receivedBeacon.canOverwrite(
                  lastIvIndex, lastTransitionDate, isIvRecoveryActive, isIvTestModeActive, flag)) {
                String numberOfHoursSinceDate =
                    ((Calendar.getInstance().getTimeInMillis()
                                - lastTransitionDate.getTimeInMillis())
                            / (3600 * 1000))
                        + "h";
                LOG.info(
                    "Discarding beacon "
                        + receivedBeacon.getIvIndex()
                        + ", last "
                        + lastIvIndex.getIvIndex()
                        + ", changed: "
                        + numberOfHoursSinceDate
                        + "ago, test mode: "
                        + ivUpdateTestModeActive);
                return;
              }

              final IvIndex receivedIvIndex = receivedBeacon.getIvIndex();
              mMeshNetwork.ivIndex =
                  new IvIndex(
                      receivedIvIndex.getIvIndex(),
                      receivedIvIndex.isIvUpdateActive(),
                      lastTransitionDate);
              ...
              if (mMeshNetwork.ivIndex.getTransmitIvIndex() > lastIvIndex.getTransmitIvIndex()) {
                ...
                final ProvisionedMeshNode node =
                    mMeshNetwork.getNode(provisioner.getProvisionerUuid());
                node.setSequenceNumber(0);
              }

              // Updating the iv recovery flag
              if (lastIvIndex != mMeshNetwork.ivIndex) {
                final boolean ivRecovery =
                    mMeshNetwork.getIvIndex().getIvIndex() > lastIvIndex.getIvIndex() + 1
              ...
              if (!mMeshNetwork.ivIndex.getIvRecoveryFlag()) {
                final Iterator<Map.Entry<Integer, List<Integer>>> iterator =
                    mMeshNetwork.networkExclusions.entrySet().iterator();
        ...
        case PDU_TYPE_PROXY_CONFIGURATION:
          // Proxy configuration
          LOG.info(
              "Received proxy configuration message: "
  ...
  public synchronized void createMeshPdu(final int dst, final MeshMessage meshMessage) {
    if (!MeshAddress.isAddressInRange(dst)) {
      throw new IllegalArgumentException(
          "Invalid address, destination address must be a valid 16-bit value.");
    }
    final Provisioner provisioner = mMeshNetwork.getSelectedProvisioner();
    if (provisioner != null && provisioner.getProvisionerAddress() != null) {
      ...
      if (MeshAddress.isValidVirtualAddress(dst)) {
        ...
        if (label == null) {
          throw new IllegalArgumentException(
              "Label UUID unavailable for the virtual address provided");
        }
      }
      mMeshMessageHandler.createMeshMessage(
          provisioner.getProvisionerAddress(), dst, label, meshMessage);
    } else {
      throw new IllegalArgumentException(
  ...
  private MeshNetwork generateMeshNetwork(int provisionerAddress) {
    ...
    final AllocatedUnicastRange unicastRange =
        new AllocatedUnicastRange(provisionerAddress, provisionerAddress);
    ...
    final Provisioner provisioner =
        network.createProvisioner("nRF Mesh Provisioner", unicastRange, groupRange, sceneRange);
  ...
  /**
   * Deletes an address from the scenes in the network. This is to be called when resetting or
  ...
  private final InternalMeshManagerCallbacks internalMeshMgrCallbacks =
      new InternalMeshManagerCallbacks() {
        @Override
        public void onNodeProvisioned(final ProvisionedMeshNode meshNode) {
          synchronized (MeshManagerApi.this) {
            updateProvisionedNodeList(meshNode);
            mMeshNetwork.sequenceNumbers.put(
                meshNode.getUnicastAddress(), meshNode.getSequenceNumber());
            mMeshNetwork.unicastAddress =
                mMeshNetwork.nextAvailableUnicastAddress(
                    meshNode.getNumberOfElements(), mMeshNetwork.getSelectedProvisioner());
        ...
        public Provisioner getProvisioner(final int unicastAddress) {
          synchronized (MeshManagerApi.this) {
            for (Provisioner provisioner : mMeshNetwork.getProvisioners()) {
              if (provisioner.isLastSelected()) return provisioner;
            }
            return null;
          }
        }
  ...
  private final UpperTransportLayerCallbacks upperTransportLayerCallbacks =
      new UpperTransportLayerCallbacks() {
  ...
  private final InternalTransportCallbacks internalTransportCallbacks =
      new InternalTransportCallbacks() {
        ...
        public void storeScene(
            final int address, final int currentScene, final List<Integer> scenes) {
          synchronized (MeshManagerApi.this) {
        ...
        private void updateNetwork(final ProvisionedMeshNode meshNode) {
          synchronized (MeshManagerApi.this) {
            ...
            mMeshNetwork.setTimestamp(System.currentTimeMillis());
            // mMeshNetworkDb.update(mMeshNetwork, mMeshNetworkDao, mNetworkKeysDao,
            // mApplicationKeysDao, mProvisionersDao, mProvisionedNodesDao,
  ...
  private byte[] removeSegmentation(final int mtuSize, final byte[] data) {
    ...
    if (chunks > 1) {
      ...
      int length;
      for (int i = 0; i < chunks; i++) {
        // when removing segmentation bits we only remove the start because the pdu type would be
