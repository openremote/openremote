...
public final class ConfigModelSubscriptionVirtualAddressDelete extends ConfigMessage {
  ...
  public static final Logger LOG =
      Logger.getLogger(ConfigModelSubscriptionVirtualAddressDelete.class.getName());
  private static final int OP_CODE =
      ConfigMessageOpCodes.CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_DELETE;

  // Length in bytes
  private static final int CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_DELETE_LENGTH = 20;
  private static final int VENDOR_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_DELETE_LENGTH =
      CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_DELETE_LENGTH + 2;
   ...
   */
  public ConfigModelSubscriptionVirtualAddressDelete(
  ...
  void assembleMessageParameters() {
    ...
    if (mModelIdentifier >= Short.MIN_VALUE && mModelIdentifier <= Short.MAX_VALUE) {
      paramsBuffer =
          ByteBuffer.allocate(CONFIG_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_DELETE_LENGTH)
      ...
      paramsBuffer =
          ByteBuffer.allocate(VENDOR_MODEL_SUBSCRIPTION_VIRTUAL_ADDRESS_DELETE_LENGTH)
              .order(ByteOrder.LITTLE_ENDIAN);
      paramsBuffer.put(elementAddress[1]);
      paramsBuffer.put(elementAddress[0]);
      paramsBuffer.put(subscriptionAddress);
