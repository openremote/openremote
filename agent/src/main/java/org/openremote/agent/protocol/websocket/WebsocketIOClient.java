...
/**
 * This is an {@link IOClient} implementation based on {@link AbstractNettyIOClient}. For custom
 * decoders, the initial message type is of type {@link String}. For custom encoders, the final
 * message type must be of type {@link String}.
 */
public class WebsocketIOClient<T> extends AbstractNettyIOClient<T, InetSocketAddress> {
  ...
  public WebsocketIOClient(
      URI uri, Map<String, List<String>> headers, OAuthGrant oAuthGrant, boolean pingDisabled) {
    this.uri = uri;
  ...
  protected void addEncodersDecoders(Channel channel) throws Exception {
    ...
    if (!pingDisabled) {
      channel
          .pipeline()
          .addFirst(
              new ReadTimeoutHandler(PING_MILLIS, TimeUnit.MILLISECONDS) {
                @Override
    ...
    channel
        .pipeline()
        .addLast(
            new io.netty.handler.codec.MessageToMessageDecoder<WebSocketFrame>() {
              @Override
    ...
    channel
        .pipeline()
        .addLast(
            new MessageToMessageEncoder<String>() {
              @Override
    ...
    channel
        .pipeline()
        .addLast(
            new io.netty.handler.codec.MessageToMessageEncoder<ByteBuf>() {
              @Override
