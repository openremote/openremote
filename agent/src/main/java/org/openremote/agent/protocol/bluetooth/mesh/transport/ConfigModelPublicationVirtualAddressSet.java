...
public class ConfigModelPublicationVirtualAddressSet extends ConfigMessage {
  ...
  public static final Logger LOG =
      Logger.getLogger(ConfigModelPublicationVirtualAddressSet.class.getName());
  private static final int OP_CODE =
      ConfigMessageOpCodes.CONFIG_MODEL_PUBLICATION_VIRTUAL_ADDRESS_SET;
   ...
   *     friendship credentials and false for master credentials. Currently supports only master
   *     credentials
   ...
   */
  public ConfigModelPublicationVirtualAddressSet(
  ...
  void assembleMessageParameters() {
    ...
    LOG.info(
        "Model: "
            + MeshParserUtils.bytesToHex(MeshAddress.addressIntToBytes(modelIdentifier), false));
    ...
    // then it is a sig model
    if (modelIdentifier >= Short.MIN_VALUE && modelIdentifier <= Short.MAX_VALUE) {
      paramsBuffer =
          ByteBuffer.allocate(SIG_MODEL_PUBLISH_SET_PARAMS_LENGTH).order(ByteOrder.LITTLE_ENDIAN);
      ...
      paramsBuffer =
          ByteBuffer.allocate(VENDOR_MODEL_PUBLISH_SET_PARAMS_LENGTH)
      ...
      final byte[] modelIdentifier =
          new byte[] {
            (byte) ((this.modelIdentifier >> 24) & 0xFF),
            (byte) ((this.modelIdentifier >> 16) & 0xFF),
            (byte) ((this.modelIdentifier >> 8) & 0xFF),
