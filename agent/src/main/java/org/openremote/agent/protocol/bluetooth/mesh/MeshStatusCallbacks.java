...
public interface MeshStatusCallbacks {
   ...
   *
   * <p>As of now this is only triggered if the incomplete timer has expired for a given segmented
   * message. The incomplete timer will wait for a minimum of 10 seconds on receiving a segmented
   * message. If all segments are not received during this period, that transaction shall be
   ...
   *
   * <p>This callback is invoked after {@link MeshManagerCallbacks#onMeshPduCreated(byte[])} where a
   * mesh pdu is created.
   *
   ...
   *
   * <p>This callback is invoked after {@link MeshManagerCallbacks#onMeshPduCreated(byte[])} where a
   * mesh pdu is created and is ready to be sent.
   *
