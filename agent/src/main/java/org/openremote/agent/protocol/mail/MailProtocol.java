...
public class MailProtocol extends AbstractMailProtocol<MailAgent, MailProtocol, MailAgentLink> {
  ...
  public String getProtocolInstanceUri() {
    return "mailClient://"
        ...
        + ":"
        + agent.getPort().map(Object::toString).orElse("")
        + "?username="
        + agent.getUsernamePassword().map(UsernamePassword::getUsername).orElse("");
  }

  @Override
  protected String getMailMessageAttributeValue(
      String assetId, Attribute<?> attribute, MailAgentLink agentLink, MailMessage mailMessage) {
  ...
  @Override
  protected Predicate<MailMessage> getAttributeMailMessageFilter(
      String assetId, Attribute<?> attribute, MailAgentLink agentLink) {
    return getAttributeMailMessageFilter(timerService, agentLink);
  }

  public static <T extends MailAgentLink> Predicate<MailMessage> getAttributeMailMessageFilter(
      TimerService timerService, T agentLink) {
    ...
    if (fromPredicate != null) {
      Predicate<Object> valuePredicate =
          fromPredicate.asPredicate(timerService::getCurrentTimeMillis);
      messageFilter =
          m -> {
    ...
    if (subjectPredicate != null) {
      Predicate<Object> valuePredicate =
          subjectPredicate.asPredicate(timerService::getCurrentTimeMillis);
      Predicate<MailMessage> subjectFilter =
          m -> {
