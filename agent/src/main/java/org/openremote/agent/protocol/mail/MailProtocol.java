...
public class MailProtocol extends AbstractMailProtocol<MailAgent, MailProtocol, MailAgentLink> {
    ...
    public String getProtocolInstanceUri() {
        return "mailClient://" + agent.getHost().orElse("") + ":" + agent.getPort().map(Object::toString).orElse("")
                + "?username=" + agent.getUsernamePassword().map(UsernamePassword::getUsername).orElse("");
    }

    @Override
    protected String getMailMessageAttributeValue(String assetId, Attribute<?> attribute, MailAgentLink agentLink,
    ...
    @Override
    protected Predicate<MailMessage> getAttributeMailMessageFilter(String assetId, Attribute<?> attribute,
            MailAgentLink agentLink) {
