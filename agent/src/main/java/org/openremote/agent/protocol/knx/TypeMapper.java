...
public class TypeMapper {
  ...
  private static final Map<String, ValueDescriptor<?>> dptToTypeMap;
  ...
  static {
    ...
    // Main number 3
    // dptToTypeMap.put(DPTXlator3BitControlled.DPT_CONTROL_DIMMING.getID(),
    // ValueType.IncreaseDecrease);
  ...
  public static DPTXlator toDPTXlator(Datapoint datapoint, Object value) throws Exception {
    ...
    if (translator instanceof DPTXlatorBoolean && ValueUtil.isBoolean(value.getClass())) {
      ((DPTXlatorBoolean) translator).setValue(ValueUtil.getBoolean(value).orElse(false));
        ...
        && ValueUtil.isNumber(value.getClass())) {
  ...
  public static Object toValue(Datapoint datapoint, byte[] data) throws Exception {
    ...
    LOG.info(
        "Received KNX data: "
            + translator.getType().getID()
            + " ("
            + translator.getType().getDescription()
            + ") is "
            + translator.getValue()
            + " ("
            + translator.getNumericValue()
            + ") - "
            + datapoint.getName());
    ...
    if (translator instanceof DPTXlator2ByteFloat) {
      return BigDecimal.valueOf(translator.getNumericValue())
