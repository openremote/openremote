...
public abstract class AbstractMailProtocol<
        T extends AbstractMailAgent<T, U, V>,
        U extends AbstractMailProtocol<T, U, V>,
        V extends AgentLink<V>>
  ...
  protected static final Logger LOG =
      SyslogCategory.getLogger(PROTOCOL, AbstractMailProtocol.class);
  ...
  protected void doStart(Container container) throws Exception {
    ...
    MailClientBuilder clientBuilder =
        new MailClientBuilder(
            ...
            .setCheckIntervalSeconds(
                getAgent()
                    ...
                    .orElse(MailClientBuilder.DEFAULT_CHECK_INTERVAL_SECONDS))
            ...
            // Set an initial delay to allow attributes to be linked before we read messages - not
            // perfect but it should do
    ...
    oAuthGrant
        .map(oAuth -> clientBuilder.setOAuth(userPassword.getUsername(), oAuth))
        .orElseGet(
            () ->
  ...
  protected void doLinkAttribute(String assetId, Attribute<?> attribute, V agentLink)
    ...
    Predicate<MailMessage> messageFilter =
        getAttributeMailMessageFilter(assetId, attribute, agentLink);

    Function<MailMessage, String> mailMessageProcessor =
        mailMessage -> {
    ...
    attributeMessageProcessorMap.put(
        new AttributeRef(assetId, attribute.getName()), mailMessageProcessor);
  }
  ...
  protected void onMailMessage(MailMessage mailMessage) {
    attributeMessageProcessorMap.forEach(
        ((attributeRef, mailMessageStringFunction) -> {
          ...
          if (value != null) {
            ...
            LOG.log(
                Level.FINEST,
                "MailMessage failed to match linked attribute:"
                    + attributeRef
                    + ", "
                    + mailMessage);
          }
  ...
  protected abstract Predicate<MailMessage> getAttributeMailMessageFilter(
      String assetId, Attribute<?> attribute, V agentLink);

  protected abstract String getMailMessageAttributeValue(
      String assetId, Attribute<?> attribute, V agentLink, MailMessage mailMessage);
}
