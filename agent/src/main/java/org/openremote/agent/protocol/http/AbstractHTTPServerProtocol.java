...
/**
 * This is an abstract protocol for creating JAX-RS deployments; a concrete implementation should be
 * created for each required JAX-RS deployment. Multiple instances of the same deployment can be
 * created by creating multiple protocol instances for the desired concrete implementation.
 *
 * <p>The deployment is deployed by creating an instance of a concrete {@link
 * AbstractHTTPServerAgent} implementation with the following {@link Attribute}s:
 *
 * <ul>
 *   <li>{@link AbstractHTTPServerAgent#DEPLOYMENT_PATH} <b>(required)</b>
 *   <li>{@link AbstractHTTPServerAgent#ALLOWED_ORIGINS}
 *   <li>{@link AbstractHTTPServerAgent#ALLOWED_HTTP_METHODS}
 * </ul>
 *
 * The path used for the deployment is determined by {@link #getDeploymentPath}. The realm that the
 * {@link Agent} belongs to will be used implicitly for all incoming requests to any deployments
 * that this protocol creates and therefore only users of that realm will be able to make calls to
 * the deployment when {@link AbstractHTTPServerAgent#ROLE_BASED_SECURITY} is true.
 */
public abstract class AbstractHTTPServerProtocol<
        T extends AbstractHTTPServerProtocol<T, U, V>,
        U extends AbstractHTTPServerAgent<U, T, V>,
        V extends AgentLink<?>>
  ...
  public static class DeploymentInstance {
    ...
    public DeploymentInstance(
        DeploymentInfo deploymentInfo, WebService.RequestHandler requestHandler) {
  ...
  /**
   * This is the default path prefix for all deployments. Should not be overridden unless you know
   * what you are doing and there is a good reason to override.
   */
  ...
  private static final Logger LOG =
      SyslogCategory.getLogger(PROTOCOL, AbstractHTTPServerProtocol.class);
  public static final HTTPMethod[] DEFAULT_ALLOWED_METHODS = HTTPMethod.values();
  protected DeploymentInstance deployment;
  protected static WebServiceExceptions.DefaultResteasyExceptionMapper
      defaultResteasyExceptionMapper;
  protected static WebServiceExceptions.ForbiddenResteasyExceptionMapper
  ...
  public void doStart(Container container) throws Exception {
    ...
    identityService =
        container.hasService(IdentityService.class)
  ...
  protected ResteasyDeployment createDeployment(Application application) {
    ...
    if (allowedOrigins != null) {
      String allowedMethods =
          Arrays.stream(agent.getAllowedHTTPMethods().orElse(DEFAULT_ALLOWED_METHODS))
              .map(Enum::name)
    ...
    if (enableSecurity) {
      if (identityService == null) {
        throw new RuntimeException(
            "Role based security can only be enabled when an identity service is available");
      }
    }

    resteasyDeployment.setSecurityEnabled(enableSecurity);

    ServletInfo resteasyServlet =
  ...
  /**
   * Should return instances of all JAX-RS interface implementations that make up this protocol's
   * deployment.
   */
  protected abstract Set<Object> getApiSingletons();

  /**
   * Get the path prefix to use for this protocol instance; should use {@value
   * #DEFAULT_DEPLOYMENT_PATH_PREFIX} unless there is a good reason to override this.
   */
  ...
  /**
   * Deployment path will always be prefixed with {@link #getDeploymentPathPrefix()}; default
   * implementation combines the prefix with the value of {@link
   * AbstractHTTPServerAgent#DEPLOYMENT_PATH}, for example:
   *
   * <ul>
   *   <li>getDeploymentPathPrefix() = {@value #DEFAULT_DEPLOYMENT_PATH_PREFIX}
   *   <li>{@link AbstractHTTPServerAgent#DEPLOYMENT_PATH} = "complaints"
   * </ul>
   *
   * <p>Full path to deployment = "/rest/complaints"
   *
   * <p>If the {@link AbstractHTTPServerAgent#DEPLOYMENT_PATH} is missing or not a String or the
   * generated path does not match the {@link #PATH_REGEX} regex then an {@link
   * IllegalArgumentException} will is thrown.
   */
    ...
    try {
      ...
      HttpHandler handlerWrapper =
          exchange -> {
            exchange
                .getRequestHeaders()
                .put(HttpString.tryFromString(Constants.REALM_PARAM_NAME), agentRealm);
            httpHandler.handleRequest(exchange);
          };
      WebService.RequestHandler requestHandler =
          pathStartsWithHandler(
              deploymentInfo.getDeploymentName(), deploymentInfo.getContextPath(), handlerWrapper);

      LOG.info(
          "Registering HTTP Server Protocol request handler '"
  ...
  protected void undeploy() {
    ...
    try {
      LOG.info(
          "Un-registering HTTP Server Protocol request handler '"
              + this.getClass().getSimpleName()
              + "' for request path: "
              + deployment.deploymentInfo.getContextPath());
      webService.getRequestHandlers().remove(deployment.requestHandler);
      DeploymentManager manager =
          Servlets.defaultContainer().getDeployment(deployment.deploymentInfo.getDeploymentName());
      if (manager != null) {
        manager.stop();
        manager.undeploy();
      }
