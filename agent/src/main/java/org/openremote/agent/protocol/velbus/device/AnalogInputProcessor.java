    ...
    public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
        ...
        if (device.getDeviceType() == VelbusDeviceType.VMBMETEO) {
            return Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                    VelbusPacket.OutboundCommand.SENSOR_READOUT.getCode(), (byte) 0x0F, (byte) 0x00));
        }
        ...
        if (device.getDeviceType() == VelbusDeviceType.VMB4AN) {
            ...
            return Arrays.asList(
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_READOUT.getCode(),
                            (byte) 0x08, (byte) 0x00),
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_READOUT.getCode(),
                            (byte) 0x09, (byte) 0x00),
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_READOUT.getCode(),
                    ...
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_READOUT.getCode(),
                            (byte) 0x0B, (byte) 0x00),
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_SETTINGS.getCode(),
                            (byte) 0x08),
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_SETTINGS.getCode(),
                            (byte) 0x09),
                    new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.SENSOR_SETTINGS.getCode(),
        ...
        switch (packetCommand) {
            ...
            case RAW_SENSOR_STATUS:
                ...
                if (device.getDeviceType() == VelbusDeviceType.VMB4AN) {
                    ...
                    double value = ((packet.getByte(3) & 0xFF) << 16) + ((packet.getByte(4) & 0xFF) << 8)
