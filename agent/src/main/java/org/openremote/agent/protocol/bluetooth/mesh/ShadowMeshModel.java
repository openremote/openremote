...
public abstract class ShadowMeshModel {
  ...
  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, ShadowMeshModel.class.getName());
  ...
  public synchronized void sendSetCommand(Object value) {
    ...
    if (key != null) {
      ...
      LOG.severe(
          "Failed to send mesh model '"
              + getModelName()
              + "' command value '"
              + value
              + "' because couldn't find application key for index '"
              + appKeyIndex
              + "', address: '"
              + element.getAddress());
    }
