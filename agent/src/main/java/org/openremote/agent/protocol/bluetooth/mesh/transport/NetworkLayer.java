...
/**
 * NetworkLayer implementation of the mesh network architecture as per the mesh profile
 * specification.
 *
 * <p>NetworkLayer encrypts/decrypts mesh messages to be sent or received by the nodes in a mesh
 * network.
 *
 * <p>
 */
abstract class NetworkLayer extends LowerTransportLayer {
  ...
  @Override
  public final synchronized Message createNetworkLayerPDU(final Message message) {
    ...
    switch (message.getPduType()) {
        ...
        for (int i = 0; i < lowerTransportPduMap.size(); i++) {
          ...
          if (i != 0) {
            ...
            final byte[] sequenceNumber =
                MeshParserUtils.getSequenceNumberBytes(node.incrementSequenceNumber());
          ...
          final byte[] nonce =
              createNetworkNonce(ctlTTL, sequenceNumbers.get(i), src, message.getIvIndex());
          ...
          LOG.info(
              "Encrypted Network payload: " + MeshParserUtils.bytesToHex(encryptedPayload, false));
        }
        break;
      case MeshManagerApi.PDU_TYPE_PROXY_CONFIGURATION:
        ...
        for (int i = 0; i < lowerTransportPduMap.size(); i++) {
          ...
          sequenceNumbers.add(message.getSequenceNumber());
          final byte[] nonce =
              createProxyNonce(message.getSequenceNumber(), src, message.getIvIndex());
          final byte[] encryptedPayload =
              encryptPdu(
                  lowerTransportPdu,
                  encryptionKey,
    ...
    for (int i = 0; i < encryptedPduPayload.size(); i++) {
      ...
      final byte[] pdu =
          ByteBuffer.allocate(1 + 1 + header.length + encryptedPayload.length)
  ...
  final synchronized Message createRetransmitNetworkLayerPDU(
      final Message message, final int segment) {
    final SecureUtils.K2Output k2Output = getK2Output(message);
    final int nid = k2Output.getNid();
    final byte[] encryptionKey = k2Output.getEncryptionKey();
    LOG.info("Encryption key: " + MeshParserUtils.bytesToHex(encryptionKey, false));
    ...
    if (message.getPduType() == MeshManagerApi.PDU_TYPE_NETWORK) {
      ...
      // node.incrementSequenceNumber();//incrementSequenceNumber(mNetworkLayerCallbacks.getProvisioner(), message.getSequenceNumber());
      final byte[] sequenceNum =
          MeshParserUtils.getSequenceNumberBytes(node.incrementSequenceNumber());
      message.setSequenceNumber(sequenceNum);
      ...
      encryptedNetworkPayload =
          encryptPdu(
              ...
              message.getDst(),
              SecureUtils.getNetMicLength(message.getCtl()));
      if (encryptedNetworkPayload == null) return null;
      LOG.info(
          "Encrypted Network payload: "
              + MeshParserUtils.bytesToHex(encryptedNetworkPayload, false));
    }
    ...
    final byte[] pdu =
        ByteBuffer.allocate(1 + 1 + header.length + encryptedNetworkPayload.length)
            .order(ByteOrder.BIG_ENDIAN)
            .put((byte) pduType)
            .put(iviNID)
            .put(header)
   ...
   *
   * <p>This method will drop messages with an invalid sequence number as all mesh messages are
   * supposed to have a sequence
   *
   ...
   */
  final synchronized Message parseMeshMessage(
    ...
    if (ctl == 1) {
      return parseControlMessage(
          key,
          provisioner.getProvisionerAddress(),
          data,
          networkHeader,
          decryptedNetworkPayload,
          src,
          sequenceNumber);
    } else {
      return parseAccessMessage(
          key, data, networkHeader, decryptedNetworkPayload, src, sequenceNumber, ivIndex);
    }
  }
   ...
   */
  private AccessMessage parseAccessMessage(
      final NetworkKey key,
      final byte[] data,
      final byte[] networkHeader,
      final byte[] decryptedNetworkPayload,
    ...
    try {
      ...
      final int dst =
          MeshParserUtils.unsignedBytesToInt(
              decryptedNetworkPayload[1], decryptedNetworkPayload[0]);
      LOG.info("Dst: " + MeshAddress.formatAddress(dst, true));

      if (isSegmentedMessage(decryptedNetworkPayload[2])) {
        LOG.info(
            "Received a segmented access message from: " + MeshAddress.formatAddress(src, false));
        ...
        if (!mMeshNode.hasUnicastAddress(src)) {
          LOG.info(
              "Segment received is from a different src than the one we are processing, let's drop it");
          return null;
        }
        ...
        final byte[] pdu =
            ByteBuffer.allocate(2 + networkHeader.length + decryptedNetworkPayload.length)
                .order(ByteOrder.BIG_ENDIAN)
                .put(data, 0, 2)
                .put(networkHeader)
        ...
        final int ttl =
            receivedTtl == 0 ? receivedTtl : mNetworkLayerCallbacks.getProvisioner().getGlobalTtl();
        final AccessMessage message =
            parseSegmentedAccessLowerTransportPDU(ttl, pdu, ivIndex, sequenceNumber);
        ...
        final byte[] pdu =
            ByteBuffer.allocate(2 + networkHeader.length + decryptedNetworkPayload.length)
                ...
                .array();
        final AccessMessage message =
            parseUnsegmentedAccessLowerTransportPDU(pdu, ivIndex, sequenceNumber);
      ...
      throw new ExtendedInvalidCipherTextException(
          ex.getMessage(), ex.getCause(), NetworkLayer.class.getSimpleName());
    }
  }
   ...
   */
  private ControlMessage parseControlMessage(
      final NetworkKey key,
      ...
      switch (pduType) {
        case MeshManagerApi.PDU_TYPE_NETWORK:
          ...
          if (isSegmentedMessage(decryptedNetworkPayload[2])) {
            ...
            return parseUnsegmentedControlMessage(
                key, data, decryptedProxyPdu, ttl, src, dst, sequenceNumber);
          }
        case MeshManagerApi.PDU_TYPE_PROXY_CONFIGURATION:
   ...
   * @return a complete {@link ControlMessage} or null if the message was unable to parsed
   */
  private ControlMessage parseUnsegmentedControlMessage(
      final NetworkKey key,
      final byte[] data,
      final byte[] decryptedProxyPdu,
      final int ttl,
   ...
   * @return a complete {@link ControlMessage} or null if the message was unable to parsed
   */
  private ControlMessage parseSegmentedControlMessage(
      final NetworkKey key,
      final byte[] data,
      final byte[] decryptedProxyPdu,
      final int ttl,
   ...
   */
  private byte[] obfuscateNetworkHeader(
      final byte ctlTTL, final byte[] sequenceNumber, final int src, final byte[] pecb) {

    final ByteBuffer buffer =
        ByteBuffer.allocate(1 + sequenceNumber.length + 2).order(ByteOrder.BIG_ENDIAN);
    buffer.put(ctlTTL);
    ...
    for (int i = 0; i < 6; i++) obfuscated[i] = (byte) (headerBuffer[i] ^ pecb[i]);
   ...
   * @return Obfuscated network header
   */
  static byte[] deObfuscateNetworkHeader(
      final byte[] pdu, final byte[] ivIndex, final byte[] privacyKey) {
    ...
    for (int i = 0; i < 6; i++) deObfuscatedData[i] = (byte) (obfuscatedData[i] ^ pecb[i]);
  ...
  private static byte[] createPECB(
      final byte[] ivIndex, final byte[] privacyRandom, final byte[] privacyKey) {
   ...
   */
  static byte[] createNetworkNonce(
      final byte ctlTTL, final byte[] sequenceNumber, final int src, final byte[] ivIndex) {
   ...
   */
  private byte[] encryptPdu(
      final byte[] lowerTransportPdu,
      final byte[] encryptionKey,
      final byte[] nonce,
      final int dst,
      final int micLength) {
    // Adding the destination address on network layer
    final byte[] unencryptedNetworkPayload =
        ByteBuffer.allocate(2 + lowerTransportPdu.length)
            .order(ByteOrder.BIG_ENDIAN)
            .putShort((short) dst)
            .put(lowerTransportPdu)
            .array();
