...
public class CounterProcessor extends FeatureProcessor {
  ...
  public static final List<PropertyDescriptor> SUPPORTED_PROPERTIES =
      Arrays.asList(
          new PropertyDescriptor(
              "counter1Enabled", "Counter 1 Enabled", "COUNTER1_ENABLED", ValueType.BOOLEAN, true),
          new PropertyDescriptor(
              "counter2Enabled", "Counter 2 Enabled", "COUNTER2_ENABLED", ValueType.BOOLEAN, true),
          new PropertyDescriptor(
              "counter3Enabled", "Counter 3 Enabled", "COUNTER3_ENABLED", ValueType.BOOLEAN, true),
          new PropertyDescriptor(
              "counter4Enabled", "Counter 4 Enabled", "COUNTER4_ENABLED", ValueType.BOOLEAN, true),
          new PropertyDescriptor(
              "counter1Units", "Counter 1 Units", "COUNTER1_UNITS", ValueType.TEXT, true),
          new PropertyDescriptor(
              "counter2Units", "Counter 2 Units", "COUNTER2_UNITS", ValueType.TEXT, true),
          new PropertyDescriptor(
              "counter3Units", "Counter 3 Units", "COUNTER3_UNITS", ValueType.TEXT, true),
          new PropertyDescriptor(
              "counter4Units", "Counter 4 Units", "COUNTER4_UNITS", ValueType.TEXT, true),
          new PropertyDescriptor(
              "counter1Instant", "Counter 1 Instant", "COUNTER1_INSTANT", ValueType.NUMBER, true),
          new PropertyDescriptor(
              "counter2Instant", "Counter 2 Instant", "COUNTER2_INSTANT", ValueType.NUMBER, true),
          new PropertyDescriptor(
              "counter3Instant", "Counter 3 Instant", "COUNTER3_INSTANT", ValueType.NUMBER, true),
          new PropertyDescriptor(
              "counter4Instant", "Counter 4 Instant", "COUNTER4_INSTANT", ValueType.NUMBER, true),
  ...
  public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
    ...
    return Arrays.asList(
        new VelbusPacket(
            ...
            VelbusPacket.OutboundCommand.COUNTER_STATUS.getCode(),
            (byte) 0x0F,
            (byte) 0x00),
        new VelbusPacket(
            device.getBaseAddress(),
            VelbusPacket.OutboundCommand.READ_MEMORY.getCode(),
            (byte) 0x03,
            (byte) 0xFE));
  }
  ...
  public List<VelbusPacket> getPropertyWritePackets(
    ...
    return ValueUtil.getValueCoerced(value, AttributeExecuteStatus.class)
        .map(
            attributeExecuteStatus -> {
              ...
              if (counter != null) {
                ...
                return Collections.singletonList(
                    new VelbusPacket(
                        device.getBaseAddress(),
                        VelbusPacket.OutboundCommand.COUNTER_RESET.getCode(),
                        (byte) (counter - 1)));
              }
  ...
  public boolean processReceivedPacket(VelbusDevice device, VelbusPacket packet) {
    ...
    switch (packetCommand) {
      case COUNTER_STATUS:
        ...
        int counter =
            ((packet.getByte(2) & 0xFF) << 24)
                + ((packet.getByte(3) & 0xFF) << 16)
                + ((packet.getByte(4) & 0xFF) << 8)
                + ((packet.getByte(5) & 0xFF));
        int period = ((packet.getByte(6) & 0xFF) << 8) + ((packet.getByte(7) & 0xFF));

        CounterUnits units =
            (CounterUnits) device.getPropertyValue("COUNTER" + channelNumber + "_UNITS");
      ...
      case MEMORY_DATA:
        if ((packet.getByte(1) & 0xFF) == 0x03 && (packet.getByte(2) & 0xFF) == 0xFE) {
          ...
          counters[0] =
              Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER1_ENABLED"))
                      .orElse(false)
                  ? CounterUnits.fromCode(counterUnits & 0x03)
                  : null;
          counters[1] =
              Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER2_ENABLED"))
                      .orElse(false)
                  ? CounterUnits.fromCode((counterUnits & 0x0C) >> 2)
                  : null;
          counters[2] =
              Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER3_ENABLED"))
                      .orElse(false)
                  ? CounterUnits.fromCode((counterUnits & 0x30) >> 4)
                  : null;
          counters[3] =
              Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER4_ENABLED"))
                      .orElse(false)
                  ? CounterUnits.fromCode((counterUnits & 0xC0) >> 6)
                  : null;
          ...
          for (int i = 0; i < 4; i++) {
            if (counters[i] == CounterUnits.KILOWATTS) {
              double val =
                  Optional.ofNullable(
                          (Double) device.getPropertyValue("COUNTER" + (i + 1) + "_INSTANT"))
                      .orElse(0d);
              val = val * 1000;
