...
public class CounterProcessor extends FeatureProcessor {
    ...
    public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
        ...
        return Arrays.asList(
                new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.COUNTER_STATUS.getCode(),
                        (byte) 0x0F, (byte) 0x00),
                new VelbusPacket(device.getBaseAddress(), VelbusPacket.OutboundCommand.READ_MEMORY.getCode(),
                        (byte) 0x03, (byte) 0xFE));
    }
        ...
        return ValueUtil.getValueCoerced(value, AttributeExecuteStatus.class).map(attributeExecuteStatus -> {
            ...
            if (counter != null) {
                ...
                return Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                        VelbusPacket.OutboundCommand.COUNTER_RESET.getCode(), (byte) (counter - 1)));
            }
    ...
    public boolean processReceivedPacket(VelbusDevice device, VelbusPacket packet) {
        ...
        switch (packetCommand) {
            case COUNTER_STATUS:
                ...
                int counter = ((packet.getByte(2) & 0xFF) << 24) + ((packet.getByte(3) & 0xFF) << 16)
                        + ((packet.getByte(4) & 0xFF) << 8) + ((packet.getByte(5) & 0xFF));
            ...
            case MEMORY_DATA:
                if ((packet.getByte(1) & 0xFF) == 0x03 && (packet.getByte(2) & 0xFF) == 0xFE) {
                    ...
                    counters[0] = Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER1_ENABLED"))
                            .orElse(false) ? CounterUnits.fromCode(counterUnits & 0x03) : null;
                    counters[1] = Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER2_ENABLED"))
                            .orElse(false) ? CounterUnits.fromCode((counterUnits & 0x0C) >> 2) : null;
                    counters[2] = Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER3_ENABLED"))
                            .orElse(false) ? CounterUnits.fromCode((counterUnits & 0x30) >> 4) : null;
                    counters[3] = Optional.ofNullable((Boolean) device.getPropertyValue("COUNTER4_ENABLED"))
                            .orElse(false) ? CounterUnits.fromCode((counterUnits & 0xC0) >> 6) : null;
                    ...
                    for (int i = 0; i < 4; i++) {
                        if (counters[i] == CounterUnits.KILOWATTS) {
                            double val = Optional
                                    .ofNullable((Double) device.getPropertyValue("COUNTER" + (i + 1) + "_INSTANT"))
                                    .orElse(0d);
                            val = val * 1000;
