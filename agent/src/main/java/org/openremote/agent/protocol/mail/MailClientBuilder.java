...
public class MailClientBuilder {
    ...
    public UsernamePassword getAuth() throws NullPointerException, ExecutionException, InterruptedException, TimeoutException {
        Objects.requireNonNull(user, "User must be supplied");

        if (oAuthGrant != null) {
            String bearerToken = WebService.getBearerToken(executorService, getClient(), oAuthGrant).get(10000, TimeUnit.MILLISECONDS);
