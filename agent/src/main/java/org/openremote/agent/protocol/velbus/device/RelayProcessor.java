...
public class RelayProcessor extends OutputChannelProcessor {
    ...
    protected final static List<PropertyDescriptor> SUPPORTED_PROPERTIES = Arrays.asList(
            ...
            new PropertyDescriptor("ch1IntermittentSeconds", "CH1 Intermittent (s)", "CH1_INTERMITTENT",
                    ValueType.NUMBER),
            new PropertyDescriptor("ch2IntermittentSeconds", "CH2 Intermittent (s)", "CH2_INTERMITTENT",
                    ValueType.NUMBER),
            new PropertyDescriptor("ch3IntermittentSeconds", "CH3 Intermittent (s)", "CH3_INTERMITTENT",
                    ValueType.NUMBER),
            new PropertyDescriptor("ch4IntermittentSeconds", "CH4 Intermittent (s)", "CH4_INTERMITTENT",
                    ValueType.NUMBER),
            new PropertyDescriptor("ch5IntermittentSeconds", "CH5 Intermittent (s)", "CH5_INTERMITTENT",
                    ValueType.NUMBER),
    ...
    public List<VelbusPacket> getStatusRequestPackets(VelbusDevice device) {
        return Collections.singletonList(new VelbusPacket(device.getBaseAddress(),
                VelbusPacket.OutboundCommand.MODULE_STATUS.getCode(), (byte) 0x1F));
    }
                    ...
                    switch (channelNumberAndPropertySuffix.value) {
                        ...
                        case "_SETTING":
                            ...
                            switch (setting.get()) {
                                case NORMAL:
                                    return Stream
                                            .of(getPackets(device, channelNumber, INHIBIT_CANCEL, 0xFFFFFF),
    ...
    public boolean processReceivedPacket(VelbusDevice device, VelbusPacket packet) {
        ...
        switch (packetCommand) {
            case RELAY_STATUS:
                ...
                ChannelSetting setting = device.getDeviceType() == VelbusDeviceType.VMB1RY ? ChannelSetting.NORMAL
                        : ChannelSetting.fromCode(packet.getByte(2) & 0xFF);
                ChannelState state = (packet.getByte(3) & 0xFF) == 0x11 ? ChannelState.INTERMITTENT
                        : ChannelState.fromCode(packet.getByte(3) & 0xFF);
                LedState ledStatus = device.getDeviceType() == VelbusDeviceType.VMB1RY ? null
    ...
    protected static List<VelbusPacket> getPackets(VelbusDevice velbusDevice, int channelNumber,
            VelbusPacket.OutboundCommand command, int durationSeconds) {
