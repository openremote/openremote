...
public class NetworkTransmitSettings {
   ...
   *
   * @param networkTransmitCount Number of transmissions for each Network PDU originating from the
   *     node
  ...
  public static int decodeNetworkTransmissionInterval(final int interval) {
    if ((interval >= 10 && interval <= 320) && (interval % 10 != 0))
      throw new IllegalArgumentException(
          "Network Transmission Interval must be 10-320 ms with a step of 10 ms");
