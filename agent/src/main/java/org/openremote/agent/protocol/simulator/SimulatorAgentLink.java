...
public class SimulatorAgentLink extends AgentLink<SimulatorAgentLink> {
  ...
  @JsonSchemaDescription(
      "Used to store a dataset of values that should be replayed (i.e. written to the"
  ...
  @JsonSchemaDescription(
      "Overwrites the possible dataset length and when it is replayed."
  ...
  public SimulatorAgentLink(
    ...
    if (schedule == null) {
      ...
      LocalDateTime end =
          getReplayData()
              .flatMap(this::getLastReplayDatapointSeconds)
              .map(start::plusSeconds)
              .orElse(start.plusDays(1));
   ...
   * @return The last (max) {@link SimulatorReplayDatapoint#timestamp} seconds. If the resolved time
   *     is 0 an empty value is returned instead.
   */
