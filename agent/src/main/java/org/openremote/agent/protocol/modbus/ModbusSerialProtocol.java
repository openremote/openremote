...
/**
 * Modbus Serial (RTU) protocol implementation. Handles Serial-specific transport: half-duplex
 * communication with timing gaps.
 */
public class ModbusSerialProtocol
    extends AbstractModbusProtocol<ModbusSerialProtocol, ModbusSerialAgent, ModbusSerialFrame> {

  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, ModbusSerialProtocol.class);
  ...
  protected NettyIOClient<ModbusSerialFrame> doCreateIoClient() throws Exception {
    String portName =
        agent.getSerialPort().orElseThrow(() -> new RuntimeException("Serial port not specified"));
  ...
  protected void onMessageReceived(ModbusSerialFrame frame) {
    LOG.finest(
        () ->
            "Received frame - UnitID: "
                + frame.getUnitId()
                + ", FC: 0x"
                + Integer.toHexString(frame.getFunctionCode() & 0xFF));
  ...
  @Override
  public ModbusSerialFrame sendModbusRequest(int unitId, byte[] pdu, long timeoutMs)
      throws Exception {
    ...
    synchronized (sendLock) {
      ...
      try {
        ...
        LOG.finest(
            () ->
                "Sent Modbus Serial request - UnitID: "
                    + unitId
                    + ", FC: 0x"
                    + Integer.toHexString(pdu[0] & 0xFF));
        ...
        LOG.finest(
            () ->
                "Received Modbus Serial response - UnitID: "
                    + response.getUnitId()
                    + ", FC: 0x"
                    + Integer.toHexString(response.getFunctionCode() & 0xFF));
        return response;
