...
public abstract class AbstractMailAgent<T extends AbstractMailAgent<T, U, V>, U extends AbstractMailProtocol<T, U, V>, V extends AgentLink<V>>
        extends Agent<T, U, V> {

    @NotNull public static final AttributeDescriptor<String> PROTOCOL = new AttributeDescriptor<>("protocol", ValueType.TEXT);
    @NotNull public static final AttributeDescriptor<UsernamePassword> USERNAME_AND_PASSWORD = Agent.USERNAME_AND_PASSWORD
            .withOptional(false);
    ...
    public static final AttributeDescriptor<Integer> CHECK_INTERVAL_SECONDS = new AttributeDescriptor<>(
            "checkIntervalSeconds", ValueType.POSITIVE_INTEGER).withOptional(true);
    public static final AttributeDescriptor<Boolean> DELETE_PROCESSED_MAIL = new AttributeDescriptor<>(
            "deleteProcessedMail", ValueType.BOOLEAN).withOptional(true);
    public static final AttributeDescriptor<Boolean> PREFER_HTML = new AttributeDescriptor<>("preferHTML",
            ValueType.BOOLEAN).withOptional(true);
    public static final AttributeDescriptor<String> MAIL_FOLDER_NAME = new AttributeDescriptor<>("mailFolderName",
            ValueType.TEXT).withOptional(true);
    public static final AttributeDescriptor<Boolean> START_TLS = new AttributeDescriptor<>("startTLS",
            ValueType.BOOLEAN).withOptional(true);
