...
abstract class ProvisionedBaseMeshNode {
  ...
  public final void setNodeName(final String nodeName) {
    if (nodeName != null && nodeName.length() > 0) this.nodeName = nodeName;
  }
   ...
   *
   * <p>This is to be used only by the library
   */
  ...
  /**
   * Secure Network Beacon state determines if a node is periodically broadcasting Secure Network
   * Beacons.
   */
  /*
  @Retention(RetentionPolicy.SOURCE)
  ...
  /**
   * The Node Identity state determines if a node is advertising with Node Identity messages on a
   * subnet. If the Mesh Proxy Service is exposed, the node can be configured to advertise with Node
   * Identity on a subnet.
   */
  /*
  @Retention(RetentionPolicy.SOURCE)
