...
public class SequenceNumberPersistencyManager {
  ...
  public static final Logger LOG =
      SyslogCategory.getLogger(
          SyslogCategory.PROTOCOL, SequenceNumberPersistencyManager.class.getName());
  ...
  public synchronized void save(NetworkKey networkKey, int unicastAddress, int sequenceNumber) {
    ...
    LOG.info(
        "Save sequence number: [sourceAddress="
            + address
            + ", sequenceNumber="
            + sequenceNumber
            + "]");
  ...
  protected synchronized void save() {
    ...
    try {
      ...
      LOG.severe(
          "Failed to save sequence number file: '"
              + file.getPath()
              + "' because: "
              + e.getMessage());
    } finally {
      if (writer != null) {
        try {
          ...
          LOG.severe(
              "Failed to close sequence number file: '"
                  + file.getPath()
                  + "' because: "
                  + e.getMessage());
        }
  ...
  public synchronized void load() {
    ...
    for (Element itemElement : itemElements) {
      ...
      try {
        ...
        LOG.log(
            Level.SEVERE, "Error while loading sequence number file: '" + file.getPath() + "'", e);
        e.printStackTrace();
      }
    }
  }
