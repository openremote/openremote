...
class MeshProvisioningHandler implements InternalProvisioningCallbacks {
   ...
   *
   * <p>This will handle the provisioning process through each step
   *
  ...
  void handleProvisioningWriteCallbacks() {
    ...
    switch (provisioningState.getState()) {
      case PROVISIONING_INVITE:
        provisioningState =
            new ProvisioningCapabilitiesState(unprovisionedMeshNode, mStatusCallbacks);
        break;
      case PROVISIONING_CAPABILITIES:
        break;
      case PROVISIONING_START:
  ...
  private void parseProvisioningState(
    ...
    if (data[1] == ProvisioningState.State.PROVISIONING_COMPLETE.getState()) {
      ...
      // Generate the network id and store it in the mesh node, this is needed to reconnect to the
      // device at a later stage.
      final ProvisionedMeshNode provisionedMeshNode =
          new ProvisionedMeshNode(unprovisionedMeshNode);
      mInternalMeshManagerCallbacks.onNodeProvisioned(provisionedMeshNode);
      mStatusCallbacks.onProvisioningCompleted(
          provisionedMeshNode, ProvisioningState.States.PROVISIONING_COMPLETE, data);
    } else {
      ...
      if (provisioningFailedState.parseData(data)) {
        mStatusCallbacks.onProvisioningFailed(
            unprovisionedMeshNode, ProvisioningState.States.PROVISIONING_FAILED, data);
      }
    }
  }
  ...
  private boolean validateProvisioningDataInput(
      final NetworkKey networkKey, final Integer flags, final Integer ivIndex) {
    String error;
   ...
   *
   * <p>This method will send a provisioning invite to the connected peripheral. This will help
   * users to identify a particular node before starting the provisioning process. This method must
   * be invoked before calling {@link #startProvisioningNoOOB(UnprovisionedMeshNode)} </p
   *
   * @param uuid Device UUID of unprovisioned node
   * @param networkKey Network key
   * @param flags Flag containing the key refresh or the iv update operations
   ...
   *
   * <p>This method will continue the provisioning process that was started by invoking {@link
   * #identify(UUID, NetworkKey, int, int, int, int)}.
   *
   * @param unprovisionedMeshNode {@link UnprovisionedMeshNode}
   */
  void startProvisioningNoOOB(final UnprovisionedMeshNode unprovisionedMeshNode)
      throws IllegalArgumentException {
  ...
  private void sendProvisioningInvite(final UnprovisionedMeshNode unprovisionedMeshNode) {
    isProvisioningPublicKeySent = false;
    isProvisioneePublicKeyReceived = false;
    final ProvisioningInviteState invite =
        new ProvisioningInviteState(
            unprovisionedMeshNode, attentionTimer, mInternalTransportCallbacks, mStatusCallbacks);
  ...
  private boolean parseProvisioningCapabilitiesMessage(
    ...
    final ProvisioningCapabilitiesState provisioningCapabilitiesState =
        new ProvisioningCapabilitiesState(unprovisionedMeshNode, mStatusCallbacks);
  ...
  private void sendProvisioningStart(final UnprovisionedMeshNode unprovisionedMeshNode) {
    final ProvisioningCapabilitiesState capabilitiesState =
        (ProvisioningCapabilitiesState) provisioningState;
    final ProvisioningCapabilities capabilities = capabilitiesState.getCapabilities();
    final ProvisioningStartState startProvisioning =
        new ProvisioningStartState(
            unprovisionedMeshNode, mInternalTransportCallbacks, mStatusCallbacks);
    startProvisioning.setProvisioningCapabilities(
        capabilities.getNumberOfElements(),
  ...
  private void sendProvisionerPublicKey(final UnprovisionedMeshNode unprovisionedMeshNode) {
    if (!isProvisioningPublicKeySent) {
      if (provisioningState instanceof ProvisioningPublicKeyState) {
        ...
        final ProvisioningPublicKeyState provisioningPublicKeyState =
            new ProvisioningPublicKeyState(
                unprovisionedMeshNode, mInternalTransportCallbacks, mStatusCallbacks);
  ...
  private void parseProvisioneePublicKeyXY(
    ...
    if (provisioningState instanceof ProvisioningPublicKeyState) {
      final ProvisioningPublicKeyState provisioningPublicKeyState =
          ((ProvisioningPublicKeyState) provisioningState);
      ...
      if (isProvisioningPublicKeySent && isProvisioneePublicKeyReceived) {
        switch (unprovisionedMeshNode.getAuthMethodUsed()) {
          case STATIC_OOB_AUTHENTICATION:
            provisioningState =
                new ProvisioningConfirmationState(
          ...
          case INPUT_OOB_AUTHENTICATION:
            provisioningState =
                new ProvisioningInputCompleteState(
                    unprovisionedMeshNode, mInternalTransportCallbacks, mStatusCallbacks);
            mStatusCallbacks.onProvisioningStateChanged(
                mUnprovisionedMeshNode,
   ...
   *
   * @param authentication authentication value input by the user this may be nullable depending on
   *     the OOB type selected by the user
   */
  void sendProvisioningConfirmation(/* @Nullable */ final String authentication) {
    ...
    // Check if the current provisioning state, if the user had selected InputOOBAction the state
    // will be ProvisioningInputCompleteState
    if (provisioningState instanceof ProvisioningInputCompleteState) {
      provisioningConfirmationState =
          new ProvisioningConfirmationState(
              this, mUnprovisionedMeshNode, mInternalTransportCallbacks, mStatusCallbacks);
  ...
  private boolean parseProvisioningInputCompleteState(final byte[] data) {
    final ProvisioningInputCompleteState inputCompleteState =
        (ProvisioningInputCompleteState) provisioningState;
    return inputCompleteState.parseData(data);
  }

  private boolean parseProvisioneeConfirmation(final byte[] data) {
    final ProvisioningConfirmationState provisioningConfirmationState =
        (ProvisioningConfirmationState) provisioningState;
  ...
  private void sendRandomConfirmationPDU(final UnprovisionedMeshNode unprovisionedMeshNode) {
    final ProvisioningRandomConfirmationState provisioningRandomConfirmation =
        new ProvisioningRandomConfirmationState(
            this, unprovisionedMeshNode, mInternalTransportCallbacks, mStatusCallbacks);
  ...
  private boolean parseProvisioneeRandom(final byte[] data) {
    final ProvisioningRandomConfirmationState provisioningRandomConfirmation =
        (ProvisioningRandomConfirmationState) provisioningState;
  ...
  private void sendProvisioningData(final UnprovisionedMeshNode unprovisionedMeshNode) {
    final ProvisioningDataState provisioningDataState =
        new ProvisioningDataState(
            this, unprovisionedMeshNode, mInternalTransportCallbacks, mStatusCallbacks);
  ...
  public final byte[] generateConfirmationInputs(
      final byte[] provisionerKeyXY, final byte[] provisioneeKeyXY) {
    // invite: 1 bytes, capabilities: 11 bytes, start: 5 bytes, provisionerKey: 64 bytes, deviceKey:
    // 64 bytes
    ...
    final ByteBuffer inviteBuffer =
        ByteBuffer.allocate(inviteLength)
            .put(mUnprovisionedMeshNode.getProvisioningInvitePdu(), offset, inviteLength);
    final byte[] invite = inviteBuffer.array();

    // We must remove the first two bytes which is the pdu type and the provisioning pdu type
    final int capabilitiesLength =
        mUnprovisionedMeshNode.getProvisioningCapabilitiesPdu().length - offset;
    final ByteBuffer capabilitiesBuffer =
        ByteBuffer.allocate(capabilitiesLength)
            .put(
                mUnprovisionedMeshNode.getProvisioningCapabilitiesPdu(),
                offset,
                capabilitiesLength);
    ...
    final ByteBuffer startDataBuffer =
        ByteBuffer.allocate(startDataLength)
