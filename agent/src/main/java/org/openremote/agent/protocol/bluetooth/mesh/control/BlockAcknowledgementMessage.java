...
public class BlockAcknowledgementMessage extends TransportControlMessage {
   ...
   *
   * <p>This method will set the segO bit to 1
   *
   ...
   *
   * <p>This method will iterate though the block acknowledgement to find out which segments needs
   * to be retransmitted.
   *
   * @param blockAcknowledgement acknowledgement payload received
   * @param segmentCount number of segments
   */
  public static List<Integer> getSegmentsToBeRetransmitted(
      final byte[] blockAcknowledgement, final int segmentCount) {
   ...
   * @param segN number of segments
   */
  public static boolean hasAllSegmentsBeenReceived(
      final Integer blockAcknowledgement, final int segN) {
    if (blockAcknowledgement == null) return false;
    ...
    return setBitCount
        == segN
            + 1; // Since segN is 0 based add 1 as the bit count represents the number of segments
  }
}
