 ...
 *
 * <p>It uses the netty component for managing the connection.
 *
 * <p>Concrete implementations are responsible for providing the {@link ChannelOutboundHandler}s and
 * {@link ChannelInboundHandler}s required to encode/decode the specific type of messages
 * sent/received to/from this client. For {@link IOClient}s that require some specific
 * encoders/decoders irrespective of the message type (e.g. {@link UDPIOClient} and {@link
 * WebsocketIOClient}) the {@link #addEncodersDecoders} method can be overridden so the {@link
 * IOClient} can exactly control the order and types of encoders/decoders in the pipeline.
 *
 * <p>Users of the {@link IOClient} can add encoders/decoders for their specific message type using
 * the {@link #setEncoderDecoderProvider}, each {@link IOClient} should make it clear to users what
 * the required output type(s) are for the last encoder and decoder that a user may wish to add, if
 * adding encoders/decoders is not supported then {@link IOClient}s should override this setter and
 * throw an {@link UnsupportedOperationException}.
 *
 * <p>Typically for outgoing messages a single {@link ChannelOutboundHandler} is sufficient and the
 * {@link MessageToByteEncoder} can be used as a base.
 *
 * <p>For inbound messages; the decoders required are very much dependent on the message type and
 * {@link IOClient} type, any number of standard netty {@link ChannelInboundHandler}s can be used
 * but the last handler should build messages of type &lt;T&gt; and pass them to the {@link
 * #onMessageReceived} method of the client; the {@link ByteToMessageDecoder} or {@link
 * MessageToByteEncoder} can be used for this purpose, which one to use will depend on the previous
 * {@link ChannelInboundHandler}s in the pipeline.
 *
 * <p><b>NOTE: Care must be taken when working with Netty {@link ByteBuf} as Netty uses reference
 * counting to manage their lifecycle. Refer to the Netty documentation for more information.</b>
 */
public abstract class AbstractNettyIOClient<T> implements NettyIOClient<T> {
   ...
   * {@link ByteBuf}; the provided {@link #decoder} should extract the messages of type &lt;T&gt;
   * from the {@link ByteBuf} and add them to the {@link List} and they will then be passed to the
   * {@link IOClient}.
   */
  public static class ByteToMessageDecoder<T> extends io.netty.handler.codec.ByteToMessageDecoder {
    ...
    public ByteToMessageDecoder(
        NettyIOClient<T> client, @NotNull BiConsumer<ByteBuf, List<T>> decoder) {
      this.client = client;
  ...
  /**
   * This is intended to be used at the end of a decoder chain where the previous decoder outputs
   * messages of type &lt;T&gt;.
   */
  ...
  /**
   * Concrete implementations must provide an encoder to fill the {@link ByteBuf} ready to be sent
   * `over the wire`.
   */
  public static class MessageToByteEncoder<T>
    ...
    public MessageToByteEncoder(
        ...
        AbstractNettyIOClient<T> client,
  ...
  protected final List<Consumer<ConnectionStatus>> connectionStatusConsumers =
  ...
  /**
   * If set the supplier will be called to provide a future that is chained to the standard {@link
   * #startChannel()} future to add implementation-specific initialization logic that if fails will
   * still trigger the retry logic with backoff. Just using the {@link #connectionStatus} and then
   * calling disconnect won't have the same effect as the retry will not use the backoff logic.
   */
  ...
  /**
   * Start the actual connection and return a future indicating completion state. Implementors can
   * also add any custom connection logic they require.
   */
  ...
  /**
   * This just adds a timeout to the connect future (just in case one wasn't added by {@link
   * #startChannel} implementation
   */
  ...
  protected CompletableFuture<Void> doConnect() {
    LOG.info(
        "Establishing connection with timeout of "
  ...
  /**
   * Inserts the decoders and encoders into the channel pipeline and configures standard exception
        ...
        .addListener(
            closedFuture -> {
              ...
              if (!closedFuture.isSuccess() && closedFuture.cause() != null) {
                LOG.info(
                    "Connection closed with exception on '"
                        + getClientUri()
                        + "': "
            ...
            new ChannelInboundHandlerAdapter() {
              ...
              public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause)
                ...
                if (cause instanceof DecoderException decoderException) {
                  ...
                  // Aggressively force close the channel on other exceptions which will cause a
                  // reconnect
                  ctx.close();
                }
              }
  ...
  protected void onDecodeException(ChannelHandlerContext ctx, DecoderException decoderException) {
    LOG.log(
        Level.FINE,
        "Decoder exception occurred on in-bound message '"
            + decoderException.getMessage()
            + "': "
            + getClientUri(),
        decoderException);
  }
  ...
  protected void onEncodeException(ChannelHandlerContext ctx, EncoderException encoderException) {
    LOG.log(
        Level.FINE,
        "Encoder exception occurred on out-bound message '"
            + encoderException.getMessage()
            + "': "
            + getClientUri(),
        encoderException);
  }
    ...
    connectionStatusConsumers.forEach(
        consumer -> {
          try {
            ...
            LOG.log(
                Level.INFO,
                "Connection status change handler threw an exception: " + getClientUri(),
                e);
          }
        });
  }
  ...
  public static CompletableFuture<Void> toCompletableFuture(Future<Void> future)
            ...
            cf -> {
              ...
              if (cf.cause() != null) {
                ...
                completableFuture.completeExceptionally(
                    new RuntimeException("Unknown connection failure occurred"));
              } else {
    ...
    throw new UnsupportedOperationException(
        "Future must be a ChannelFuture or already a CompletableFuture");
  }
}
