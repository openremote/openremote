...
public abstract class AbstractNettyIOClient<T, U extends SocketAddress>
  ...
  public static class ByteToMessageDecoder<T> extends io.netty.handler.codec.ByteToMessageDecoder {
    ...
    public ByteToMessageDecoder(
        AbstractNettyIOClient<T, ?> client, @NotNull BiConsumer<ByteBuf, List<T>> decoder) {
      this.client = client;
  ...
  public static class MessageToMessageDecoder<T> extends SimpleChannelInboundHandler<T> {
    ...
    public MessageToMessageDecoder(
        Class<? extends T> typeClazz, AbstractNettyIOClient<T, ?> client) {
      super(typeClazz);
      this.client = client;
    }
                ...
                (execution) -> {
                  LOG.info(
                      "Connection attempt failed '"
                          ...
                          + getClientUri()
                          + ", error="
                          + (execution.getLastException() != null
                              ? execution.getLastException().getMessage()
                              : null));
                  doDisconnect();
                })
                ...
                (execution) -> {
                  LOG.fine(
                      "Connection attempt '"
                          ...
                          + "' for: "
                          + getClientUri());
                  // Connection future should timeout so we just wait for it but add additional
  ...
  protected void onConnectionStatusChanged(ConnectionStatus connectionStatus) {
    ...
    if (!connectionStatusConsumers.isEmpty()) {
      LOG.finest(
          "Notifying connection status consumers: count=" + connectionStatusConsumers.size());
    }
