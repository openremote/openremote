...
public abstract class AbstractModbusProtocol<
        S extends AbstractModbusProtocol<S, T>, T extends ModbusAgent<T, S>>
  ...
  protected void doStart(Container container) throws Exception {
    ...
    try {
      ...
      setConnectionStatus(
          client.isConnected() ? ConnectionStatus.CONNECTED : ConnectionStatus.ERROR);
    } catch (Exception e) {
  ...
  protected void doLinkedAttributeWrite(
    ...
    switch (agentLink.getWriteMemoryArea()) {
      ...
      case HOLDING ->
          builder.addTagAddress(
              "holdingRegisters", "holding-register:" + writeAddress, processedValue);
      default ->
          throw new IllegalStateException(
              "No other memory area is allowed to write to a Modbus server");
    }
    ...
    try {
      ...
      if (response.getResponseCode(response.getTagNames().stream().findFirst().orElseThrow())
          != PlcResponseCode.OK) {
  ...
  protected ScheduledFuture<?> scheduleModbusPollingReadRequest(
    ...
    int readAmountOfRegisters =
        (amountOfRegisters.isEmpty() || amountOfRegisters.get() < 1)
            ? dataType.getRegisterCount()
            : amountOfRegisters.get();
    String amountOfRegistersString =
        readAmountOfRegisters <= 1 ? "" : "[" + readAmountOfRegisters + "]";

    int address =
        readAddress.orElseThrow(
            () -> new RuntimeException("Read Address is empty! Unable to schedule read request."));

    switch (readType) {
      case COIL ->
          builder.addTagAddress(
              "coils", "coil:" + address + ":" + dataType + amountOfRegistersString);
      case DISCRETE ->
          builder.addTagAddress(
              "discreteInputs",
              "discrete-input:" + address + ":" + dataType + amountOfRegistersString);
      case HOLDING ->
          builder.addTagAddress(
              "holdingRegisters",
              "holding-register:" + address + ":" + dataType + amountOfRegistersString);
      case INPUT ->
          builder.addTagAddress(
              "inputRegisters",
              "input-register:" + address + ":" + dataType + amountOfRegistersString);
    ...
    LOG.log(
        Level.FINE,
        "Scheduling Modbus Read Value polling request '"
            + "clientRequest"
            + "' to execute every "
            + pollingMillis
            + "ms for attributeRef: "
    ...
    return scheduledExecutorService.scheduleWithFixedDelay(
        () -> {
          try {
            ...
            // We currently only request one thing (with the above tag), so we get it from there. If
            // it doesn't exist,
                    ...
                    .orElseThrow(
                        () ->
                            new RuntimeException(
                                "Could not retrieve the requested value from the response"));
            ...
            LOG.log(
                Level.FINE,
                "Exception thrown during Modbus Read Value polling request '"
