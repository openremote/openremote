...
/**
 * Abstract base class for Modbus protocol implementations (TCP and Serial/RTU). Manages the IO
 * client lifecycle and contains shared Modbus logic for batching, polling, data conversion, and PDU
 * building.
 *
 ...
 */
public abstract class AbstractModbusProtocol<
        T extends AbstractModbusProtocol<T, U, F>,
        U extends ModbusAgent<U, T>,
        F extends ModbusFrame>
    extends AbstractProtocol<U, ModbusAgentLink> {

  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, AbstractModbusProtocol.class);
  ...
  /**
   * Send a Modbus request and wait for the response. This is transport-specific (TCP uses
   * transaction IDs, Serial uses timing).
   */
  ...
  protected void doStart(Container container) throws Exception {
    ...
    if (deviceConfigOpt.isEmpty() || deviceConfigOpt.get().isEmpty()) {
      LOG.info(
          getProtocolName()
              + " - Initializing deviceConfig with default configuration for agent: "
              + agent.getName());
      ...
      sendAttributeEvent(
          new AttributeEvent(agent.getId(), ModbusAgent.DEVICE_CONFIG, defaultConfig));
    }
  }
  ...
  protected void doLinkAttribute(
      String assetId, Attribute<?> attribute, ModbusAgentLink agentLink) {
    ...
    boolean hasReadConfig =
        agentLink.getReadMemoryArea() != null
    ...
    if (hasReadConfig) {
      if (agentLink.getRequestInterval() != null) {
        ...
        if (!batchReadIntervalTasks.containsKey(groupKey)) {
          ScheduledFuture<?> batchTask =
              scheduleBatchedReadIntervalTask(
                  groupKey,
                  agentLink.getReadMemoryArea(),
                  agentLink.getRequestInterval(),
                  agentLink.getUnitId());
          ...
          LOG.fine(
              getProtocolName()
                  + " - Scheduled new read interval task for batch group "
                  + groupKey);
        }
      ...
      LOG.fine(
          getProtocolName()
              + " - Skipping read interval for "
              + ref
              + " - read configuration incomplete (unitId, readMemoryArea, readValueType, and readAddress all required)");
    }

    // Check if write interval is enabled (requestInterval set, writeAddress present, no read
    // address)
    if (agentLink.getRequestInterval() != null
        && agentLink.getWriteAddress() != null
        && agentLink.getReadAddress() == null) {
      ScheduledFuture<?> writeTask = scheduleModbusWriteRequestInterval(ref, agentLink);
      writeIntervalMap.put(ref, writeTask);
      LOG.fine(
          getProtocolName()
              + " - Scheduled write interval task for attribute "
              + ref
              + " every "
              + agentLink.getRequestInterval()
              + "ms");
  ...
  protected void doLinkedAttributeWrite(
      ModbusAgentLink agentLink, AttributeEvent event, Object processedValue) {
    ...
    int writeAddress =
        AgentLink.getOrThrowAgentLinkProperty(
            Optional.ofNullable(agentLink.getWriteAddress()), "write address");
    ...
    try {
      ...
      switch (agentLink.getWriteMemoryArea()) {
        case COIL -> {
          ...
          LOG.fine(
              getProtocolName()
                  ...
                  + writeAddress
                  + " (0x"
                  + Integer.toHexString(protocolAddress)
                  + "), Value: "
                  + value);
        }
        case HOLDING -> {
          if (registersCount == 1) {
            ...
            byte[] registerData =
                ModbusDataConverter.convertValueToRegisterBytes(
                    processedValue, registersCount, agentLink.getReadValueType(), endianFormat);
            pdu = ModbusPduBuilder.buildWriteMultipleRegistersPDU(protocolAddress, registerData);
            LOG.fine(
                getProtocolName()
                    + " Write Multiple Registers - UnitId: "
                    + unitId
                    + ", Address: "
                    + writeAddress
                    + " (0x"
        ...
        default ->
            throw new IllegalStateException(
                "Only COIL and HOLDING memory areas are supported for writing");
      }
      ...
      LOG.fine(
          getProtocolName() + " Write Success - UnitId: " + unitId + ", Address: " + writeAddress);
      ...
      if (event.getSource() != null) {
        if (agentLink.getReadAddress() == null) {
          ...
          LOG.finest(
              getProtocolName()
                  + " - Triggering verification read after write for "
                  + event.getRef());
  ...
  protected List<BatchReadRequest> createBatchRequests(
    ...
    Map<ModbusAgentLink.ReadMemoryArea, List<Map.Entry<AttributeRef, ModbusAgentLink>>>
        groupedByMemoryArea =
    ...
    for (Map.Entry<ModbusAgentLink.ReadMemoryArea, List<Map.Entry<AttributeRef, ModbusAgentLink>>>
        group : groupedByMemoryArea.entrySet()) {
      List<Map.Entry<AttributeRef, ModbusAgentLink>> sortedAttributes =
          group.getValue().stream()
              .sorted(
                  Comparator.comparingInt(
                      e -> Optional.ofNullable(e.getValue().getReadAddress()).orElse(0)))
      ...
      for (Map.Entry<AttributeRef, ModbusAgentLink> entry : sortedAttributes) {
        ...
        int registerCount =
            Optional.ofNullable(link.getRegistersAmount())
                .orElse(link.getReadValueType().getRegisterCount());
          ...
          if (!hasIllegalInGap && newQuantity <= maxRegisterLength) {
            ...
            LOG.finest(
                getProtocolName()
                    + " - Added register "
                    + address
                    + " to batch starting at "
                    + currentBatch.getStartAddress()
                    + " (new quantity="
                    + newQuantity
                    + ")");
            continue;
            ...
            String reason =
                hasIllegalInGap
                    ? "illegal register "
                        + firstIllegalRegister
                        + " detected in gap (registers "
                        + currentEnd
                        + "-"
                        + (address - 1)
                        + ")"
                    : "would exceed maxRegisterLength ("
                        + newQuantity
                        + " > "
                        + maxRegisterLength
                        + ")";
            LOG.fine(
                getProtocolName() + " - Split batch before register " + address + ": " + reason);
          }
        }
      ...
      if (currentBatch != null) {
        ...
        LOG.fine(
            getProtocolName()
                + " - Finalized batch: startAddress="
                + currentBatch.getStartAddress()
                + ", quantity="
                + currentBatch.getQuantity()
                + ", attributes="
                + currentBatch.getAttributes().size());
      }
    }
  ...
  protected ScheduledFuture<?> scheduleBatchedReadIntervalTask(
      String groupKey,
    ...
    LOG.fine(
        getProtocolName()
            + " - Scheduling batched read interval task for group "
            + groupKey
            + " every "
            + requestInterval
            + "ms");
          ...
          try {
            ...
            synchronized (cachedBatches) {
              ...
              if (batches == null) {
                ...
                Set<RegisterRange> illegalRegistersForUnit =
                    parseIllegalRegisters(illegalRegistersStr);

                LOG.fine(
                    getProtocolName()
                        + " - Creating batch requests for group "
                        + groupKey
                        + " with "
                        + group.size()
                        + " attribute(s) (unitId="
            ...
            for (int i = 0; i < batches.size(); i++) {
              ...
              LOG.finest(
                  getProtocolName()
                      ...
                      + batches.size()
                      + ": registers="
                      + batch.getStartAddress()
                      + "-"
                      + endAddress
                      + " (quantity="
                      + batch.getQuantity()
                      + ", attributes="
                      + batch.getAttributes().size()
                      + ")");
  ...
  protected void executeBatchRead(
      BatchReadRequest batch,
      ModbusAgentLink.ReadMemoryArea memoryArea,
      Map<AttributeRef, ModbusAgentLink> group,
      Integer unitId) {
    if (getConnectionStatus() != ConnectionStatus.CONNECTED) {
      LOG.fine(
          "Skipping batch read operation - not connected (status: " + getConnectionStatus() + ")");
      return;
    }
    ...
    try {
      ...
      byte[] pdu =
          ModbusPduBuilder.buildReadRequestPDU(functionCode, protocolAddress, batch.getQuantity());

      LOG.fine(
          getProtocolName()
              + " Read Request - MemoryArea: "
              + memoryArea
              + ", UnitId: "
              + effectiveUnitId
              + ", StartAddress: "
      ...
      for (int i = 0; i < batch.getAttributes().size(); i++) {
        ...
        try {
          ...
          Object value =
              ModbusDataConverter.extractValueFromBatchResponse(
                  data, offset, registerCount, dataType, endianFormat, functionCode);

          if (value != null) {
            LOG.finest(
                getProtocolName()
                    + " - Successfully extracted value for "
                    + ref
                    + " - Value: "
                    + value
                    + ", DataType: "
      ...
      if (e instanceof TimeoutException) {
        ...
        LOG.log(
            Level.WARNING, getProtocolName() + " - " + operation + " failed: " + e.getMessage(), e);
      }
    }
  }
          ...
          try {
            int registerCount =
                Optional.ofNullable(agentLink.getRegistersAmount())
                    .orElse(agentLink.getReadValueType().getRegisterCount());
            BatchReadRequest batch =
                new BatchReadRequest(agentLink.getReadAddress(), registerCount);
            ...
            executeBatchRead(
                batch, agentLink.getReadMemoryArea(), tempGroup, agentLink.getUnitId());
            ...
            LOG.log(
                Level.WARNING,
                getProtocolName()
                    + " - Exception during one-time read for "
                    + ref
                    + ": "
                    + e.getMessage(),
                e);
  ...
  protected ScheduledFuture<?> scheduleModbusWriteRequestInterval(
      AttributeRef ref, ModbusAgentLink agentLink) {
    LOG.fine(
        "Scheduling "
            + getProtocolName()
            + " Write interval request to execute every "
            + agentLink.getRequestInterval()
            + "ms for attributeRef: "
            + ref);
        ...
        () -> {
          try {
            ...
            LOG.log(
                Level.WARNING,
                getProtocolName()
                    + " - Exception during write interval for "
                    + ref
                    + ": "
                    + e.getMessage(),
                e);
        ...
        () -> {
          try {
            ...
            LOG.log(
                Level.WARNING,
                getProtocolName()
                    + " - Failed to execute verification read for "
                    + ref
                    + ": "
                    + e.getMessage(),
                e);
