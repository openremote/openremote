...
public class ProvisioningConfirmationState extends ProvisioningState {
  ...
  private static final byte[] NO_OOB_AUTH = {
  ...
  public ProvisioningConfirmationState(
  ...
  public void executeSend() {
    ...
    mStatusCallbacks.onProvisioningStateChanged(
        mNode, States.PROVISIONING_CONFIRMATION_SENT, provisioningConfirmationPDU);
  ...
  public boolean parseData(final byte[] data) {
    mStatusCallbacks.onProvisioningStateChanged(
        mNode, States.PROVISIONING_CONFIRMATION_RECEIVED, data);
  ...
  private byte[] createProvisioningConfirmation() {
    ...
    final byte[] confirmationInputs =
        provisioningCallbacks.generateConfirmationInputs(
            mNode.getProvisionerPublicKeyXY(), mNode.getProvisioneePublicKeyXY());
    ...
    LOG.info(
        "Provisioning confirmation: "
            + MeshParserUtils.bytesToHex(provisioningConfirmationPDU, false));
  ...
  private byte[] generateAuthenticationValue() {
    switch (mNode.authMethodUsed) {
      ...
      case INPUT_OOB_AUTHENTICATION:
        ...
        return InputOOBAction.generateInputOOBAuthenticationValue(
            inputOOBAction, mNode.getInputAuthentication());
    }
    return null;
  }
