 ...
 *
 * <p>This class contains properties such as Model Identifier, bound keys, key indexes, subscription
...
public abstract class MeshModel {
  ...
  public void setBoundAppKeyIndex(final int appKeyIndex) {
    if (!mBoundAppKeyIndexes.contains(appKeyIndex)) mBoundAppKeyIndexes.add(appKeyIndex);
  }
  ...
  protected void setPublicationStatus(
      final ConfigModelPublicationStatus status, final UUID labelUUID) {
    if (status.isSuccessful()) {
      if (!MeshAddress.isValidUnassignedAddress(status.getPublishAddress())) {
        mPublicationSettings =
            new PublicationSettings(
                status.getPublishAddress(),
                labelUUID,
  ...
  protected void updatePublicationStatus(final ConfigModelPublicationStatus status) {
    if (status.isSuccessful()) {
      if (mPublicationSettings != null) {
        ...
        mPublicationSettings.setPublishRetransmitIntervalSteps(
            status.getPublishRetransmitIntervalSteps());
      }
    }
  }
  ...
  /**
   * Overwrites the subscription addresses in a mesh model by clearing the existing addresses and
