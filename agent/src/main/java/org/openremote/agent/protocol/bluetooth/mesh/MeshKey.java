...
abstract class MeshKey implements Cloneable {
  ...
  public void setName(@NotNull final String name) throws IllegalArgumentException {
    if (TextUtil.isNullOrEmpty(name)) throw new IllegalArgumentException("Name cannot be empty!");
    this.name = name;
  }
   ...
   * Sets a network key.
   *
   * <p>In order to change the key call {@link BaseMeshNetwork#updateNetKey(NetworkKey, String)} or
  ...
  protected boolean valid(@NotNull final byte[] key) {
    Objects.requireNonNull(key);
    if (key.length != 16) throw new IllegalArgumentException("Key must be of length 16!");
    return true;
  }

  protected boolean distributeKey(@NotNull final byte[] key) {
    Objects.requireNonNull(key);
    if (!Arrays.equals(this.key, oldKey)) setOldKey(this.key);
    setKey(key);
    return true;
  }
}
