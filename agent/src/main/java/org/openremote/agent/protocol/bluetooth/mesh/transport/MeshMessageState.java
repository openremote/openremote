 ...
 *
 * <p>This class handles sending, resending and parsing mesh messages. Each message sent by the
 * library has its own state. {@link ConfigMessageState} and {@link GenericMessageState} extends
 * this class based on the type of the message. Currently the library supports basic Configuration
...
abstract class MeshMessageState implements LowerTransportLayerCallbacks {
   ...
   * @param callbacks {@link InternalMeshMsgHandlerCallbacks} Internal mesh message handler
   *     callbacks
   */
  MeshMessageState(
    ...
    if (message.getNetworkLayerPdu().size() > 0 && !retransmitPduIndexes.isEmpty()) {
      for (int i = 0; i < retransmitPduIndexes.size(); i++) {
        ...
        if (message.getNetworkLayerPdu().get(segO) != null) {
          ...
          final Message retransmitMeshMessage =
              mMeshTransport.createRetransmitMeshMessage(message, segO);
          mInternalTransportCallbacks.onMeshPduCreated(
              mDst, retransmitMeshMessage.getNetworkLayerPdu().get(segO));
        }
      }
    }
  }
  ...
  public synchronized void sendSegmentAcknowledgementMessage(final ControlMessage controlMessage) {
    ...
    final ControlMessage message =
        mMeshTransport.createSegmentBlockAcknowledgementMessage(controlMessage);
    LOG.info(
        "Sending acknowledgement: "
            + MeshParserUtils.bytesToHex(message.getNetworkLayerPdu().get(0), false));
    mInternalTransportCallbacks.onMeshPduCreated(
        message.getDst(), message.getNetworkLayerPdu().get(0));
