...
public abstract class AbstractMQTT_IOClient<S> implements IOClient<MQTTMessage<S>> {
  ...
  protected final ConcurrentMap<String, Pair<MqttQos, Set<Consumer<MQTTMessage<S>>>>>
      topicConsumerMap = new ConcurrentHashMap<>();
                ...
                context -> {
                  ...
                  if (disconnected) {
                    ...
                    if (this.usernamePassword != null) {
                      ((Mqtt3ClientDisconnectedContext) context)
                          .getReconnector()
                          .connectWith()
                          .simpleAuth()
                  ...
                  if (context.getCause() instanceof Mqtt3DisconnectException) {
                    ...
                    LOG.info(
                        "Connection rejected by the broker '"
                            + getClientUri()
                            + "': reasonCode="
  ...
  public void sendMessage(MQTTMessage<S> message) {
    ...
    // TODO: Might need to put some timeout logic here as this call can block if client becomes
    // disconnected see (https://github.com/hivemq/hivemq-mqtt-client/issues/554)
    client
        .publishWith()
        ...
        .whenComplete(
            (publish, throwable) -> {
              if (throwable != null) {
                ...
                LOG.log(
                    Level.INFO,
                    "Failed to publish to MQTT broker '" + getClientUri() + "'",
                    throwable);
              } else {
                // Success
  ...
  public boolean addMessageConsumer(
      String topic, MqttQos qos, Consumer<MQTTMessage<S>> messageConsumer) {
    if (client == null) {
      return false;
    }

    Pair<MqttQos, Set<Consumer<MQTTMessage<S>>>> consumers =
        topicConsumerMap.computeIfAbsent(
            topic, t -> new Pair<>(qos != null ? qos : MqttQos.AT_LEAST_ONCE, new HashSet<>()));
  ...
  protected boolean doClientSubscription(String topic) {
    ...
    Consumer<MQTTMessage<S>> messageConsumer =
        message -> {
    ...
    Pair<MqttQos, Set<Consumer<MQTTMessage<S>>>> topicConsumers = topicConsumerMap.get(topic);
    try {
      Mqtt3Subscribe subscribeMessage =
          Mqtt3Subscribe.builder().topicFilter(topic).qos(topicConsumers.getKey()).build();
              ...
              .subscribe(
                  ...
                  publish -> {
                    try {
                      ...
                      LOG.log(
                          Level.WARNING,
                          "Failed to process published message on client '" + getClientUri() + "'",
                          e);
                    }
                  })
              .get();
          ...
          .whenComplete(
              (subAck, throwable) -> {
                if (throwable != null) {
                  ...
                  LOG.fine(
                      "Unsubscribed from topic '" + topic + "' on client '" + getClientUri() + "'");
                }
              });
    }
  }
  ...
  public ConnectionStatus getConnectionStatus() {
    ...
    // We cannot differentiate between connecting and waiting with the callbacks provided so just
    // use waiting state
  ...
  public void connect() {
    ...
    Mqtt3ConnectBuilder.Send<CompletableFuture<Mqtt3ConnAck>> completableFutureSend =
        client
            .connectWith()
            .cleanSession(
                true) // Always clean session as there's some inconsistency in how HiveMQ client
            // handles this on reconnects
            .keepAlive(5);
  ...
  /**
   * Allows an override to be provided as {@link #getConnectionStatus} doesn't always return the
   * correct status when a server initiated disconnect occurs (some sort of timing issue).
   */
        ...
        .whenComplete(
            (unused, throwable) -> {
              ...
              if (throwable != null) {
                LOG.log(
                    Level.INFO,
                    "Disconnect error '" + getClientUri() + "':" + throwable.getMessage());
              }
