    ...
    Mqtt3ClientBuilder builder =
        MqttClient.builder()
            ...
            .addDisconnectedListener(
                context -> {
                  // Remove all subscriptions so we can add them again on connect and get a
                  // completable future for them
        ...
        (connAck, throwable) -> {
          if (connAck != null) {
            ...
            if (throwable instanceof CancellationException) {
              ...
              LOG.info(
                  "Connection failed uri=" + getClientUri() + ", error=" + throwable.getMessage());
            }
          }
        });
