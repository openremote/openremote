...
public abstract class ChannelProcessor extends FeatureProcessor {
  ...
  protected static int getMaxChannelNumber(VelbusDeviceType velbusDeviceType) {
    if (velbusDeviceType == VelbusDeviceType.VMBGPOD
        || velbusDeviceType == VelbusDeviceType.VMBGPO) {
      return 32;
    }

    if (velbusDeviceType == VelbusDeviceType.VMB7IN
        || velbusDeviceType == VelbusDeviceType.VMB1TS) {
      return 7;
    }
  ...
  protected Optional<Pair<Integer, String>> getChannelNumberAndPropertySuffix(
      VelbusDevice device, Pattern channelRegex, String property) {
