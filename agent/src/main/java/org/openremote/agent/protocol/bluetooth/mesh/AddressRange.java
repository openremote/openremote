...
public abstract class AddressRange extends Range {
  ...
  public boolean overlaps(final Range otherRange) {
    if (otherRange instanceof AddressRange) {
      ...
      return overlaps(
          lowAddress,
          highAddress,
          otherAddressRange.getLowAddress(),
   ...
   */
  public static List<AllocatedGroupRange> minus(
      final List<AllocatedGroupRange> ranges, final AllocatedGroupRange other) {
  ...
  List<AllocatedGroupRange> minus(final AllocatedGroupRange other) {
    ...
    if (other.lowAddress > lowAddress) {
      final AllocatedGroupRange leftSlice =
          new AllocatedGroupRange(lowAddress, (Math.min(highAddress, other.lowAddress - 1)));
    ...
    if (other.highAddress < highAddress) {
      final AllocatedGroupRange rightSlice =
          new AllocatedGroupRange(Math.max(other.highAddress + 1, lowAddress), highAddress);
   ...
   */
  public static List<AllocatedUnicastRange> minus(
      final List<AllocatedUnicastRange> ranges, final AllocatedUnicastRange other) {
  ...
  List<AllocatedUnicastRange> minus(final AllocatedUnicastRange other) {
    ...
    // Result: |---|                             |-----------|                 |--|
    if (other.lowAddress > lowAddress) {
      final AllocatedUnicastRange leftSlice =
          new AllocatedUnicastRange(lowAddress, (Math.min(highAddress, other.lowAddress - 1)));
      results.add(leftSlice);
    }
    ...
    // Result:                      |--|                      |--|                     |--------|
    if (other.highAddress < highAddress) {
      final AllocatedUnicastRange rightSlice =
          new AllocatedUnicastRange(Math.max(other.highAddress + 1, lowAddress), highAddress);
      results.add(rightSlice);
    }
    return results;
  }
