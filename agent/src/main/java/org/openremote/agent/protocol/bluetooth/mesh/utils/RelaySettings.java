...
public class RelaySettings {
  ...
  public static final int RELAY_FEATURE_DISABLED =
      0x00; // The node support Relay feature that is disabled
  public static final int RELAY_FEATURE_ENABLED =
   ...
   *
   * @param relayTransmitCount Number of retransmissions on advertising bearer for each Network PDU
   *     relayed by the node
  ...
  /**
   * Returns the number of retransmissions on advertising bearer for each Network PDU relayed by the
   * node
   */
  ...
  public static int decodeRelayRetransmitInterval(final int interval) {
    if ((interval >= 10 && interval <= 320) && (interval % 10 != 0))
      throw new IllegalArgumentException(
          "Relay Retransmit Interval must be in range of 10-320 ms.");
