...
public class BluetoothMeshNetwork extends BluetoothCentralManagerCallback
  ...
  public static final Logger LOG =
      SyslogCategory.getLogger(SyslogCategory.PROTOCOL, BluetoothMeshNetwork.class.getName());
  ...
  public BluetoothMeshNetwork(
      BluetoothCentralManager bluetoothCentral,
      SequenceNumberPersistencyManager sequenceNumberManager,
      MainThreadManager mainThread,
      String proxyAddress,
      int unicastAddress,
      NetworkKey networkKey,
      Map<Integer, ApplicationKey> applicationKeyMap,
      int mtu,
      int sequenceNumber,
  ...
  @Override
  public synchronized void onDisconnectedPeripheral(
      BluetoothPeripheral peripheral, BluetoothCommandStatus status) {
    if (proxyScanner != null) {
  ...
  @Override
  public synchronized void onDiscoveredPeripheral(
      BluetoothPeripheral peripheral, ScanResult scanResult) {
    if (proxyScanner != null) {
  ...
  public synchronized void onMeshPduCreated(byte[] pdu) {
    ...
    if (bluetoothMeshProxy != null) {
      bluetoothMeshProxy.sendData(
          getMtu(),
          pdu,
          (proxy, data, isSuccess) -> {
            if (isSuccess) {
  ...
  public synchronized void onMeshMessageReceived(int src, MeshMessage meshMessage) {
    LOG.info(
        "Received mesh message: address='"
            + String.format("0x%04X", src)
            + "', message='"
    ...
    if (element != null) {
      ...
      LOG.info(
          "Could not find element to process the received mesh message: address='"
              + String.format("0x%04X", src)
              + "', message='"
              + meshMessage.getClass().getName()
              + "'");
  ...
  public void onMessageDecryptionFailed(String meshLayer, String errorMessage) {
    LOG.warning(
        "Failed to decrypt Bluetooth mesh message: [meshLayer: '"
            + meshLayer
            + "', errorMessage: '"
            + errorMessage
            + "']");
  ...
  public synchronized void addMeshModel(int address, int modelId, int appKeyIndex) {
    ...
    if (model == null) {
      LOG.severe(
          "Bluetooth Mesh model with ID='"
              + String.format("0x%04X", modelId)
              + "' is not supported.");
      return;
    }
    ...
    if (address < 0xC000) {
      ...
      if (node == null) {
        ...
        node =
            new ProvisionedMeshNode(network.getSelectedProvisioner(), networkKeys, applicationKeys);
      ...
      if (getNode(address) == null) {
        ...
        network.unicastAddress =
            network.nextAvailableUnicastAddress(
                node.getNumberOfElements(), network.getSelectedProvisioner());
  ...
  public synchronized void addSensorValueConsumer(
      int address, int modelId, Consumer<Object> consumer) {
  ...
  public synchronized void removeSensorValueConsumer(
      int address, int modelId, Consumer<Object> consumer) {
    ShadowMeshModel model = searchShadowModel(address, modelId);
    if (model != null) {
  ...
  public synchronized void sendMeshSetCommand(int address, int modelId, Object value) {
    if (isConnected()) {
      ...
      if (model != null) {
        ...
        String modelName =
            sigModel != null ? sigModel.getModelName() : String.format("0x%04X", modelId);
        LOG.warning(
            "Failed to send mesh command value '"
                + value
                + "' because couldn't find mesh model: address:'"
  ...
  private synchronized void connect() {
    ...
    proxyScanner =
        new BluetoothMeshProxyScanner(
            mainThreadManager, bluetoothCentral, executorService, scheduledExecutorService);
    proxyScanner.start(
        networkKey,
        proxyAddress,
        SCAN_DURATION,
        new BluetoothMeshProxyScannerCallback() {
          @Override
          public void onMeshProxiesScanned(
              List<BluetoothMeshProxy> meshProxies, Integer errorCode) {
            LOG.info("Finished scanning Bluetooth mesh proxies.");
            if (errorCode == null) {
              for (BluetoothMeshProxy curMeshProxy : meshProxies) {
                LOG.info(
                    "Scan found Bluetooth mesh proxy: [Name="
                        + curMeshProxy.getPeripheral().getName()
                        + ", Address="
                        + curMeshProxy.getPeripheral().getAddress()
                        + ", Rssi="
              ...
              if (meshProxies.size() > 0) {
                synchronized (BluetoothMeshNetwork.this) {
                  ...
                  BluetoothMeshNetwork.this.bluetoothMeshProxy.setRxDataCallback(
                      BluetoothMeshNetwork.this);
                  BluetoothMeshNetwork.this.bluetoothMeshProxy.connect(
                      statusConsumer,
                      new BluetoothMeshProxyConnectCallback() {
                        @Override
                        public void onMeshProxyConnected(
                            BluetoothPeripheral peripheral,
                            boolean isSuccess,
                            boolean isConnectionLoss) {
                          synchronized (BluetoothMeshNetwork.this) {
                            if (isSuccess) {
                              LOG.info(
                                  "Successfully connected to Bluetooth mesh proxy: [Name="
                                      + peripheral.getName()
                                      + ", Address="
                              ...
                              LOG.warning(
                                  "Failed to connect to Bluetooth mesh proxy: [Name="
                                      + peripheral.getName()
                                      + ", Address="
                                      + peripheral.getAddress()
                                      + "]");
